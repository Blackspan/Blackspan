...
class Inline {
	...
	static public function dump($value) {
		...
		case is_numeric($value):
			...
			$repr = is_string($value) ? "'$value'"
					: (is_infinite($value) ? str_ireplace('INF', '.Inf',
	 ...
	 */
	static public function parseScalar($scalar, $delimiters = null,
			$stringDelimiters = array('"', "'"), &$i = 0, $evaluate = true) {
	...
	static private function parseQuotedScalar($scalar, &$i) {
		if (!preg_match('/' . self::REGEX_QUOTED_STRING . '/Au',
				substr($scalar, $i), $match)) {
			throw new ParseException(
					sprintf('Malformed inline YAML string (%s).',
							substr($scalar, $i)));
		}
		...
		while ($i < $len) {
			...
			while ($i < $len) {
				...
				case ' ':
					break;
				default:
					$output[$key] = self::parseScalar($mapping,
							array(',', '}'), array('"', "'"), $i);
					$done = true;
					--$i;
				}
	...
	static private function evaluateScalar($scalar) {
		...
		case ctype_digit($scalar):
			...
			return '0' == $scalar[0] ? octdec($scalar)
		...
		case is_numeric($scalar):
			return '0x' == $scalar[0] . $scalar[1] ? hexdec($scalar)
					: floatval($scalar);
