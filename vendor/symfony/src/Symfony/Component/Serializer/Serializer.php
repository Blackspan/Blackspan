...
class Serializer implements SerializerInterface {
	...
	public final function serialize($data, $format) {
		if (!$this->supportsSerialization($format)) {
			throw new UnexpectedValueException(
					'Serialization for the format ' . $format
							. ' is not supported');
		}
	...
	public final function deserialize($data, $type, $format) {
		if (!$this->supportsDeserialization($format)) {
			throw new UnexpectedValueException(
					'Deserialization for the format ' . $format
							. ' is not supported');
		}
	...
	private function normalizeObject($object, $format = null) {
		if (!$this->normalizers) {
			throw new LogicException(
					'You must register at least one normalizer to be able to normalize objects.');
		}
		...
		throw new UnexpectedValueException(
				'Could not normalize object of type ' . $class
	...
	private function denormalizeObject($data, $class, $format = null) {
		...
		throw new UnexpectedValueException(
				'Could not denormalize object of type ' . $class
						. ', no supporting normalizer found.');
	}
	...
	public function getEncoder($format) {
		if (!isset($this->encoders[$format])) {
			throw new RuntimeException(
					sprintf('No encoder found for format "%s".', $format));
		}
