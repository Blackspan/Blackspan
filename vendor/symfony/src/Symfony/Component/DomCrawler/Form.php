...
class Form extends Link implements \ArrayAccess {
	...
	public function getUri() {
		...
		if (!in_array($this->getMethod(), array('POST', 'PUT', 'DELETE'))
				&& $queryString = http_build_query($this->getValues(), null,
						'&')) {
	...
	public function getMethod() {
		...
		return $this->node->getAttribute('method') ? strtoupper(
						$this->node->getAttribute('method')) : 'GET';
	}
	...
	public function get($name) {
		if (!$this->has($name)) {
			throw new \InvalidArgumentException(
					sprintf('The form has no "%s" field', $name));
		}
	...
	private function initialize() {
		...
		foreach ($xpath
				->query(
						'descendant::input | descendant::textarea | descendant::select',
						$root) as $node) {
	...
	public function offsetGet($name) {
		if (!$this->has($name)) {
			throw new \InvalidArgumentException(
					sprintf('The form field "%s" does not exist', $name));
		}
	...
	protected function setNode(\DOMNode $node) {
		...
		if ('button' == $node->nodeName
			...
			throw new \LogicException(
					sprintf('Unable to submit on a "%s" tag.', $node->nodeName));
		}
