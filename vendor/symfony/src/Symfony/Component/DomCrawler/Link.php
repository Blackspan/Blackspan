...
class Link {
	...
	public function __construct(\DOMNode $node, $currentUri, $method = 'GET') {
		if (!in_array(substr($currentUri, 0, 4), array('http', 'file'))) {
			throw new \InvalidArgumentException(
					sprintf('Current URI must be an absolute URL ("%s").',
							$currentUri));
		}
	...
	public function getUri() {
		...
		// relative path
		return substr($this->currentUri, 0, strrpos($this->currentUri, '/') + 1)
				. $uri;
	}
	...
	protected function setNode(\DOMNode $node) {
		if ('a' != $node->nodeName) {
			throw new \LogicException(
					sprintf('Unable to click on a "%s" tag.', $node->nodeName));
		}
