...
class RequestMatcher implements RequestMatcherInterface {
	...
	public function __construct($path = null, $host = null, $methods = null,
			$ip = null, array $attributes = array()) {
		$this->path = $path;
		$this->host = $host;
	...
	public function matchMethod($method) {
		$this->methods = array_map('strtoupper',
	...
	public function matches(Request $request) {
		...
		if (null !== $this->host
				&& !preg_match('#' . str_replace('#', '\\#', $this->host) . '#',
						$request->getHost())) {
			return false;
		}

		if (null !== $this->ip
				&& !$this->checkIp($request->getClientIp(), $this->ip)) {
			return false;
		}
	...
	protected function checkIp4($requestIp, $ip) {
		...
		return 0
				=== substr_compare(sprintf('%032b', ip2long($requestIp)),
