...
class Response {
	...
	static public $statusTexts = array(100 => 'Continue',
			101 => 'Switching Protocols', 200 => 'OK', 201 => 'Created',
	...
	public function __toString() {
		...
		return sprintf('HTTP/%s %s %s', $this->version, $this->statusCode,
				$this->statusText) . "\r\n" . $this->headers . "\r\n"
				. $this->getContent();
	}
	...
	public function prepare() {
		...
		if (!$this->headers->has('Content-Type')) {
			$this->headers->set('Content-Type', 'text/html; charset='
							. $charset);
		} elseif (0 === strpos($this->headers->get('Content-Type'), 'text/')
				&& false
						=== strpos($this->headers->get('Content-Type'),
								'charset')) {
			// add the charset
			$this->headers
					->set('Content-Type',
	...
	public function sendHeaders() {
		...
		// status
		header(
				sprintf('HTTP/%s %s %s', $this->version, $this->statusCode,
						$this->statusText));
	...
	public function setStatusCode($code, $text = null) {
		...
		$this->statusText = false === $text ? ''
				: (null === $text ? self::$statusTexts[$this->statusCode]
						: $text);
	}
	...
	public function isCacheable() {
		if (!in_array($this->statusCode,
				array(200, 203, 300, 301, 302, 404, 410))) {
			return false;
		}
	...
	public function mustRevalidate() {
		return $this->headers->hasCacheControlDirective('must-revalidate')
				|| $this->headers->has('must-proxy-revalidate');
	}
	...
	public function getMaxAge() {
		...
		if (null !== $this->getExpires()) {
			return $this->getExpires()->format('U')
	...
	public function setCache(array $options) {
		if ($diff = array_diff(array_keys($options),
				array('etag', 'last_modified', 'max_age', 's_maxage',
						'private', 'public'))) {
			throw new \InvalidArgumentException(
					sprintf(
	...
	public function setNotModified() {
		...
		foreach (array('Allow', 'Content-Encoding', 'Content-Language',
	...
	public function isNotModified(Request $request) {
		...
		if ($etags = $request->getEtags()) {
			$notModified = (in_array($this->getEtag(), $etags)
					|| in_array('*', $etags))
					&& (!$lastModified
