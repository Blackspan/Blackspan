...
class ArgvInput extends Input {
	...
	private function parseShortOption($token) {
		...
		if (strlen($name) > 1) {
			if ($this->definition->hasShortcut($name[0])
					&& $this->definition->getOptionForShortcut($name[0])
							->acceptValue()) {
	...
	private function parseLongOption($token) {
		...
		if (false !== $pos = strpos($name, '=')) {
			$this
					->addLongOption(substr($name, 0, $pos),
							substr($name, $pos + 1));
		} else {
	...
	private function parseArgument($token) {
		...
		if ($this->definition->hasArgument($c)) {
			...
			$this->arguments[$arg->getName()] = $arg->isArray() ? array($token)
					: $token;
	...
	private function addShortOption($shortcut, $value) {
		...
		$this
				->addLongOption(
						$this->definition->getOptionForShortcut($shortcut)
								->getName(), $value);
	}
