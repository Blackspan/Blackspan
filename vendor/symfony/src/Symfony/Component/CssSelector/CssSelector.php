...
class CssSelector {
	...
	static public function toXPath($cssExpr, $prefix = 'descendant-or-self::') {
		if (is_string($cssExpr)) {
			...
			if (preg_match('~^(\w*)#(\w+)\s*$~u', $cssExpr, $match)) {
				return sprintf("%s%s[@id = '%s']", $prefix,
			...
			if (preg_match('#^(\w*)\.(\w+)\s*$#u', $cssExpr, $match)) {
				return sprintf(
						...
						$prefix, $match[1] ? $match[1] : '*', $match[2]);
			}
		...
		if (!$expr) {
			throw new ParseException(
					sprintf('Got None for xpath expression from %s.', $cssExpr));
		}
	...
	public function parse($string) {
		...
		try {
			...
			throw new $class(
					sprintf('%s at %s -> %s', $e->getMessage(),
							implode($stream->getUsed(), ''), $stream->peek()),
					0, $e);
		}
	}
	...
	private function parseSelector($stream) {
		...
		while (true) {
			...
			if ($consumed == count($stream->getUsed())) {
				throw new ParseException(
						sprintf("Expected selector, got '%s'", $stream->peek()));
			}

			$result = new Node\CombinedSelectorNode($result, $combinator,
					$nextSelector);
		}
	...
	private function parseSimpleSelector($stream) {
		...
		if ('*' != $peek && !$peek->isType('Symbol')) {
			...
			if ('*' != $next && !$next->isType('Symbol')) {
				throw new ParseException(
						sprintf("Expected symbol, got '%s'", $next));
			}
		...
		while (true) {
			...
			if ('#' == $peek) {
				...
				if (!$ident || !$ident->isType('Symbol')) {
					throw new ParseException(
							sprintf("Expected symbol, got '%s'", $ident));
				}
				...
				if ($stream->peek() == '(') {
					...
					if (')' != $next) {
						throw new ParseException(
								sprintf("Expected ')', got '%s' and '%s'",
										$next, $selector));
					}

					$result = new Node\FunctionNode($result, $type, $ident,
							$selector);
				} else {
	...
	private function parseAttrib($selector, $stream) {
		...
		if ($stream->peek() == ']') {
			return new Node\AttribNode($selector, $namespace, $attrib,
					'exists', null);
		}
		...
		if (!in_array($op, array('^=', '$=', '*=', '=', '~=', '|=', '!='))) {
			throw new ParseException(
					sprintf("Operator expected, got '%s'", $op));
		}

		$value = $stream->next();
		if (!$value->isType('Symbol') && !$value->isType('String')) {
			throw new ParseException(
					sprintf("Expected string or symbol, got '%s'", $value));
		}
