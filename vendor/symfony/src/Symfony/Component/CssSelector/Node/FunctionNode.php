...
class FunctionNode implements NodeInterface {
	static protected $unsupported = array('target', 'lang', 'enabled',
			'disabled');
	 ...
	 * @return XPathExpr
	 */
	protected function _xpath_nth_child($xpath, $expr, $last = false,
			$addNameTest = true) {
	...
	protected function _xpath_contains($xpath, $expr) {
		...
		$xpath
				->addCondition(
						sprintf('contains(string(.), %s)',
								XPathExpr::xpathLiteral($expr)));
