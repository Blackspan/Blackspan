...
class ValidatorFactory implements ValidatorContextInterface {
	 ...
	 *                                      ".yml" nor ".yaml"
	 */
	static public function buildDefault(array $mappingFiles = array(),
			$annotations = true, $staticMethod = null) {
		...
		foreach ($mappingFiles as $file) {
			...
			if ($extension === 'xml') {
				...
				throw new MappingException(
						'The only supported mapping file formats are XML and YAML');
			}
		}
	...
	public function __construct(
			ValidatorContextInterface $defaultContext = null) {
		$this->defaultContext = null === $defaultContext ? new ValidatorContext()
				: $defaultContext;
	}
