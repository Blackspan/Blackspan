...
class FormBuilder {
	 ...
	 */
	public function __construct($name, FormFactoryInterface $factory,
			EventDispatcherInterface $dispatcher, $dataClass = null) {
		$this->name = $name;
	 ...
	 * @return FormBuilder The current builder
	 */
	public function appendNormTransformer(
	 ...
	 * @return FormBuilder The current builder
	 */
	public function prependNormTransformer(
			DataTransformerInterface $normTransformer) {
	 ...
	 * @return FormBuilder The current builder
	 */
	public function appendClientTransformer(
	 ...
	 * @return FormBuilder The current builder
	 */
	public function prependClientTransformer(
			DataTransformerInterface $clientTransformer) {
	...
	public function add($child, $type = null, array $options = array()) {
		...
		if (!is_string($child)) {
			throw new UnexpectedTypeException($child,
					'string or Symfony\Component\Form\FormBuilder');
		}
		...
		if ($this->currentLoadingType
				&& ($type instanceof FormTypeInterface ? $type->getName()
						: $type) == $this->currentLoadingType) {
			throw new CircularReferenceException(
					is_string($type) ? $this->getFormFactory()->getType($type)
							: $type);
		}

		$this->children[$child] = array('type' => $type,
				'options' => $options,);
	...
	public function create($name, $type = null, array $options = array()) {
		...
		if (null !== $type) {
			return $this->getFormFactory()
					->createNamedBuilder($type, $name, null, $options);
		}

		return $this->getFormFactory()
				->createBuilderForProperty($this->dataClass, $name, null,
						$options);
	}
	...
	public function get($name) {
		if (!isset($this->children[$name])) {
			throw new FormException(
					sprintf('The field "%s" does not exist', $name));
		}

		if (!$this->children[$name] instanceof FormBuilder) {
			$this->children[$name] = $this
					->create($name, $this->children[$name]['type'],
	...
	public function getForm() {
		$instance = new Form($this->getName(), $this->buildDispatcher(),
				$this->getTypes(), $this->getClientTransformers(),
				$this->getNormTransformers(), $this->getDataMapper(),
				$this->getValidators(), $this->getRequired(),
				$this->getReadOnly(), $this->getErrorBubbling(),
				$this->getEmptyData(), $this->getAttributes());
	...
	protected function buildChildren() {
		...
		foreach ($this->children as $name => $builder) {
			if (!$builder instanceof FormBuilder) {
				$builder = $this
						->create($name, $builder['type'], $builder['options']);
			}
