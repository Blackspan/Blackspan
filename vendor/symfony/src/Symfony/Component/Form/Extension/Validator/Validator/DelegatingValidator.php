...
class DelegatingValidator implements FormValidatorInterface {
	 ...
	 */
	static public function validateFormData(FormInterface $form,
		...
		if (is_object($form->getData()) || is_array($form->getData())) {
			...
			foreach (self::getFormValidationGroups($form) as $group) {
				$graphWalker
						->walkReference($form->getData(), $group,
								$propertyPath, true);
			}
		}
	}
	...
	private function buildFormPathMapping(FormInterface $form, array &$mapping,
			$formPath = 'children', $namePath = '') {
		foreach ($form->getAttribute('error_mapping') as $nestedDataPath => $nestedNamePath) {
			$mapping['/^' . preg_quote($formPath . '.data.' . $nestedDataPath)
					. '(?!\w)/'] = $namePath . '.' . $nestedNamePath;
		}
		...
		foreach ($iterator as $child) {
			...
			if ($child->hasChildren()) {
				$this
						->buildFormPathMapping($child, $mapping,
								$nestedFormPath, $nestedNamePath);
			}
	...
	private function buildDataPathMapping(FormInterface $form, array &$mapping,
			$dataPath = 'data', $namePath = '') {
		foreach ($form->getAttribute('error_mapping') as $nestedDataPath => $nestedNamePath) {
			$mapping['/^' . preg_quote($dataPath . '.' . $nestedDataPath)
					. '(?!\w)/'] = $namePath . '.' . $nestedNamePath;
		}
		...
		foreach ($iterator as $child) {
			...
			if ($child->hasChildren()) {
				...
				if (is_array($child->getData())
						|| $child->getData() instanceof \Traversable) {
					$this
							->buildDataPathMapping($child, $mapping, $dataPath,
									$nestedNamePath);
				}

				foreach ($nestedDataPaths as $nestedDataPath) {
					$this
							->buildDataPathMapping($child, $mapping,
									$nestedDataPath, $nestedNamePath);
				}
			}
	...
	private function buildNamePathMapping(FormInterface $form, array &$forms,
			$namePath = '') {
		$iterator = new VirtualFormAwareIterator($form->getChildren());
		$iterator = new \RecursiveIteratorIterator($iterator);
			...
			if (is_string($form)) {
				if (!isset($forms[$form])) {
					throw new FormException(
							sprintf(
									'The child form with path "%s" does not exist',
									$form));
				}
