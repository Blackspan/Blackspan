...
class ChoiceType extends AbstractType {
	...
	public function buildForm(FormBuilder $builder, array $options) {
		if ($options['choice_list']
				&& !$options['choice_list'] instanceof ChoiceListInterface) {
			throw new FormException(
					'The "choice_list" must be an instance of "Symfony\Component\Form\Extension\Core\ChoiceList\ChoiceListInterface".');
		}
		...
		if ($options['expanded']) {
			...
			foreach ($options['choices'] as $choice => $value) {
				if ($options['multiple']) {
					...
					$builder
							->add((string) $choice, 'radio',
									array('value' => $choice, 'label' => $value,));
				}
			}
		}
		...
		$builder->setAttribute('choice_list', $options['choice_list'])
				->setAttribute('preferred_choices',
		...
		if ($options['expanded']) {
			if ($options['multiple']) {
				$builder
						->appendClientTransformer(
								new ArrayToBooleanChoicesTransformer(
										$options['choice_list']));
			} else {
				$builder
						->appendClientTransformer(
								new ScalarToBooleanChoicesTransformer(
										$options['choice_list']))
			...
			if ($options['multiple']) {
				$builder
						->appendClientTransformer(
								new ArrayToChoicesTransformer());
			} else {
				$builder
						->appendClientTransformer(
								new ScalarToChoiceTransformer());
			}
		}
	...
	public function buildView(FormView $view, FormInterface $form) {
		...
		$view->set('multiple', $form->getAttribute('multiple'))
				...
				->set('preferred_choices',
						array_intersect_key($choices, $preferred))
	...
	public function getDefaultOptions(array $options) {
		...
		return array('multiple' => false, 'expanded' => false,
				'choice_list' => null, 'choices' => array(),
				'preferred_choices' => array(),
				'empty_data' => $multiple || $expanded ? array() : '',
				'empty_value' => $multiple || $expanded
						|| !isset($options['empty_value']) ? null : '',
