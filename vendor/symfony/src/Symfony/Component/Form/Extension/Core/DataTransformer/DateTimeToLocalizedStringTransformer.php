...
class DateTimeToLocalizedStringTransformer extends BaseDateTimeTransformer {
	...
	public function __construct($inputTimezone = null, $outputTimezone = null,
			...
			$calendar = \IntlDateFormatter::GREGORIAN, $pattern = null) {
		...
		if (!in_array($dateFormat, self::$formats, true)) {
			throw new UnexpectedTypeException($dateFormat,
					implode('", "', self::$formats));
		}

		if (!in_array($timeFormat, self::$formats, true)) {
			throw new UnexpectedTypeException($timeFormat,
					implode('", "', self::$formats));
		}
	...
	public function transform($dateTime) {
		...
		$value = $this->getIntlDateFormatter()
				->format((int) $dateTime->format('U'));
	...
	protected function getIntlDateFormatter() {
		...
		return new \IntlDateFormatter(\Locale::getDefault(), $dateFormat,
				$timeFormat, $timezone, $calendar, $pattern);
	}
}
