...
class DateType extends AbstractType {
	...
	public function buildForm(FormBuilder $builder, array $options) {
		...
		$allowedFormatOptionValues = array(\IntlDateFormatter::FULL,
		...
		if (!in_array($format, $allowedFormatOptionValues, true)) {
			if (is_string($format)) {
				...
				throw new CreationException(
						'The "format" option must be one of the IntlDateFormatter constants (FULL, LONG, MEDIUM, SHORT) or a string representing a custom pattern');
			}
		}

		$formatter = new \IntlDateFormatter(\Locale::getDefault(), $format,
				\IntlDateFormatter::NONE, 'UTC', \IntlDateFormatter::GREGORIAN,
				$pattern);
		...
		if ($options['widget'] === 'single_text') {
			...
			if ($options['widget'] === 'choice') {
				if (is_array($options['empty_value'])) {
					$options['empty_value'] = array_merge(
							array('year' => null, 'month' => null,
									'day' => null), $options['empty_value']);
				} else {
					$options['empty_value'] = array(
							'year' => $options['empty_value'],
							'month' => $options['empty_value'],
							'day' => $options['empty_value']);
				}

				// Only pass a subset of the options to children
				$yearOptions = array(
						'choice_list' => new PaddedChoiceList(
								array_combine($options['years'],
										$options['years']), 4, '0',
								STR_PAD_LEFT),
						'empty_value' => $options['empty_value']['year'],
						'required' => $options['required'],);
				$monthOptions = array(
						'choice_list' => new MonthChoiceList($formatter,
								$options['months']),
						'empty_value' => $options['empty_value']['month'],
						'required' => $options['required'],);
				$dayOptions = array(
						'choice_list' => new PaddedChoiceList(
								array_combine($options['days'],
										$options['days']), 2, '0',
								STR_PAD_LEFT),
			...
			$builder->add('year', $options['widget'], $yearOptions)
	...
	public function getDefaultOptions(array $options) {
		return array('years' => range(date('Y') - 5, date('Y') + 5),
				'months' => range(1, 12), 'days' => range(1, 31),
				'widget' => 'choice', 'input' => 'datetime',
				'format' => \IntlDateFormatter::MEDIUM,
				'data_timezone' => null, 'user_timezone' => null,
				'empty_value' => null, 
				// Don't modify \DateTime classes by reference, we treat
				// them like immutable value objects
				'by_reference' => false, 'error_bubbling' => false,);
	}
	...
	public function getAllowedOptionValues(array $options) {
		return array(
				...
				'widget' => array('single_text', 'text', 'choice',),);
	}
