...
class CsrfType extends AbstractType {
	...
	public function buildForm(FormBuilder $builder, array $options) {
		...
		$validator = function (FormInterface $form) use ($csrfProvider,
				$intention) {
			if ((!$form->hasParent() || $form->getParent()->isRoot())
					&& !$csrfProvider
							->isCsrfTokenValid($intention, $form->getData())) {
				$form
						->addError(
								new FormError(
	...
	public function getDefaultOptions(array $options) {
		return array('csrf_provider' => $this->csrfProvider,
				'intention' => null, 'property_path' => false,);
	}
