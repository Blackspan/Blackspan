...
abstract class Client {
	...
	public function __construct(array $server = array(),
			History $history = null, CookieJar $cookieJar = null) {
	...
	public function setServerParameters(array $server) {
		$this->server = array_merge(
				array('HTTP_HOST' => 'localhost',
						'HTTP_USER_AGENT' => 'Symfony2 BrowserKit',), $server);
	}
	...
	public function submit(Form $form, array $values = array()) {
		...
		return $this
				->request($form->getMethod(), $form->getUri(),
	...
	public function request($method, $uri, array $parameters = array(),
		...
		$request = new Request($uri, $method, $parameters, $files,
				$this->cookieJar->allValues($uri), $server, $content);
		...
		return $this->crawler = $this
				->createCrawlerFromContent($request->getUri(),
						$response->getContent(),
	...
	protected function doRequestInProcess($request) {
		...
		$process = new PhpProcess($this->getScript($request), null,
				array('TMPDIR' => sys_get_temp_dir(),
	...
	protected function getScript($request) {
		...
		throw new \LogicException(
				'To insulate requests, you need to override the getScript() method.');
		// @codeCoverageIgnoreEnd
	}
	...
	protected function getAbsoluteUri($uri) {
		...
		if (!$this->history->isEmpty()) {
			...
			$currentUri = sprintf('http%s://%s/',
					...
					isset($this->server['HTTP_HOST']) ? $this
	 ...
	 * @return Crawler
	 */
	protected function requestFromRequest(Request $request,
			$changeHistory = true) {
		return $this
				->request($request->getMethod(), $request->getUri(),
						$request->getParameters(), $request->getFiles(),
						$request->getServer(), $request->getContent(),
						$changeHistory);
	}
}
