...
class Translator implements TranslatorInterface {
	 ...
	 * @api
	 */
	public function addResource($format, $resource, $locale,
			$domain = 'messages') {
	...
	public function trans($id, array $parameters = array(),
		...
		return strtr($this->catalogues[$locale]->get((string) $id, $domain),
				$parameters);
	}
				...
				$this->selector
						->choose(
								$this->catalogues[$locale]
										->get((string) $id, $domain),
								(int) $number, $locale), $parameters);
	}
	...
	protected function loadCatalogue($locale) {
		...
		if (isset($this->resources[$locale])) {
			foreach ($this->resources[$locale] as $resource) {
				...
				$this->catalogues[$locale]
						->addCatalogue(
								$this->loaders[$resource[0]]
										->load($resource[1], $locale,
												$resource[2]));
			}
		}
	...
	private function addFallbackCatalogue($locale) {
		...
		if ($fallback != $locale) {
			$this->catalogues[$locale]
					->addFallbackCatalogue($this->catalogues[$fallback]);
		}
	}
}
