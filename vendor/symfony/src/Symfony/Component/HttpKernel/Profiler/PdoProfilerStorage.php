...
abstract class PdoProfilerStorage implements ProfilerStorageInterface {
	...
	public function find($ip, $url, $limit) {
		...
		$tokens = $this
				->fetch($db,
						'SELECT token, ip, url, time, parent FROM sf_profiler_data '
								. $criteria . ' ORDER BY time DESC LIMIT '
								. ((integer) $limit), $args);
		$this->close($db);
	...
	public function read($token) {
		...
		$data = $this
				->fetch($db,
						'SELECT data, parent, ip, url, time FROM sf_profiler_data WHERE token = :token LIMIT 1',
						$args);
		$this->close($db);
	...
	public function write(Profile $profile) {
		$db = $this->initDb();
		$args = array(':token' => $profile->getToken(),
				':parent' => $profile->getParent() ? $profile->getParent()
								->getToken() : '',
				':data' => base64_encode(serialize($profile->getCollectors())),
				':ip' => $profile->getIp(),
				':url' => $profile->getUrl(),
				':time' => $profile->getTime(), ':created_at' => time(),);

		if ($this->read($profile->getToken())) {
			try {
				$this
						->exec($db,
								'UPDATE sf_profiler_data SET parent = :parent, data = :data, ip = :ip, url = :url, time = :time, created_at = :created_at WHERE token = :token',
								$args);
				$this->cleanup();
				$status = true;
			} catch (\Exception $e) {
			...
			try {
				$this
						->exec($db,
								'INSERT INTO sf_profiler_data (token, parent, data, ip, url, time, created_at) VALUES (:token, :parent, :data, :ip, :url, :time, :created_at)',
								$args);
				$this->cleanup();
				$status = true;
			} catch (\Exception $e) {
	...
	protected function cleanup() {
		...
		$this
				->exec($db,
						'DELETE FROM sf_profiler_data WHERE created_at < :time',
	...
	protected function exec($db, $query, array $args = array()) {
		...
		if (!$success) {
			throw new \RuntimeException(
					sprintf('Error executing query "%s"', $query));
		}
	}
	...
	protected function prepareStatement($db, $query) {
		...
		if (false === $stmt) {
			throw new \RuntimeException(
					'The database cannot successfully prepare the statement');
		}
	...
	protected function readChildren($token, $parent) {
		...
		$data = $this
				->fetch($db,
						'SELECT token, data, ip, url, time FROM sf_profiler_data WHERE parent = :token',
						array(':token' => $token));
		$this->close($db);
