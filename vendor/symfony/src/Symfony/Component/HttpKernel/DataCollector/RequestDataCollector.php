...
class RequestDataCollector extends DataCollector {
	...
	public function collect(Request $request, Response $response,
		...
		foreach ($response->headers->getCookies() as $cookie) {
			$cookies[] = $this
					->getCookieHeader($cookie->getName(), $cookie->getValue(),
		...
		foreach ($request->attributes->all() as $key => $value) {
			$attributes[$key] = is_object($value) ? sprintf('Object(%s)',
		...
		$this->data = array(
				...
				'content' => $content,
				'content_type' => $response->headers->get('Content-Type') ? $response
				...
				'session_attributes' => $request->hasSession() ? $request
								->getSession()->all() : array(),);
	}
	...
	private function getCookieHeader($name, $value, $expires, $path, $domain,
			$secure, $httponly) {
		...
		if (0 !== $expires) {
			if (is_numeric($expires)) {
				...
				if (false === $expires || -1 == $expires) {
					throw new \InvalidArgumentException(
							sprintf(
									'The "expires" cookie parameter is not valid.',
									$expires));
				}
			}
			...
			$cookie .= '; expires='
					. substr(
							\DateTime::createFromFormat('U', $expires,
