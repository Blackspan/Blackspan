...
class Store implements StoreInterface {
	...
	public function lock(Request $request) {
		if (false
				!== $lock = @fopen(
						$path = $this
								->getPath($this->getCacheKey($request) . '.lck'),
						'x')) {
			fclose($lock);
	...
	public function lookup(Request $request) {
		...
		foreach ($entries as $entry) {
			if ($this
					->requestsMatch(
							isset($entry[1]['vary']) ? $entry[1]['vary'][0] : '',
							$request->headers->all(), $entry[0])) {
				$match = $entry;
	...
	public function invalidate(Request $request) {
		...
		foreach (array('Location', 'Content-Location') as $header) {
			if ($uri = $request->headers->get($header)) {
				$subRequest = Request::create($uri, 'get', array(), array(),
	...
	public function purge($url) {
		if (file_exists(
				$path = $this
						->getPath($this->getCacheKey(Request::create($url))))) {
			unlink($path);
	...
	private function save($key, $data) {
		...
		if (!is_dir(dirname($path))
				&& false === @mkdir(dirname($path), 0777, true)) {
			return false;
		}
	...
	public function getPath($key) {
		return $this->root . DIRECTORY_SEPARATOR . substr($key, 0, 2)
				. DIRECTORY_SEPARATOR . substr($key, 2, 2)
				. DIRECTORY_SEPARATOR . substr($key, 4, 2)
				. DIRECTORY_SEPARATOR . substr($key, 6);
	}
