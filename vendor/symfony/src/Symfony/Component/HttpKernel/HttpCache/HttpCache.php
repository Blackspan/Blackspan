...
class HttpCache implements HttpKernelInterface {
	...
	public function __construct(HttpKernelInterface $kernel,
			StoreInterface $store, Esi $esi = null, array $options = array()) {
		$this->store = $store;
		$this->kernel = $kernel;
		...
		$this->options = array_merge(
				array('debug' => false, 'default_ttl' => 0,
						'private_headers' => array('Authorization', 'Cookie'),
						'allow_reload' => false,
						'allow_revalidate' => false,
						'stale_while_revalidate' => 2,
						'stale_if_error' => 60,), $options);
		$this->esi = $esi;
		$this->traces = array();
	}
	...
	protected function validate(Request $request, Response $entry,
		...
		if (304 == $response->getStatusCode()) {
			...
			if ($etag && in_array($etag, $requestEtags)
			...
			foreach (array('Date', 'Expires', 'Cache-Control', 'ETag',
					'Last-Modified') as $name) {
	...
	protected function fetch(Request $request, $catch = false) {
		...
		} elseif ($this->options['default_ttl'] > 0
	 ...
	 * @return Response A Response instance
	 */
	protected function forward(Request $request, $catch = false,
			Response $entry = null) {
		if ($this->esi) {
	...
	protected function isFreshEnough(Request $request, Response $entry) {
		...
		if ($this->options['allow_revalidate']
				&& null
						!== $maxAge = $request->headers
	...
	private function restoreResponseBody(Request $request, Response $response) {
		...
		if ($response->headers->has('X-Body-Eval')) {
			...
			$response
					->setContent(
							file_get_contents(
									$response->headers->get('X-Body-File')));
		} else {
			return;
		}
