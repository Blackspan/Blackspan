...
class ExceptionHandler {
	...
	public function createResponse(\Exception $exception) {
		...
		$title = '';
		try {
			$code = $exception instanceof HttpExceptionInterface ? $exception
							->getStatusCode() : 500;
	...
	private function getContent($exception) {
		...
		foreach ($exception->toArray() as $position => $e) {
			...
			foreach ($e['trace'] as $i => $trace) {
				$content .= '<li>';
				if ($trace['function']) {
					$content .= sprintf('at %s%s%s()',
							$this->abbrClass($trace['class']), $trace['type'],
							$trace['function']);
				}
				if (isset($trace['file']) && isset($trace['line'])) {
					$content .= sprintf(' in %s line %s', $trace['file'],
							$trace['line']);
				}
				$content .= '</li>';
			}
...
EOF;
	...
	private function abbrClass($class) {
		...
		return sprintf("<abbr title=\"%s\">%s</abbr>", $class,
				array_pop($parts));
	}
}
