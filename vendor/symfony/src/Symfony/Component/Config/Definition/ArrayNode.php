...
class ArrayNode extends BaseNode implements PrototypeNodeInterface {
	...
	public function getDefaultValue() {
		if (!$this->hasDefaultValue()) {
			throw new \RuntimeException(
					sprintf('The node at path "%s" has no default value.',
							$this->getPath()));
		}
	...
	public function addChild(NodeInterface $node) {
		...
		if (isset($this->children[$name])) {
			throw new \InvalidArgumentException(
					sprintf('A child node named "%s" already exists.', $name));
		}
	...
	protected function finalizeValue($value) {
		if (false === $value) {
			$msg = sprintf('Unsetting key for path "%s", value: %s',
					$this->getPath(), json_encode($value));
	...
	protected function normalizeValue($value) {
		...
		if (count($value) && !$this->ignoreExtraKeys) {
			$msg = sprintf('Unrecognized options "%s" under "%s"',
					implode(', ', array_keys($value)), $this->getPath());
			$ex = new InvalidConfigurationException($msg);
			$ex->setPath($this->getPath());
	...
	protected function remapXml($value) {
		foreach ($this->xmlRemappings as $transformation) {
			...
			$value[$plural] = Processor::normalizeConfig($value, $singular,
					$plural);
