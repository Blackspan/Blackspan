...
class XmlDumper extends Dumper {
	...
	public function dump(array $options = array()) {
		...
		$container = $this->document
				->createElementNS('http://symfony.com/schema/dic/services',
						'container');
		$container
	...
	private function addService($definition, $id, \DOMElement $parent) {
		...
		if ($definition->getFactoryMethod()) {
			$service
					->setAttribute('factory-method',
							$definition->getFactoryMethod());
		}
		if ($definition->getFactoryService()) {
			$service
					->setAttribute('factory-service',
							$definition->getFactoryService());
		}
		if (ContainerInterface::SCOPE_CONTAINER
				...
				$configurator
						->setAttribute(
								(is_object($callable[0])
										&& $callable[0] instanceof Reference ? 'service'
										: 'class'), $callable[0]);
	 ...
	 * @return void
	 */
	private function convertParameters($parameters, $type, \DOMElement $parent,
			$keyAttribute = 'key') {
		$withKeys = array_keys($parameters)
