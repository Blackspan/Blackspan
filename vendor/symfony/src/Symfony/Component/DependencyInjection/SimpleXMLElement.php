...
class SimpleXMLElement extends \SimpleXMLElement {
	...
	public function getArgumentsAsPhp($name, $lowercase = true) {
		...
		foreach ($this->$name as $arg) {
			...
			$key = isset($arg['key']) ? (string) $arg['key']
					: (!$arguments ? 0 : max(array_keys($arguments)) + 1);
	...
	static public function phpize($value) {
		...
		case is_numeric($value):
			return '0x' == $value[0] . $value[1] ? hexdec($value)
					: floatval($value);
