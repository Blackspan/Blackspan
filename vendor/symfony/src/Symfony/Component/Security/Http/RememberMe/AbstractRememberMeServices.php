 ...
 */
abstract class AbstractRememberMeServices implements
		RememberMeServicesInterface, LogoutHandlerInterface {
	 ...
	 */
	public function __construct(array $userProviders, $key, $providerKey,
		...
		if (0 === count($userProviders)) {
			throw new \InvalidArgumentException(
					'You must provide at least one user provider.');
		}
		...
		try {
			...
			if (!$user instanceof UserInterface) {
				throw new \RuntimeException(
						'processAutoLoginCookie() must return a UserInterface implementation.');
			}
			...
			if (null !== $this->logger) {
				$this->logger
						->warn(
								'User class for remember-me cookie not supported.');
			}
	 ...
	 * @return void
	 */
	public final function loginSuccess(Request $request, Response $response,
			TokenInterface $token) {
		if (!$token->getUser() instanceof UserInterface) {
			if (null !== $this->logger) {
				$this->logger
						->debug(
								'Remember-me ignores token since it does not contain a UserInterface implementation.');
			}
	 ...
	 * @return TokenInterface
	 */
	abstract protected function processAutoLoginCookie(array $cookieParts,
			Request $request);
	 ...
	 * @return void
	 */
	abstract protected function onLoginSuccess(Request $request,
			Response $response, TokenInterface $token);
	...
	protected final function getUserProvider($class) {
		...
		throw new UnsupportedUserException(
				sprintf('There is no user provider that supports class "%s".',
						$class));
	}
	...
	protected function cancelCookie(Request $request) {
		if (null !== $this->logger) {
			$this->logger
					->debug(
							sprintf('Clearing remember-me cookie "%s"',
									$this->options['name']));
		}

		$request->attributes
				->set(self::COOKIE_ATTR_NAME,
						new Cookie($this->options['name'], null, 1,
								$this->options['path'],
								$this->options['domain']));
	}
	...
	protected function isRememberMeRequested(Request $request) {
		...
		$parameter = $request->request
				->get($this->options['remember_me_parameter'], null, true);
		...
		return $parameter === 'true' || $parameter === 'on'
				|| $parameter === '1' || $parameter === 'yes';
	}
}
