...
class PersistentTokenBasedRememberMeServices extends AbstractRememberMeServices {
	...
	protected function processAutoLoginCookie(array $cookieParts,
		...
		if ($persistentToken->getLastUsed()->getTimestamp()
		...
		$this->tokenProvider
				->updateToken($series, $tokenValue, new \DateTime());
		$request->attributes
				->set(self::COOKIE_ATTR_NAME,
						new Cookie($this->options['name'],
								$this
										->encodeCookie(
												array($series, $tokenValue)),
		...
		return $this->getUserProvider($persistentToken->getClass())
				->loadUserByUsername($persistentToken->getUsername());
	}
		...
		$this->tokenProvider
				->createNewToken(
						new PersistentToken(
								...
								$user->getUsername(), $series, $tokenValue,
								new \DateTime()));
