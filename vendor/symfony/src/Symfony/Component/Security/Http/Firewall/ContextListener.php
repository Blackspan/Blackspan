...
class ContextListener implements ListenerInterface {
	...
	public function __construct(SecurityContext $context, array $userProviders,
			$contextKey, LoggerInterface $logger = null,
	...
	public function handle(GetResponseEvent $event) {
		...
		$session = $request->hasPreviousSession() ? $request->getSession()
				: null;

		if (null === $session
				|| null
						=== $token = $session
								->get('_security_' . $this->contextKey)) {
	...
	private function refreshUser(TokenInterface $token) {
			...
			try {
				...
				if (null !== $this->logger) {
					$this->logger
							->debug(
									sprintf(
											'Username "%s" was reloaded from user provider.',
											$user->getUsername()));
				}
		...
		throw new \RuntimeException(
				sprintf('There is no user provider for user "%s".',
						get_class($user)));
	}
}
