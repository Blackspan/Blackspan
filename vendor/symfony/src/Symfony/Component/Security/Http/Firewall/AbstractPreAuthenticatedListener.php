...
abstract class AbstractPreAuthenticatedListener implements ListenerInterface {
	...
	public final function handle(GetResponseEvent $event) {
		...
		if (null !== $this->logger) {
			$this->logger
					->debug(
							sprintf('Checking secure context token: %s',
		...
		if (null !== $this->logger) {
			$this->logger
					->debug(
							sprintf('Trying to pre-authenticate user "%s"',
									$user));
		}
		...
		try {
			$token = $this->authenticationManager
					->authenticate(
							new PreAuthenticatedToken($user, $credentials,
									$this->providerKey));

			if (null !== $this->logger) {
				$this->logger
						->info(sprintf('Authentication success: %s', $token));
			}
			...
			if (null !== $this->dispatcher) {
				...
				$this->dispatcher
						->dispatch(SecurityEvents::INTERACTIVE_LOGIN,
								$loginEvent);
			}
