...
class DigestAuthenticationEntryPoint implements AuthenticationEntryPointInterface {
	...
	public function __construct($realmName, $key, $nonceValiditySeconds = 300,
	...
	public function start(Request $request,
		...
		$authenticateHeader = sprintf(
				'Digest realm="%s", qop="auth", nonce="%s"', $this->realmName,
				$nonceValueBase64);
		...
		if (null !== $this->logger) {
			$this->logger
					->debug(
							sprintf(
									'WWW-Authenticate header sent to user agent: "%s"',
									$authenticateHeader));
		}
		...
		$response
				->setStatusCode(401,
						$authException ? $authException->getMessage() : null);
