...
class HttpUtils {
	 ...
	 */
	public function createRedirectResponse(Request $request, $path,
			$status = 302) {
		if ('/' === $path[0]) {
	...
	public function createRequest(Request $request, $path) {
		...
		$newRequest = Request::create($path, 'get', array(),
		...
		if ($request->attributes
				->has(SecurityContextInterface::AUTHENTICATION_ERROR)) {
		...
		if ($request->attributes
				->has(SecurityContextInterface::ACCESS_DENIED_ERROR)) {
			$newRequest->attributes
					->set(SecurityContextInterface::ACCESS_DENIED_ERROR,
							$request->attributes
									->get(
		...
		if ($request->attributes->has(SecurityContextInterface::LAST_USERNAME)) {
			$newRequest->attributes
					->set(SecurityContextInterface::LAST_USERNAME,
							$request->attributes
									->get(
	...
	private function generateUrl($route, $absolute = false) {
		if (null === $this->router) {
			throw new \LogicException(
					'You must provide a RouterInterface instance to be able to use routes.');
		}
