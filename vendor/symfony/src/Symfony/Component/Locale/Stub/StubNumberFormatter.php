...
class StubNumberFormatter {
	 ...
	 * @var array
	 */
	protected $errorMessages = array(self::U_ZERO_ERROR => 'U_ZERO_ERROR',
	 ...
	 * @var array
	 */
	private $attributes = array(self::FRACTION_DIGITS => 0,
	 ...
	 * @var array
	 */
	static private $supportedStyles = array('CURRENCY' => self::CURRENCY,
	 ...
	 * @var array
	 */
	static private $int32Range = array('positive' => 2147483647,
			'negative' => -2147483648);
	 ...
	 * @var array
	 */
	static private $int64Range = array('positive' => 9223372036854775807,
	...
	public function __construct($locale = 'en', $style = null, $pattern = null) {
		if ('en' != $locale) {
			throw new MethodArgumentValueNotImplementedException(__METHOD__,
					'locale', $locale, 'Only the \'en\' locale is supported');
		}

		if (!in_array($style, self::$supportedStyles)) {
			$message = sprintf('The available styles are: %s.',
		...
		if (null !== $pattern) {
			throw new MethodArgumentNotImplementedException(__METHOD__,
					'pattern');
		}
	 ...
	 * @throws MethodArgumentNotImplementedException       When the pattern value is different than null
	 */
	static public function create($locale = 'en', $style = null,
			$pattern = null) {
	...
	public function format($value, $type = self::TYPE_DEFAULT) {
		...
		if ($type == self::TYPE_CURRENCY) {
			trigger_error(__METHOD__ . '(): Unsupported format type ' . $type,
					\E_USER_WARNING);
		...
		if ($this->style == self::CURRENCY) {
			throw new \RuntimeException(
					sprintf(
							...
							__METHOD__,
							NotImplementedException::INTL_INSTALL_MESSAGE));
		}
	...
	public function setAttribute($attr, $value) {
		if (!in_array($attr, self::$supportedAttributes)) {
			$message = sprintf('The available attributes are: %s',
		...
		if (self::$supportedAttributes['ROUNDING_MODE'] == $attr
				&& $this->isInvalidRoundingMode($value)) {
			$message = sprintf(
	...
	private function round($value, $precision) {
		...
		$roundingMode = self::$phpRoundingMap[$this
				->getAttribute(self::ROUNDING_MODE)];
	...
	private function formatNumber($value, $precision) {
		...
		return number_format($value, $precision, '.',
				$this->getAttribute(self::GROUPING_USED) ? ',' : '');
	}
	...
	private function getInt32Value($value) {
		if ($value > self::$int32Range['positive']
	...
	private function getInt64Value($value) {
		if ($value > self::$int64Range['positive']
				|| $value < self::$int64Range['negative']) {
			return false;
		}

		if (PHP_INT_SIZE !== 8
				&& ($value > self::$int32Range['positive']
						|| $value <= self::$int32Range['negative'])) {
			return (float) $value;
		}
