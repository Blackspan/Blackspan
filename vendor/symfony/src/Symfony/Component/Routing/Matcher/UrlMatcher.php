...
class UrlMatcher implements UrlMatcherInterface {
	...
	public function match($pathinfo) {
		...
		throw 0 < count($this->allow) ? new MethodNotAllowedException(
						array_unique(array_map('strtoupper', $this->allow)))
	...
	protected function matchCollection($pathinfo, RouteCollection $routes) {
		...
		foreach ($routes as $name => $route) {
			if ($route instanceof RouteCollection) {
				if (false === strpos($route->getPrefix(), '{')
						&& $route->getPrefix()
								!== substr($pathinfo, 0,
			...
			return array_merge(
					$this->mergeDefaults($matches, $route->getDefaults()),
					array('_route' => $name));
		}
	}
