...
class UrlGenerator implements UrlGeneratorInterface {
	...
	public function generate($name, $parameters = array(), $absolute = false) {
		...
		return $this
				->doGenerate($this->cache[$name]->getVariables(),
						$route->getDefaults(), $route->getRequirements(),
						$this->cache[$name]->getTokens(), $parameters, $name,
						$absolute);
	}
	...
	protected function doGenerate($variables, $defaults, $requirements, $tokens,
		...
		$parameters = array_replace($this->context->getParameters(),
				$parameters);
		...
		foreach ($tokens as $token) {
			if ('variable' === $token[0]) {
				if (false === $optional
						...
						|| (isset($parameters[$token[3]])
								&& (string) $parameters[$token[3]]
										!= (string) $defaults[$token[3]])) {
					if (!$isEmpty = in_array($tparams[$token[3]],
					...
					if (!$isEmpty || !$optional) {
						$url = $token[1]
								. strtr(rawurlencode($tparams[$token[3]]),
										$this->decodedChars) . $url;
					}
		...
		if ($this->context->getHost()) {
			...
			if (isset($requirements['_scheme'])
					&& ($req = strtolower($requirements['_scheme']))
					&& $scheme != $req) {
				$absolute = true;
				$scheme = $req;
			}
			...
			if ($absolute) {
				...
				$url = $scheme . '://' . $this->context->getHost() . $port
						. $url;
			}
		}
