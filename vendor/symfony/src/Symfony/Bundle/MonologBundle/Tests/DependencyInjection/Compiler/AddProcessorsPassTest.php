...
class AddProcessorsPassTest extends TestCase {
	public function testHandlerProcessors() {
		...
		$this
				->assertEquals(
						array('pushProcessor', array(new Reference('test'))),
						$calls[0]);
		...
		$this
				->assertEquals(
						array('pushProcessor', array(new Reference('test2'))),
						$calls[0]);
	}
	...
	protected function getContainer() {
		...
		$container
				->setDefinition('monolog.handler.test',
						new Definition('%monolog.handler.null.class%',
								array(100, false)));
		$container
				->setDefinition('handler_test',
						new Definition('%monolog.handler.null.class%',
								array(100, false)));
		...
		$definition
				->addMethodCall('pushHandler',
						array(new Reference('monolog.handler.test2')));
