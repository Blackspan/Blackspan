...
abstract class SecurityExtensionTest extends \PHPUnit_Framework_TestCase {
	...
	public function testRolesHierarchy() {
		...
		$this
				->assertEquals(
						array('ROLE_ADMIN' => array('ROLE_USER'),
								'ROLE_SUPER_ADMIN' => array('ROLE_USER',
										'ROLE_ADMIN', 'ROLE_ALLOWED_TO_SWITCH'),
								'ROLE_REMOTE' => array('ROLE_USER',
										'ROLE_ADMIN'),),
						$container
	...
	public function testUserProviders() {
		...
		$providers = array_values(
				array_filter($container->getServiceIds(),
						function ($key) {
							return 0
									=== strpos($key,
											'security.user.provider.concrete');
						}));

		$expectedProviders = array('security.user.provider.concrete.default',
		...
		$this
				->assertEquals(array(),
						array_diff($expectedProviders, $providers));
		$this
				->assertEquals(array(),
						array_diff($providers, $expectedProviders));
	...
	public function testFirewalls() {
		...
		$arguments = $container->getDefinition('security.firewall.map')
				->getArguments();
		$listeners = array();
		...
		$this
				->assertEquals(
						array(array(),
								array('security.channel_listener',
	...
	public function testAccess() {
		...
		$rules = array();
		foreach ($container->getDefinition('security.access_map')
		...
		foreach ($rules as $rule) {
			...
			if (1 === $i) {
				...
				$this
						->assertEquals(array('IS_AUTHENTICATED_ANONYMOUSLY'),
								$roles);
	...
	public function testMerge() {
		...
		$this
				->assertEquals(
						array('FOO' => array('MOO'), 'ADMIN' => array('USER'),),
						$container
								->getParameter('security.role_hierarchy.roles'));
	}
						...
						array(
								array(
										...
										'JMS\FooBundle\Entity\User3' => array(
												...
												'arguments' => array('md5',
														true, 5000),),
										'JMS\FooBundle\Entity\User4' => new Reference(
												'security.encoder.foo'),)),
						$container
								->getDefinition(
	...
	public function testAcl() {
		...
		$this
				->assertTrue(
						$container->hasDefinition('security.acl.dbal.provider'));
		$this
				->assertEquals('security.acl.dbal.provider',
						(string) $container->getAlias('security.acl.provider'));
	}
