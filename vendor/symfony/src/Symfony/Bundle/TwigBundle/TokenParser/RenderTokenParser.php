...
class RenderTokenParser extends \Twig_TokenParser {
	...
	public function parse(\Twig_Token $token) {
		...
		if ($this->parser->getStream()->test(\Twig_Token::NAME_TYPE, 'with')) {
			...
			$hasAttributes = true;
			$attributes = $this->parser->getExpressionParser()
					->parseExpression();
		} else {
			$hasAttributes = false;
			$attributes = new \Twig_Node_Expression_Array(array(),
					$token->getLine());
		}
		...
		if ($hasAttributes
			...
			$options = new \Twig_Node_Expression_Array(array(),
					$token->getLine());
		}

		$this->parser->getStream()->expect(\Twig_Token::BLOCK_END_TYPE);

		return new RenderNode($expr, $attributes, $options, $token->getLine(),
				$this->getTag());
	}
