...
class ProfilerPassTest extends \PHPUnit_Framework_TestCase {
	...
	public function testTemplateNoIdThrowsException() {
		...
		$builder->expects($this->atLeastOnce())->method('findTaggedServiceIds')
	...
	public function testValidCollector() {
		// one service, with a template key, but no id
		$services = array(
				'my_collector_service' => array(
						0 => array('template' => 'foo', 'id' => 'my_collector')),);
		...
		$container->expects($this->once())->method('setParameter')
				->with('data_collector.templates',
						array(
								'my_collector_service' => array(
										'my_collector', 'foo')));
