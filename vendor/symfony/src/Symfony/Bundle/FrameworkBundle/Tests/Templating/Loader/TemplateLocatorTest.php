...
class TemplateLocatorTest extends TestCase {
	public function testLocateATemplate() {
		$template = new TemplateReference('bundle', 'controller', 'name',
				'format', 'engine');

		$fileLocator = $this->getFileLocator();

		$fileLocator->expects($this->once())->method('locate')
	...
	public function testThrowsExceptionWhenTemplateNotFound() {
		...
		try {
			...
			$this
					->fail(
							'->locate() should throw an exception when the file is not found.');
	...
	protected function getFileLocator() {
		return $this->getMockBuilder('Symfony\Component\Config\FileLocator')
				->setMethods(array('locate'))
				->setConstructorArgs(array('/path/to/fallback'))->getMock();
	}
}
