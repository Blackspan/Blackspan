...
class RouterListener {
	...
	public function __construct(RouterInterface $router, $httpPort = 80,
			$httpsPort = 443, LoggerInterface $logger = null) {
		$this->router = $router;
	...
	public function onEarlyKernelRequest(GetResponseEvent $event) {
		...
		$context
				->setHttpPort(
						$request->isSecure() ? $this->httpPort
								: $request->getPort());
		$context
				->setHttpsPort(
	...
	public function onKernelRequest(GetResponseEvent $event) {
		...
		try {
			...
			if (null !== $this->logger) {
				$this->logger
						->info(
								sprintf('Matched route "%s" (parameters: %s)',
										$parameters['_route'],
			...
			$message = sprintf('No route found for "%s %s"',
			...
			$message = sprintf(
					...
					$request->getMethod(), $request->getPathInfo(),
					strtoupper(implode(', ', $e->getAllowedMethods())));
