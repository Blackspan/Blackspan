...
class FormHelper extends Helper {
	 ...
	 */
	protected function renderSection(FormView $view, $section,
			array $variables = array()) {
		...
		do {
			...
			if ($template) {
				...
				$this->context[] = array('variables' => $variables,
						'view' => $view,);
	...
	public function renderBlock($name, $variables = array()) {
		if (0 == count($this->context)) {
			throw new FormException(
					sprintf(
							'This method should only be called while rendering a form element.',
							$name));
		}
		...
		if (false === $template) {
			throw new FormException(
					sprintf('No block "%s" found while rendering the form.',
							$name));
		}
	...
	protected function lookupTemplate(FormView $view, $block) {
		...
		if (!isset($this->templates[$id][$block])) {
			...
			for ($i = count($themes) - 1; $i >= 0; --$i) {
				if ($this->engine
						->exists($templateName = $themes[$i] . ':' . $file)) {
