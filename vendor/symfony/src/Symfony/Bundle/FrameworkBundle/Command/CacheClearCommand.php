...
EOF
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		if (!is_writable($realCacheDir)) {
			throw new \RuntimeException(
					sprintf('Unable to write in the "%s" directory',
							$realCacheDir));
		}
	...
	protected function warmup($warmupDir) {
		...
		$finder = new Finder();
		foreach ($finder->files()->name(get_class($kernel->getContainer())
						. '*')->in($warmupDir) as $file) {
			$content = file_get_contents($file);
			$content = preg_replace($regex, '', $content);

			// fix absolute paths to the cache directory
			$content = preg_replace('/' . preg_quote($warmupDir, '/') . '/',
					preg_replace('/_new$/', '', $warmupDir), $content);
		...
		foreach ($finder->files()->name('*.meta')->in($warmupDir) as $file) {
			$content = preg_replace(
					'/C\:\d+\:"'
							. preg_quote($class . $this->getTempKernelSuffix(),
									'"/') . '"/',
	...
	protected function getTempKernel(KernelInterface $parent, $namespace,
			$class, $warmupDir) {
