...
EOF
	...
	protected function outputRoutes(OutputInterface $output, $routes) {
		...
		foreach ($routes as $name => $route) {
			...
			$method = isset($requirements['_method']) ? strtoupper(
							is_array($requirements['_method']) ? implode(', ',
	...
	protected function outputRoute(OutputInterface $output, $routes, $name) {
		$output
				->writeln(
						$this->getHelper('formatter')
								->formatSection('router',
										sprintf('Route "%s"', $name)));
		...
		$output
				->writeln(
						sprintf('<comment>Pattern</comment>      %s',
								$route->getPattern()));
		$output
				->writeln(
		...
		$output
				->writeln(
						sprintf('<comment>Defaults</comment>     %s', $defaults));
		...
		$output
				->writeln(
						sprintf('<comment>Requirements</comment> %s',
								$requirements));
		...
		$output
				->writeln(
						sprintf('<comment>Options</comment>      %s', $options));
