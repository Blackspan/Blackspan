...
class TemplateFinder implements TemplateFinderInterface {
	...
	public function __construct(KernelInterface $kernel,
			TemplateNameParser $parser, $rootDir) {
		$this->kernel = $kernel;
		$this->parser = $parser;
	...
	public function findAllTemplates() {
		...
		foreach ($this->kernel->getBundles() as $name => $bundle) {
			$templates = array_merge($templates,
					$this->findTemplatesInBundle($bundle));
		}

		$templates = array_merge($templates,
				$this->findTemplatesInFolder($this->rootDir . '/views'));
		...
		if (is_dir($dir)) {
			...
			foreach ($finder->files()->followLinks()->in($dir) as $file) {
				$template = $this->parser
						->parseFromFilename($file->getRelativePathname());
				if (false !== $template) {
					$templates[] = $template;
				}
			}
	...
	private function findTemplatesInBundle(BundleInterface $bundle) {
		$templates = $this
				->findTemplatesInFolder($bundle->getPath() . '/Resources/views');
		$name = $bundle->getName();
