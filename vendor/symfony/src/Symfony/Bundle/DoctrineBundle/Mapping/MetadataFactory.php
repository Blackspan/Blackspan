...
class MetadataFactory {
	...
	public function getBundleMetadata(BundleInterface $bundle) {
		...
		if (!$metadata->getMetadata()) {
			throw new \RuntimeException(
					sprintf(
							'Bundle "%s" does not contain any mapped entities.',
							$bundle->getName()));
		}

		$path = $this
				->getBasePathForClass($bundle->getName(),
						$bundle->getNamespace(), $bundle->getPath());
	...
	public function getClassMetadata($class, $path = null) {
		...
		if (class_exists($class)) {
			...
			$path = $this
					->getBasePathForClass($class, $r->getNamespaceName(),
							dirname($r->getFilename()));
			$metadata->setNamespace($r->getNamespacename());
		} elseif (!$path) {
			throw new \RuntimeException(
					sprintf(
							'Unable to determine where to save the "%s" class (use the --path option).',
							$class));
		}
	...
	public function getNamespaceMetadata($namespace, $path = null) {
		...
		if (!$metadata->getMetadata()) {
			throw new \RuntimeException(
					sprintf(
							'Namespace "%s" does not contain any mapped entities.',
							$namespace));
		}
		...
		if (class_exists($all[0]->name)) {
			$r = new \ReflectionClass($all[0]->name);
			$path = $this
					->getBasePathForClass($namespace, $r->getNamespaceName(),
							dirname($r->getFilename()));
		} elseif (!$path) {
			throw new \RuntimeException(
					sprintf(
	...
	private function getBasePathForClass($name, $namespace, $path) {
		...
		if ($c != 1) {
			throw new \RuntimeException(
					sprintf(
							'Can\'t find base path for "%s" (path: "%s", destination: "%s").',
