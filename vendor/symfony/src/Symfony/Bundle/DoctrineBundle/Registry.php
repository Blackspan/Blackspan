...
class Registry implements RegistryInterface {
	...
	public function __construct(ContainerInterface $container,
			array $connections, array $entityManagers, $defaultConnection,
	...
	public function getConnection($name = null) {
		...
		if (!isset($this->connections[$name])) {
			throw new \InvalidArgumentException(
					sprintf('Doctrine Connection named "%s" does not exist.',
							$name));
		}
	...
	public function getEntityManager($name = null) {
		...
		if (!isset($this->entityManagers[$name])) {
			throw new \InvalidArgumentException(
					sprintf(
							'Doctrine EntityManager named "%s" does not exist.',
							$name));
		}
	...
	public function getEntityNamespace($alias) {
		foreach (array_keys($this->entityManagers) as $name) {
			try {
				return $this->getEntityManager($name)->getConfiguration()
						->getEntityNamespace($alias);
	...
	public function getEntityManagerForClass($class) {
		...
		foreach ($this->entityManagers as $id) {
			...
			if (!$em->getConfiguration()->getMetadataDriverImpl()
					->isTransient($class)) {
				return $em;
			}
		}
