...
class DoctrineBundle extends Bundle {
	...
	public function boot() {
		...
		if ($this->container->hasParameter('doctrine.orm.proxy_namespace')) {
			...
			$this->autoloader = function ($class) use ($namespace, $dir,
					&$container) {
				if (0 === strpos($class, $namespace)) {
					...
					if (!file_exists($file)
						...
						if (1 === Version::compare('2.2.0')) {
							...
							$originalClassName = ClassUtils::getRealClass(
									$className);
						...
						foreach ($registry->getEntityManagers() as $em) {
							...
							if ($em->getConfiguration()
									->getAutoGenerateProxyClasses()) {
								$classes = $em->getMetadataFactory()
										->getAllMetadata();
						...
						if (!file_exists($file)) {
							throw new \RuntimeException(
									sprintf(
											'The proxy file "%s" does not exist. If you still have objects serialized in the session, you need to clear the session manually.',
											$file));
						}
					}
