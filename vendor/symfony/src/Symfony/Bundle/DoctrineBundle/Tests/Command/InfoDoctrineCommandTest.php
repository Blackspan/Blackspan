...
require_once __DIR__
		. '/../DependencyInjection/Fixtures/Bundles/YamlBundle/Entity/Test.php';
...
class InfoDoctrineCommandTest extends TestCase {
	public function testAnnotationsBundle() {
		...
		$output = $this
				->getMock('Symfony\Component\Console\Output\OutputInterface');
		$output->expects($this->at(0))->method('writeln')
				->with(
						$this
								->equalTo(
										'Found <info>1</info> entities mapped in entity manager <info>default</info>:'));
		$output->expects($this->at(1))->method('writeln')
				->with(
						$this
								->equalTo(
										'<info>[OK]</info>   Fixtures\Bundles\YamlBundle\Entity\Test'));
