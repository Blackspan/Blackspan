...
class XMLSchemaTest extends \PHPUnit_Framework_TestCase {
	static public function dataValidateSchemaFiles() {
		...
		foreach ($di as $element) {
			if ($element->isFile()
					&& strpos($element->getFilename(), ".xml") !== false) {
	...
	public function testValidateSchema($file) {
		...
		$dbalElements = $dom
				->getElementsByTagNameNS(
						"http://symfony.com/schema/dic/doctrine", "config");
		if ($dbalElements->length) {
			...
			$this
					->assertTrue($ret,
							"DoctrineBundle Dependency Injection XMLSchema did not validate this XML instance.");
			$found = true;
		}
		...
		$this
				->assertTrue($found,
						"Neither <doctrine:orm> nor <doctrine:dbal> elements found in given XML. Are namespaces configured correctly?");
	}
}
