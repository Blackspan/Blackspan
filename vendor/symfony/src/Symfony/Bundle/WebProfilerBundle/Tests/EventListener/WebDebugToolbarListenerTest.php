...
class WebDebugToolbarListenerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testRedirectionIsIntercepted() {
		foreach (array(301, 302) as $statusCode) {
			...
			$event = new FilterResponseEvent($this->getKernelMock(),
					$this->getRequestMock(),
					HttpKernelInterface::MASTER_REQUEST, $response);

			$listener = new WebDebugToolbarListener(
					$this->getTemplatingMock('Redirection'), true);
	...
	public function testToolbarIsInjected() {
		...
		$event = new FilterResponseEvent($this->getKernelMock(),
				$this->getRequestMock(), HttpKernelInterface::MASTER_REQUEST,
				$response);
	...
	public function testToolbarIsNotInjectedOnRedirection() {
		foreach (array(301, 302) as $statusCode) {
			$response = new Response(
					'<html><head></head><body></body></html>', $statusCode);
			$response->headers->set('X-Debug-Token', 'xxxxxxxx');
			$event = new FilterResponseEvent($this->getKernelMock(),
					$this->getRequestMock(),
					HttpKernelInterface::MASTER_REQUEST, $response);
	...
	public function testToolbarIsNotInjectedWhenOnSubRequest() {
		...
		$event = new FilterResponseEvent($this->getKernelMock(),
				$this->getRequestMock(), HttpKernelInterface::SUB_REQUEST,
				$response);
	...
	protected function getRequestMock($isXmlHttpRequest = false,
			$requestFormat = 'html') {
		$session = $this
				->getMock('Symfony\Component\HttpFoundation\Session', array(),
						array(), '', false);
		$request = $this
				->getMock('Symfony\Component\HttpFoundation\Request',
						array('getSession', 'isXmlHttpRequest',
								'getRequestFormat'), array(), '', false);
		$request->expects($this->any())->method('isXmlHttpRequest')
				->will($this->returnValue($isXmlHttpRequest));
		$request->expects($this->any())->method('getRequestFormat')
				->will($this->returnValue($requestFormat));
		$request->expects($this->any())->method('getSession')
	...
	protected function getTemplatingMock($render = 'WDT') {
		$templating = $this
				->getMock('Symfony\Bundle\TwigBundle\TwigEngine', array(),
						array(), '', false);
		$templating->expects($this->any())->method('render')
	...
	protected function getKernelMock() {
		return $this
				->getMock('Symfony\Component\HttpKernel\Kernel', array(),
						array(), '', false);
	}
}
