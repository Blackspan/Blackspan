...
class ProfilerController extends ContainerAware {
	...
	public function panelAction($token) {
		...
		$panel = $this->container->get('request')->query
				->get('panel', 'request');
		...
		if (!$profile->hasCollector($panel)) {
			throw new NotFoundHttpException(
					sprintf('Panel "%s" is not available for token "%s".',
							$panel, $token));
		}

		return $this->container->get('templating')
				->renderResponse($this->getTemplateName($profiler, $panel),
						array('token' => $token, 'profile' => $profile,
	...
	public function exportAction($token) {
		...
		return new Response($profiler->export($profile), 200,
	...
	public function purgeAction() {
		...
		return new RedirectResponse(
				$this->container->get('router')
						->generate('_profiler', array('token' => '-')));
	}
	...
	public function importAction() {
		...
		if (!$profile = $profiler
				->import(file_get_contents($file->getPathname()))) {
			throw new \RuntimeException(
					'Problem uploading the data (token already exists).');
		}

		return new RedirectResponse(
				$this->container->get('router')
						->generate('_profiler',
								array('token' => $profile->getToken())));
	}
	...
	public function toolbarAction($token, $position = null) {
		...
		if (null === $position) {
			$position = false
					=== strpos(
							$this->container->get('request')->headers
									->get('user-agent'), 'Mobile') ? 'fixed'
					: 'absolute';
		}
		...
		try {
			$url = $this->container->get('router')
					->generate('_profiler', array('token' => $token));
		} catch (\Exception $e) {
	...
	public function searchBarAction() {
		...
		return $this->container->get('templating')
				->renderResponse(
						...
						array('token' => $token, 'ip' => $ip, 'url' => $url,
								'limit' => $limit,));
	}
	...
	public function searchAction() {
		...
		return new RedirectResponse(
				$this->container->get('router')
						->generate('_profiler_search_results',
								array(
	...
	protected function getTemplateNames($profiler) {
		...
		foreach ($this->container->getParameter('data_collector.templates') as $id => $arguments) {
			...
			if (!$this->container->get('templating')
	...
	protected function getTemplateName($profiler, $panel) {
		...
		if (!isset($templates[$panel])) {
			throw new NotFoundHttpException(
					sprintf('Panel "%s" is not registered.', $panel));
		}
	...
	protected function getTemplates($profiler) {
		...
		foreach ($templates as $name => $template) {
			$templates[$name] = $this->container->get('twig')
