...
class UniqueEntityValidator extends ConstraintValidator {
	...
	public function isValid($entity, Constraint $constraint) {
		...
		foreach ($fields as $fieldName) {
			...
			$criteria[$fieldName] = $class->reflFields[$fieldName]
					->getValue($entity);
			...
			if (isset($class->associationMappings[$fieldName])) {
				...
				$relatedId = $relatedClass
						->getIdentifierValues($criteria[$fieldName]);
		 ...
		 */
		if (0 === count($result)
				|| (1 === count($result) && $entity === $result[0])) {
			return true;
		}
		...
		$this->context
				->setPropertyPath(
						empty($oldPath) ? $fields[0] : $oldPath . '.'
								. $fields[0]);
		$this->context
