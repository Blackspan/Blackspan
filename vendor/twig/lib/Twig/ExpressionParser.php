...
class Twig_ExpressionParser {
	...
	public function __construct(Twig_Parser $parser, array $unaryOperators,
	...
	public function parseExpression($precedence = 0) {
		...
		while ($this->isBinary($token)
			...
			if (isset($op['callable'])) {
				...
				$expr1 = $this
						->parseExpression(
								self::OPERATOR_LEFT === $op['associativity'] ? $op['precedence']
												+ 1 : $op['precedence']);
				$class = $op['class'];
	...
	protected function parseConditionalExpression($expr) {
		while ($this->parser->getStream()
				->test(Twig_Token::PUNCTUATION_TYPE, '?')) {
			$this->parser->getStream()->next();
			$expr2 = $this->parseExpression();
			$this->parser->getStream()
					->expect(Twig_Token::PUNCTUATION_TYPE, ':',
							'The ternary operator must have a default value');
	...
	public function parsePrimaryExpression() {
		...
		case Twig_Token::NAME_TYPE:
			...
			case 'true':
			case 'TRUE':
				$node = new Twig_Node_Expression_Constant(true,
						$token->getLine());
				break;

			case 'false':
			case 'FALSE':
				$node = new Twig_Node_Expression_Constant(false,
						$token->getLine());
				break;
			...
			case 'null':
			case 'NULL':
				$node = new Twig_Node_Expression_Constant(null,
						$token->getLine());
				break;
			...
			default:
				if ('(' === $this->parser->getCurrentToken()->getValue()) {
					$node = $this
							->getFunctionNode($token->getValue(),
									$token->getLine());
				} else {
					$node = new Twig_Node_Expression_Name($token->getValue(),
							$token->getLine());
				}
			}
			break;
		...
		case Twig_Token::NUMBER_TYPE:
			$this->parser->getStream()->next();
			$node = new Twig_Node_Expression_Constant($token->getValue(),
					$token->getLine());
			break;
		...
		default:
			if ($token->test(Twig_Token::PUNCTUATION_TYPE, '[')) {
				...
				throw new Twig_Error_Syntax(
						sprintf('Unexpected token "%s" of value "%s"',
								Twig_Token::typeToEnglish($token->getType(),
										$token->getLine()), $token->getValue()),
						$token->getLine());
			}
		}
	...
	public function parseStringExpression() {
		...
		while (true) {
			if ($stream->test(Twig_Token::STRING_TYPE) && $nextCanBeString) {
				$token = $stream->next();
				$nodes[] = new Twig_Node_Expression_Constant(
		...
		foreach ($nodes as $node) {
			$expr = new Twig_Node_Expression_Binary_Concat($expr, $node,
					$node->getLine());
		}
	...
	public function parseArrayExpression() {
		...
		$node = new Twig_Node_Expression_Array(array(),
	...
	public function parseHashExpression() {
		...
		while (!$stream->test(Twig_Token::PUNCTUATION_TYPE, '}')) {
			...
			if ($stream->test(Twig_Token::STRING_TYPE)
				...
				$token = $stream->next();
				$key = new Twig_Node_Expression_Constant($token->getValue(),
						$token->getLine());
				...
				throw new Twig_Error_Syntax(
						sprintf(
								...
								Twig_Token::typeToEnglish($current->getType(),
										$current->getLine()),
								$current->getValue()), $current->getLine());
			}
	...
	public function getFunctionNode($name, $line) {
		...
		case 'parent':
			if (!count($this->parser->getBlockStack())) {
				throw new Twig_Error_Syntax(
						'Calling "parent" outside a block is forbidden', $line);
			}
			...
			return new Twig_Node_Expression_Parent(
					$this->parser->peekBlockStack(), $line);
		case 'block':
			return new Twig_Node_Expression_BlockReference($args->getNode(0),
					false, $line);
		case 'attribute':
			if (count($args) < 2) {
				throw new Twig_Error_Syntax(
			...
			return new Twig_Node_Expression_GetAttr($args->getNode(0),
					$args->getNode(1),
					count($args) > 2 ? $args->getNode(2)
							: new Twig_Node_Expression_Array(array(), $line),
					Twig_TemplateInterface::ANY_CALL, $line);
		default:
			if (null !== $alias = $this->parser->getImportedFunction($name)) {
				...
				$node = new Twig_Node_Expression_MethodCall($alias['node'],
						$alias['name'], $arguments, $line);
	...
	public function parseSubscriptExpression($node) {
		...
		if ($token->getValue() == '.') {
			...
			if ($token->getType() == Twig_Token::NAME_TYPE
					...
					|| ($token->getType() == Twig_Token::OPERATOR_TYPE
							&& preg_match(Twig_Lexer::REGEX_NAME,
									$token->getValue()))) {
				$arg = new Twig_Node_Expression_Constant($token->getValue(),
						$lineno);
			...
			if ($stream->test(Twig_Token::PUNCTUATION_TYPE, ':')) {
				...
				if ($stream->test(Twig_Token::PUNCTUATION_TYPE, ']')) {
					$length = new Twig_Node_Expression_Constant(null,
							$token->getLine());
				} else {
				...
				$filter = new $class($node,
						new Twig_Node_Expression_Constant('slice',
								$token->getLine()), $arguments,
						$token->getLine());
		...
		return new Twig_Node_Expression_GetAttr($node, $arg, $arguments, $type,
				$lineno);
	}
	...
	public function parseFilterExpressionRaw($node, $tag = null) {
		while (true) {
			...
			$name = new Twig_Node_Expression_Constant($token->getValue(),
					$token->getLine());
			if (!$this->parser->getStream()
					->test(Twig_Token::PUNCTUATION_TYPE, '(')) {
			...
			$node = new $class($node, $name, $arguments, $token->getLine(),
					$tag);
	...
	public function parseArguments() {
		...
		$stream
				->expect(Twig_Token::PUNCTUATION_TYPE, '(',
						'A list of arguments must be opened by a parenthesis');
		while (!$stream->test(Twig_Token::PUNCTUATION_TYPE, ')')) {
			if (!empty($args)) {
				$stream
						->expect(Twig_Token::PUNCTUATION_TYPE, ',',
	...
	public function parseAssignmentExpression() {
		$targets = array();
		while (true) {
			$token = $this->parser->getStream()
					->expect(Twig_Token::NAME_TYPE, null,
							'Only variables can be assigned to');
			if (in_array($token->getValue(), array('true', 'false', 'none'))) {
				throw new Twig_Error_Syntax(
						sprintf('You cannot assign a value to "%s"',
								$token->getValue()), $token->getLine());
			}
	...
	protected function getFunctionNodeClass($name) {
		...
		if (isset($functionMap[$name])
				&& $functionMap[$name] instanceof Twig_Function_Node) {
	...
	protected function getFilterNodeClass($name) {
		...
		if (isset($filterMap[$name])
				&& $filterMap[$name] instanceof Twig_Filter_Node) {
