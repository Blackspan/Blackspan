...
class Twig_Error extends Exception {
	...
	public function __construct($message, $lineno = -1, $filename = null,
		...
		if (-1 === $lineno || null === $filename) {
			list($lineno, $filename) = $this
					->findTemplateInfo(null !== $previous ? $previous : $this,
							$lineno, $filename);
		}
	...
	public function __call($method, $arguments) {
		...
		throw new BadMethodCallException(
				sprintf('Method "Twig_Error::%s()" does not exist.', $method));
	}
	...
	protected function updateRepr() {
		...
		if (null !== $this->filename) {
			$this->message .= sprintf(' in %s',
					is_string($this->filename) ? '"' . $this->filename . '"'
							: json_encode($this->filename));
		}
	...
	protected function findTemplateInfo(Exception $e, $currentLine,
			$currentFile) {
		...
		foreach ($traces as $i => $trace) {
			...
			if (0 === $i) {
				...
				$line = isset($traces[$i - 1]['line']) ? $traces[$i - 1]['line']
						: -log(0);
			}
			...
			foreach ($tokens as $token) {
				...
				if (T_COMMENT === $token[0] && null === $template
						&& preg_match('#/\* +(.+) +\*/#', $token[1], $match)) {
					$template = $match[1];
				} elseif (T_COMMENT === $token[0]
						&& preg_match('#^//\s*line (\d+)\s*$#', $token[1],
								$match)) {
