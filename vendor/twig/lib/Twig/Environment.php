...
class Twig_Environment {
	 ...
	 */
	public function __construct(Twig_LoaderInterface $loader = null,
			$options = array()) {
		if (null !== $loader) {
		...
		$options = array_merge(
				array('debug' => false, 'charset' => 'UTF-8',
						'base_template_class' => 'Twig_Template',
						'strict_variables' => false,
						'autoescape' => true, 'cache' => false,
						'auto_reload' => null, 'optimizations' => -1,),
				$options);

		$this->debug = (bool) $options['debug'];
		$this->charset = $options['charset'];
		$this->baseTemplateClass = $options['base_template_class'];
		$this->autoReload = null === $options['auto_reload'] ? $this->debug
				: (bool) $options['auto_reload'];
		$this->extensions = array('core' => new Twig_Extension_Core(),
				'escaper' => new Twig_Extension_Escaper(
						(bool) $options['autoescape']),
	...
	public function getCacheFilename($name) {
		...
		$class = substr($this->getTemplateClass($name),
				strlen($this->templateClassPrefix));

		return $this->getCache() . '/' . substr($class, 0, 2) . '/'
				. substr($class, 2, 2) . '/' . substr($class, 4) . '.php';
	}
	...
	public function getTemplateClass($name) {
		return $this->templateClassPrefix
				. md5($this->loader->getCacheKey($name));
	}
	...
	public function clearCacheFiles() {
		...
		foreach (new RecursiveIteratorIterator(
				new RecursiveDirectoryIterator($this->cache),
	...
	public function compileSource($source, $name = null) {
		try {
			return $this
					->compile($this->parse($this->tokenize($source, $name)));
		} catch (Twig_Error $e) {
	...
	public function getExtension($name) {
		if (!isset($this->extensions[$name])) {
			throw new Twig_Error_Runtime(
					sprintf('The "%s" extension is not enabled.', $name));
		}
	...
	public function getNodeVisitors() {
		if (null === $this->visitors) {
			$this->visitors = isset($this->staging['visitors']) ? $this
							->staging['visitors'] : array();
			foreach ($this->getExtensions() as $extension) {
				$this->visitors = array_merge($this->visitors,
	...
	public function getFilters() {
		if (null === $this->filters) {
			$this->filters = isset($this->staging['filters']) ? $this
							->staging['filters'] : array();
			foreach ($this->getExtensions() as $extension) {
	...
	public function getTests() {
		if (null === $this->tests) {
			$this->tests = isset($this->staging['tests']) ? $this
							->staging['tests'] : array();
			foreach ($this->getExtensions() as $extension) {
	...
	public function getFunctions() {
		if (null === $this->functions) {
			$this->functions = isset($this->staging['functions']) ? $this
							->staging['functions'] : array();
			foreach ($this->getExtensions() as $extension) {
	...
	public function getGlobals() {
		if (null === $this->globals) {
			$this->globals = isset($this->staging['globals']) ? $this
							->staging['globals'] : array();
			foreach ($this->getExtensions() as $extension) {
				$this->globals = array_merge($this->globals,
		...
		foreach ($this->getExtensions() as $extension) {
			...
			if (2 !== count($operators)) {
				throw new InvalidArgumentException(
						sprintf(
								'"%s::getOperators()" does not return a valid operators array.',
								get_class($extension)));
			}

			$this->unaryOperators = array_merge($this->unaryOperators,
					$operators[0]);
			$this->binaryOperators = array_merge($this->binaryOperators,
					$operators[1]);
		}
	}
	...
	protected function writeCacheFile($file, $content) {
		...
		if (!is_dir($dir)) {
			if (false === @mkdir($dir, 0777, true) && !is_dir($dir)) {
				throw new RuntimeException(
						sprintf("Unable to create the cache directory (%s).",
								$dir));
			}
		} elseif (!is_writable($dir)) {
			throw new RuntimeException(
					sprintf("Unable to write in the cache directory (%s).",
							$dir));
		}
		...
		if (false !== @file_put_contents($tmpFile, $content)) {
			...
			if (@rename($tmpFile, $file)
					|| (@copy($tmpFile, $file) && unlink($tmpFile))) {
				chmod($file, 0644);
