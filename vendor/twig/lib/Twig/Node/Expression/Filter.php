...
class Twig_Node_Expression_Filter extends Twig_Node_Expression {
	public function __construct(Twig_NodeInterface $node,
		...
		parent::__construct(
				array('node' => $node, 'filter' => $filterName,
						'arguments' => $arguments), array(), $lineno, $tag);
	}
	...
	public function compile(Twig_Compiler $compiler) {
		...
		if (false === $filter = $compiler->getEnvironment()->getFilter($name)) {
			...
			if ($alternatives = $compiler->getEnvironment()
					->computeAlternatives($name,
							array_keys(
									$compiler->getEnvironment()->getFilters()))) {
	...
	protected function compileFilter(Twig_Compiler $compiler,
			Twig_FilterInterface $filter) {
