...
class LazyAssetManager extends AssetManager {
	...
	public function getFormula($name) {
		...
		if (!isset($this->formulae[$name])) {
			throw new \InvalidArgumentException(
					sprintf('There is no "%s" formula.', $name));
		}
	...
	public function load() {
		...
		if ($diff = array_diff(array_keys($this->resources),
				array_keys($this->loaders))) {
			throw new \LogicException(
					'The following loader(s) are not registered: '
							. implode(', ', $diff));
		}
		...
		foreach ($this->resources as $loader => $resources) {
			foreach ($resources as $resource) {
				$this->formulae = array_replace($this->formulae,
						$this->loaders[$loader]->load($resource));
			}
		}
	...
	public function get($name) {
		...
		if (!parent::has($name) && isset($this->formulae[$name])) {
			...
			$options['name'] = $name;
			parent::set($name,
					$this->factory->createAsset($inputs, $filters, $options));
		}
	...
	public function getNames() {
		...
		return array_unique(
				array_merge(parent::getNames(), array_keys($this->formulae)));
	}
