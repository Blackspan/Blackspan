...
class CssImportFilter extends BaseCssFilter {
	...
	public function filterLoad(AssetInterface $asset) {
		...
		$callback = function ($matches) use ($importFilter, $sourceRoot,
				$sourcePath) {
			...
			if (false !== strpos($matches['url'], '://')) {
				...
				// protocol-relative
				list($importHost, $importPath) = explode('/',
						substr($matches['url'], 2), 2);
			...
			if (false !== strpos($importSource, '://')
					|| 0 === strpos($importSource, '//')) {
				$import = new HttpAsset($importSource, array($importFilter),
						true);
			} elseif (!file_exists($importSource)) {
				// ignore not found imports
				return $matches[0];
			} else {
				$import = new FileAsset($importSource, array($importFilter),
