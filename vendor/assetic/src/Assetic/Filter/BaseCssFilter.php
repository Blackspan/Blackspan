...
abstract class BaseCssFilter implements FilterInterface {
	 ...
	 */
	protected function filterReferences($content, $callback, $limit = -1,
			&$count = 0) {
	 ...
	 * @return string The filtered CSS
	 */
	protected function filterUrls($content, $callback, $limit = -1,
			&$count = 0) {
		return preg_replace_callback('/url\((["\']?)(?<url>.*?)(\\1)\)/',
				$callback, $content, $limit, $count);
	}
	 ...
	 * @return string The filtered CSS
	 */
	protected function filterImports($content, $callback, $limit = -1,
			&$count = 0, $includeUrl = true) {
		$pattern = $includeUrl ? '/@import (?:url\()?(\'|"|)(?<url>[^\'"\)\n\r]*)\1\)?;?/'
				: '/@import (?!url\()(\'|"|)(?<url>[^\'"\)\n\r]*)\1;?/';

		return preg_replace_callback($pattern, $callback, $content, $limit,
				$count);
	}
}
