...
class CssRewriteFilter extends BaseCssFilter {
	...
	public function filterDump(AssetInterface $asset) {
		...
		if (false !== strpos($sourceBase, '://')) {
			list($scheme, $url) = explode('://', $sourceBase . '/'
					. $sourcePath, 2);
			...
			if ('.' == dirname($sourcePath)) {
				...
				$path .= ltrim(
						substr(dirname($sourcePath) . '/', strlen($targetDir)),
						'/');
			}
		}

		$content = $this
				->filterReferences($asset->getContent(),
						function ($matches) use ($host, $path) {
							...
							while (0 === strpos($url, '../')
									&& 2 <= substr_count($path, '/')) {
								$path = substr($path, 0,
										strrpos(rtrim($path, '/'), '/') + 1);
								$url = substr($url, 3);
							}

							return str_replace($matches['url'],
									$host . $path . $url, $matches[0]);
						});
