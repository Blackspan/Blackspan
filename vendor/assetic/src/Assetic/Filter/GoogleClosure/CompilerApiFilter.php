...
class CompilerApiFilter extends BaseCompilerFilter {
	public function filterDump(AssetInterface $asset) {
		$query = array('js_code' => $asset->getContent(),
				'output_format' => 'json', 'output_info' => 'compiled_code',);
		...
		if (null !== $this->excludeDefaultExterns) {
			$query['exclude_default_externs'] = $this->excludeDefaultExterns ? 'true'
					: 'false';
		}
		...
		if (null !== $this->useClosureLibrary) {
			$query['use_closure_library'] = $this->useClosureLibrary ? 'true'
					: 'false';
		}
		...
		$response = file_get_contents(
				'http://closure-compiler.appspot.com/compile', false, $context);
		...
		if (isset($data->serverErrors) && 0 < count($data->serverErrors)) {
			...
			throw new \RuntimeException(
					sprintf(
							'The Google Closure Compiler API threw some server errors: '
									. print_r($data->serverErrors, true)));
		...
		if (isset($data->errors) && 0 < count($data->errors)) {
			...
			throw new \RuntimeException(
					sprintf(
							'The Google Closure Compiler API threw some errors: '
