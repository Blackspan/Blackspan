...
class CompassFilter implements FilterInterface {
	...
	public function filterLoad(AssetInterface $asset) {
		...
		$pb = new ProcessBuilder(
				array($this->compassPath, 'compile', $tempDir,));
		...
		if ($this->cacheLocation) {
			$optionsConfig['sass_options']['cache_location'] = $this
					->cacheLocation;
		}
		...
		if ($this->httpJavascriptsPath) {
			$optionsConfig['http_javascripts_path'] = $this
					->httpJavascriptsPath;
		}
		...
		if (count($optionsConfig)) {
			...
			foreach ($optionsConfig as $name => $value) {
				if (!is_array($value)) {
					$config[] = sprintf('%s = "%s"', $name,
							addcslashes($value, '\\'));
				} elseif (!empty($value)) {
					$config[] = sprintf('%s = %s', $name,
							$this->formatArrayToRuby($value));
				}
			}
	...
	private function formatArrayToRuby($array) {
		...
		if (count(array_filter(array_keys($array), "is_numeric"))
				!= count($array)) {
