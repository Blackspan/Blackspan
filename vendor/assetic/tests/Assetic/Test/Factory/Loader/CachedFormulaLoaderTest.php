...
class CachedFormulaLoaderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->configCache = $this
				->getMockBuilder('Assetic\\Cache\\ConfigCache')
	...
	public function testNotDebug() {
		$expected = array('foo' => array(array(), array(), array()),
				'bar' => array(array(), array(), array()),);

		$this->configCache->expects($this->once())->method('has')
				->with($this->isType('string'))
				->will($this->returnValue(false));
		$this->loader->expects($this->once())->method('load')
				->with($this->resource)->will($this->returnValue($expected));
		$this->configCache->expects($this->once())->method('set')
				->with($this->isType('string'), $expected);

		$loader = new CachedFormulaLoader($this->loader, $this->configCache);
		$this
				->assertEquals($expected, $loader->load($this->resource),
						'->load() returns formulae');
	}
	...
	public function testNotDebugCached() {
		...
		$this->configCache->expects($this->once())->method('get')
				->with($this->isType('string'))
	...
	public function testDebugCached() {
		...
		$this->configCache->expects($this->once())->method('has')
				->with($this->isType('string'))->will($this->returnValue(true));
		$this->configCache->expects($this->once())->method('getTimestamp')
				->with($this->isType('string'))
				->will($this->returnValue($timestamp));
		$this->resource->expects($this->once())->method('isFresh')
				->with($timestamp)->will($this->returnValue(true));
	...
	public function testDebugCachedStale() {
		...
		$this->configCache->expects($this->once())->method('getTimestamp')
				...
				->will($this->returnValue($timestamp));
		$this->resource->expects($this->once())->method('isFresh')
				->with($timestamp)->will($this->returnValue(false));
		$this->loader->expects($this->once())->method('load')
				->with($this->resource)->will($this->returnValue($expected));
		$this->configCache->expects($this->once())->method('set')
				->with($this->isType('string'), $expected);
