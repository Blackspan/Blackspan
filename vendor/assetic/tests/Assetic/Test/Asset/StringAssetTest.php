...
class StringAssetTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetFilters() {
		$asset = new StringAsset('');
		$this
				->assertInternalType('array', $asset->getFilters(),
						'->getFilters() returns an array');
	}
	...
	public function testLoadAppliesAdditionalFilter() {
		...
		$filter = $this->getMock('Assetic\\Filter\\FilterInterface');
		$filter->expects($this->once())->method('filterLoad')->with($asset);
	...
	public function testDumpAppliesAdditionalFilter() {
		...
		$filter = $this->getMock('Assetic\\Filter\\FilterInterface');
		$filter->expects($this->once())->method('filterDump')->with($asset);
	...
	public function testLastModified() {
		...
		$this
				->assertEquals(123, $asset->getLastModified(),
						'->getLastModified() return the set last modified value');
	}
}
