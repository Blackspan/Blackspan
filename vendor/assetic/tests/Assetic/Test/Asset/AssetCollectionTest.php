...
class AssetCollectionTest extends \PHPUnit_Framework_TestCase {
	public function testInterface() {
		...
		$this
				->assertInstanceOf('Assetic\\Asset\\AssetInterface', $coll,
						'AssetCollection implements AssetInterface');
	}
	...
	public function testNestedCollectionLoad() {
		...
		$filter = new CallablesFilter(
				function ($asset) use ($content, &$matches, &$count) {
					++$count;
		...
		$this
				->assertEquals(1, count($matches),
						'->load() applies filters to leaves');
		$this
				->assertEquals(1, $count,
						'->load() applies filters to leaves only');
	}
	...
	public function testMixedIteration() {
		...
		$this
				->assertEquals(array('asset', 'nested'), $contents,
						'->load() iterates over multiple levels');
	}
	...
	public function testLoadDedupBySourceUrl() {
		...
		$this
				->assertEquals('asset', $coll->getContent(),
						'->load() detects duplicate assets based on source URL');
	}
	...
	public function testLoadDedupByStrictEquality() {
		...
		$this
				->assertEquals('foo', $coll->getContent(),
						'->load() detects duplicate assets based on strict equality');
	}
	...
	public function testDumpDedupBySourceUrl() {
		...
		$this
				->assertEquals('asset', $coll->dump(),
						'->dump() detects duplicate assets based on source URL');
	}
	...
	public function testDumpDedupByStrictEquality() {
		...
		$this
				->assertEquals('foo', $coll->dump(),
						'->dump() detects duplicate assets based on strict equality');
	}
	...
	public function testIterationFilters() {
		...
		$this
				->assertEquals(1, $count,
						'collection filters are called when child assets are iterated over');
	}
	...
	public function testSetContent() {
		...
		$this
				->assertEquals('asdf', $coll->getContent(),
	...
	public function testGetLastModified($timestamps, $expected) {
		...
		for ($i = 0; $i < count($timestamps); $i++) {
			$asset->expects($this->at($i))->method('getLastModified')
		...
		$this
				->assertEquals($expected, $coll->getLastModified(),
						'->getLastModifed() returns the highest last modified');
	}

	public function getTimestampsAndExpected() {
		return array(array(array(1, 2, 3), 3), array(array(5, 4, 3), 5),
				array(array(3, 8, 5), 8), array(array(3, 8, null), 8),);
	}
	...
	public function testRecursiveIteration() {
		...
		$this
				->assertEquals(4, $i,
						'iteration with a recursive iterator is recursive');
	}
	...
	public function testIteration() {
		...
		$this
				->assertEquals(2, $count,
						'iterator filters duplicates based on url');
	}
	...
	public function testBasenameCollision() {
		...
		$this
				->assertEquals(2, count(array_unique($urls)),
						'iterator prevents basename collisions');
	}

	public function testEmptyMtime() {
		$coll = new AssetCollection();
		$this
				->assertNull($coll->getLastModified(),
						'->getLastModified() returns null on empty collection');
	}
	...
	public function testLeafManipulation() {
		...
		foreach ($coll as $leaf) {
			$this
					->assertEquals('asdf', $leaf->getTargetPath(),
							'leaf changes persist between iterations');
		}
	}
}
