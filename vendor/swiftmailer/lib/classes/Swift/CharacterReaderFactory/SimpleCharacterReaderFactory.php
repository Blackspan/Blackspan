...
class Swift_CharacterReaderFactory_SimpleCharacterReaderFactory implements
	...
	public function __construct() {
		...
		$singleByte = array('class' => $prefix . 'GenericFixedWidthReader',
				'constructor' => array(1));

		$doubleByte = array('class' => $prefix . 'GenericFixedWidthReader',
				'constructor' => array(2));

		$fourBytes = array('class' => $prefix . 'GenericFixedWidthReader',
				'constructor' => array(4));

		//Utf-8
		$this->_map['utf-?8'] = array('class' => $prefix . 'Utf8Reader',
				'constructor' => array());
			...
			if (preg_match($re, $charset)) {
				if (!array_key_exists($pattern, $this->_loaded)) {
					...
					if ($reflector->getConstructor()) {
						$reader = $reflector
								->newInstanceArgs($spec['constructor']);
					} else {
