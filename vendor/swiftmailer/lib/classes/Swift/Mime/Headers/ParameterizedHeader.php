...
class Swift_Mime_Headers_ParameterizedHeader extends
		Swift_Mime_Headers_UnstructuredHeader implements
	...
	public function getParameter($parameter) {
		...
		return array_key_exists($parameter, $params) ? $params[$parameter]
				: null;
	}
	...
	public function getFieldBody() //TODO: Check caching here
 {
	...
	private function _createParameter($name, $value) {
		...
		$maxValueLength = $this->getMaxLineLength() - strlen($name . '=*N"";')
				- 1;
		$firstLineOffset = 0;
		...
		if (!preg_match('/^' . $this->_tokenRe . '$/D', $value)) {
			...
			//... and it's not ascii
			if (!preg_match(
					'/^' . $this->getGrammar()->getDefinition('text') . '*$/D',
					$value)) {
				$encoded = true;
		...
		if ($encoded || strlen($value) > $maxValueLength) {
			if (isset($this->_paramEncoder)) {
				$value = $this->_paramEncoder
						->encodeString($origValue, $firstLineOffset,
								$maxValueLength);
			} else //We have to go against RFC 2183/2231 in some areas for interoperability
 {
		...
		if (count($valueLines) > 1) {
			...
			return $name
					. $this
							->_getEndOfParameterValue($valueLines[0], $encoded,
									true);
		}
	}
	 ...
	 * @access private
	 */
	private function _getEndOfParameterValue($value, $encoded = false,
			$firstLine = false) {
		if (!preg_match('/^' . $this->_tokenRe . '$/D', $value)) {
			$value = '"' . $value . '"';
		}
