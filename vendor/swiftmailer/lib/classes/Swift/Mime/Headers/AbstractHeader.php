...
abstract class Swift_Mime_Headers_AbstractHeader implements Swift_Mime_Header {
	...
	protected function createPhrase(Swift_Mime_Header $header, $string,
			$charset, Swift_Mime_HeaderEncoder $encoder = null,
			$shorten = false) {
		//Treat token as exactly what was given
		$phraseStr = $string;
		//If it's not valid
		if (!preg_match(
				'/^' . $this->getGrammar()->getDefinition('phrase') . '$/D',
				$phraseStr)) {
			...
			if (preg_match(
					...
					$phraseStr)) {
				$phraseStr = $this->getGrammar()
						->escapeSpecials($phraseStr, array('"'),
		...
		if ($firstLineOffset >= 75) //Does this logic need to be here?
 {
		...
		$encodedTextLines = explode("\r\n",
				$this->_encoder
						->encodeString($token, $firstLineOffset,
								75 - $encodingWrapperLength));

		foreach ($encodedTextLines as $lineNum => $line) {
			$encodedTextLines[$lineNum] = '=?' . $charsetDecl . '?'
					. $this->_encoder->getName() . '?' . $line . '?=';
		}
