...
class Swift_Events_SimpleEventDispatcher implements Swift_Events_EventDispatcher {
	 ...
	 */
	public function createCommandEvent(Swift_Transport $source, $command,
			$successCodes = array()) {
	 ...
	 */
	public function createResponseEvent(Swift_Transport $source, $response,
			$valid) {
	...
	private function _bubble(Swift_Events_EventObject $evt, $target) {
		if (!$evt->bubbleCancelled()
				&& $listener = array_shift($this->_bubbleQueue)) {
			$listener->$target($evt);
			$this->_bubble($evt, $target);
		}
	}
