...
abstract class Swift_Transport_AbstractSmtpTransport implements Swift_Transport {
	...
	public function start() {
		if (!$this->_started) {
			if ($evt = $this->_eventDispatcher
					->createTransportChangeEvent($this)) {
	...
	public function executeCommand($command, $codes = array(),
		...
		if ($evt = $this->_eventDispatcher
				->createCommandEvent($this, $command, $codes)) {
	...
	protected function _doMailFromCommand($address) {
		$this
				->executeCommand(sprintf("MAIL FROM: <%s>\r\n", $address),
						array(250));
	}
	...
	protected function _doRcptToCommand($address) {
		$this
				->executeCommand(sprintf("RCPT TO: <%s>\r\n", $address),
						array(250, 251, 252));
	}
	...
	protected function _throwException(Swift_TransportException $e) {
		if ($evt = $this->_eventDispatcher
				->createTransportExceptionEvent($this, $e)) {
	...
	protected function _assertResponseCode($response, $wanted) {
		...
		if ($evt = $this->_eventDispatcher
				->createResponseEvent($this, $response, $valid)) {
	...
	private function _sendTo(Swift_Mime_Message $message, $reversePath,
		...
		return $this
				->_doMailTransaction($message, $reversePath, array_keys($to),
						$failedRecipients);
	}
	...
	private function _sendCc(Swift_Mime_Message $message, $reversePath,
		...
		return $this
				->_doMailTransaction($message, $reversePath, array_keys($cc),
						$failedRecipients);
	}
	...
	private function _sendBcc(Swift_Mime_Message $message, $reversePath,
		...
		foreach ($bcc as $forwardPath => $name) {
			...
			$sent += $this
					->_doMailTransaction($message, $reversePath,
							array($forwardPath), $failedRecipients);
		}
		return $sent;
	}
