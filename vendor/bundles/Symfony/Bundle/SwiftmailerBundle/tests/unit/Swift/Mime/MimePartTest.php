...
class Swift_Mime_MimePartTest extends Swift_Mime_AbstractMimeEntityTest {
	...
	public function testNestingLevelIsSubpart() {
		...
		$this
				->assertEqual(Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE,
	...
	public function testCharsetIsReturnedFromHeader() {
		...
		$cType = $this
				->_createHeader('Content-Type', 'text/plain',
		...
		$this
				->_checking(
						Expectations::create()->one($encoder)
								->charsetChanged('utf-8')->ignoring($encoder));
		$part = $this
				->_createMimePart($this->_createHeaderSet(), $encoder,
						$this->_createCache());
		$part->setCharset('utf-8');
	}
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->charsetChanged('utf-8')->ignoring($headers));
		$part = $this
				->_createMimePart($headers, $this->_createEncoder(),
						$this->_createCache());
		$part->setCharset('utf-8');
	}
		...
		$this
				->assertSame($part,
						$part->setContentType('text/plain')
								...
								->setCharset('utf-8')->setFormat('flowed')
								->setDelSp(true));
	}
