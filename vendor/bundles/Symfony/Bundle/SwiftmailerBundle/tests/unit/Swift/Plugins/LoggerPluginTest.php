...
class Swift_Plugins_LoggerPluginTest extends Swift_Tests_SwiftUnitTestCase {
	...
	public function testLoggerDelegatesAddingEntries() {
		...
		$this
				->_checking(
						Expectations::create()->one($logger)->add('foo'));
	...
	public function testLoggerDelegatesDumpingEntries() {
		...
		$this
				->_checking(
						Expectations::create()->one($logger)->dump()
								->returns('foobar'));
	...
	public function testLoggerDelegatesClearingEntries() {
		...
		$this
				->_checking(
						Expectations::create()->one($logger)->clear());
	...
	public function testCommandIsSentToLogger() {
		...
		$this
				->_checking(
						Expectations::create()->one($logger)
								->add(pattern('~foo\r\n~')));
	...
	public function testTransportBeforeStartChangeIsSentToLogger() {
		...
		$this
				->_checking(
						Expectations::create()->one($logger)->add(any()));
	...
	private function _createCommandEvent($command) {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($evt)->getCommand()
								->returns($command)->ignoring($evt));
		return $evt;
	}
	...
	private function _createResponseEvent($response) {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($evt)->getResponse()
								->returns($response)->ignoring($evt));
		return $evt;
	}
	...
	private function _createTransportChangeEvent() {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($evt)->getSource()
								->returns($this->_createTransport())
								->ignoring($evt));
		return $evt;
	}
	...
	private function _createTransportExceptionEvent() {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($evt)->getException()
								->returns(new Swift_TransportException(''))
								->ignoring($evt));
		return $evt;
	}
