...
class Swift_Mime_AttachmentTest extends Swift_Mime_AbstractMimeEntityTest {
	...
	public function testNestingLevelIsAttachment() {
		...
		$this
				->assertEqual(Swift_Mime_MimeEntity::LEVEL_MIXED,
	...
	public function testDispositionIsReturnedFromHeader() {
		...
		$disposition = $this
				->_createHeader('Content-Disposition', 'attachment');
		$attachment = $this
				->_createAttachment(
						$this
								->_createHeaderSet(
	...
	public function testDispositionIsSetInHeader() {
		$disposition = $this
				->_createHeader('Content-Disposition', 'attachment', array(),
						false);
		$this
				->_checking(
						Expectations::create()->one($disposition)
								->setFieldBodyModel('inline')
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								...
								->ignoring($headers));
		$attachment = $this
				->_createAttachment($headers, $this->_createEncoder(),
						$this->_createCache());
		$attachment->setDisposition('inline');
	}
	...
	public function testDefaultContentTypeInitializedToOctetStream() {
		$cType = $this
				->_createHeader('Content-Type', '', array(), false);
		$this
				->_checking(
						Expectations::create()->one($cType)
								->setFieldBodyModel('application/octet-stream')
								->ignoring($cType));
		$attachment = $this
				->_createAttachment(
						$this
	...
	public function testFilnameCanBeReadFromFileStream() {
		...
		$this
				->_checking(
						Expectations::create()->one($disposition)
								->setParameter('filename', 'file.ext')
								->ignoring($disposition));
		$attachment = $this
				->_createAttachment(
						$this
	...
	public function testContentTypeCanBeSetViaSetFile() {
		...
		$headers = $this
				->_createHeaderSet(
						array('Content-Disposition' => $disposition,
								'Content-Type' => $ctype));
		$this
				->_checking(
						Expectations::create()->one($disposition)
								...
								->ignoring($disposition)->ignoring($ctype));
		$attachment = $this
				->_createAttachment($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function XtestContentTypeCanBeLookedUpFromCommonListIfNotProvided() {
		...
		$this
				->_checking(
						Expectations::create()->one($disposition)
								->setParameter('filename', 'file.zip')
								->one($ctype)
								->setFieldBodyModel('application/zip')
								->ignoring($disposition)->ignoring($ctype));
		$attachment = $this
				->_createAttachment($headers, $this->_createEncoder(),
						$this->_createCache(),
		...
		$this
				->assertSame($attachment,
						$attachment->setContentType('application/pdf')
								...
								->setId('foo@bar')->setDescription('my pdf')
								->setMaxLineLength(998)->setBody('xx')
								->setBoundary('xyz')->setChildren(array())
								->setDisposition('inline')
								->setFilename('afile.txt')->setSize(123)
								->setFile(
										$this->_createFileStream('foo.txt', '')));
	}
	...
	protected function _createAttachment($headers, $encoder, $cache,
		...
		return new Swift_Mime_Attachment($headers, $encoder, $cache,
				new Swift_Mime_Grammar(), $mimeTypes);
	}
	...
	protected function _createFileStream($path, $data, $stub = true) {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($file)->getPath()
								->returns($path)->ignoring($file)
								->read(optional())->returns($data)
								->when($pos->isNot('at end'))
								->then($pos->is('at end'))->ignoring($file)
								->read(optional())->returns(false));
		if ($stub) {
			$this
					->_checking(
							Expectations::create()->ignoring($file));
		}
		return $file;
	}
