...
abstract class Swift_Transport_AbstractSmtpTest extends
	...
	public function testStartAccepts220ServiceGreeting() {
		...
		$this
				->_checking(
						Expectations::create()->one($buf)->initialize()
								->inSequence($s)->one($buf)->readLine(0)
								->inSequence($s)
								->returns("220 some.server.tld bleh\r\n"));
		$this->_finishBuffer($buf);
		try {
			$this
					->assertFalse($smtp->isStarted(),
							'%s: SMTP should begin non-started');
			$smtp->start();
			$this
					->assertTrue($smtp->isStarted(),
	...
	public function testBadGreetingCausesException() {
		...
		$this
				->_checking(
						Expectations::create()->one($buf)->initialize()
								->inSequence($s)->one($buf)->readLine(0)
								->inSequence($s)->returns("554 I'm busy\r\n"));
		$this->_finishBuffer($buf);
		try {
			$this
					->assertFalse($smtp->isStarted(),
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								->returns(array('me@domain.com' => 'Me'))
								->allowing($message)->getTo()
								->returns(array('foo@bar' => null))
								->allowing($message)
								->one($buf)
								->write("MAIL FROM: <me@domain.com>\r\n")
								->returns(1)->one($buf)->readLine(1)
								->returns('250 OK' . "\r\n"));
		$this->_finishBuffer($buf);
		try {
			$smtp->start();
	...
	public function testSenderIsPreferredOverFrom() {
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								->returns(array('me@domain.com' => 'Me'))
								->allowing($message)->getSender()
								->returns(
										array('another@domain.com' => 'Someone'))
								->allowing($message)->getTo()
								->returns(array('foo@bar' => null))
								->allowing($message)
								->one($buf)
								->write("MAIL FROM: <another@domain.com>\r\n")
								->returns(1)->one($buf)->readLine(1)
								->returns('250 OK' . "\r\n"));
		$this->_finishBuffer($buf);
		$smtp->start();
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->returns(
										array('another@domain.com' => 'Someone'))
								->allowing($message)->getReturnPath()
								->returns('more@domain.com')
								->allowing($message)->getTo()
								->returns(array('foo@bar' => null))
								->allowing($message)
								->one($buf)
								->write("MAIL FROM: <more@domain.com>\r\n")
								->returns(1)->one($buf)->readLine(1)
								->returns('250 OK' . "\r\n"));
		$this->_finishBuffer($buf);
		$smtp->start();
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->returns(1)->one($buf)->readLine(1)
								->returns('250 OK' . "\r\n")->one($buf)
								->write("RCPT TO: <zip@button>\r\n")
								->returns(2)->one($buf)->readLine(2)
								->returns('250 OK' . "\r\n")->one($buf)
								->write("RCPT TO: <test@domain>\r\n")
								->returns(3)->one($buf)->readLine(3)
								->returns('250 OK' . "\r\n"));
		$this->_finishBuffer($buf);
		$smtp->start();
		$smtp->send($message);
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->allowing($message)->getTo()
								->returns(array('foo@bar' => null))
								->allowing($message)->getCc()
								->returns(
										array('zip@button' => 'Zip Button',
												'test@domain' => 'Test user'))
								->allowing($message)
								->one($buf)->write("RCPT TO: <foo@bar>\r\n")
								->returns(1)->one($buf)->readLine(1)
	...
	public function testSendReturnsNumberOfSuccessfulRecipients() {
		...
		$smtp->start();
		$this
				->assertEqual(2, $smtp->send($message),
	...
	public function testRsetIsSentIfNoSuccessfulRecipients() {
		...
		$smtp->start();
		$this
				->assertEqual(0, $smtp->send($message),
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->returns(1)->one($buf)->readLine(1)
								->returns('354 OK' . "\r\n")->one($message)
								->toByteStream($buf)->inSequence($s)->one($buf)
								->write("\r\n.\r\n")->inSequence($s)
								->returns(2)->one($buf)->readLine(2)
								->returns('250 OK' . "\r\n")
								->allowing($message));
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->atLeast(1)->of($message)->setBcc(array())
								->one($message)
								->setBcc(array('zip@button' => 'Zip Button'))
								->one($message)
								->setBcc(array('test@domain' => 'Test user'))
								->atLeast(1)->of($message)
								->setBcc(
										array('zip@button' => 'Zip Button',
												'test@domain' => 'Test user'))
								->allowing($message)
								...
								->one($buf)
								->write("MAIL FROM: <me@domain.com>\r\n")
								->returns(5)->one($buf)->readLine(5)
								->returns("250 OK\r\n")->one($buf)
								->write("RCPT TO: <zip@button>\r\n")
								->returns(6)->one($buf)->readLine(6)
								->returns("250 OK\r\n")->one($buf)
								->write("DATA\r\n")->returns(7)->one($buf)
								->readLine(7)->returns("354 OK\r\n")->one($buf)
								->write("\r\n.\r\n")->returns(8)->one($buf)
								->readLine(8)->returns("250 OK\r\n")
								->one($buf)
								->write("MAIL FROM: <me@domain.com>\r\n")
								->returns(9)->one($buf)->readLine(9)
								->returns("250 OK\r\n")->one($buf)
								->write("RCPT TO: <test@domain>\r\n")
								->returns(10)->one($buf)->readLine(10)
								->returns("250 OK\r\n")->one($buf)
								->write("DATA\r\n")->returns(11)->one($buf)
								->readLine(11)->returns("354 OK\r\n")
								->one($buf)->write("\r\n.\r\n")->returns(12)
								->one($buf)->readLine(12)
								->returns("250 OK\r\n"));
		$this->_finishBuffer($buf);
				...
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->returns(
										array('zip@button' => 'Zip Button',
												'test@domain' => 'Test user'))
								->one($message)->setBcc(array())->one($message)
								->setBcc(
										array('zip@button' => 'Zip Button',
												'test@domain' => 'Test user'))
								->allowing($message)->one($buf)
								->write("MAIL FROM: <me@domain.com>\r\n")
								->returns(1)->one($buf)->readLine(1)
								->returns("250 OK\r\n")->one($buf)
								->write("RCPT TO: <foo@bar>\r\n")->returns(2)
	...
	public function testBufferCanBeWrittenToUsingExecuteCommand() {
		...
		$this
				->_checking(
						Expectations::create()->one($buf)->write("FOO\r\n")
								->returns(1)->one($buf)->readLine(1)
								->returns("250 OK\r\n")->ignoring($buf));
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->returns("500 Bad\r\n")->one($buf)
								->write("RSET\r\n")->returns(7)->one($buf)
								->readLine(7)->returns("250 OK\r\n")
								->one($buf)
								->write("MAIL FROM: <me@domain.com>\r\n")
								->returns(8)->one($buf)->readLine(8)
								->returns("250 OK\r\n")->one($buf)
								->write("RCPT TO: <test@domain>\r\n")
								->returns(9)->one($buf)->readLine(9)
								->returns("500 Bad\r\n")->one($buf)
								->write("RSET\r\n")->returns(10)->one($buf)
								->readLine(10)->returns("250 OK\r\n"));
		$this->_finishBuffer($buf);
		$smtp->start();
		$this->assertEqual(1, $smtp->send($message, $failures));
		$this
				->assertEqual(array('zip@button', 'test@domain'), $failures,
	...
	protected function _finishBuffer($buf) {
		$this
				->_checking(
						Expectations::create()->ignoring($buf)->readLine(0)
								->returns('220 server.com foo' . "\r\n")
								->ignoring($buf)
								->write(pattern('~^(EH|HE)LO .*?\r\n$~D'))
								->returns($x = uniqid())->ignoring($buf)
								->readLine($x)
								->returns('250 ServerName' . "\r\n")
								->ignoring($buf)
								->write(pattern('~^MAIL FROM: <.*?>\r\n$~D'))
								->returns($x = uniqid())->ignoring($buf)
								->readLine($x)->returns('250 OK' . "\r\n")
								->ignoring($buf)
								->write(pattern('~^RCPT TO: <.*?>\r\n$~D'))
								->returns($x = uniqid())->ignoring($buf)
								->readLine($x)->returns('250 OK' . "\r\n")
								->ignoring($buf)->write("DATA\r\n")
								->returns($x = uniqid())->ignoring($buf)
								->readLine($x)->returns('354 OK' . "\r\n")
								->ignoring($buf)->write("\r\n.\r\n")
								->returns($x = uniqid())->ignoring($buf)
								->readLine($x)->returns('250 OK' . "\r\n")
