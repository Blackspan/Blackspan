...
class Swift_Mime_SimpleMessageTest extends Swift_Mime_MimePartTest {
	...
	public function testNestingLevelIsSubpart() { //Overridden
	}
	...
	public function testNestingLevelIsTop() {
		...
		$this
				->assertEqual(Swift_Mime_MimeEntity::LEVEL_TOP,
	...
	public function testDateIsSetInHeader() {
		...
		$this
				->_checking(
						Expectations::create()->one($date)
								->setFieldBodyModel(1234)->ignoring($date));
		$message = $this
				->_createMessage(
						$this->_createHeaderSet(array('Date' => $date)),
						$this->_createEncoder(), $this->_createCache());
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addDateHeader('Date', 1234)
								->ignoring($headers));
		$message = $this
				->_createMessage($headers, $this->_createEncoder(),
						$this->_createCache());
		$message->setDate(1234);
	}
	...
	public function testDateHeaderIsAddedDuringConstruction() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addDateHeader('Date', pattern('/^[0-9]+$/D'))
								->ignoring($headers));
		$message = $this
				->_createMessage($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testIdIsSetInHeader() {
		...
		$this
				->_checking(
						Expectations::create()->one($messageId)
								->setFieldBodyModel('x@y')
								->ignoring($messageId));
		$message = $this
				->_createMessage(
						$this
	...
	public function testSubjectIsSetInHeader() {
		...
		$this
				->_checking(
						Expectations::create()->one($subject)
								->setFieldBodyModel('foo')->ignoring($subject));
		$message = $this
				->_createMessage(
						$this->_createHeaderSet(array('Subject' => $subject)),
						$this->_createEncoder(), $this->_createCache());
	...
	public function testSubjectHeaderIsCreatedIfNotPresent() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addTextHeader('Subject', 'example subject')
								->ignoring($headers));
		$message = $this
				->_createMessage($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testReturnPathIsSetInHeader() {
		...
		$this
				->_checking(
						Expectations::create()->one($path)
								->setFieldBodyModel('bounces@domain')
								->ignoring($path));
		$message = $this
				->_createMessage(
						$this->_createHeaderSet(array('Return-Path' => $path)),
	...
	public function testReturnPathHeaderIsAddedIfNoneSet() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addPathHeader('Return-Path', 'bounces@domain')
								->ignoring($headers));
		$message = $this
				->_createMessage($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testSenderIsReturnedFromHeader() {
		...
		$sender = $this
				->_createHeader('Sender', array('sender@domain' => 'Name'));
		$message = $this
				->_createMessage(
						$this->_createHeaderSet(array('Sender' => $sender)),
						$this->_createEncoder(), $this->_createCache());
		$this
				->assertEqual(array('sender@domain' => 'Name'),
						$message->getSender());
	}
	...
	public function testSenderIsSetInHeader() {
		...
		$this
				->_checking(
						Expectations::create()->one($sender)
								->setFieldBodyModel(
										array('other@domain' => 'Other'))
								->ignoring($sender));
		$message = $this
				->_createMessage(
						$this->_createHeaderSet(array('Sender' => $sender)),
	...
	public function testFromIsSetInHeader() {
		$from = $this
				->_createHeader('From', array('from@domain' => 'Name'),
						array(), false);
		$this
				->_checking(
						Expectations::create()->one($from)
								->setFieldBodyModel(
										array('other@domain' => 'Other'))
								->ignoring($from));
		$message = $this
	...
	public function testReplyToIsReturnedFromHeader() {
		...
		$reply = $this
				->_createHeader('Reply-To', array('reply@domain' => 'Name'));
		$message = $this
				->_createMessage(
						$this->_createHeaderSet(array('Reply-To' => $reply)),
						$this->_createEncoder(), $this->_createCache());
		$this
				->assertEqual(array('reply@domain' => 'Name'),
						$message->getReplyTo());
	}
	...
	public function testReplyToIsAddedToHeadersDuringAddReplyTo() {
		$replyTo = $this
				->_createHeader('Reply-To', array('from@domain' => 'Name'),
						array(), false);
		$this
				->_checking(
						Expectations::create()->one($replyTo)
								->setFieldBodyModel(
										array('from@domain' => 'Name',
												'other@domain' => 'Other'))
								->ignoring($replyTo));
	...
	public function testToIsReturnedFromHeader() {
		...
		$message = $this
				->_createMessage($this->_createHeaderSet(array('To' => $to)),
						$this->_createEncoder(), $this->_createCache());
	...
	public function testToIsSetInHeader() {
		$to = $this
				->_createHeader('To', array('to@domain' => 'Name'), array(),
						false);
		$this
				->_checking(
						Expectations::create()->one($to)
								->setFieldBodyModel(
										array('other@domain' => 'Other'))
								->ignoring($to));
		$message = $this
	...
	public function testToIsAddedToHeadersDuringAddTo() {
		$to = $this
				->_createHeader('To', array('from@domain' => 'Name'), array(),
						false);
		$this
				->_checking(
						Expectations::create()->one($to)
	...
	public function testToHeaderIsAddedIfNoneSet() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addMailboxHeader('To', (array) 'to@domain')
								->ignoring($headers));
		$message = $this
				->_createMessage($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testCcIsReturnedFromHeader() {
		...
		$message = $this
				->_createMessage($this->_createHeaderSet(array('Cc' => $cc)),
						$this->_createEncoder(), $this->_createCache());
	...
	public function testCcIsSetInHeader() {
		$cc = $this
				->_createHeader('Cc', array('cc@domain' => 'Name'), array(),
						false);
		$this
				->_checking(
						Expectations::create()->one($cc)
								->setFieldBodyModel(
										array('other@domain' => 'Other'))
								->ignoring($cc));
		$message = $this
	...
	public function testCcIsAddedToHeadersDuringAddCc() {
		$cc = $this
				->_createHeader('Cc', array('from@domain' => 'Name'), array(),
						false);
		$this
				->_checking(
						Expectations::create()->one($cc)
	...
	public function testCcHeaderIsAddedIfNoneSet() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addMailboxHeader('Cc', (array) 'cc@domain')
								->ignoring($headers));
		$message = $this
				->_createMessage($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testBccIsSetInHeader() {
		$bcc = $this
				->_createHeader('Bcc', array('bcc@domain' => 'Name'), array(),
						false);
		$this
				->_checking(
						Expectations::create()->one($bcc)
								->setFieldBodyModel(
										array('other@domain' => 'Other'))
								->ignoring($bcc));
		$message = $this
	...
	public function testBccIsAddedToHeadersDuringAddBcc() {
		$bcc = $this
				->_createHeader('Bcc', array('from@domain' => 'Name'), array(),
						false);
		$this
				->_checking(
						Expectations::create()->one($bcc)
	...
	public function testBccHeaderIsAddedIfNoneSet() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addMailboxHeader('Bcc', (array) 'bcc@domain')
								->ignoring($headers));
		$message = $this
				->_createMessage($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testNameCanBeUsedInBcc() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addMailboxHeader('Bcc',
										array('bcc@domain' => 'Name'))
								->ignoring($headers));
		$message = $this
				->_createMessage($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testPriorityIsSetInHeader() {
		...
		$this
				->_checking(
						Expectations::create()->one($prio)
								->setFieldBodyModel('5 (Lowest)')
								->ignoring($prio));
		$message = $this
				->_createMessage(
						$this->_createHeaderSet(array('X-Priority' => $prio)),
	...
	public function testPriorityHeaderIsAddedIfNoneSet() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addTextHeader('X-Priority', '4 (Low)')
								->ignoring($headers));
		$message = $this
				->_createMessage($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testReadReceiptAddressReadFromHeader() {
		$rcpt = $this
				->_createHeader('Disposition-Notification-To',
		...
		$message = $this
				->_createMessage(
						...
						$this->_createEncoder(), $this->_createCache());
		$this
				->assertEqual(array('chris@swiftmailer.org' => 'Chris'),
						$message->getReadReceiptTo());
	}

	public function testReadReceiptIsSetInHeader() {
		$rcpt = $this
				->_createHeader('Disposition-Notification-To', array(),
						array(), false);
		$this
				->_checking(
						Expectations::create()->one($rcpt)
								->setFieldBodyModel('mark@swiftmailer.org')
								->ignoring($rcpt));
		$message = $this
				->_createMessage(
						$this
	...
	public function testEmbedReturnsValidCid() {
		$child = $this
				->_createChild(Swift_Mime_MimeEntity::LEVEL_RELATED, '', false);
		$this
				->_checking(
						Expectations::create()->ignoring($child)->getId()
								->returns('foo@bar')->ignoring($child));
		$message = $this
				->_createMessage($this->_createHeaderSet(),
						$this->_createEncoder(), $this->_createCache());
		...
		$this
				->assertSame($message,
						$message->setContentType('text/plain')
								...
								->setCharset('iso-8859-1')->setFormat('flowed')
								->setDelSp(false)->setSubject('subj')
								->setDate(123)->setReturnPath('foo@bar')
								->setSender('foo@bar')
								->setFrom(array('x@y' => 'XY'))
								->setReplyTo(array('ab@cd' => 'ABCD'))
								->setTo(
										array('chris@site.tld', 'mark@site.tld'))
								->setCc('john@somewhere.tld')
								->setBcc(array('one@site', 'two@site' => 'Two'))
								->setPriority(4)->setReadReceiptTo('a@b')
								->attach($child)->detach($child));
	}
