...
class Swift_Mime_SimpleHeaderSetTest extends Swift_Tests_SwiftUnitTestCase {
	...
	public function testAddMailboxHeaderDelegatesToFactory() {
		...
		$this
				->_checking(
						Expectations::create()->one($factory)
								->createMailboxHeader('From',
										array('person@domain' => 'Person'))
	...
	public function testAddTextHeaderDelegatesToFactory() {
		...
		$this
				->_checking(
						Expectations::create()->one($factory)
								->createTextHeader('Subject', 'some text')
	...
	public function testAddParameterizedHeaderDelegatesToFactory() {
		...
		$this
				->_checking(
						Expectations::create()->one($factory)
								->createParameterizedHeader('Content-Type',
										'text/plain',
										array('charset' => 'utf-8'))
								->returns($this->_createHeader('Content-Type')));
		$set = $this->_createSet($factory);
		$set
				->addParameterizedHeader('Content-Type', 'text/plain',
						array('charset' => 'utf-8'));
	}
	...
	public function testAddIdHeaderDelegatesToFactory() {
		...
		$this
				->_checking(
						Expectations::create()->one($factory)
								->createIdHeader('Message-ID', 'some@id')
	...
	public function testAddPathHeaderDelegatesToFactory() {
		...
		$this
				->_checking(
						Expectations::create()->one($factory)
								->createPathHeader('Return-Path', 'some@path')
	...
	public function testAddedMailboxHeaderIsSeenByHas() {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($factory)
								->createMailboxHeader('From',
										array('person@domain' => 'Person'))
								->returns($this->_createHeader('From')));
		$set = $this->_createSet($factory);
		...
		$this
				->_checking(
						Expectations::create()->ignoring($factory)
								...
								->returns($this->_createHeader('Message-ID'))
								->ignoring($factory)
								->createIdHeader('Message-ID', 'other@id')
								->returns($this->_createHeader('Message-ID')));
		$set = $this->_createSet($factory);
		$set->addIdHeader('Message-ID', 'some@id');
	...
	public function testGetWithSpeiciedOffset() {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($factory)
								->createIdHeader('Message-ID', 'some@id')
								->returns($header0)->ignoring($factory)
								->createIdHeader('Message-ID', 'other@id')
								->returns($header1)->ignoring($factory)
								->createIdHeader('Message-ID', 'more@id')
								->returns($header2));
		$set = $this->_createSet($factory);
		$set->addIdHeader('Message-ID', 'some@id');
		$set->addIdHeader('Message-ID', 'other@id');
	...
	public function testGetAllReturnsAllHeadersMatchingName() {
		...
		$this
				->assertEqual(array($header0, $header1, $header2),
		...
		$this
				->_checking(
						Expectations::create()->ignoring($factory)
								->createIdHeader('Message-ID', 'some@id')
								->returns($header0)->ignoring($factory)
								->createIdHeader('Subject', 'thing')
								->returns($header1)->ignoring($factory)
								->createIdHeader('To', 'person@example.org')
								->returns($header2));
		$set = $this->_createSet($factory);
		$set->addIdHeader('Message-ID', 'some@id');
	...
	public function testToStringJoinsHeadersTogether() {
		...
		$this
				->_checking(
						Expectations::create()->one($factory)
								->createTextHeader('Foo', 'bar')
								->returns($this->_createHeader('Foo', 'bar'))
								->one($factory)
								->createTextHeader('Zip', 'buttons')
								->returns(
										$this->_createHeader('Zip', 'buttons')));
		$set = $this->_createSet($factory);
	...
	public function testHeaderSequencesCanBeSpecified() {
		...
		$this
				->_checking(
						Expectations::create()->one($factory)
								->createTextHeader('First', 'one')
								->returns($this->_createHeader('First', 'one'))
								->one($factory)
								->createTextHeader('Second', 'two')
								->returns($this->_createHeader('Second', 'two'))
								->one($factory)
								->createTextHeader('Third', 'three')
								->returns(
										$this->_createHeader('Third', 'three')));
		$set = $this->_createSet($factory);
				...
				->_checking(
						Expectations::create()->one($factory)
								...
								->returns(
										$this->_createHeader('Third', 'three'))
								->one($factory)
								->createTextHeader('Fourth', 'four')
								->returns(
										$this->_createHeader('Fourth', 'four'))
								->one($factory)
								->createTextHeader('Fifth', 'five')
	...
	public function testSettingCharsetNotifiesAlreadyExistingHeaders() {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($factory)
								->createTextHeader('Subject', 'some text')
								->returns($subject)->ignoring($factory)
								->createTextHeader('X-Header', 'some text')
								->returns($xHeader)->ignoring($factory)
	...
	public function testCharsetChangeNotifiesFactory() {
		...
		$this
				->_checking(
						Expectations::create()->one($factory)
								->charsetChanged('utf-8')->ignoring($factory));
		$set = $this->_createSet($factory);
		...
		$this
				->_checking(
						Expectations::create()->ignoring($header)
								...
								->ignoring($header)->toString()
								->returns(sprintf("%s: %s\r\n", $name, $body))
								->ignoring($header)->getFieldBody()
								->returns($body));
		return $header;
	}
