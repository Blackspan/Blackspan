...
class Swift_StreamFilters_ByteArrayReplacementFilterTest extends
	...
	public function testBasicReplacementsAreMade() {
		...
		$this
				->assertEqual(array(0x59, 0x60, 0x63, 0x64, 0x65),
	...
	public function testShouldBufferReturnsTrueIfPartialMatchAtEndOfBuffer() {
		$filter = $this->_createFilter(array(0x61, 0x62), array(0x63, 0x64));
		$this
				->assertTrue($filter->shouldBuffer(array(0x59, 0x60, 0x61)),
	...
	public function testFilterCanMakeMultipleReplacements() {
		$filter = $this
				->_createFilter(array(array(0x61), array(0x62)), array(0x63));
		$this
				->assertEqual(array(0x60, 0x63, 0x60, 0x63, 0x60),
						$filter->filter(array(0x60, 0x61, 0x60, 0x62, 0x60)));
	}

	public function testMultipleReplacementsCanBeDifferent() {
		$filter = $this
				->_createFilter(array(array(0x61), array(0x62)),
						array(array(0x63), array(0x64)));
		$this
				->assertEqual(array(0x60, 0x63, 0x60, 0x64, 0x60),
						$filter->filter(array(0x60, 0x61, 0x60, 0x62, 0x60)));
	}
	...
	public function testShouldBufferReturnsFalseIfPartialMatchNotAtEndOfString() {
		...
		$this
				->assertFalse(
						$filter
								->shouldBuffer(
										array(0x61, 0x62, 0x0D, 0x0A, 0x63)),
	...
	public function testShouldBufferReturnsTrueIfAnyOfMultipleMatchesAtEndOfString() {
		$filter = $this
				->_createFilter(array(array(0x61, 0x62), array(0x63)),
						array(0x64));
		$this
				->assertTrue($filter->shouldBuffer(array(0x59, 0x60, 0x61)),
	...
	public function testConvertingAllLineEndingsToCRLFWhenInputIsLF() {
		...
		$this
				->assertEqual(
						array(0x60, 0x0D, 0x0A, 0x61, 0x0D, 0x0A, 0x62, 0x0D,
								0x0A, 0x63),
						$filter
								->filter(
										array(0x60, 0x0A, 0x61, 0x0A, 0x62,
												0x0A, 0x63)));
	}
	...
	public function testConvertingAllLineEndingsToCRLFWhenInputIsCR() {
		...
		$this
				->assertEqual(
						...
						$filter
								->filter(
										array(0x60, 0x0D, 0x61, 0x0D, 0x62,
												0x0D, 0x63)));
	}
	...
	public function testConvertingAllLineEndingsToCRLFWhenInputIsCRLF() {
		...
		$this
				->assertEqual(
						...
						$filter
								->filter(
										array(0x60, 0x0D, 0x0A, 0x61, 0x0D,
												0x0A, 0x62, 0x0D, 0x0A, 0x63)));
	}
	...
	public function testConvertingAllLineEndingsToCRLFWhenInputIsLFCR() {
		...
		$this
				->assertEqual(
						array(0x60, 0x0D, 0x0A, 0x0D, 0x0A, 0x61, 0x0D, 0x0A,
								0x0D, 0x0A, 0x62, 0x0D, 0x0A, 0x0D, 0x0A, 0x63),
						$filter
								->filter(
										array(0x60, 0x0A, 0x0D, 0x61, 0x0A,
												0x0D, 0x62, 0x0A, 0x0D, 0x63)));
	}
	...
	public function testConvertingAllLineEndingsToCRLFWhenInputContainsLFLF() {
		...
		$this
				->assertEqual(
						...
						$filter
								->filter(
										array(0x60, 0x0A, 0x0A, 0x61, 0x0A,
												0x0A, 0x62, 0x0A, 0x0A, 0x63)));
	}

	// -- Creation methods

	private function _createFilter($search, $replace) {
		return new Swift_StreamFilters_ByteArrayReplacementFilter($search,
				$replace);
	}
