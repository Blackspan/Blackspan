...
class Swift_Mime_HeaderEncoder_QpHeaderEncoderTest extends
	...
	public function testNameIsQ() {
		$encoder = $this
				->_createEncoder($this->_createCharacterStream(true));
		...
		$this
				->_checking(
						Expectations::create()->one($charStream)
								...
								->one($charStream)->readBytes(any())
								->returns(array(0x20))->one($charStream)
								->readBytes(any())->returns(array(ord('b')))
								->allowing($charStream)->readBytes(any())
								->returns(false)->ignoring($charStream));

		$encoder = $this->_createEncoder($charStream);
		$this
				->assertNoPattern('~[ \t]~', $encoder->encodeString("a \t b"),
	...
	public function testSpaceIsRepresentedByUnderscore() {
		...
		$encoder = $this->_createEncoder($charStream);
		$this
				->assertEqual('a_b', $encoder->encodeString('a b'),
	...
	public function testEqualsAndQuestionAndUnderscoreAreEncoded() {
		...
		$this
				->_checking(
						Expectations::create()->one($charStream)
								->readBytes(any())->returns(array(ord('=')))
								->one($charStream)->readBytes(any())
								->returns(array(ord('?')))->one($charStream)
								->readBytes(any())->returns(array(ord('_')))
								->allowing($charStream)->readBytes(any())
								->returns(false)->ignoring($charStream));

		$encoder = $this->_createEncoder($charStream);
		$this
				->assertEqual('=3D=3F=5F', $encoder->encodeString('=?_'),
	...
	public function testParensAndQuotesAreEncoded() {
		...
		$encoder = $this->_createEncoder($charStream);
		$this
				->assertEqual('=28=22=29', $encoder->encodeString('(")'),
	...
	public function testOnlyCharactersAllowedInPhrasesAreUsed() {
		...
		$allowedBytes = array_merge(range(ord('a'), ord('z')),
		...
		foreach (range(0x00, 0xFF) as $byte) {
			...
			$this
					->_checking(
							Expectations::create()->one($charStream)
									->readBytes(any())->returns(array($byte))
									->allowing($charStream)->readBytes(any())
									->returns(false)->ignoring($charStream));
			...
			if (in_array($byte, $allowedBytes)) {
				$this
						->assertEqual($char, $encodedChar,
								'%s: Character ' . $char
										. ' should not be encoded.');
			} elseif (0x20 == $byte) //Special case
 {
				$this
						->assertEqual('_', $encodedChar,
								'%s: Space character should be replaced.');
			} else {
				$this
						->assertEqual(sprintf('=%02X', $byte), $encodedChar,
