...
class Swift_Encoder_Rfc2231EncoderTest extends Swift_Tests_SwiftUnitTestCase {
	...
	public function testEncodingAsciiCharactersProducesValidToken() {
			...
			$this
					->_checking(
							Expectations::create()->one($charStream)
									->read(optional())->inSequence($seq)
									->returns($char));
		}
		$this
				->_checking(
						Expectations::create()->atLeast(1)->of($charStream)
								->read(optional())->inSequence($seq)
								->returns(false)->one($charStream)
								->importString($string)->ignoring($charStream)
								->flushContents());
		...
		foreach (explode("\r\n", $encoded) as $line) {
			$this
					->assertPattern($this->_rfc2045Token, $line,
