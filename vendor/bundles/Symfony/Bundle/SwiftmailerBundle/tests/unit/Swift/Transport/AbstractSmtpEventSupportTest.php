...
abstract class Swift_Transport_AbstractSmtpEventSupportTest extends
	...
	public function testRegisterPluginLoadsPluginInEventDispatcher() {
		...
		$this
				->_checking(
						Expectations::create()->one($dispatcher)
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								->returns(array('chris@swiftmailer.org' => null))
								->allowing($message)->getTo()
								->returns(array('mark@swiftmailer.org' => 'Mark'))
								->ignoring($message)->one($dispatcher)
								->createSendEvent(optional())->returns($evt)
								->one($dispatcher)
								->dispatchEvent($evt, 'beforeSendPerformed')
								->ignoring($dispatcher)->ignoring($evt));
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->createSendEvent(optional())->returns($evt)
								->one($dispatcher)
								->dispatchEvent($evt, 'sendPerformed')
								->ignoring($dispatcher)->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
		$this->assertEqual(1, $smtp->send($message));
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->allowing($message)->getTo()
								->returns(array('mark@swiftmailer.org' => 'Mark'))
								->ignoring($message)->one($buf)
								->write(
										"MAIL FROM: <chris@swiftmailer.org>\r\n")
								->returns(1)->one($buf)->readLine(1)
								->returns("250 OK\r\n")->one($buf)
								->write("RCPT TO: <mark@swiftmailer.org>\r\n")
								->returns(2)->one($buf)->readLine(2)
								->returns("500 Not now\r\n")
								->allowing($dispatcher)
								->createSendEvent($smtp, optional())
								->returns($evt)->one($evt)
								->setFailedRecipients(
										array('mark@swiftmailer.org'))
								->ignoring($dispatcher)->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->allowing($message)->getTo()
								->returns(
										array('mark@swiftmailer.org' => 'Mark',
				...
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->returns(
										array('mark@swiftmailer.org' => 'Mark',
												'chris@site.tld' => 'Chris'))
								->ignoring($message)->allowing($dispatcher)
								->createSendEvent($smtp, optional())
								->returns($evt)->one($evt)
								->setResult(
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getFrom()
								...
								->ignoring($message)->allowing($dispatcher)
								->createSendEvent($smtp, optional())
								->returns($evt)->one($dispatcher)
								->dispatchEvent($evt, 'beforeSendPerformed')
								->ignoring($dispatcher)->atLeast(1)->of($evt)
								->bubbleCancelled()->returns(true)
								->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
		$this->assertEqual(0, $smtp->send($message));
	...
	public function testStartingTransportDispatchesTransportChangeEvent() {
		...
		$this
				->_checking(
						Expectations::create()->allowing($dispatcher)
								->createTransportChangeEvent($smtp, optional())
								->returns($evt)->one($dispatcher)
								->dispatchEvent($evt, 'transportStarted')
								->ignoring($dispatcher)->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
	}
		...
		$this
				->_checking(
						Expectations::create()->allowing($dispatcher)
								->createTransportChangeEvent($smtp, optional())
								->returns($evt)->one($dispatcher)
								->dispatchEvent($evt, 'beforeTransportStarted')
								->ignoring($dispatcher)->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
	}
		...
		$this
				->_checking(
						Expectations::create()->allowing($dispatcher)
								...
								->returns($evt)->one($dispatcher)
								->dispatchEvent($evt, 'beforeTransportStarted')
								->allowing($evt)->bubbleCancelled()
								->returns(true)->ignoring($dispatcher)
								->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
		...
		$this
				->_checking(
						Expectations::create()->allowing($dispatcher)
								->createTransportChangeEvent($smtp, optional())
								->returns($evt)->one($dispatcher)
								->dispatchEvent($evt, 'transportStopped')
								->ignoring($dispatcher)->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
		$smtp->stop();
		...
		$this
				->_checking(
						Expectations::create()->allowing($dispatcher)
								->createTransportChangeEvent($smtp, optional())
								->returns($evt)->one($dispatcher)
								->dispatchEvent($evt, 'beforeTransportStopped')
								->ignoring($dispatcher)->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
		$smtp->stop();
		...
		$this
				->_checking(
						Expectations::create()->allowing($dispatcher)
								...
								->returns($evt)->one($dispatcher)
								->dispatchEvent($evt, 'beforeTransportStopped')
								->inSequence($seq)->allowing($evt)
								->bubbleCancelled()->inSequence($seq)
								->returns(true)->ignoring($dispatcher)
								->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
	...
	public function testResponseEventsAreGenerated() {
		...
		$this
				->_checking(
						Expectations::create()->allowing($dispatcher)
								->createResponseEvent($smtp, optional())
								->returns($evt)->one($dispatcher)
								->dispatchEvent($evt, 'responseReceived')
								->ignoring($dispatcher)->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
	}
	...
	public function testCommandEventsAreGenerated() {
		...
		$this
				->_checking(
						Expectations::create()->allowing($dispatcher)
								->createCommandEvent($smtp, optional())
								->returns($evt)->one($dispatcher)
								->dispatchEvent($evt, 'commandSent')
								->ignoring($dispatcher)->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
	}
	...
	public function testExceptionsCauseExceptionEvents() {
		...
		$this
				->_checking(
						Expectations::create()->atLeast(1)->of($buf)
								->readLine(any())
								->returns("503 I'm sleepy, go away!\r\n")
								->allowing($dispatcher)
								->createTransportExceptionEvent($smtp,
										optional())->returns($evt)
								->one($dispatcher)
								->dispatchEvent($evt, 'exceptionThrown')
								->ignoring($dispatcher)->ignoring($evt));
		$this->_finishBuffer($buf);
		try {
			$smtp->start();
		...
		$this
				->_checking(
						Expectations::create()->atLeast(1)->of($buf)
								...
								->one($dispatcher)
								->dispatchEvent($evt, 'exceptionThrown')
								->atLeast(1)->of($evt)->bubbleCancelled()
								->returns(true)->ignoring($dispatcher)
								->ignoring($evt));
		$this->_finishBuffer($buf);
		$smtp->start();
	...
	protected function _createEventDispatcher($stub = true) {
		return $stub ? $this->_stub('Swift_Events_EventDispatcher')
