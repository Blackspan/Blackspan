...
class Swift_Plugins_ThrottlerPluginTest extends Swift_Tests_SwiftUnitTestCase {
	...
	public function testBytesPerMinuteThrottling() {
		...
		//10MB/min
		$plugin = new Swift_Plugins_ThrottlerPlugin(10000000,
				Swift_Plugins_ThrottlerPlugin::BYTES_PER_MINUTE, $sleeper,
				$timer);

		$this
				->_checking(
						Expectations::create()->one($timer)->getTimestamp()
								->returns(0)->one($timer)->getTimestamp()
								->returns(1) //expected 0.6
	...
	public function testMessagesPerMinuteThrottling() {
		...
		//60/min
		$plugin = new Swift_Plugins_ThrottlerPlugin(60,
				Swift_Plugins_ThrottlerPlugin::MESSAGES_PER_MINUTE, $sleeper,
				$timer);
	...
	private function _createMessageWithByteCount($bytes) {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($msg)
								->toByteStream(any())
								->calls(array($this, '_write')));
		return $msg;
	}
