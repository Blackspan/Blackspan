...
class Swift_Mime_EmbeddedFileTest extends Swift_Mime_AttachmentTest {
	...
	public function testNestingLevelIsAttachment() { //Overridden
	}
	...
	public function testNestingLevelIsEmbedded() {
		...
		$this
				->assertEqual(Swift_Mime_MimeEntity::LEVEL_RELATED,
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addIdHeader('Content-ID',
										pattern('/^.*?@.*?$/D'))
								->ignoring($headers));
		$file = $this
				->_createEmbeddedFile($headers, $this->_createEncoder(),
						$this->_createCache());
	}

	public function testDefaultDispositionIsAttachment() { //Overridden
	}
	...
	protected function _createAttachment($headers, $encoder, $cache,
			$mimeTypes = array()) {
		return $this
				->_createEmbeddedFile($headers, $encoder, $cache, $mimeTypes);
	}
