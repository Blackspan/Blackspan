...
class Swift_Mime_Headers_UnstructuredHeaderTest extends
	...
	public function testTypeIsTextHeader() {
		...
		$this
				->assertEqual(Swift_Mime_Header::TYPE_TEXT,
						$header->getFieldType());
	}
	...
	public function testLongHeadersAreFoldedAtWordBoundary() {
		...
		$header = $this
				->_getHeader('X-Custom-Header', $this->_getEncoder('Q', true));
		...
		$this
				->_checking(
						Expectations::create()->one($encoder)
								->encodeString($nonAsciiChar, any(), any())
								->returns('=8F')->ignoring($encoder));
	...
	public function testEncodedWordsAreUsedToEncodedNonPrintableAscii() {
		...
		foreach ($nonPrintableBytes as $byte) {
			...
			$this
					->_checking(
							Expectations::create()->one($encoder)
									->encodeString($char, any(), any())
									->returns($encodedChar)->ignoring($encoder));
	...
	public function testEncodedWordsAreNoMoreThan75CharsPerLine() {
		...
		$this
				->_checking(
						Expectations::create()->one($encoder)
								->encodeString($nonAsciiChar, 8, 63)
								->returns('=8F')->ignoring($encoder));
	...
	public function testAdjacentWordsAreEncodedTogether() {
		...
		$this
				->_checking(
						Expectations::create()->one($encoder)
								->encodeString($word . ' ' . $word, any(),
										any())->returns('w=8Frd_w=8Frd')
								->one($encoder)
								->encodeString($word, any(), any())
								->returns('w=8Frd')->ignoring($encoder));
		...
		$this
				->assertEqual(
						'X-Test: start =?' . $this->_charset . '?Q?'
								. 'w=8Frd_w=8Frd?= then end =?'
								. $this->_charset . '?Q?' . 'w=8Frd?=' . "\r\n",
						$headerString,
	...
	public function testLanguageInformationAppearsInEncodedWords() {
		...
		$header->setValue($value);
		$this
				->assertEqual("Subject: =?utf-8*en?Q?fo=8Fbar?=\r\n",
						$header->toString());
	}
