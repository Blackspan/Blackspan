...
class Swift_Mime_ContentEncoder_QpContentEncoderTest extends
	...
	public function testPermittedCharactersAreNotEncoded() {
		...
		foreach (array_merge(range(33, 60), range(62, 126)) as $ordinal) {
			...
			$this
					->_checking(
							Expectations::create()->one($charStream)
									->flushContents()->one($charStream)
									->importByteStream($os)->one($charStream)
									->readBytes(any())
									->returns(array($ordinal))
									->allowing($charStream)->readBytes(any())
									->returns(false)
									->allowing($is)->write(any(), optional())
									->will($collection)->ignoring($is)
									->ignoring($os));
		...
		$this
				->_checking(
						Expectations::create()->one($charStream)
								->flushContents()->one($charStream)
								->importByteStream($os)->one($charStream)
								->readBytes(any())->returns(array(ord('a')))
								->one($charStream)->readBytes(any())
								->returns(array(0x09))->one($charStream)
								->readBytes(any())->returns(array(0x09))
								->one($charStream)->readBytes(any())
								->returns(array(0x0D))->one($charStream)
								->readBytes(any())->returns(array(0x0A))
								->one($charStream)->readBytes(any())
								->returns(array(ord('b')))
								->allowing($charStream)->readBytes(any())
								->returns(false)
		...
		$this
				->_checking(
						Expectations::create()->one($charStream)
								...
								->readBytes(any())->returns(array(ord('a')))
								->one($charStream)->readBytes(any())
								->returns(array(0x20))->one($charStream)
								->readBytes(any())->returns(array(0x20))
								->one($charStream)->readBytes(any())
								->returns(array(0x0D))->one($charStream)
								->readBytes(any())->returns(array(0x0A))
								->one($charStream)->readBytes(any())
		...
		$this
				->_checking(
						Expectations::create()->one($charStream)
								...
								->readBytes(any())->returns(array(0x0A))
								->one($charStream)->readBytes(any())
								->returns(array(ord('b')))->one($charStream)
								->readBytes(any())->returns(array(0x0D))
								->one($charStream)->readBytes(any())
								->returns(array(0x0A))->one($charStream)
								->readBytes(any())->returns(array(ord('c')))
								->one($charStream)->readBytes(any())
								->returns(array(0x0D))->one($charStream)
								->readBytes(any())->returns(array(0x0A))
								->allowing($charStream)->readBytes(any())
	...
	public function testLinesLongerThan76CharactersAreSoftBroken() {
		...
		$this
				->_checking(
						Expectations::create()->allowing($charStream)
								->readBytes(any())->returns(false));
	...
	public function testBytesBelowPermittedRangeAreEncoded() {
		...
		foreach (range(0, 32) as $ordinal) {
			...
			$encoder->encodeByteStream($os, $is);
			$this
					->assertEqual(sprintf('=%02X', $ordinal),
							$collection->content);
		}
	}
		...
		$this
				->_checking(
						Expectations::create()->one($charStream)
								...
								->importByteStream($os)
								->one($charStream)->readBytes(any())
								->returns(array(61))->allowing($charStream)
								->readBytes(any())->returns(false)
								->allowing($is)->write(any(), optional())
								->will($collection)->ignoring($is)
								->ignoring($os));
	...
	public function testObserverInterfaceCanChangeCharset() {
		...
		$this
				->_checking(
						Expectations::create()->one($stream)
								->setCharacterSet('windows-1252')
								->ignoring($stream));
	...
	private function _createCharacterStream($stub = false) {
		return $stub ? $this->_stub('Swift_CharacterStream')
	...
	private function _createOutputByteStream($stub = false) {
		return $stub ? $this->_stub('Swift_OutputByteStream')
				: $this->_mock('Swift_OutputByteStream');
	}

	private function _createInputByteStream($stub = false) {
		return $stub ? $this->_stub('Swift_InputByteStream')
