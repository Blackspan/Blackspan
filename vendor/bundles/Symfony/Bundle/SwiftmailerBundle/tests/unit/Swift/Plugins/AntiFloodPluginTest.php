...
class Swift_Plugins_AntiFloodPluginTest extends Swift_Tests_SwiftUnitTestCase {
	...
	public function testPluginStopsConnectionAfterThreshold() {
		...
		$this
				->_checking(
						Expectations::create()->one($transport)->start()
								->one($transport)->stop()->ignoring($transport));
	...
	public function testPluginCanStopAndStartMultipleTimes() {
		...
		$this
				->_checking(
						Expectations::create()->exactly(5)->of($transport)
								->start()->exactly(5)->of($transport)->stop()
								->ignoring($transport));
	...
	public function testPluginCanSleepDuringRestart() {
		...
		$this
				->_checking(
						Expectations::create()->one($sleeper)->sleep(10)
								->one($transport)->start()->one($transport)
								->stop()->ignoring($transport));
		...
		$this
				->_checking(
						Expectations::create()->ignoring($evt)->getSource()
								->returns($transport)->ignoring($evt)
								->getTransport()->returns($transport));
		return $evt;
	}
