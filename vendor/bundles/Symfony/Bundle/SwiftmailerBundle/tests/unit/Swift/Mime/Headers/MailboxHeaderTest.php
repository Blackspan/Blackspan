...
class Swift_Mime_Headers_MailboxHeaderTest extends Swift_Tests_SwiftUnitTestCase {
	...
	public function testTypeIsMailboxHeader() {
		...
		$this
				->assertEqual(Swift_Mime_Header::TYPE_MAILBOX,
						$header->getFieldType());
	}
	...
	public function testMailboxIsSetForAddress() {
		...
		$this
				->assertEqual(array('chris@swiftmailer.org'),
	...
	public function testMailboxIsRenderedForNameAddress() {
		...
		$this
				->assertEqual(array('Chris Corbyn <chris@swiftmailer.org>'),
						$header->getNameAddressStrings());
	}
		...
		$this
				->_checking(
						Expectations::create()->one($encoder)
								->encodeString($name, any(), any())
								->returns('C=8Frbyn')->ignoring($encoder));
	...
	public function testGetValueReturnsMailboxStringValue() {
		...
		$header
				->setNameAddresses(
						array('chris@swiftmailer.org' => 'Chris Corbyn'));
		$this
				->assertEqual('Chris Corbyn <chris@swiftmailer.org>',
						$header->getFieldBody());
	}
	...
	public function testRemoveAddressesWithSingleValue() {
		...
		$this
				->assertEqual(array('mark@swiftmailer.org'),
						$header->getAddresses());
	}
	...
	public function testSetBodyModel() {
		...
		$this
				->assertEqual(array('chris@swiftmailer.org' => null),
						$header->getNameAddresses());
	}
