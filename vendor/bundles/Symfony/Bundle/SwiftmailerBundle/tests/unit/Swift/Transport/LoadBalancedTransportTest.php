...
class Swift_Transport_LoadBalancedTransportTest extends
	...
	public function testEachTransportIsUsedInTurn() {
		...
		$context
				->checking(
						Expectations::create()->ignoring($message1)
								->ignoring($message2)->allowing($t1)
								->isStarted()->returns(false)
								->when($con1->is('off'))->allowing($t1)
								->isStarted()->returns(true)
								->when($con1->is('on'))->allowing($t1)->start()
								->when($con1->is('off'))->then($con1->is('on'))
								->one($t1)->send($message1, optional())
								->returns(1)->when($con1->is('on'))->never($t1)
								->send($message2, optional())->ignoring($t1)
								->allowing($t2)->isStarted()->returns(false)
								->when($con2->is('off'))->allowing($t2)
								->isStarted()->returns(true)
								->when($con2->is('on'))->allowing($t2)->start()
								->when($con2->is('off'))->then($con2->is('on'))
								->one($t2)->send($message2, optional())
								->returns(1)->when($con2->is('on'))->never($t2)
								->send($message1, optional())->ignoring($t2));
		...
		$context
				->checking(
						Expectations::create()->ignoring($message1)
								...
								->one($t1)->send($message1, optional())
								->returns(1)->when($con1->is('on'))->never($t1)
								->send($message2, optional())->one($t1)
								->send($message3, optional())->returns(1)
								->when($con1->is('on'))->never($t1)
								->send($message4, optional())->ignoring($t1)
								->allowing($t2)->isStarted()->returns(false)
								->when($con2->is('off'))->allowing($t2)
								->isStarted()->returns(true)
								->when($con2->is('on'))->allowing($t2)->start()
								...
								->one($t2)->send($message2, optional())
								->returns(1)->when($con2->is('on'))->never($t2)
								->send($message1, optional())->one($t2)
								->send($message4, optional())->returns(1)
								->when($con2->is('on'))->never($t2)
								->send($message3, optional())->ignoring($t2));
	...
	public function testMessageCanBeTriedOnNextTransportIfExceptionThrown() {
		...
		$context
				->checking(
						Expectations::create()->ignoring($message)
								->allowing($t1)->isStarted()->returns(false)
								->when($con1->is('off'))->allowing($t1)
								->isStarted()->returns(true)
								->when($con1->is('on'))->one($t1)->start()
								->when($con1->isNot('on'))
								->then($con1->is('on'))->one($t1)
								->send($message, optional())->throws($e)
								->when($con1->is('on'))->ignoring($t1)
								->allowing($t2)->isStarted()->returns(false)
								->when($con2->is('off'))->allowing($t2)
								->isStarted()->returns(true)
								->when($con2->is('on'))->one($t2)->start()
								->when($con2->isNot('on'))
								->then($con2->is('on'))->one($t2)
								->send($message, optional())->returns(1)
								->when($con2->is('on'))->ignoring($t2));
		...
		$context
				->checking(
						Expectations::create()->ignoring($message)
								...
								->when($con1->isNot('on'))
								->then($con1->is('on'))->one($t1)
								->send($message, optional())->returns(0)
								->when($con1->is('on'))->ignoring($t1)
								->allowing($t2)->isStarted()->returns(false)
								->when($con2->is('off'))->allowing($t2)
								->isStarted()->returns(true)
	...
	public function testTransportsWhichThrowExceptionsAreNotRetried() {
		...
		$context
				->checking(
						Expectations::create()->ignoring($message1)
								->ignoring($message2)->ignoring($message3)
								->ignoring($message4)->allowing($t1)
								->isStarted()->returns(false)
								->when($con1->is('off'))->allowing($t1)
								->isStarted()->returns(true)
								->when($con1->is('on'))->one($t1)->start()
								->when($con1->isNot('on'))
								->then($con1->is('on'))->one($t1)
								->send($message1, optional())->throws($e)
								->when($con1->is('on'))->never($t1)
								->send($message2, optional())->never($t1)
								->send($message3, optional())->never($t1)
								->send($message4, optional())->ignoring($t1)
								->allowing($t2)->isStarted()->returns(false)
								->when($con2->is('off'))->allowing($t2)
								->isStarted()->returns(true)
								...
								->when($con2->isNot('on'))
								->then($con2->is('on'))->one($t2)
								->send($message1, optional())->returns(1)
								->when($con2->is('on'))->one($t2)
								->send($message2, optional())->returns(1)
								->when($con2->is('on'))->one($t2)
								->send($message3, optional())->returns(1)
								->when($con2->is('on'))->one($t2)
								->send($message4, optional())->returns(1)
	...
	public function testStoppingTransportStopsAllDelegates() {
		...
		$context
				->checking(
						Expectations::create()->allowing($t1)->isStarted()
								->returns(true)->when($con1->is('on'))
								->one($t1)->stop()->when($con1->is('on'))
								->then($con1->is('off'))->ignoring($t1)
								->allowing($t2)->isStarted()->returns(true)
								->when($con2->is('on'))->one($t2)->stop()
								->when($con2->is('on'))->then($con2->is('off'))
								->ignoring($t2));
		...
		$context
				->checking(
						Expectations::create()->ignoring($message1)
								...
								->when($con1->is('off'))->allowing($t1)
								->isStarted()->returns(true)
								->when($con1->is('on'))->exactly(2)->of($t1)
								->start()->when($con1->isNot('on'))
								->then($con1->is('on'))->one($t1)
								->send($message1, optional())->throws($e)
								->when($con1->is('on'))->then($con1->is('off'))
								->one($t1)->send($message2, optional())
								->returns(10)->when($con1->is('on'))
								->ignoring($t1)->allowing($t2)->isStarted()
								->returns(false)->when($con2->is('off'))
								->allowing($t2)->isStarted()->returns(true)
								->when($con2->is('on'))->one($t2)->start()
								->when($con2->isNot('on'))
								->then($con2->is('on'))->one($t2)
								->send($message1, optional())->throws($e)
								->when($con2->is('on'))->never($t2)
								->send($message2, optional())->ignoring($t2));
	...
	public function testFailureReferenceIsPassedToDelegates() {
		...
		$context
				->checking(
						Expectations::create()->ignoring($message)
								->allowing($t1)->isStarted()->returns(false)
								->when($con->is('off'))->allowing($t1)
								->isStarted()->returns(true)
								->when($con->is('on'))->one($t1)->start()
								->when($con->isNot('on'))->then($con->is('on'))
								->one($t1)
								->send($message, reference($failures))
								->returns(1)->when($con->is('on'))
								->ignoring($t1));
	...
	public function testRegisterPluginDelegatesToLoadedTransports() {
		...
		$context
				->checking(
						Expectations::create()->one($t1)
								->registerPlugin($plugin)->one($t2)
								->registerPlugin($plugin)->ignoring($t1)
								->ignoring($t2));
