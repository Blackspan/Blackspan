...
class Swift_MailerTest extends Swift_Tests_SwiftUnitTestCase {
	...
	public function testTransportIsStartedWhenSending() {
		...
		$this
				->_checking(
						Expectations::create()->allowing($transport)
								->isStarted()->returns(false)
								->when($con->is('off'))->allowing($transport)
								->isStarted()->returns(false)
								->when($con->is('on'))->one($transport)
								->start()->when($con->is('off'))
								->then($con->is('on'))->ignoring($transport)
								->ignoring($message));
	...
	public function testMessageIsPassedToTransport() {
		...
		$this
				->_checking(
						Expectations::create()->one($transport)
								->send($message, optional())
								->ignoring($transport)->ignoring($message));
	...
	public function testSendReturnsCountFromTransport() {
		...
		$this
				->_checking(
						Expectations::create()->one($transport)
								->send($message, optional())->returns(57)
								->ignoring($transport)->ignoring($message));
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getTo()
								->returns(
										array('foo&invalid' => 'Foo',
												'bar@valid.tld' => 'Bar'))
								->one($transport)
								->send($message, reference($failures))
								->throws($rfcException)->ignoring($transport)
								->ignoring($message));

		$mailer = $this->_createMailer($transport);
		$this
				->assertEqual(0, $mailer->send($message, $failures),
						'%s: Should return 0');
		$this
				->assertEqual(array('foo&invalid', 'bar@valid.tld'), $failures,
