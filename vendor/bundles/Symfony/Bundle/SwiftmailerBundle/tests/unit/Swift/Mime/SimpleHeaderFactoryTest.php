...
class Swift_Mime_SimpleHeaderFactoryTest extends Swift_Tests_SwiftUnitTestCase {
	...
	public function testMailboxHeaderHasCorrectModel() {
		$header = $this->_factory
				->createMailboxHeader('X-Foo', array('foo@bar' => 'FooBar'));
		$this
				->assertEqual(array('foo@bar' => 'FooBar'),
	...
	public function testParameterizedHeaderHasCorrectParams() {
		$header = $this->_factory
				->createParameterizedHeader('X-Foo', 'bar',
						array('zip' => 'button'));
		...
		$this
				->_checking(
						Expectations::create()->one($encoder)
								->charsetChanged('utf-8')->one($paramEncoder)
	...
	private function _createHeaderEncoder($stub = true) {
		return $stub ? $this->_stub('Swift_Mime_HeaderEncoder')
				: $this->_mock('Swift_Mime_HeaderEncoder');
	}

	private function _createParamEncoder($stub = true) {
		return $stub ? $this->_stub('Swift_Encoder')
