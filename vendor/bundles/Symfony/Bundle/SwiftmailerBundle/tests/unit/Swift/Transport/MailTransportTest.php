...
class Swift_Transport_MailTransportTest extends Swift_Tests_SwiftUnitTestCase {
	...
	public function testTransportInvokesMailOncePerMessage() {
		...
		$this
				->_checking(
						Expectations::create()->one($invoker)
								->mail(any(), any(), any(), any(), optional())
								->ignoring($dispatcher)->ignoring($headers)
								->ignoring($message));
	...
	public function testTransportUsesToFieldBodyInSending() {
		...
		$headers = $this->_createHeaders(array('To' => $to));
		$message = $this->_createMessage($headers);

		$this
				->_checking(
						Expectations::create()->allowing($to)->getFieldBody()
								->returns("Foo <foo@bar>")->one($invoker)
								->mail("Foo <foo@bar>", any(), any(), any(),
										optional())->ignoring($dispatcher)
								->ignoring($headers)->ignoring($message)
								->ignoring($to));
	...
	public function testTransportUsesSubjectFieldBodyInSending() {
		...
		$headers = $this->_createHeaders(array('Subject' => $subj));
		$message = $this->_createMessage($headers);

		$this
				->_checking(
						Expectations::create()->allowing($subj)->getFieldBody()
								->returns("Thing")->one($invoker)
								->mail(any(), "Thing", any(), any(), optional())
								->ignoring($dispatcher)->ignoring($headers)
								->ignoring($message)->ignoring($subj));
	...
	public function testTransportUsesBodyOfMessage() {
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->toString()
								->returns(
										"To: Foo <foo@bar>\r\n" . "\r\n"
												. "This body")->one($invoker)
								->mail(any(), any(), "This body", any(),
										optional())->ignoring($dispatcher)
								->ignoring($headers)->ignoring($message));
						...
						Expectations::create()->allowing($message)->toString()
								->returns(
										"Subject: Stuff\r\n" . "\r\n"
												. "This body")->one($invoker)
								->mail(any(), any(), any(),
										"Subject: Stuff" . PHP_EOL, optional())
								->ignoring($dispatcher)->ignoring($headers)
								->ignoring($message));
	...
	public function testTransportReturnsCountOfAllRecipientsIfInvokerReturnsTrue() {
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getTo()
								->returns(
										array('foo@bar' => null,
												'zip@button' => null))
								->allowing($message)->getCc()
								->returns(array('test@test' => null))
								->one($invoker)
								->mail(any(), any(), any(), any(), optional())
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)->getTo()
								...
								->one($invoker)
								->mail(any(), any(), any(), any(), optional())
								->returns(false)->ignoring($dispatcher)
								->ignoring($headers)->ignoring($message));
	...
	public function testToHeaderIsRemovedFromHeaderSetDuringSending() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)->remove('To')
								->one($invoker)
								->mail(any(), any(), any(), any(), optional())
								->ignoring($dispatcher)->ignoring($headers)
								->ignoring($message)->ignoring($to));
	...
	public function testSubjectHeaderIsRemovedFromHeaderSetDuringSending() {
		...
		$headers = $this
				->_createHeaders(array('Subject' => $subject));
		$message = $this->_createMessage($headers);

		$this
				->_checking(
						Expectations::create()->one($headers)
								->remove('Subject')->one($invoker)
								->mail(any(), any(), any(), any(), optional())
								->ignoring($dispatcher)->ignoring($headers)
								->ignoring($message)->ignoring($subject));
	...
	public function testSubjectHeaderIsPutBackAfterSending() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->set($subject, optional())->one($invoker)
								->mail(any(), any(), any(), any(), optional())
								->ignoring($dispatcher)->ignoring($headers)
								->ignoring($message)->ignoring($subject));
	...
	private function _createMessage($headers) {
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)
	...
	private function _createHeaders($headers = array()) {
		...
		if (count($headers) > 0) {
			foreach ($headers as $name => $header) {
				$this
						->_checking(
								Expectations::create()->allowing($set)
										->get($name)->returns($header)
										->allowing($set)->has($name)
										->returns(true));
			}
		}
		...
		$this
				->_checking(
						Expectations::create()->allowing($set)->get(any())
								->returns($header)->allowing($set)->has(any())
								->returns(true)->ignoring($header));
