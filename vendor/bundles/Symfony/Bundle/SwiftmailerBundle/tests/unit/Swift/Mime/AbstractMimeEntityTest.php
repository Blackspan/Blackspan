...
abstract class Swift_Mime_AbstractMimeEntityTest extends
		Swift_Tests_SwiftUnitTestCase {

	public function testGetHeadersReturnsHeaderSet() {
		$headers = $this->_createHeaderSet();
		$entity = $this
				->_createEntity($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testContentTypeIsSetInHeader() {
		$ctype = $this
				->_createHeader('Content-Type', 'text/plain', array(), false);
		$headers = $this->_createHeaderSet(array('Content-Type' => $ctype));
		$this
				->_checking(
						Expectations::create()->one($ctype)
								->setFieldBodyModel('image/jpeg')
								->ignoring($ctype));
		$entity = $this
				->_createEntity($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testContentTypeHeaderIsAddedIfNoneSet() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addParameterizedHeader('Content-Type',
										'image/jpeg')->ignoring($headers));
		$entity = $this
				->_createEntity($headers, $this->_createEncoder(),
						$this->_createCache());
		$entity->setContentType('image/jpeg');
	...
	public function testGetEncoderFromConstructor() {
		...
		$entity = $this
				->_createEntity($this->_createHeaderSet(), $encoder,
						$this->_createCache());
	...
	public function testSettingEncoderUpdatesTransferEncoding() {
		$encoder = $this->_createEncoder('base64');
		$encoding = $this
				->_createHeader('Content-Transfer-Encoding', '8bit', array(),
						false);
		$headers = $this
				->_createHeaderSet(
						array('Content-Transfer-Encoding' => $encoding));
		$this
				->_checking(
						Expectations::create()->one($encoding)
								->setFieldBodyModel('base64')
								->ignoring($encoding));
		$entity = $this
				->_createEntity($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testSettingEncoderAddsEncodingHeaderIfNonePresent() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addTextHeader('Content-Transfer-Encoding',
										'something')->ignoring($headers));
		$entity = $this
				->_createEntity($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testIdIsSetInHeader() {
		...
		$this
				->_checking(
						Expectations::create()->one($cid)
								->setFieldBodyModel('foo@bar')->ignoring($cid));
		$entity = $this
				->_createEntity($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testIdIsAutoGenerated() {
		$entity = $this
				->_createEntity($this->_createHeaderSet(),
	...
	public function testDescriptionIsReadFromHeader() {
		...
		$headers = $this
				->_createHeaderSet(array('Content-Description' => $desc));
		$entity = $this
				->_createEntity($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testDescriptionIsSetInHeader() {
		...
		$this
				->_checking(
						Expectations::create()->one($desc)
								->setFieldBodyModel('whatever')
								->ignoring($desc));
		$entity = $this
				->_createEntity($headers, $this->_createEncoder(),
						$this->_createCache());
	...
	public function testDescriptionHeaderIsAddedIfNotPresent() {
		...
		$this
				->_checking(
						Expectations::create()->one($headers)
								->addTextHeader('Content-Description',
										'whatever')->ignoring($headers));
		$entity = $this
				->_createEntity($headers, $this->_createEncoder(),
						$this->_createCache());
		$entity->setDescription('whatever');
	...
	public function testEncoderIsUsedForStringGeneration() {
		...
		$this
				->_checking(
						Expectations::create()->one($encoder)
								->encodeString('blah', optional())
								->ignoring($encoder));
		$entity = $this
				->_createEntity($this->_createHeaderSet(), $encoder,
						$this->_createCache());
	...
	public function testHeadersAppearInString() {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($headers)->toString()
								->returns(
	...
	public function testChildrenOfLevelAttachmentAndLessCauseMultipartMixed() {
		for ($level = Swift_Mime_MimeEntity::LEVEL_MIXED; $level
				> Swift_Mime_MimeEntity::LEVEL_TOP; $level /= 2) {
			$child = $this->_createChild($level);
			$cType = $this
					->_createHeader('Content-Type', 'text/plain', array(),
							false);
			$this
					->_checking(
							Expectations::create()->one($cType)
									->setFieldBodyModel('multipart/mixed')
									->ignoring($cType));
			$entity = $this
					->_createEntity(
							$this
	...
	public function testChildrenOfLevelAlternativeAndLessCauseMultipartAlternative() {
		for ($level = Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE; $level
			...
			$this
					->_checking(
							Expectations::create()->one($cType)
									->setFieldBodyModel('multipart/alternative')
									->ignoring($cType));
			$entity = $this
					->_createEntity(
							$this
	...
	public function testChildrenOfLevelRelatedAndLessCauseMultipartRelated() {
		for ($level = Swift_Mime_MimeEntity::LEVEL_RELATED; $level
				> Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE; $level /= 2) {
			$child = $this->_createChild($level);
			$cType = $this
					->_createHeader('Content-Type', 'text/plain', array(),
							false);
			$this
					->_checking(
							Expectations::create()->one($cType)
									->setFieldBodyModel('multipart/related')
									->ignoring($cType));
			$entity = $this
					->_createEntity(
							$this
	...
	public function testHighestLevelChildDeterminesContentType() {
		$combinations = array(
				array(
						'levels' => array(Swift_Mime_MimeEntity::LEVEL_MIXED,
		...
		foreach ($combinations as $combination) {
			...
			$this
					->_checking(
							Expectations::create()->one($cType)
									->setFieldBodyModel($combination['type'])
									->ignoring($cType));
			$entity = $this
					->_createEntity(
							$this
	...
	public function testChildrenAppearNestedInString() {
		...
		$child1 = $this
				->_createChild(Swift_Mime_MimeEntity::LEVEL_ALTERNATIVE,
	...
	public function testMixingLevelsIsHierarchical() {
		...
		$attachment = $this
				->_createChild(Swift_Mime_MimeEntity::LEVEL_MIXED,
		...
		$this
				->_checking(
						Expectations::create()->ignoring($headers)->toString()
								...
								->ignoring($headers)->newInstance()
								->returns($newHeaders)->ignoring($headers)
								->ignoring($newHeaders)->toString()
								->returns(
	...
	public function testSettingEncoderNotifiesChildren() {
		...
		$this
				->_checking(
						Expectations::create()->one($child)
								->encoderChanged($encoder)->ignoring($child));
	...
	public function testEntityIsWrittenToByteStream() {
		...
		$this
				->_checking(
						Expectations::create()->atLeast(1)->of($is)
								->write(any())->ignoring($is));
	...
	public function testEntityHeadersAreComittedToByteStream() {
		...
		$this
				->_checking(
						Expectations::create()->atLeast(1)->of($is)->commit()
		...
		$this
				->_checking(
						Expectations::create()->ignoring($headers)->toString()
								->returns(
										"Content-Type: multipart/alternative; boundary=\"xxx\"\r\n")
								->ignoring($headers)->ignoring($htmlChild)
								->getContentType()->returns('text/html')
								->ignoring($htmlChild)->ignoring($textChild)
	...
	public function testUnsettingChildrenRestoresContentType() {
		...
		$this
				->_checking(
						Expectations::create()->one($cType)
								->setFieldBodyModel('image/jpeg')
								->inSequence($s)->one($cType)
								->setFieldBodyModel('multipart/alternative')
								->inSequence($s)->one($cType)
								->setFieldBodyModel('image/jpeg')
								->inSequence($s)->ignoring($cType));
	...
	public function testBodyIsReadFromCacheWhenUsingToStringIfPresent() {
		...
		$this
				->_checking(
						Expectations::create()->one($cache)
								->hasKey(any(), 'body')->returns(true)
								->one($cache)->getString(any(), 'body')
								->returns("\r\ncache\r\ncache!")
								->ignoring($cache));

		$entity = $this
				->_createEntity($headers, $this->_createEncoder(), $cache);
	...
	public function testBodyIsClearedFromCacheIfNewBodySet() {
		...
		$this
				->_checking(
						Expectations::create()->one($cache)
								->clearKey(any(), 'body')->ignoring($cache));
	...
	public function testBodyIsNotClearedFromCacheIfSameBodySet() {
		...
		$this
				->_checking(
						Expectations::create()->never($cache)
								->clearKey(any(), 'body')->ignoring($cache));
		...
		$this
				->_checking(
						Expectations::create()->one($cache)
								->hasKey(any(), 'body')->returns(true)
								->one($cache)
								->exportToByteStream(any(), 'body', $is)
								->ignoring($cache));
		...
		$this
				->assertSame($entity,
						$entity->setContentType('text/plain')
								...
								->setMaxLineLength(998)->setBody('xx')
								->setBoundary('xyz')->setChildren(array()));
	}
	...
	protected function _createChild($level = null, $string = '', $stub = true) {
		...
		if (isset($level)) {
			$this
					->_checking(
							Expectations::create()->ignoring($child)
									->getNestingLevel()->returns($level));
		}
		$this
				->_checking(
						Expectations::create()->ignoring($child)->toString()
								->returns($string));
		if ($stub) {
			$this
					->_checking(
	...
	protected function _createEncoder($name = 'quoted-printable', $stub = true) {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($encoder)->getName()
								->returns($name));

		if ($stub) {
			$this
					->_checking(
							Expectations::create()->ignoring($encoder)
									->encodeString(any(), optional())
									->calls(
											array($this,
													'returnStringFromEncoder'))
									->ignoring($encoder));
		}
		return $encoder;
	}
	...
	protected function _createCache($stub = true) {
		...
		if ($stub) {
			$this
					->_checking(
							Expectations::create()->ignoring($cache));
		}
		return $cache;
	}
	...
	protected function _createHeaderSet($headers = array(), $stub = true) {
		...
		foreach ($headers as $key => $header) {
			$this
					->_checking(
							Expectations::create()->ignoring($set)->has($key)
									->returns(true)->ignoring($set)->get($key)
									->returns($header));
		}
		if ($stub) {
			$this
					->_checking(
							Expectations::create()->ignoring($set)
									->newInstance()->returns($set)
									->ignoring($set));
		}
		return $set;
	}

	protected function _createHeader($name, $model = null, $params = array(),
			$stub = true) {
		$header = $this->_mock('Swift_Mime_ParameterizedHeader');
		$this
				->_checking(
						Expectations::create()->ignoring($header)
								->getFieldName()->returns($name)
								->ignoring($header)->getFieldBodyModel()
								->returns($model));
		foreach ($params as $key => $value) {
			$this
					->_checking(
							Expectations::create()->ignoring($header)
									->getParameter($key)->returns($value));
		}
		if ($stub) {
			$this
					->_checking(
	...
	protected function _createOutputStream($data = null, $stub = true) {
		...
		if (isset($data)) {
			$pos = $this->_mockery()->states('position')
					->startsAs('at beginning');
			$this
					->_checking(
							Expectations::create()->ignoring($os)
									->read(optional())->returns($data)
									->when($pos->isNot('at end'))
									->then($pos->is('at end'))
									->ignoring($os)->read(optional())
									->returns(false));
			if ($stub) {
				$this
						->_checking(
	...
	protected function _createInputStream($stub = true) {
		...
		if ($stub) {
			$this
					->_checking(Expectations::create()->ignoring($is));
		}
		return $is;
	}
