...
class Swift_CharacterStream_ArrayCharacterStreamTest extends
	...
	public function testValidatorAlgorithmOnImportString() {
		...
		$stream = new Swift_CharacterStream_ArrayCharacterStream($factory,
				'utf-8');

		$seq = $this->_mockery()->sequence('read-sequence');
		$this
				->_checking(
						Expectations::create()->ignoring($reader)
								->getInitialByteSize()->returns(1)
								->one($reader)
								->validateByteSequence(array(0xD0), 1)
								->inSequence($seq)->returns(1)->one($reader)
								->validateByteSequence(array(0xD0), 1)
								->inSequence($seq)->returns(1)->one($reader)
								->validateByteSequence(array(0xD0), 1)
								->inSequence($seq)->returns(1)->one($reader)
								->validateByteSequence(array(0xD1), 1)
								->inSequence($seq)->returns(1)->one($reader)
								->validateByteSequence(array(0xD0), 1)
								->inSequence($seq)->returns(1)->one($reader)
								->validateByteSequence(array(0xD0), 1)
								->inSequence($seq)->returns(1));

		$stream
				->importString(
						pack('C*', 0xD0, 0x94, 0xD0, 0xB6, 0xD0, 0xBE, 0xD1,
								0x8D, 0xD0, 0xBB, 0xD0, 0xB0));
	}
	...
	public function testCharactersWrittenUseValidator() {
		...
		$stream
				->write(
						pack('C*', 0xD0, 0xBB, 0xD1, 0x8E, 0xD0, 0xB1, 0xD1,
								0x8B, 0xD1, 0x85));
	}
	...
	public function testReadCharactersAreInTact() {
		...
		$this
				->assertIdenticalBinary(pack('C*', 0xD0, 0xB6, 0xD0, 0xBE),
						$stream->read(2));
	...
	public function testCharactersCanBeReadAsByteArrays() {
		...
		$this
				->assertEqual(array(0xD0, 0xB6, 0xD0, 0xBE),
						$stream->readBytes(2));
		$this->assertEqual(array(0xD0, 0xBB), $stream->readBytes(1));
		$this
				->assertEqual(array(0xD1, 0x8E, 0xD0, 0xB1, 0xD1, 0x8B),
						$stream->readBytes(3));
	...
	public function testRequestingByteArrayCountPastEndOfStream() {
		...
		$this
				->assertEqual(array(0xD0, 0x94, 0xD0, 0xB6, 0xD0, 0xBE),
	...
	public function testByteStreamCanBeImportingUsesValidator() {
		...
		$this
				->_checking(
						Expectations::create()->between(0, 1)->of($os)
								->setReadPointer(0)->one($os)->read(any())
								->inSequence($seq)->returns(pack('C*', 0xD0))
								->one($os)->read(any())->inSequence($seq)
								->returns(pack('C*', 0x94))->one($os)
								->read(any())->inSequence($seq)
								->returns(pack('C*', 0xD0))->one($os)
								->read(any())->inSequence($seq)
								->returns(pack('C*', 0xB6))->one($os)
								->read(any())->inSequence($seq)
								->returns(pack('C*', 0xD0))->one($os)
								->read(any())->inSequence($seq)
								->returns(pack('C*', 0xBE))->ignoring($os)
								->read(any())->returns(false));
	...
	public function testAlgorithmWithFixedWidthCharsets() {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($reader)
								->getInitialByteSize()->returns(2)
								->one($reader)
								->validateByteSequence(array(0xD1, 0x8D), 2)
								->inSequence($seq)->one($reader)
								->validateByteSequence(array(0xD0, 0xBB), 2)
								->inSequence($seq)->one($reader)
								->validateByteSequence(array(0xD0, 0xB0), 2)
								->inSequence($seq));
	...
	private function _getFactory($reader) {
		...
		$this
				->_checking(
						Expectations::create()->allowing($factory)
								->getReaderFor('utf-8')->returns($reader));
		return $factory;
	}
