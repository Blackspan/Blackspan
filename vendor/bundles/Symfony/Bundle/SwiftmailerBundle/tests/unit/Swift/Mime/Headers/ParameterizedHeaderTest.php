...
class Swift_Mime_Headers_ParameterizedHeaderTest extends
	...
	public function testTypeIsParameterizedHeader() {
		$header = $this
				->_getHeader('Content-Type',
						$this->_getHeaderEncoder('Q', true),
						$this->_getParameterEncoder(true));
		$this
				->assertEqual(Swift_Mime_Header::TYPE_PARAMETERIZED,
						$header->getFieldType());
	}
	...
	public function testSpaceInParamResultsInQuotedString() {
		$header = $this
				->_getHeader('Content-Disposition',
						$this->_getHeaderEncoder('Q', true),
						$this->_getParameterEncoder(true));
		$header->setValue('attachment');
		$header->setParameters(array('filename' => 'my file.txt'));
		$this
				->assertEqual('attachment; filename="my file.txt"',
						$header->getFieldBody());
	}
		...
		$this
				->assertEqual(
						'attachment; ' . 'filename*0*=utf-8\'\''
								...
								. ";\r\n " . 'filename*2*='
								. str_repeat('a', 54), $header->getFieldBody());
	}
	...
	public function testEncodedParamDataIncludesCharsetAndLanguage() {
		...
		$this
				->assertEqual(
						'attachment; filename*=' . $this->_charset . "'"
								. $this->_lang . "'" . str_repeat('a', 20)
								. '%8F' . str_repeat('a', 10),
						$header->getFieldBody());
	}
	...
	public function testMultipleEncodedParamLinesAreFormattedCorrectly() {
		...
		$this
				->assertEqual(
						'attachment; filename*0*=' . $this->_charset . "'"
								. $this->_lang . "'" . str_repeat('a', 20)
								. '%8F' . str_repeat('a', 28) . ";\r\n "
	...
	public function testValueCanBeEncodedIfNonAscii() {
		...
		$this
				->_checking(
						Expectations::create()->one($encoder)
								->encodeString($value, any(), any())
								->returns('fo=8Fbar')->ignoring($encoder));

		$header = $this
				->_getHeader('X-Foo', $encoder,
						$this->_getParameterEncoder(true));
	...
	public function testValueAndParamCanBeEncodedIfNonAscii() {
		...
		$this
				->_checking(
						Expectations::create()->one($paramEncoder)
								->encodeString($value, any(), any())
								->returns('fo%8Fbar')->ignoring($paramEncoder));
	...
	public function testParamsAreEncodedWithEncodedWordsIfNoParamEncoderSet() {
		...
		$header->setParameters(array('says' => $value));
		$this
				->assertEqual("X-Foo: bar; says=\"=?utf-8?Q?fo=8Fbar?=\"\r\n",
						$header->toString());
	}
	...
	public function testSetParameter() {
		...
		$this
				->assertEqual(array('charset' => 'utf-8', 'delsp' => 'no'),
