...
class Swift_Transport_FailoverTransportTest extends
		...
		$context
				->checking(
						Expectations::create()->ignoring($message1)
								...
								->returns(1)->when($con->is('on'))->one($t1)
								->send($message2, optional())->returns(1)
								->when($con->is('on'))->ignoring($t1)
								->never($t2)->start()->never($t2)
								->send(any(), optional())->ignoring($t2));
		...
		$context
				->checking(
						Expectations::create()->ignoring($message)
								...
								->when($con->is('on'))->one($t1)->start()
								->when($con->isNot('on'))->then($con->is('on'))
								->one($t1)->send($message, optional())
								->returns(0)->when($con->is('on'))
								->ignoring($t1));
