...
class Swift_Transport_EsmtpTransportTest extends
	...
	public function testTimeoutCanBeSetAndFetched() {
		...
		$this
				->_checking(
						Expectations::create()->one($buf)
								->setParam('timeout', 10));
		$smtp->setTimeout(10);
		$this
				->assertEqual(10, $smtp->getTimeout(),
	...
	public function testEncryptionCanBeSetAndFetched() {
		...
		$this
				->assertEqual('tls', $smtp->getEncryption(),
						'%s: Crypto should be returned');
	}

	public function testStartSendsHeloToInitiate() {//Overridden for EHLO instead
	}
	...
	public function testInvalidHeloResponseCausesException() {//Overridden to first try EHLO
		$buf = $this->_getBuffer();
		$smtp = $this->_getTransport($buf);
		$s = $this->_sequence('SMTP-convo');
		$this
	...
	public function testDomainNameIsPlacedInHelo() { //Overridden to include ESMTP
	/* -- RFC 2821, 4.1.4.
	...
	public function testFluidInterface() {
		...
		$ref = $smtp->setHost('foo')->setPort(25)->setEncryption('tls')
				->setTimeout(30);
