...
class Swift_KeyCache_ArrayKeyCacheTest extends Swift_Tests_SwiftUnitTestCase {
	...
	public function testStringDataCanBeOverwritten() {
		...
		$cache
				->setString($this->_key1, 'foo', 'test',
						Swift_KeyCache::MODE_WRITE);
		$cache
				->setString($this->_key1, 'foo', 'whatever',
						Swift_KeyCache::MODE_WRITE);
	...
	public function testByteStreamCanBeImported() {
		...
		$this
				->_checking(
						Expectations::create()->one($os)->read(optional())
								->returns('abc')->one($os)->read(optional())
								->returns('def')->one($os)->read(optional())
								->returns(false)->ignoring($os));
		$is = $this->_createKeyCacheInputStream(true);
		$cache = $this->_createCache($is);
		$cache
				->importFromByteStream($this->_key1, 'foo', $os,
						Swift_KeyCache::MODE_WRITE);
	...
	public function testByteStreamCanBeAppended() {
		...
		$this
				->_checking(
						Expectations::create()->one($os1)->read(optional())
								->returns('abc')->one($os1)->read(optional())
								->returns('def')->one($os1)->read(optional())
								->returns(false)->ignoring($os1)
		...
		$cache
				->importFromByteStream($this->_key1, 'foo', $os1,
						Swift_KeyCache::MODE_APPEND);
		$cache
				->importFromByteStream($this->_key1, 'foo', $os2,
						Swift_KeyCache::MODE_APPEND);

		$this
				->assertEqual('abcdefxyzuvw',
						$cache->getString($this->_key1, 'foo'));
	}
	...
	private function _createKeyCacheInputStream($stub = false) {
		return $stub ? $this->_stub('Swift_KeyCache_KeyCacheInputStream')
