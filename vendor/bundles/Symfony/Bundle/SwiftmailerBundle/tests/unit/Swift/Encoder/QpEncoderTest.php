...
class Swift_Encoder_QpEncoderTest extends Swift_Tests_SwiftUnitTestCase {
			...
			$this
					->_checking(
							Expectations::create()->one($charStream)
									...
									->importString($char)->one($charStream)
									->readBytes(optional())
									->returns(array($ordinal))->atLeast(1)
									->of($charStream)->readBytes(optional())
									->returns(false));
	...
	public function testWhiteSpaceAtLineEndingIsEncoded() {
		...
		$this
				->_checking(
						Expectations::create()->one($charStream)
								->flushContents()->one($charStream)
								->importString($string)->one($charStream)
								->readBytes(optional())->inSequence($seq)
								->returns(array(ord('a')))->one($charStream)
								->readBytes(optional())->inSequence($seq)
		...
		$this
				->_checking(
						Expectations::create()->one($charStream)
								...
								->returns(array(0x0A))->one($charStream)
								->readBytes(optional())->inSequence($seq)
								->returns(array(ord('c')))->one($charStream)
								->readBytes(optional())->inSequence($seq)
								->returns(array(0x0D))->one($charStream)
								->readBytes(optional())->inSequence($seq)
								->returns(array(0x0A))->one($charStream)
	...
	public function testLinesLongerThan76CharactersAreSoftBroken() {
		...
		$exps->one($charStream)->flushContents()->one($charStream)
				->importString($input);

		$output = '';
		for ($i = 0; $i < 140; ++$i) {
			$exps->one($charStream)->readBytes(optional())->inSequence($seq)
					->returns(array(ord('a')));
		...
		$this
				->_checking(
						Expectations::create()->one($charStream)
								->flushContents()->one($charStream)
								->importString($char)->one($charStream)
								->readBytes(optional())->returns(array(61))
								->atLeast(1)->of($charStream)
								->readBytes(optional())->returns(false));
	...
	public function testFirstLineLengthCanBeDifferent() {
		...
		$encoder = new Swift_Encoder_QpEncoder($charStream);
		$this
				->assertEqual($output, $encoder->encodeString($input, 22),
