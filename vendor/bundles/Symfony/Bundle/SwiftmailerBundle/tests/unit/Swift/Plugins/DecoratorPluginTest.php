...
class Swift_Plugins_DecoratorPluginTest extends Swift_Tests_SwiftUnitTestCase {
	public function testMessageBodyReceivesReplacements() {
		$message = $this
				->_createMessage($this->_createHeaders(),
						array('zip@button.tld' => 'Zipathon'),
						array('chris.corbyn@swiftmailer.org' => 'Chris'),
						'Subject', 'Hello {name}, you are customer #{id}');
		$this
				->_checking(
						Expectations::create()->one($message)
								->setBody('Hello Zip, you are customer #456')
								->ignoring($message));

		$plugin = $this
				->_createPlugin(
						array(
								'zip@button.tld' => array('{name}' => 'Zip',
										'{id}' => '456')));
		...
		$this
				->_checking(
						Expectations::create()->one($message)
								...
								->one($message)
								->setBody('Hello Foo, you are customer #123')
								->ignoring($message));

		$plugin = $this
				->_createPlugin(
						array(
								'foo@bar.tld' => array('{name}' => 'Foo',
										'{id}' => '123'),
								'zip@button.tld' => array('{name}' => 'Zip',
										'{id}' => '456')));
	...
	public function testReplacementsCanBeMadeInHeaders() {
		$headers = $this
				->_createHeaders(
						array(
								$returnPathHeader = $this
										->_createHeader('Return-Path',
												'foo-{id}@swiftmailer.org'),
								$toHeader = $this
										->_createHeader('Subject',
												'A message for {name}!')));
		...
		$this
				->_checking(
						Expectations::create()->one($message)
								->setBody('Hello Zip, you are customer #456')
								->one($toHeader)
								->setFieldBodyModel('A message for Zip!')
								->one($returnPathHeader)
								->setFieldBodyModel('foo-456@swiftmailer.org')
								->ignoring($message)->ignoring($toHeader)
	...
	public function testReplacementsAreMadeOnSubparts() {
		$part1 = $this
				->_createPart('text/plain', 'Your name is {name}?', '1@x');
		$part2 = $this
				->_createPart('text/html', 'Your <em>name</em> is {name}?',
						'2@x');
		$message = $this
				->_createMessage($this->_createHeaders(),
						array('zip@button.tld' => 'Zipathon'),
		...
		$this
				->_checking(
						Expectations::create()->ignoring($message)
								->getChildren()->returns(array($part1, $part2))
								->one($part1)->setBody('Your name is Zip?')
								->one($part2)
								->setBody('Your <em>name</em> is Zip?')
								->ignoring($part1)->ignoring($part2)
								->ignoring($message));
	...
	public function testReplacementsCanBeTakenFromCustomReplacementsObject() {
		...
		$this
				->_checking(
						Expectations::create()->one($message)
								->setBody('Something b')->one($message)
								->setBody('Something c')->one($replacements)
								->getReplacementsFor('foo@bar')
								->returns(array('{a}' => 'b'))
								->one($replacements)
								->getReplacementsFor('zip@zap')
								->returns(array('{a}' => 'c'))
								->ignoring($message));
	...
	private function _createMessage($headers, $to = array(), $from = null,
			$subject = null, $body = null) {
		$message = $this->_mock('Swift_Mime_Message');
		foreach ($to as $addr => $name) {
			$this
					->_checking(
							Expectations::create()->one($message)->getTo()
		...
		$this
				->_checking(
						Expectations::create()->allowing($message)
								...
								->ignoring($message)->getSubject()
								->returns($subject)->ignoring($message)
	...
	private function _createSendEvent(Swift_Mime_Message $message) {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($evt)->getMessage()
								->returns($message)->ignoring($evt));
		return $evt;
	}
	...
	private function _createPart($type, $body, $id) {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($part)
								->getContentType()->returns($type)
	...
	private function _createHeaders($headers = array()) {
		...
		$this
				->_checking(
						Expectations::create()->allowing($set)->getAll()
								->returns($headers)->ignoring($set));
