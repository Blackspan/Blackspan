...
class Swift_Transport_Esmtp_AuthHandlerTest extends
	...
	public function testMixinMethods() {
		...
		$this
				->assertTrue(in_array('getUsername', $mixins),
						'%s: getUsername() should be accessible via mixin');
		$this
				->assertTrue(in_array('setUsername', $mixins),
						'%s: setUsername() should be accessible via mixin');
		$this
				->assertTrue(in_array('getPassword', $mixins),
						'%s: getPassword() should be accessible via mixin');
		$this
				->assertTrue(in_array('setPassword', $mixins),
						'%s: setPassword() should be accessible via mixin');
		$this
				->assertTrue(in_array('setAuthMode', $mixins),
						'%s: setAuthMode() should be accessible via mixin');
		$this
				->assertTrue(in_array('getAuthMode', $mixins),
	...
	public function testAuthenticatorsAreCalledAccordingToParamsAfterEhlo() {
		...
		$this
				->_checking(
						Expectations::create()->never($a1)
								->authenticate($this->_agent, 'jack', 'pass')
								->one($a2)
								->authenticate($this->_agent, 'jack', 'pass')
								->returns(true));
	...
	public function testSeveralAuthenticatorsAreTriedIfNeeded() {
		...
		$this
				->_checking(
						Expectations::create()->one($a1)
								->authenticate($this->_agent, 'jack', 'pass')
								->returns(false)->one($a2)
								->authenticate($this->_agent, 'jack', 'pass')
								->returns(true));
	...
	private function _createMockAuthenticator($type) {
		...
		$this
				->_checking(
						Expectations::create()->ignoring($authenticator)
								->getAuthKeyword()->returns($type));
