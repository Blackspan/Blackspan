...
class Swift_Events_SimpleEventDispatcherTest extends
	...
	public function testCommandEventCanBeCreated() {
		...
		$evt = $this->_dispatcher
				->createCommandEvent($buf, "FOO\r\n", array(250));
	...
	public function testResponseEventCanBeCreated() {
		$buf = $this->_stub('Swift_Transport');
		$evt = $this->_dispatcher
				->createResponseEvent($buf, "250 Ok\r\n", true);
	...
	public function testTransportExceptionEventCanBeCreated() {
		...
		$evt = $this->_dispatcher
				->createTransportExceptionEvent($transport, $ex);
	...
	public function testListenersAreNotifiedOfDispatchedEvent() {
		...
		$this
				->_checking(
						Expectations::create()->one($listenerA)
								->transportStarted($evt)->one($listenerB)
								->transportStarted($evt));
	...
	public function testListenersAreOnlyCalledIfImplementingCorrectInterface() {
		...
		$this
				->_checking(
						Expectations::create()->one($targetListener)
								->sendPerformed($evt)->never($otherListener));
	...
	public function testListenersCanCancelBubblingOfEvent() {
		...
		$this
				->_checking(
						Expectations::create()->one($listenerA)
								->sendPerformed($evt)
								->calls(array($this, '_cancelBubble'))
								->never($listenerB));
	...
	public function testAddingListenerTwiceDoesNotReceiveEventTwice() {
		...
		$this
				->_checking(
						Expectations::create()->one($listener)
								->transportStarted($evt)->never($listener)
								->transportStarted($evt));
