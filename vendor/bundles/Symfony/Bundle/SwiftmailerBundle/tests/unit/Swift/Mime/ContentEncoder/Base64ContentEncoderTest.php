...
class Swift_Mime_ContentEncoder_Base64ContentEncoderTest extends
	...
	public function testInputOutputRatioIs3to4Bytes() {
		...
		$this
				->_checking(
						Expectations::create()->allowing($is)
								->write(any(), optional())->will($collection)
								->ignoring($is)
								->one($os)->read(optional())->returns('123')
								->allowing($os)->read(optional())
								->returns(false)
								->ignoring($os));
			...
			$this
					->_checking(
							Expectations::create()->allowing($is)
									...
									->one($os)->read(optional())
									->returns(pack('C', rand(0, 255)))
									->allowing($os)->read(optional())
									->returns(false)->ignoring($os));

			$this->_encoder->encodeByteStream($os, $is);
			$this
					->assertPattern('~^[a-zA-Z0-9/\+]{2}==$~',
							$collection->content,
		...
		$this
				->_checking(
						Expectations::create()->allowing($is)
								...
								->ignoring($is)
								->one($os)->read(optional())
								->returns('abcdefghijkl') //12
								->one($os)->read(optional())
								->returns('mnopqrstuvwx') //24
								->one($os)->read(optional())
								->returns('yzabc1234567') //36
								->one($os)->read(optional())
								->returns('890ABCDEFGHI') //48
								->one($os)->read(optional())
								->returns('JKLMNOPQRSTU') //60
								->one($os)->read(optional())
								->returns('VWXYZ1234567') //72
								->one($os)->read(optional())
								->returns('abcdefghijkl') //84
								->allowing($os)->read(optional())
								->returns(false)->ignoring($os));
