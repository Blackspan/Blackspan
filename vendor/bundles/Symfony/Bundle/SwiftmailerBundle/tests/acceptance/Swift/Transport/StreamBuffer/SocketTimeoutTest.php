...
class Swift_Transport_StreamBuffer_SocketTimeoutTest extends
	...
	public function skip() {
		...
		for ($i = 0; $i < 5; ++$i) {
			...
			$this->_server = stream_socket_server(
					'tcp://127.0.0.1:' . $this->_randomHighPort);
			if ($this->_server) {
				$serverStarted = true;
			}
		}
		$this
				->skipUnless(SWIFT_SMTP_HOST,
	...
	protected function _initializeBuffer() {
		...
		$this->_buffer
				->initialize(
						array('type' => Swift_Transport_IoBuffer::TYPE_SOCKET,
								'host' => $host, 'port' => $port,
								'protocol' => 'tcp', 'blocking' => 1,
								'timeout' => 1));
	}
