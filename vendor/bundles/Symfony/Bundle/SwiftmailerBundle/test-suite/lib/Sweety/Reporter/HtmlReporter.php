...
class Sweety_Reporter_HtmlReporter implements Sweety_Reporter {
	...
	public function reportSkip($message, $path) {
		$this->_tplVars['messages'][] = array('type' => 'skip',
				'path' => $path, 'text' => $message);
	}
	...
	public function reportFail($message, $path) {
		...
		$this->_tplVars['messages'][] = array('type' => 'fail',
				'path' => $path, 'text' => $message);
	}
	...
	public function reportException($message, $path) {
		...
		$this->_tplVars['messages'][] = array('type' => 'exception',
				'path' => $path, 'text' => $message);
	}
	...
	public function reportOutput($output, $path) {
		$this->_tplVars['messages'][] = array('type' => 'output',
				'path' => $path, 'text' => $output);
	}
	...
	public function finish() {
		...
		$incomplete = $this->_tplVars['caseCount']
				- $this->_tplVars['runCount'];

		if (0 < $incomplete) {
			$this->_tplVars['messages'][] = array('type' => 'internal',
					'path' => '',
