...
abstract class Sweety_Runner_AbstractTestRunner implements Sweety_Runner {
	...
	protected function parseXml($xml, $testCase) {
		...
		$xml = preg_replace('/[^\x01-\x7F]/e', 'sprintf("&#%d;", ord("$0"));',
				$xml); //Do something better?
		if (!empty($xml)) {
				...
				->reportException(
						'Invalid XML response: '
								. trim(
										strip_tags(
												preg_replace(
														'/^\s*<\?xml.+<\/(?:name|pass|fail|exception)>/s',
														'', $xml))), $testCase);
	}
	...
	private function _parseTestCase(SimpleXMLElement $testCase, $path = '',
		...
		foreach ($testCase->xpath('./test') as $testMethod) {
			$testMethodName = (string) $this
					->_firstNodeValue($testMethod->xpath('./name'));

			foreach ($testMethod->xpath('./formatted') as $formatted) {
				$this
						->_parseFormatted($formatted,
								$path . ' -> ' . $testMethodName, $reporter);
			}
			...
			foreach ($testMethod->xpath('./pass') as $pass) {
				$this
						->_parsePass($pass, $path . ' -> ' . $testMethodName,
								$reporter);
			}
	...
	private function _parseResults(SimpleXMLElement $document, $path = '',
		...
		if (!empty($groups)) {
			foreach ($groups as $group) {
				$groupName = (string) $this
						->_firstNodeValue($group->xpath('./name'));
				$this
						->_parseResults($group, $path . ' -> ' . $groupName,
								$reporter);
			}
		} else {
