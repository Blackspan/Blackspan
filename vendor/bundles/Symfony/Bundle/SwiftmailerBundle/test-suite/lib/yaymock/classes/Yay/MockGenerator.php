...
class Yay_MockGenerator {
	...
	private function _materializeMockCode($typeHint) {
		...
		$mockData = array('className' => $this->applyNamingScheme($typeHint),
	...
	private function _getMethods(ReflectionClass $reflector) {
		...
		foreach ($reflector->getMethods() as $reflectionMethod) {
			...
			if ($reflectionMethod->isPublic()
				...
				$methods[] = array('name' => $reflectionMethod->getName(),
						'access' => $reflectionMethod->isPublic() ? 'public'
								: 'protected',
						'modifiers' => $reflectionMethod->isStatic() ? 'static'
								: '',
						'returnReference' => $reflectionMethod
								->returnsReference(),
						'parameters' => $this
								->_getParameters($reflectionMethod));
			}
		}
		return $methods;
	}
	...
	private function _getParameters(ReflectionMethod $method) {
		...
		foreach ($method->getParameters() as $reflectionParameter) {
			...
			$parameters[] = array('hint' => $hint,
					'byReference' => $reflectionParameter
							->isPassedByReference(),
		...
		if ($warning) {
			trigger_error(
					sprintf(
							...
							$reflector->getName(), $warning), E_USER_WARNING);
		}
		return $canExtend;
	}
