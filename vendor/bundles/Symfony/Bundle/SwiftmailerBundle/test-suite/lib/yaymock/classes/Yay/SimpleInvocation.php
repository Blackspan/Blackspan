...
class Yay_SimpleInvocation implements Yay_Invocation {
	...
	private function _describeArgument($arg, $format) {
		...
		if (is_int($arg)) {
			...
			$description = sprintf($format, 'float',
					preg_replace('/^(.{8}).+/', '$1..', $arg));
		} elseif (is_numeric($arg)) {
			$description = sprintf($format, 'number',
					preg_replace('/^(.{8}).+/', '$1..', $arg));
		} elseif (is_string($arg)) {
			$description = sprintf($format, 'string',
					preg_replace('/^(.{8}).+/', '$1..', $arg));
			...
			$description = sprintf($format, gettype($arg),
					preg_replace('/^(.{8}).+/', '$1..', (string) $arg));
		}
		return $description;
	}
