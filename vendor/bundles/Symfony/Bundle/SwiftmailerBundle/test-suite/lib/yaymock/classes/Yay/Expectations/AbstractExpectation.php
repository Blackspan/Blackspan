 ...
 * @package Yay
 */
abstract class Yay_Expectations_AbstractExpectation implements Yay_Expectation,
		Yay_InvocationRecorder {
	 ...
	 */
	abstract public function notifyMatchedInvocation(
			Yay_Invocation $invocation);
	...
	public function isExpected(Yay_Invocation $invocation) {
		...
		if ($object === $this->_object) {
			...
			if ($matches && isset($this->_sequence)) {
				$matches = $this->_sequence
						->isInSequence($this->_wantedSequenceId);
			}
		} else {
			$matches = false;
		}
	...
	public function describeTo(Yay_Description $description) {
		...
		$description
				->appendText(
						sprintf(' of %s;', $this->_getInvocationSignature()));
