 ...
 * @package Yay
 */
class Yay_Expectations_BetweenExpectation extends
		Yay_Expectations_AbstractExpectation {
	...
	public function isExpected(Yay_Invocation $invocation) {
		return parent::isExpected($invocation)
	...
	public function describeBounds(Yay_Description $description) {
		$description
				->appendText(
						sprintf('Between %d and %d', $this->_min, $this->_max));
	}
	...
	public function describeSatisfaction(Yay_Description $description) {
		...
		$description
				->appendText(
						sprintf(' occurred %d times',
								(($this->_matched < $this->_max) ? $this
												->_matched : $this->_max)));
	}
