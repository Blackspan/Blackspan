...
class SimpleTest {
	...
	static function preferred($classes) {
		...
		for ($i = count($registry['Preferred']) - 1; $i >= 0; $i--) {
			foreach ($classes as $class) {
				if (SimpleTestCompatibility::isA($registry['Preferred'][$i],
						$class)) {
	...
	protected static function getDefaults() {
		return array('MockBaseClass' => 'SimpleMock', 'IgnoreList' => array(),
				'DefaultProxy' => false, 'DefaultProxyUsername' => false,
