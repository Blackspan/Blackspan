 ...
 */
foreach (array('LEXER_ENTER', 'LEXER_MATCHED', 'LEXER_UNMATCHED', 'LEXER_EXIT',
	...
	protected function getCompoundedRegex() {
		if ($this->regex == null) {
			for ($i = 0, $count = count($this->patterns); $i < $count; $i++) {
				$this->patterns[$i] = '('
						. str_replace(array('/', '(', ')'),
								array('\/', '\(', '\)'), $this->patterns[$i])
						. ')';
			}
			$this->regex = "/" . implode("|", $this->patterns) . "/"
...
class SimpleLexer {
	...
	protected function reduce($raw) {
		if ($action = $this->regexes[$this->mode->getCurrent()]
				->match($raw, $match)) {
...
class SimpleHtmlLexer extends SimpleLexer {
	...
	protected function getParsedTags() {
		return array('a', 'base', 'title', 'form', 'input', 'button',
				'textarea', 'select', 'option', 'frameset', 'frame', 'label');
	}
...
class SimpleHtmlSaxParser {
	...
	function acceptStartToken($token, $event) {
		...
		if ($event == LEXER_EXIT) {
			$success = $this->listener
					->startElement($this->tag, $this->attributes);
			$this->tag = '';
			$this->attributes = array();
			return $success;
		}
		if ($token != '=') {
			$this->current_attribute = strtolower(
	...
	function acceptAttributeToken($token, $event) {
		if ($this->current_attribute) {
			if ($event == LEXER_UNMATCHED) {
				$this->attributes[$this->current_attribute] .= SimpleHtmlSaxParser::decodeHtml(
						$token);
			}
			if ($event == LEXER_SPECIAL) {
				$this->attributes[$this->current_attribute] .= preg_replace(
						'/^=\s*/', '', SimpleHtmlSaxParser::decodeHtml($token));
			}
		}
		return true;
	}
	...
	static function normalise($html) {
		...
		$text = preg_replace('|<img[^>]*alt\s*=\s*"([^"]*)"[^>]*>|', ' \1 ',
				$text);
		$text = preg_replace('|<img[^>]*alt\s*=\s*\'([^\']*)\'[^>]*>|', ' \1 ',
				$text);
		$text = preg_replace('|<img[^>]*alt\s*=\s*([a-zA-Z_]+)[^>]*>|', ' \1 ',
				$text);
