...
class SimpleDumper {
	...
	protected function describeGenericDifference($first, $second) {
		return "as [" . $this->describeValue($first) . "] does not match ["
	...
	protected function describeStringDifference($first, $second, $identical) {
		...
		$message .= " with [" . $this->clipString($first, 200, $position)
				. "] and [" . $this->clipString($second, 200, $position) . "]";
		return $message;
	}
	...
	protected function describeIntegerDifference($first, $second, $identical) {
		...
		return "because [" . $this->describeValue($first) . "] differs from ["
	...
	protected function describeArrayDifference($first, $second, $identical) {
		...
		if (!$this->isMatchingKeys($first, $second, $identical)) {
			return "as key list [" . implode(", ", array_keys($first))
					. "] does not match key list ["
		...
		foreach (array_keys($first) as $key) {
			...
			return "with member [$key] "
					. $this
							->describeDifference($first[$key], $second[$key],
									$identical);
		}
		return "";
	}
	...
	protected function describeObjectDifference($first, $second, $identical) {
		...
		return $this
				->describeArrayDifference(get_object_vars($first),
						get_object_vars($second), $identical);
	}
