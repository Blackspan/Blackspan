...
class SimpleTagBuilder {
	...
	function createTag($name, $attributes) {
		static $map = array('a' => 'SimpleAnchorTag',
				'title' => 'SimpleTitleTag', 'base' => 'SimpleBaseTag',
				'button' => 'SimpleButtonTag',
				'textarea' => 'SimpleTextAreaTag',
				'option' => 'SimpleOptionTag', 'label' => 'SimpleLabelTag',
				'form' => 'SimpleFormTag', 'frame' => 'SimpleFrameTag');
	...
	protected function createInputTag($attributes) {
		...
		$map = array('submit' => 'SimpleSubmitTag',
				'image' => 'SimpleImageSubmitTag',
				'checkbox' => 'SimpleCheckboxTag',
				'radio' => 'SimpleRadioButtonTag', 'text' => 'SimpleTextTag',
				'hidden' => 'SimpleTextTag', 'password' => 'SimpleTextTag',
...
class SimplePageBuilder extends SimpleSaxListener {
	...
	function endElement($name) {
		...
		if ($this->hasNamedTagOnOpenTagStack($name)) {
			...
			if ($tag->isPrivateContent()
					&& $this->private_content_tag->getTagName() == $name) {
...
class SimplePage {
	...
	function acceptLabelEnd() {
		if (isset($this->label)) {
			if (isset($this->last_widget)) {
				...
				$this->left_over_labels[] = SimpleTestCompatibility::copy(
						$this->label);
			}
			unset($this->label);
		}
	...
	function setField($selector, $value, $position = false) {
		$is_set = false;
		for ($i = 0; $i < count($this->complete_forms); $i++) {
			if ($this->complete_forms[$i]
					->setField($selector, $value, $position)) {
				$is_set = true;
			}
		}
		return $is_set;
