...
class SimpleTestCase {
	...
	function pass($message = "Pass") {
		...
		$this->reporter
				->paintPass($message . $this->getAssertionLine());
		return true;
	}
	...
	function fail($message = "Fail") {
		...
		$this->reporter
				->paintFail($message . $this->getAssertionLine());
		return false;
	}
	...
	function getAssertionLine() {
		$trace = new SimpleStackTrace(
				array('assert', 'expect', 'pass', 'fail', 'skip'));
...
class SimpleFileLoader {
	...
	protected function scrapeClassesFromFile($test_file) {
		preg_match_all(
				...
				file_get_contents($test_file), $matches);
		return $matches[1];
	}
	...
	function createSuiteFromClasses($title, $classes) {
		if (count($classes) == 0) {
			$suite = new BadTestSuite($title,
					"No runnable test cases in [$title]");
			return $suite;
		}
...
class TestSuite {
	...
	function getLabel() {
		if (!$this->label) {
			return ($this->getSize() == 1) ? get_class($this->test_cases[0])
					: get_class($this);
		} else {
			return $this->label;
		}
