...
class ParametersExpectation extends SimpleExpectation {
	...
	function testMessage($parameters) {
		if ($this->test($parameters)) {
			return "Expectation of " . count($this->expected)
					. " arguments of ["
					. $this->renderArguments($this->expected) . "] is correct";
		} else {
	...
	protected function describeDifference($expected, $parameters) {
		if (count($expected) != count($parameters)) {
			return "Expected " . count($expected) . " arguments of ["
					. $this->renderArguments($expected) . "] but got "
					. count($parameters) . " arguments of ["
...
class CallCountExpectation extends SimpleExpectation {
	...
	function testMessage($compare) {
		return 'Expected call count for [' . $this->method . '] was ['
				. $this->count . '] got [' . $compare . ']';
	}
}
...
class MinimumCallCountExpectation extends SimpleExpectation {
	 ...
	 */
	function testMessage($compare) {
		return 'Minimum call count for [' . $this->method . '] was ['
				. $this->count . '] got [' . $compare . ']';
	}
}
...
class MaximumCallCountExpectation extends SimpleExpectation {
	 ...
	 */
	function testMessage($compare) {
		return 'Maximum call count for [' . $this->method . '] was ['
				. $this->count . '] got [' . $compare . ']';
	}
}
...
class SimpleCallSchedule {
	...
	function expectArguments($method, $args, $message) {
		...
		$this->expected_args[strtolower($method)] = new ParametersExpectation(
				$args, $message);
...
class SimpleMock {
	...
	protected function checkArgumentsIsArray($args, $task) {
		if (!is_array($args)) {
			trigger_error("Cannot $task as \$args parameter is not an array",
					E_USER_ERROR);
		}
	}
	...
	function returnsAt($timing, $method, $value, $args = false) {
		...
		$this->actions
				->registerAt($timing, $method, $args, new SimpleReturn($value));
	}
	...
	function setReturnReferenceAt($timing, $method, &$reference, $args = false) {
		...
		$this->actions
				->registerAt($timing, $method, $args,
						new SimpleByReference($reference));
	}
	...
	function expectAt($timing, $method, $args, $message = '%s') {
		...
		$method = strtolower($method);
		$message .= Mock::getExpectationLine();
		$this->expected_args_at[$timing][$method] = new ParametersExpectation(
				$args, $message);
	}
	...
	function expectCallCount($method, $count, $message = '%s') {
		...
		$message .= Mock::getExpectationLine();
		$this->expected_counts[strtolower($method)] = new CallCountExpectation(
	...
	function expectMaximumCallCount($method, $count, $message = '%s') {
		...
		$message .= Mock::getExpectationLine();
		$this->max_counts[strtolower($method)] = new MaximumCallCountExpectation(
				$method, $count, $message);
	}
	...
	function expectMinimumCallCount($method, $count, $message = '%s') {
		...
		$message .= Mock::getExpectationLine();
		$this->expected_counts[strtolower($method)] = new MinimumCallCountExpectation(
				$method, $count, $message);
	}
	 ...
	 *    method call.
	 */
	function errorOn($method, $error = 'A mock error', $args = false,
			$severity = E_USER_ERROR) {
		$this->dieOnNoMethod($method, "error on");
		$this->actions
				->register($method, $args,
						new SimpleErrorThrower($error, $severity));
	}
	 ...
	 *    method call.
	 */
	function errorAt($timing, $method, $error = 'A mock error', $args = false,
			$severity = E_USER_ERROR) {
	...
	protected function checkExpectations($method, $args, $timing) {
		...
		if (isset($this->expected_args_at[$timing][$method])) {
			$test
					->assert($this->expected_args_at[$timing][$method], $args,
							"Mock method [$method] at [$timing] -> %s");
		} elseif (isset($this->expected_args[$method])) {
			$test
					->assert($this->expected_args[$method], $args,
...
class MockGenerator {
	...
	function generatePartial($methods) {
		...
		if ($mock_reflection->classExistsSansAutoload()) {
			trigger_error(
					'Partial mock class [' . $this->mock_class
							. '] already exists');
			return false;
		}
	...
	protected function createClassCode($methods) {
		...
		$code = "class " . $this->mock_class . " extends " . $this->mock_base
				. " $implements {\n";
