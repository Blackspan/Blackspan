...
class SimpleRoute {
	...
	function createConnection($method, $timeout) {
		...
		$socket = $this
				->createSocket(
						$this->url->getScheme() ? $this->url->getScheme()
								: 'http', $this->url->getHost(),
						$this->url->getPort() ? $this->url->getPort()
...
class SimpleProxyRoute extends SimpleRoute {
	...
	function createConnection($method, $timeout) {
		$socket = $this
				->createSocket(
						$this->proxy->getScheme() ? $this->proxy->getScheme()
								: 'http', $this->proxy->getHost(),
...
class SimpleHttpRequest {
	...
	function fetch($timeout) {
		$socket = $this->route
				->createConnection($this->encoding->getMethod(), $timeout);
		if (!$socket->isError()) {
	...
	protected function createResponse($socket) {
		$response = new SimpleHttpResponse($socket, $this->route->getUrl(),
				$this->encoding);
		$socket->close();
		return $response;
	}
...
class SimpleHttpHeaders {
	...
	function isChallenge() {
		return ($this->response_code == 401) && (boolean) $this->authentication
				&& (boolean) $this->realm;
	}
	...
	protected function parseHeaderLine($header_line) {
		...
		if (preg_match('/WWW-Authenticate:\s+(\S+)\s+realm=\"(.*?)\"/i',
				$header_line, $matches)) {
	...
	protected function parseCookie($cookie_line) {
		...
		return new SimpleCookie($cookie[1], trim($cookie[2]),
...
class SimpleHttpResponse extends SimpleStickyError {
	...
	function __construct($socket, $url, $encoding) {
		...
		if ($socket->isError()) {
			$this
					->setError(
							'Error reading socket [' . $socket->getError()
									. ']');
			return;
		}
		$this->parse($raw);
