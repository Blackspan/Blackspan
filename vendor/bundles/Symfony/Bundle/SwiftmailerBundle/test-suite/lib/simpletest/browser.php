...
class SimpleBrowserHistory {
	...
	protected function atEnd() {
		return ($this->position + 1 >= count($this->sequence))
				&& !$this->isEmpty();
	}
...
class SimpleBrowser {
	...
	function __construct() {
		...
		$this->user_agent
				->useProxy(SimpleTest::getDefaultProxy(),
	...
	protected function parse($response, $depth = 0) {
		...
		if ($this->ignore_frames || !$page->hasFrames()
				|| ($depth > $this->maximum_nested_frames)) {
			return $page;
		}
	...
	function getCurrentCookieValue($name) {
		return $this->user_agent
				->getBaseCookieValue($name, $this->page->getUrl());
	}
	...
	function head($url, $parameters = false) {
		...
		$response = &$this->user_agent
				->fetchResponse($url, new SimpleHeadEncoding($parameters));
	...
	function retry() {
		...
		if (count($frames) > 0) {
			$this
					->loadFrame($frames, $this->page->getUrl(),
							$this->page->getRequestData());
			return $this->page->getRaw();
		}
	...
	function authenticate($username, $password) {
		...
		$this->user_agent
				->setIdentity($url->getHost(), $this->page->getRealm(),
						$username, $password);
		return $this->retry();
	}
	...
	function setField($label, $value, $position = false) {
		return $this->page
				->setField(new SimpleByLabelOrName($label), $value, $position);
	}
	...
	function setFieldByName($name, $value, $position = false) {
		return $this->page
				->setField(new SimpleByName($name), $value, $position);
	}
	...
	function clickSubmit($label = 'Submit', $additional = false) {
		...
		$success = $this
				->load($form->getAction(),
						$form
								->submitButton(new SimpleByLabel($label),
										$additional));
	...
	function clickSubmitByName($name, $additional = false) {
		...
		$success = $this
				->load($form->getAction(),
						$form
								->submitButton(new SimpleByName($name),
										$additional));
		return ($success ? $this->getContent() : $success);
	}
	...
	function isSubmit($label) {
		return (boolean) $this->page
				->getFormBySubmit(new SimpleByLabel($label));
	}
	...
	function clickImage($label, $x = 1, $y = 1, $additional = false) {
		...
		$success = $this
				->load($form->getAction(),
						$form
								->submitImage(new SimpleByLabel($label), $x,
										$y, $additional));
		return ($success ? $this->getContent() : $success);
	}
	...
	function clickImageByName($name, $x = 1, $y = 1, $additional = false) {
		...
		$success = $this
				->load($form->getAction(),
						$form
								->submitImage(new SimpleByName($name), $x, $y,
										$additional));
		return ($success ? $this->getContent() : $success);
	}
	...
	function clickImageById($id, $x = 1, $y = 1, $additional = false) {
		...
		$success = $this
				->load($form->getAction(),
						$form
								->submitImage(new SimpleById($id), $x, $y,
										$additional));
		return ($success ? $this->getContent() : $success);
	}
	...
	function submitFormById($id) {
		...
		$success = $this
				->load($form->getAction(), $form->submit());
		return ($success ? $this->getContent() : $success);
	}
	...
	function isClickable($label) {
		return $this->isSubmit($label) || ($this->getLink($label) !== false)
				|| $this->isImage($label);
	}
}
?>