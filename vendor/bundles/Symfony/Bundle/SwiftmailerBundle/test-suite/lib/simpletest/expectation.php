...
class SimpleExpectation {
	...
	static function isExpectation($expectation) {
		return is_object($expectation)
				&& SimpleTestCompatibility::isA($expectation,
						'SimpleExpectation');
	}
}
	...
	function testMessage($compare) {
		...
		if ($this->test($compare)) {
			...
			return "Not equal expectation fails ["
					. $dumper->describeValue($this->getValue()) . "] matches";
		}
	}
}
...
class WithinMarginExpectation extends SimpleExpectation {
	...
	protected function withinMessage($compare) {
		return "Within expectation ["
				. $this->dumper->describeValue($this->lower) . "] and ["
	...
	function testMessage($compare) {
		...
		if ($this->test($compare)) {
			...
			return "Identical expectation ["
					...
					. "] "
					. $dumper
							->describeDifference($this->getValue(), $compare,
									TYPE_MATTERS);
		}
	}
}
...
class PatternExpectation extends SimpleExpectation {
	...
	function testMessage($compare) {
		if ($this->test($compare)) {
			...
			$dumper = $this->getDumper();
			return "Pattern [" . $this->getPattern() . "] not detected in ["
					. $dumper->describeValue($compare) . "]";
		}
	}
	...
	protected function describePatternMatch($pattern, $subject) {
		...
		return "Pattern [$pattern] detected at character [$position] in ["
				. $dumper->describeValue($subject) . "] as [" . $matches[0]
				. "] in region ["
...
class IsAExpectation extends SimpleExpectation {
	...
	function test($compare) {
		if (is_object($compare)) {
			...
			return (strtolower(gettype($compare))
					== $this->canonicalType($this->type));
		}
	}
	...
	protected function canonicalType($type) {
		...
		$map = array('bool' => 'boolean', 'float' => 'double',
				'real' => 'double', 'int' => 'integer');
		if (isset($map[$type])) {
			$type = $map[$type];
		}
		return $type;
...
class MethodExistsExpectation extends SimpleExpectation {
	 ...
	 */
	function test($compare) {
		return (boolean) (is_object($compare)
				&& method_exists($compare, $this->method));
	}
