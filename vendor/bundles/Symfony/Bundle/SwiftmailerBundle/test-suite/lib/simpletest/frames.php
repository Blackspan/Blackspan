...
class SimpleFrameset {
	...
	function getFrameFocus() {
		...
		return array_merge(array($this->getPublicNameFromIndex($this->focus)),
	...
	function getFrames() {
		$report = array();
		for ($i = 0; $i < count($this->frames); $i++) {
			$report[$this->getPublicNameFromIndex($i)] = $this->frames[$i]
					->getFrames();
		}
		return $report;
	}
	...
	function getUrlsByLabel($label) {
		...
		foreach ($this->frames as $index => $frame) {
			$urls = array_merge($urls,
					$this
							->tagUrlsWithFrame($frame->getUrlsByLabel($label),
									$index));
		}
		return $urls;
	}
	...
	protected function findForm($method, $attribute) {
		if (is_integer($this->focus)) {
			return $this
					->findFormInFrame($this->frames[$this->focus],
							$this->focus, $method, $attribute);
		}
		for ($i = 0; $i < count($this->frames); $i++) {
			$form = $this
					->findFormInFrame($this->frames[$i], $i, $method,
							$attribute);
			if ($form) {
				return $form;
			}
