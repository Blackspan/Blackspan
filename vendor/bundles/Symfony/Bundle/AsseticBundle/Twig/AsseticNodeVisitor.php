...
class AsseticNodeVisitor implements \Twig_NodeVisitorInterface {
	...
	public function __construct() {
		$this->useNodeName = version_compare(\Twig_Environment::VERSION,
				'1.2.0-DEV', '<');
	}
	...
	public function leaveNode(\Twig_NodeInterface $node, \Twig_Environment $env) {
		...
		return new \Twig_Node_Expression_Conditional(
				new \Twig_Node_Expression_GetAttr(
						...
						new \Twig_Node_Expression_Constant('use_controller',
								$line), new \Twig_Node(),
						\Twig_TemplateInterface::ARRAY_CALL, $line),
				...
				new \Twig_Node_Expression_Function(
						...
						new \Twig_Node(
								array($node,
										new \Twig_Node_Expression_Constant(
												isset($options['package']) ? $options['package']
														: null, $line))),
						$line), $line);
	}
	 ...
	 */
	private function checkNode(\Twig_NodeInterface $node,
			\Twig_Environment $env) {
		if ($node instanceof \Twig_Node_Expression_Function) {
			$name = $this->useNodeName ? $node->getNode('name')
			...
			if ($env->getFunction($name) instanceof AsseticFilterFunction) {
				...
				$invoker = $env->getExtension('assetic')
						->getFilterInvoker($name);
				$factory = $invoker->getFactory();

				$inputs = isset($arguments[0]) ? (array) $arguments[0] : array();
				$filters = $invoker->getFilters();
				$options = array_replace($invoker->getOptions(),
