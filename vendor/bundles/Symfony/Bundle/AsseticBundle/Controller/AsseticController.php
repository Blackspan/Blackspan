...
class AsseticController {
	...
	public function __construct(Request $request, LazyAssetManager $am,
			CacheInterface $cache, $enableProfiler = false,
	...
	public function render($name, $pos = null) {
		...
		if (!$this->am->has($name)) {
			throw new NotFoundHttpException(
					sprintf('The "%s" asset could not be found.', $name));
		}
		...
		if (null !== $pos && !$asset = $this->findAssetLeaf($asset, $pos)) {
			throw new NotFoundHttpException(
					sprintf(
							'The "%s" asset does not include a leaf at position %d.',
							$name, $pos));
		}
