...
class AsseticHelperTest extends \PHPUnit_Framework_TestCase {
	...
	public function testUrls($debug, $count, $message) {
		$helper = new AsseticHelperForTest(new AssetFactory('/foo', $debug),
				$debug);
		$urls = $helper
				->javascripts(array('js/jquery.js', 'js/jquery.plugin.js'));

		$this
				->assertInstanceOf('Traversable', $urls,
						'->javascripts() returns an array');
	...
	public function getDebugAndCount() {
		return array(
				array(false, 1,
						'->javascripts() returns one url when not in debug mode'),
				array(true, 2,
						'->javascripts() returns many urls when in debug mode'),);
	}
}
