...
class CoalescingDirectoryResourceTest extends \PHPUnit_Framework_TestCase {
	public function testFiltering() {
		$dir1 = $this
				->getMock(
						'Assetic\\Factory\\Resource\\IteratorResourceInterface');
		$file1a = $this
		...
		$dir1->expects($this->any())->method('getIterator')
				->will(
						$this
								->returnValue(
										new \ArrayIterator(
												array($file1a, $file1b))));
		$file1a->expects($this->any())->method('__toString')
				->will($this->returnValue('FooBundle:Foo:file1.foo.bar'));
		$file1b->expects($this->any())->method('__toString')
				->will($this->returnValue('FooBundle:Foo:file2.foo.bar'));

		$dir2->expects($this->any())->method('getIterator')
				->will(
						$this
								->returnValue(
										new \ArrayIterator(
												array($file2a, $file2c))));
		$file2a->expects($this->any())->method('__toString')
				->will($this->returnValue('BarBundle:Foo:file1.foo.bar'));
		$file2c->expects($this->any())->method('__toString')
		...
		$expected = array('FooBundle:Foo:file1.foo.bar',
				'FooBundle:Foo:file2.foo.bar', 'BarBundle:Foo:file3.foo.bar',);
