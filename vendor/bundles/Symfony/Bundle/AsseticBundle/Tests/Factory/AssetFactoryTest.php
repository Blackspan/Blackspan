...
class AssetFactoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->factory = new AssetFactory($this->kernel, $this->container,
				$this->parameterBag, '/path/to/web');
	}
	...
	public function testBundleNotation() {
		...
		$this->parameterBag->expects($this->once())->method('resolveValue')
				->will($this->returnCallback(function ($v) {
							return $v;
						}));
		$this->kernel->expects($this->once())->method('getBundle')
				->with('MyBundle')->will($this->returnValue($bundle));
		$this->kernel->expects($this->once())->method('locateResource')
				->with($input)
				->will(
						$this
								->returnValue(
										'/path/to/MyBundle/Resources/css/main.css'));
		$bundle->expects($this->once())->method('getPath')
		...
		$this
				->assertEquals('/path/to/MyBundle', $asset->getSourceRoot(),
						'->createAsset() sets the asset root');
		$this
				->assertEquals('Resources/css/main.css',
						$asset->getSourcePath(),
						'->createAsset() sets the asset path');
	}
	...
	public function testBundleGlobNotation($input) {
		...
		$this
				->assertNull($asset->getSourcePath(),
						'->createAsset() sets the asset path to null');
	}

	public function getGlobs() {
		return array(array('@MyBundle/Resources/css/*'),
