...
class AsseticControllerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->controller = new AsseticController($this->request, $this->am,
				$this->cache);
	}
	...
	public function testRenderNotFound() {
		...
		$this->am->expects($this->once())->method('has')->with($name)
	...
	public function testRenderLastModifiedFresh() {
		...
		$asset->expects($this->any())->method('getFilters')
				->will($this->returnValue(array()));
		$this->am->expects($this->once())->method('has')->with($name)
				->will($this->returnValue(true));
		$this->am->expects($this->once())->method('get')->with($name)
				->will($this->returnValue($asset));
		$asset->expects($this->once())->method('getLastModified')
		...
		$asset->expects($this->never())->method('dump');

		$response = $this->controller->render($name);
		$this
				->assertEquals(304, $response->getStatusCode(),
						'->render() sends a Not Modified response when If-Modified-Since is fresh');
	}
	...
	public function testRenderLastModifiedStale() {
		...
		$lastModified = strtotime('2010-10-10 10:10:10');
		$ifModifiedSince = gmdate('D, d M Y H:i:s', $lastModified - 300)
				. ' GMT';
		...
		$this->am->expects($this->once())->method('get')->with($name)
				->will($this->returnValue($asset));
		$asset->expects($this->exactly(2))->method('getLastModified')
				->will($this->returnValue($lastModified));
		$this->headers->expects($this->once())->method('get')
				->with('If-Modified-Since')
				->will($this->returnValue($ifModifiedSince));
		...
		$this
				->assertEquals(200, $response->getStatusCode(),
						'->render() sends an OK response when If-Modified-Since is stale');
		$this
				->assertEquals($content, $response->getContent(),
						'->render() sends the dumped asset as the response content');
	}
	...
	public function testRenderETagFresh() {
		...
		$this->am->expects($this->once())->method('hasFormula')->with($name)
				->will($this->returnValue(true));
		$this->am->expects($this->once())->method('getFormula')->with($name)
				->will($this->returnValue($formula));
		$this->request->expects($this->once())->method('getETags')
		...
		$this
				->assertEquals(304, $response->getStatusCode(),
						'->render() sends a Not Modified response when If-None-Match is fresh');
	}
	...
	public function testRenderETagStale() {
		...
		$this
				->assertEquals(200, $response->getStatusCode(),
						'->render() sends an OK response when If-None-Match is stale');
		$this
				->assertEquals($content, $response->getContent(),
						'->render() sends the dumped asset as the response content');
	}
