...
class DumpCommand extends ContainerAwareCommand {
	...
	protected function configure() {
		$this->setName('assetic:dump')
				...
				->addArgument('write_to', InputArgument::OPTIONAL,
						'Override the configured asset root')
				->addOption('watch', null, InputOption::VALUE_NONE,
						'Check for changes every second, debug mode only')
				->addOption('force', null, InputOption::VALUE_NONE,
						'Force an initial generation of all assets (used with --watch)')
				->addOption('period', null, InputOption::VALUE_REQUIRED,
						'Set the polling period in seconds (used with --watch)',
						1);
	}
	...
	protected function initialize(InputInterface $input,
		...
		$this->basePath = $input->getArgument('write_to') ? 
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		$output
				->writeln(
						sprintf('Dumping all <comment>%s</comment> assets.',
								$input->getOption('env')));
		$output
				->writeln(
						sprintf('Debug mode is <comment>%s</comment>.',
								$input->getOption('no-debug') ? 'off' : 'on'));
		$output->writeln('');
		...
		if (!$this->am->isDebug()) {
			throw new \RuntimeException(
					'The --watch option is only available in debug mode.');
		}
	...
	private function watch(InputInterface $input, OutputInterface $output) {
		...
		$cache = sys_get_temp_dir() . '/assetic_watch_'
				. substr(sha1($this->basePath), 0, 7);
	...
	private function checkAsset($name, array &$previously) {
		$formula = $this->am->hasFormula($name) ? serialize(
						$this->am->getFormula($name)) : null;
		$asset = $this->am->get($name);
		$mtime = $asset->getLastModified();

		if (isset($previously[$name])) {
			$changed = $previously[$name]['mtime'] != $mtime
					|| $previously[$name]['formula'] != $formula;
		} else {
			$changed = true;
		}
	...
	private function dumpAsset($name, OutputInterface $output) {
		...
		if (isset($formula[2]['debug']) ? $formula[2]['debug']
				: $this->am->isDebug()) {
	...
	private function doDump(AssetInterface $asset, OutputInterface $output) {
		$target = rtrim($this->basePath, '/') . '/'
				. str_replace('_controller/', '', $asset->getTargetPath());
