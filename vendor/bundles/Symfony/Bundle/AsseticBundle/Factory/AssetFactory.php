...
class AssetFactory extends BaseAssetFactory {
	...
	public function __construct(KernelInterface $kernel,
			ContainerInterface $container, ParameterBagInterface $parameterBag,
			$baseDir, $debug = false) {
		$this->kernel = $kernel;
	...
	protected function parseInput($input, array $options = array()) {
		...
		if ('@' == $input[0] && false !== strpos($input, '/')) {
			...
			$options['root'] = array(
					$this->kernel->getBundle($bundle)->getPath());
	...
	protected function createAssetReference($name) {
		if (!$this->getAssetManager()) {
			$this
					->setAssetManager(
							$this->container->get('assetic.asset_manager'));
		}
	...
	protected function getFilter($name) {
		if (!$this->getFilterManager()) {
			$this
					->setFilterManager(
							$this->container->get('assetic.filter_manager'));
		}
