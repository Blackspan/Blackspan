...
class SecureMethodInvocationsPass implements CompilerPassInterface {
	...
	public function __construct($cacheDir) {
		...
		if (false === is_writable($cacheDir)) {
			throw new \RuntimeException(
					'Cannot write to cache folder: ' . $cacheDir);
		}
	...
	public function process(ContainerBuilder $container) {
		...
		$secureNotAll = !$container
				->getParameter('security.extra.secure_all_services');
		...
		foreach ($container->getDefinitions() as $id => $definition) {
			...
			if ((null === $class = $definition->getClass())
				...
				if ($secureNotAll) {
					throw new \RuntimeException(
							sprintf('Could not find class "%s" for "%s".',
									$class, $id));
				}
			...
			if (null !== $definition->getFactoryMethod()
					|| $definition->isAbstract() || $definition->isSynthetic()) {
				if ($secureNotAll) {
					throw new \RuntimeException(
							sprintf(
									'You cannot secure service "%s", because it is either created by a factory, or an abstract/synthetic service.',
									$id));
				}
	...
	private function processDefinition(ContainerBuilder $container, $id,
		...
		if ($this->needsReAssessment($id, $definition)) {
			...
			if (true === $metadata->isProxyRequired()) {
				list($newClassName, $content) = $this->generator
						->generate($definition, $metadata);
				file_put_contents(
						$path = $this->cacheDir . 'SecurityProxies/'
								. $newClassName . '.php', $content);
				$definition->setFile($path);
				$definition
						->setClass(
								$proxyClass = 'SecurityProxies\\'
										. $newClassName);
				$definition
						->addMethodCall(
								'jmsSecurityExtraBundle__setMethodSecurityInterceptor',
								array(
										new Reference(
												'security.access.method_interceptor')));
			} else if (isset($this->cacheMetadata[$id]['proxy_class'])) {
				@unlink(
						$this->cacheDir
								. $this->cacheMetadata[$id]['proxy_class']
								. '.php');
			}

			$this->cacheMetadata[$id] = array(
					'class' => $definition->getClass(),
					'proxy_class' => $proxyClass, 'proxy_file' => $path,
					'analyze_time' => time(), 'files' => $files,);
		} else {
	...
	private function createOrLoadCacheMetadata() {
		if (file_exists($this->cacheDir . 'cache.meta')) {
			if (!is_readable($this->cacheDir . 'cache.meta')) {
				throw new \RuntimeException(
						'Cannot load security cache meta data from: '
								. $this->cacheDir . 'cache.meta');
			}

			$this->cacheMetadata = unserialize(
					file_get_contents($this->cacheDir . 'cache.meta'));
		} else {
			@set_time_limit(0);
	...
	private function writeCacheMetadata() {
		if (false
				=== file_put_contents($this->cacheDir . 'cache.meta',
						serialize($this->cacheMetadata))) {
			throw new \RuntimeException(
					'Could not write to cache file: ' . $this->cacheDir
							. 'cache.meta');
		}
	}
}
