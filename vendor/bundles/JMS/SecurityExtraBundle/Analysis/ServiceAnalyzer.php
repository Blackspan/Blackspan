...
class ServiceAnalyzer {
	...
	public function __construct($class, Reader $reader) {
		...
		$this->driver = new DriverChain(
				array(new AnnotationDriver($reader),));
		$this->analyzed = false;
	}
	...
	public function getFiles() {
		if (!$this->analyzed) {
			throw new \LogicException(
					'Data not yet available, run analyze() first.');
		}
	...
	private function normalizeMetadata() {
		...
		foreach ($this->metadata->classMetadata as $class) {
			...
			foreach ($class->methodMetadata as $name => $method) {
				if ($method->reflection->isStatic()
						|| $method->reflection->isFinal()) {
					throw new \RuntimeException(
							'Annotations cannot be defined on final, or static methods.');
				}
				...
				if (!isset($secureMethods[$name])) {
					...
					$secureMethods[$name]->merge($method);
						...
						&& $method->reflection->getDeclaringClass()->getName()
								!== $secureMethods[$name]->reflection
										->getDeclaringClass()->getName()) {
					throw new \RuntimeException(
							sprintf(
									'Unresolved security metadata conflict for method "%s::%s" in "%s". Please copy the respective annotations, and add @SatisfiesParentSecurityPolicy to the child method.',
									$secureMethods[$name]->reflection
											->getDeclaringClass()->getName(),
									$name,
									$secureMethods[$name]->reflection
		...
		foreach ($secureMethods as $name => $method) {
			if ($method->reflection->isAbstract()) {
				$previous = null;
				$abstractClass = $method->reflection->getDeclaringClass()
						->getName();
	...
	private function analyzeControlFlow() {
		...
		while (true) {
			foreach ($rootClass->getMethods() as $method) {
				...
				if ($secureMethods[$name]->reflection->getDeclaringClass()
						->getName() !== $rootClass->getName()) {
					throw new \RuntimeException(
							sprintf(
	...
	private function hasMethod(\ReflectionClass $class, $name) {
		...
		return $class->getName()
				=== $class->getMethod($name)->getDeclaringClass()->getName();
	}
}
