...
class AnnotationDriverTest extends \PHPUnit_Framework_TestCase {
	public function testLoadMetadataFromClass() {
		...
		$metadata = $driver
				->loadMetadataForClass(
						new \ReflectionClass(
								'JMS\SecurityExtraBundle\Tests\Mapping\Driver\FooService'));
		...
		$this
				->assertEquals(array(0 => array('VIEW')),
						$method->paramPermissions);

		$metadata = $driver
				->loadMetadataForClass(
						new \ReflectionClass(
								'JMS\SecurityExtraBundle\Tests\Mapping\Driver\FooInterface'));
		$this->assertTrue(isset($metadata->methodMetadata['foo']));
		$method = $metadata->methodMetadata['foo'];
		$this->assertEquals(array(), $method->roles);
		$this
				->assertEquals(array(0 => array('OWNER'), 1 => array('EDIT')),
						$method->paramPermissions);
	...
	public function testLoadMetadataFromClassDoesNotProcessMethodsForWhichNoSecurityMetadataExists() {
		...
		$this
				->assertTrue(
						class_exists(
								'JMS\SecurityExtraBundle\Tests\Fixtures\Annotation\NonSecurityAnnotation',
								false));
