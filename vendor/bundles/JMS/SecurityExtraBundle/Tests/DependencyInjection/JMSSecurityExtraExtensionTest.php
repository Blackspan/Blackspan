...
class JMSSecurityExtraExtensionTest extends \PHPUnit_Framework_TestCase {
	public function testConfigLoad() {
		...
		$this
				->assertFalse(
						$container
								->getParameter(
										'security.extra.secure_all_services'));
		$this
				->assertFalse(
						$container->getDefinition('security.extra.iddqd_voter')
								->hasTag('security.voter'));
	}
	...
	public function testConfigLoadSecureAll() {
		...
		$extension
				->load(
						array(
								array('secure_all_services' => true,
	...
	public function testConfigLoadEnableIddqdAttribute() {
		$extension = new JMSSecurityExtraExtension();
		$extension
				->load(array(array('enable_iddqd_attribute' => true)),
						$container = new ContainerBuilder());
