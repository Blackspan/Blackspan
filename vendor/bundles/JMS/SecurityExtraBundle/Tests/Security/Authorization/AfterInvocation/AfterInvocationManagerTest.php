...
class AfterInvocationManagerTest extends \PHPUnit_Framework_TestCase {
	public function testDecide() {
		...
		$provider1->expects($this->once())->method('decide')
				->with(
						$this
								->isInstanceOf(
										'Symfony\Component\Security\Core\Authentication\Token\TokenInterface'),
						$this->anything(), $this->equalTo($attributes),
						$this->equalTo('foo'))->will($this->returnValue('bar'));

		$provider2 = $this->getProvider();
		$provider2->expects($this->once())->method('decide')
				->with(
						$this
								->isInstanceOf(
										'Symfony\Component\Security\Core\Authentication\Token\TokenInterface'),
						$this->anything(), $this->equalTo($attributes),
						$this->equalTo('bar'))->will($this->returnValue('moo'));
		...
		$this
				->assertEquals('moo',
						$manager->decide($token, 'sth', $attributes, 'foo'));
	}
	...
	public function testSupportsAttribute($attribute, $supported) {
		...
		$provider->expects($this->once())->method('supportsAttribute')
	...
	public function testSupportsClass($class, $supported) {
		$provider = $this->getProvider();
		$provider->expects($this->once())->method('supportsClass')
	...
	public function getSupportsTests() {
		return array(array('FOO', true), array('BAR', false),);
	}
