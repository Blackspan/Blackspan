...
class AclAfterInvocationProviderTest extends \PHPUnit_Framework_TestCase {
	public function testDecideReturnsNullWhenObjectIsNull() {
		...
		$this
				->assertNull(
						$provider
								->decide($this->getToken(), null, array(), null));
	}
	...
	public function testDecideDoesNotModifyReturnedObjectWhenNoAttributeIsSupported() {
		...
		$this
				->assertSame($returnedObject,
						$provider
								->decide($this->getToken(), null,
										array('foo', 'moo'), $returnedObject));
	}
	...
	public function testDecideDoesNotModifyReturnedObjectWhenNoObjectIdentityCanBeRetrieved() {
		...
		$provider = new AclAfterInvocationProvider($this->getAclProvider(),
				$oidStrategy, $this->getSidStrategy(), $permissionMap);
		$this
				->assertSame($returnedObject,
						$provider
								->decide($this->getToken(), null, array('foo'),
										$returnedObject));
	}
	...
	public function testDecideThrowsAccessDeniedExceptionWhenNoAclIsFound() {
		...
		$aclProvider->expects($this->once())->method('findAcl')
				->will(
						$this
								->throwException(
										new AclNotFoundException('No ACL')));

		$provider = new AclAfterInvocationProvider($aclProvider, $oidStrategy,
				$sidStrategy, $permissionMap);
	...
	public function testDecideThrowsAccessDeniedExceptionWhenNoAceIsFound() {
		...
		$acl = $this
				->getMock('Symfony\Component\Security\Acl\Model\AclInterface');
	...
	public function testSupportsAttribute() {
		...
		$permissionMap->expects($this->at(0))->method('contains')
				->with($this->equalTo('foo'))->will($this->returnValue(true));
		$permissionMap->expects($this->at(1))->method('contains')
				->with($this->equalTo('asdf'))->wilL($this->returnValue(false));
