...
class MethodSecurityInterceptorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testInvokeAuthenticatesTokenIfItIsNotYetAuthenticated() {
		list($interceptor, $securityContext, $authManager, , , ) = $this
				->getInterceptor();

		$token = $this
				->getMock(
						'Symfony\Component\Security\Core\Authentication\Token\TokenInterface');
		$token->expects($this->once())->method('isAuthenticated')
				->will($this->returnValue(false));

		$securityContext->expects($this->once())->method('getToken')
				->will($this->returnValue($token));

		$authManager->expects($this->once())->method('authenticate')
				->will(
						$this
								->throwException(
	...
	public function testInvokeCallsADMForRolesAndThrowsExceptionWhenInsufficientPriviledges() {
		list($interceptor, $context, $authManager, $adm, , ) = $this
				->getInterceptor();
		...
		$context->expects($this->once())->method('getToken')
				->will($this->returnValue($token));
		$context->expects($this->once())->method('setToken')
		...
		$adm->expects($this->once())->method('decide')
				->with($this->equalTo($token),
						$this->equalTo(array('ROLE_FOO')),
						$this->equalTo($method))
				->will($this->returnValue(false));
	...
	public function testInvokeCallsADMForEachParamPermissionsAndThrowsExceptionOnInsufficientPermissions() {
		...
		$metadata['param_permissions'] = array(
				$p0 = array('ROLE_FOO', 'ROLE_ASDF'), $p1 = array('ROLE_MOO'),);
		$method = $this->getInvocation();
		$adm->expects($this->at(0))->method('decide')
				->with($this->equalTo($token), $this->equalTo($p0),
						$this->equalTo(new \stdClass()))
				->will($this->returnValue(true));
		$adm->expects($this->at(1))->method('decide')
				->with($this->equalTo($token), $this->equalTo($p1),
						$this->equalTo(new \stdClass()))
				->will($this->returnValue(false));
	...
	public function testInvokehandlesExceptionsFromWithintheInvokedMethodGracefully() {
		...
		list($interceptor, $context, , , , $runAsManager) = $this
				->getInterceptor();
		...
		$runAsToken = new RunAsUserToken('asdf', 'user', 'foo',
				array('ROLE_FOO'), $token);
		$runAsManager->expects($this->once())->method('buildRunAs')
				->will($this->returnValue($runAsToken));

		$context->expects($this->exactly(2))->method('setToken');
	...
	protected function getMetadata(array $roles = array(),
			array $runAsRoles = array(), array $paramPermissions = array(),
			array $returnPermissions = array()) {
		return array('roles' => $roles, 'run_as_roles' => $runAsRoles,
	...
	protected function getInterceptor() {
		...
		return array(
				new MethodSecurityInterceptor($securityContext,
						$authenticationManager, $accessDecisionManager,
						$afterInvocationManager, $runAsManager),
	...
	protected function getInvocation($method = 'foo', $arguments = array()) {
		...
		return new MethodInvocation($object = new SecureService(), $method,
				$object, $arguments);
	}
}
