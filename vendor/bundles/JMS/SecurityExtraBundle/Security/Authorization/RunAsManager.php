...
class RunAsManager implements RunAsManagerInterface {
	 ...
	 * {@inheritDoc}
	 */
	public function buildRunAs(TokenInterface $token, $secureObject,
			array $attributes) {
		$roles = array();
		...
		return new RunAsUserToken($this->key, $token->getUser(),
				$token->getCredentials(), $roles, $token);
	}
	...
	public function supportsAttribute($attribute) {
		return !empty($attribute)
				&& 0 === strpos($attribute, $this->rolePrefix);
	}
