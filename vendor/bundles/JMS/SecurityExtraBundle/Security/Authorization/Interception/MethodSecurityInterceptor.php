...
class MethodSecurityInterceptor {
	...
	public function __construct(SecurityContext $securityContext,
			...
			RunAsManagerInterface $runAsManager, LoggerInterface $logger = null) {
	...
	public function invoke(MethodInvocation $method, array $metadata) {
		...
		try {
			$returnValue = $method
					->invokeArgs($method->getThis(), $method->getArguments());
			...
			return $this
					->afterInvocation($method, $metadata, $runAsToken,
							$returnValue);
	...
	protected function beforeInvocation(MethodInvocation $method,
			array $metadata) {
		...
		if ($metadata['roles']
				&& false
						=== $this->accessDecisionManager
								->decide($token, $metadata['roles'], $method)) {
		...
		if ($metadata['param_permissions']) {
			foreach ($method->getArguments() as $index => $argument) {
				if (null !== $argument
						&& isset($metadata['param_permissions'][$index])
						&& false
								=== $this->accessDecisionManager
										->decide($token,
												$metadata['param_permissions'][$index],
												$argument)) {
					throw new AccessDeniedException(
							sprintf(
									'Token does not have the required permissions for method "%s::%s".',
		...
		if ($metadata['run_as_roles']) {
			$runAsToken = $this->runAsManager
					->buildRunAs($token, $method, $metadata['run_as_roles']);

			if (null !== $this->logger) {
				$this->logger
						->debug('Populating security context with RunAsToken');
			}

			if (null === $runAsToken) {
				throw new \RuntimeException(
						'RunAsManager must not return null from buildRunAs().');
			}
	...
	protected function afterInvocation(MethodInvocation $method,
			array $metadata, $runAsToken, $returnValue) {
		if (!$metadata['return_permissions']) {
			return $returnValue;
		}

		return $this->afterInvocationManager
				->decide($this->securityContext->getToken(), $method,
						$metadata['return_permissions'], $returnValue);
	}

	protected function restoreOriginalToken(RunAsUserToken $runAsToken) {
		if (null !== $this->logger) {
			$this->logger
					->debug('Populating security context with original Token.');
		}
