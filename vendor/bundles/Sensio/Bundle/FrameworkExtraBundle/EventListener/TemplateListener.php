...
class TemplateListener {
	...
	public function onKernelController(FilterControllerEvent $event) {
		...
		if (!$configuration->getTemplate()) {
			$configuration
					->setTemplate(
							$this
									->guessTemplateName($controller, $request,
	...
	public function onKernelView(GetResponseForControllerResultEvent $event) {
		...
		if (null === $parameters) {
			if (!$vars = $request->attributes->get('_template_vars')) {
				if (!$vars = $request->attributes
						->get('_template_default_vars')) {
					return;
				}
			}
	...
	protected function guessTemplateName($controller, Request $request,
		...
		if (!preg_match('/Controller\\\(.+)Controller$/',
				get_class($controller[0]), $matchController)) {
		...
		return new TemplateReference($bundle->getName(), $matchController[1],
				$matchAction[1], $request->getRequestFormat(), $engine);
	}
