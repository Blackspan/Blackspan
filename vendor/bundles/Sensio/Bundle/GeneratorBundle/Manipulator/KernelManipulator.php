...
class KernelManipulator extends Manipulator {
	...
	public function addBundle($bundle) {
		...
		$lines = array_slice($src, $method->getStartLine() - 1,
		...
		$this
				->setCode(token_get_all('<?php ' . implode('', $lines)),
						$method->getStartLine());
		while ($token = $this->next()) {
			...
			while ($token = $this->next()) {
				...
				$lines = array_merge(array_slice($src, 0, $this->line - 2),
				...
				file_put_contents($this->reflected->getFilename(),
						implode('', $lines));
