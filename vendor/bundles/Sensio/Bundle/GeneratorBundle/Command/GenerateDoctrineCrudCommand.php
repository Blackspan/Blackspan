		...
		$this
				->setDefinition(
						array(
								...
								new InputOption('route-prefix', '',
										InputOption::VALUE_REQUIRED,
										'The route prefix'),
								new InputOption('with-write', '',
...
EOT
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$entityClass = $this->getContainer()->get('doctrine')
		...
		$generator
				->generate($bundle, $entity, $metadata[0], $format, $prefix,
						$withWrite);
	...
	protected function interact(InputInterface $input, OutputInterface $output) {
		...
		$prefix = $dialog
				->ask($output,
						$dialog->getQuestion('Routes prefix', '/' . $prefix),
						'/' . $prefix);
		...
		$output
				->writeln(
						array('',
								...
								sprintf(
										"You are going to generate a CRUD controller for \"<info>%s:%s</info>\"",
										$bundle, $entity),
								sprintf(
	...
	private function updateRouting($dialog, InputInterface $input,
			OutputInterface $output, $bundle, $format, $entity, $prefix) {
		$auto = true;
		if ($input->isInteractive()) {
			$auto = $dialog
					->askConfirmation($output,
		...
		$this->getContainer()->get('filesystem')
				->mkdir($bundle->getPath() . '/Resources/config/');
		$routing = new RoutingManipulator(
				$bundle->getPath() . '/Resources/config/routing.yml');
		try {
			$ret = $auto ? $routing
							->addResource($bundle->getName(), $format,
									'/' . $prefix,
									'routing/'
											. strtolower(
													str_replace('\\', '_',
	...
	protected function getRoutePrefix(InputInterface $input, $entity) {
		$prefix = $input->getOption('route-prefix') ? 
				: strtolower(str_replace(array('\\', '/'), '_', $entity));
