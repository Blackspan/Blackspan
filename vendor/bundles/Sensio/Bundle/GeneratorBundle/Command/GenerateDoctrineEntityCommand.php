...
class GenerateDoctrineEntityCommand extends GenerateDoctrineCommand {
	...
	protected function configure() {
		$this->setName('doctrine:generate:entity')
				...
				->addOption('format', null, InputOption::VALUE_REQUIRED,
						...
						'annotation')
				->addOption('with-repository', null, InputOption::VALUE_NONE,
						'Whether to generate the entity repository or not')
				->setHelp(
						<<<EOT
...
EOT
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$generator
				->generate($bundle, $entity, $format, array_values($fields),
						$input->getOption('with-repository'));
	...
	protected function interact(InputInterface $input, OutputInterface $output) {
		...
		while (true) {
			...
			try {
				...
				$output
						->writeln(
								sprintf(
										'<bg=red>Entity "%s:%s" already exists</>.',
										$bundle, $entity));
			} catch (\Exception $e) {
				$output
						->writeln(
								sprintf(
										'<bg=red>Bundle "%s" does not exist.</>',
										$bundle));
			}
		}
		...
		// repository?
		$output->writeln('');
		$withRepository = $dialog
				->askConfirmation($output,
						$dialog
								->getQuestion(
										'Do you want to generate an empty repository class',
										$input->getOption('with-repository') ? 'yes'
												: 'no', '?'),
						$input->getOption('with-repository'));
		...
		$output
				->writeln(
						array('',
								...
								sprintf(
										"You are going to generate a \"<info>%s:%s</info>\" Doctrine2 entity",
										$bundle, $entity),
								sprintf(
										"using the \"<info>%s</info>\" format.",
										$format), '',));
	}
	...
	private function addFields(InputInterface $input, OutputInterface $output,
		...
		$lengthValidator = function ($length) {
			...
			if (false === $result) {
				throw new \InvalidArgumentException(
						sprintf('Invalid length "%s".', $length));
			}
		...
		while (true) {
					...
					->askAndValidate($output,
							...
							function ($name) use ($fields, $self) {
								if (isset($fields[$name]) || 'id' == $name) {
									throw new \InvalidArgumentException(
											sprintf(
													'Field "%s" is already defined.',
													$name));
								}

								// check reserved words
								if ($self->getGenerator()
			...
			$type = $dialog
					->askAndValidate($output,
							$dialog->getQuestion('Field type', $defaultType),
			...
			if ($type == 'string') {
				$data['length'] = $dialog
						->askAndValidate($output,
								$dialog->getQuestion('Field length', 255),
								$lengthValidator, false, 255);
			}
	...
	public function getGenerator() {
		if (null === $this->generator) {
			$this->generator = new DoctrineEntityGenerator(
