...
class UserManagerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		if (!class_exists('Doctrine\ODM\CouchDB\Version')) {
			$this
					->markTestSkipped(
							'Doctrine CouchDB has to be installed for this test to run.');
		}
		...
		$this->dm = $this
				->getMock('Doctrine\ODM\CouchDB\DocumentManager',
						array('getRepository', 'persist', 'remove', 'flush',
								'getClassMetadata'), array(), '', false);
		$this->repository = $this
				->getMock('Doctrine\ODM\CouchDB\DocumentRepository',
						array('findBy', 'findAll'), array(), '', false);
		$this->dm->expects($this->any())->method('getRepository')
				->with($this->equalTo(self::USERTYPE))
				->will($this->returnValue($this->repository));
		$this->dm->expects($this->any())->method('getClassMetadata')
				->with($this->equalTo(self::USERTYPE))
				->will($this->returnValue($class));
		$this->userManager = new UserManager($ef, $c, $c, $this->dm,
				self::USERTYPE);
	}

	public function testDeleteUser() {
		$user = new DummyUser();
		$this->dm->expects($this->once())->method('remove')
