...
class UniqueValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$options = array('property' => 'username',);
		$context = $this
				->getMockBuilder('Symfony\Component\Validator\ExecutionContext')
				->disableOriginalConstructor()->getMock();
		$this->constraint = new Unique($options);
		$this->userManagerMock = $this
				->getMock('FOS\UserBundle\Model\UserManagerInterface');
	...
	public function testFalseOnDuplicateUserProperty() {
		$this->userManagerMock->expects($this->once())
				->method('validateUnique')->will($this->returnValue(false))
				->with($this->equalTo($this->user),
						$this->equalTo($this->constraint));

		$this
				->assertFalse(
						$this->validator
								->isValid($this->user, $this->constraint));
	}

	public function testTrueOnUniqueUserProperty() {
		$this->userManagerMock->expects($this->once())
				->method('validateUnique')->will($this->returnValue(true))
				->with($this->equalTo($this->user),
						$this->equalTo($this->constraint));
