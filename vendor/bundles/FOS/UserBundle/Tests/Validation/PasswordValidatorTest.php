...
class PasswordValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$options = array('passwordProperty' => 'current',
				'userProperty' => 'user');
		$context = $this
		...
		$this->changePasswordObject = new ChangePassword(
				$this->getMock('FOS\UserBundle\Model\UserInterface'));
	...
	public function testFalseOnInvalidPassword() {
		...
		$this->encoder->expects($this->once())->method('isPasswordValid')
				->will($this->returnValue(false));

		$this
				->assertFalse(
						$this->validator
								->isValid($this->changePasswordObject,
										$this->constraint));
	}
