...
class PropelUserManagerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testFindUserByUsername() {
		$this->userManager->expects($this->once())->method('findUserBy')
				->with($this->equalTo(array('usernameCanonical' => 'jack')));
		$this->userManager->expects($this->once())
				->method('canonicalizeUsername')->with($this->equalTo('jack'))
		...
		$this->userManager->expects($this->once())->method('findUserBy')
				->with(
						$this
								->equalTo(
										array(
												'emailCanonical' => 'jack@email.org')));
		$this->userManager->expects($this->once())->method('canonicalizeEmail')
	...
	public function testLoadUserByUsernameWithExistingUser() {
		$userMock = $this
				->getMock('FOS\UserBundle\Propel\User', array(), array('sha1'));

		$manager = $this->getMockBuilder('FOS\UserBundle\Propel\UserManager')
				->disableOriginalConstructor()
				->setMethods(array('findUserByUsername'))->getMock();

		$manager->expects($this->once())->method('findUserByUsername')
	...
	public function testLoadUserByUsernameWithMissingUser() {
		...
		$manager->expects($this->once())->method('findUserByUsername')
				->with($this->equalTo('jack'))->will($this->returnValue(null));
	...
	protected function getManagerMock() {
		return $this->getMockBuilder('FOS\UserBundle\Propel\UserManager')
				->disableOriginalConstructor()
				->setMethods(
						array('findUserBy', 'canonicalizeUsername',
