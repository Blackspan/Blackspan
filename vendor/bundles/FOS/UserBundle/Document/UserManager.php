...
class UserManager extends BaseUserManager {
	...
	public function validateUnique(UserInterface $value, Constraint $constraint) {
		...
		$query = $this
				->getQueryArray($classMetadata, $value, $constraint->property);
		...
		if ($document instanceof Proxy) {
			$classMetadata
					->setIdentifierValue($document, $document->__identifier);
		}
		// check if document has the same identifier as the current one
		if ($classMetadata->getIdentifierValue($document)
				=== $classMetadata->getIdentifierValue($value)) {
			return true;
		}
	...
	protected function getQueryArray($classMetadata, $value, $fieldName) {
		...
		if (!isset($classMetadata->fieldMappings[$field])) {
			throw new \LogicException(
					"Mapping for '$fieldName' doesn't exist for "
							. $this->class);
		}
		...
		if (isset($mapping['reference']) && $mapping['reference']) {
			throw new \LogicException(
					'Cannot determine uniqueness of referenced document values');
		}

		$criteria[$field] = $value instanceOf UserInterface ? $classMetadata
						->getFieldValue($value, $field) : $value;
