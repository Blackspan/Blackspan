...
class Mailer implements MailerInterface {
	...
	public function __construct($mailer, RouterInterface $router,
			EngineInterface $templating, array $parameters) {
		$this->mailer = $mailer;
		$this->router = $router;
	...
	public function sendConfirmationEmailMessage(UserInterface $user) {
		...
		$rendered = $this->templating
				->render($template,
						array('user' => $user, 'confirmationUrl' => $url));
		$this
				->sendEmailMessage($rendered,
	...
	public function sendResettingEmailMessage(UserInterface $user) {
		...
		$url = $this->router
				->generate('fos_user_resetting_reset',
						array('token' => $user->getConfirmationToken()), true);
		$rendered = $this->templating
				->render($template,
						array('user' => $user, 'confirmationUrl' => $url));
	...
	protected function sendEmailMessage($renderedTemplate, $fromEmail, $toEmail) {
		...
		$message = \Swift_Message::newInstance()->setSubject($subject)
				->setFrom($fromEmail)->setTo($toEmail)->setBody($body);
