...
abstract class RoleCommand extends ContainerAwareCommand {
				...
				->setDefinition(
						array(
								new InputArgument('username',
										...
										'The username'),
								new InputArgument('role',
										InputArgument::OPTIONAL, 'The role'),
								new InputOption('super', null,
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$this
				->executeRoleCommand($manipulator, $output, $username, $super,
						$role);
	}
	...
	abstract protected function executeRoleCommand(
			UserManipulator $manipulator, OutputInterface $output, $username,
			$super, $role);
	...
	protected function interact(InputInterface $input, OutputInterface $output) {
		...
		if ((true !== $input->getOption('super'))
				&& !$input->getArgument('role')) {
			$role = $this->getHelper('dialog')
					->askAndValidate($output, 'Please choose a role:',
							function ($role) {
								if (empty($role)) {
									throw new \Exception(
