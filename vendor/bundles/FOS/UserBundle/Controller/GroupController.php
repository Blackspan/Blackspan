...
class GroupController extends ContainerAware {
	...
	public function editAction($groupname) {
		...
		return $this->container->get('templating')
				->renderResponse(
						'FOSUserBundle:Group:edit.html.' . $this->getEngine(),
	...
	public function newAction() {
		...
		if ($process) {
			...
			$url = $this->container->get('router')
					->generate('fos_user_group_show', $parameters);
		...
		return $this->container->get('templating')
				->renderResponse(
						'FOSUserBundle:Group:new.html.' . $this->getEngine(),
						array('form' => $form->createview(),
								'theme' => $this->container
										->getParameter(
												'fos_user.template.theme'),));
	...
	protected function findGroupBy($key, $value) {
		if (!empty($value)) {
			$group = $this->container->get('fos_user.group_manager')
					->{'findGroupBy' . ucfirst($key)}($value);
		}
