				...
				->setDefinition(
						array(
								new InputArgument('name',
										...
										'A service name (foo)  or search (foo*)'),
								new InputOption('show-private', null,
...
EOF
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		if ($name) {
			...
			$this
					->outputServices($output, $serviceIds,
							$input->getOption('show-private'));
		}
	}
	...
	protected function outputServices(OutputInterface $output, $serviceIds,
		...
		$output
				->writeln(
						sprintf($format1, '<comment>Name</comment>',
	...
	protected function outputService(OutputInterface $output, $serviceId) {
		...
		if ($definition instanceof Definition) {
			$output
					->writeln(
							sprintf('<comment>Service Id</comment>   %s',
									$serviceId));
			$output
					->writeln(
							sprintf('<comment>Class</comment>        %s',
									$definition->getClass()));

			$tags = $definition->getTags() ? implode(', ',
							array_keys($definition->getTags())) : '-';
			$output
					->writeln(
							sprintf('<comment>Tags</comment>         %s', $tags));

			$output
					->writeln(
							sprintf('<comment>Scope</comment>        %s',
									$definition->getScope()));

			$public = $definition->isPublic() ? 'yes' : 'no';
			$output
					->writeln(
							sprintf('<comment>Public</comment>       %s',
									$public));
		} elseif ($definition instanceof Alias) {
			$alias = $definition;
			$output
