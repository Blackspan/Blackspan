 ...
 */
class TraceableEventDispatcher extends ContainerAwareEventDispatcher implements
		...
		if (!is_callable($listener)) {
			if (is_string($listener)) {
				...
				$typeDefinition = '[array] '
						. (is_object($listener[0]) ? get_class($listener[0])
			...
			throw new \RuntimeException(
					sprintf(
							'The given callback (%s) for event "%s" is not callable.',
							$typeDefinition, $eventName));
		}
						...
						if ($skipped) {
							if (is_object($skippedListener)) {
								...
								if (is_object($skippedListener[0])) {
									$typeDefinition = get_class(
											$skippedListener[0]);
								} else {
									$typeDefinition = implode('::',
											$skippedListener);
								}
							} else {
	...
	private function getListenerInfo($listener, $eventName) {
		...
		if ($listener instanceof \Closure) {
			...
			$info += array('type' => 'Function', 'function' => $listener,
					'file' => $file, 'line' => $line, 'pretty' => $listener,);
		} elseif (is_array($listener)
			...
			$info += array('type' => 'Method', 'class' => $class,
					'method' => $listener[1], 'file' => $file,
					'line' => $line, 'pretty' => $class . '::' . $listener[1],);
		}
