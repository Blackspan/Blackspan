...
class SessionTest extends WebTestCase {
	...
	public function testWelcome($config, $insulate) {
		...
		$this
				->assertContains('You are new here and gave no name.',
						$crawler->text());

		// remember name
		$crawler = $client->request('GET', '/session/drak');
		$this
				->assertContains('Hello drak, nice to meet you.',
						$crawler->text());

		// prove remembered name
		$crawler = $client->request('GET', '/session');
		$this
				->assertContains('Welcome back drak, nice to meet you.',
						$crawler->text());
	...
	public function testTwoClients($config, $insulate) {
		...
		$this
				->assertContains('Hello client1, nice to meet you.',
						$crawler1->text());
		...
		$this
				->assertContains('Hello client2, nice to meet you.',
						$crawler2->text());

		// prove remembered name of client1
		$crawler1 = $client1->request('GET', '/session');
		$this
				->assertContains('Welcome back client1, nice to meet you.',
						$crawler1->text());

		// prove remembered name of client2
		$crawler2 = $client2->request('GET', '/session');
		$this
				->assertContains('Welcome back client2, nice to meet you.',
						$crawler2->text());
	...
	public function getConfigs() {
		return array(
		// configfile, insulate
		array('config.yml', true), array('config.yml', false),);
	}
