...
class RouterListenerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->router->expects($this->any())->method('getContext')
	 ...
	 */
	public function testPort($defaultHttpPort, $defaultHttpsPort, $uri,
			$expectedHttpPort, $expectedHttpsPort) {
		$listener = new RouterListener($this->router, $defaultHttpPort,
				$defaultHttpsPort);
		...
		$this
				->assertEquals(0 === strpos($uri, 'https') ? 'https' : 'http',
						$this->context->getScheme());
	}

	public function getPortData() {
		return array(array(80, 443, 'http://localhost/', 80, 443),
	...
	private function createGetResponseEventForUri($uri) {
		...
		return new GetResponseEvent($kernel, $request,
