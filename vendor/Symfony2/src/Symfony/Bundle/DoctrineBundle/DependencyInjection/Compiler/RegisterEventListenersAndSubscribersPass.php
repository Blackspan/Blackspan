...
class RegisterEventListenersAndSubscribersPass implements CompilerPassInterface {
	...
	protected function registerSubscriber($subscriberId, $instances) {
		...
		foreach ($connections as $name) {
			$this->getEventManager($name, $subscriberId)
					->addMethodCall('addEventSubscriber',
							array(new Reference($subscriberId)));
		}
	}
	...
	protected function registerListener($listenerId, $instances) {
		...
		foreach ($instances as $attributes) {
			if (!isset($attributes['event'])) {
				throw new \InvalidArgumentException(
						sprintf(
								'Doctrine event listener "%s" must specify the "event" attribute.',
								$listenerId));
			}
		...
		foreach ($connections as $name => $events) {
			$this->getEventManager($name, $listenerId)
					->addMethodCall('addEventListener',
							array(array_unique($events),
	...
	private function getEventManager($name, $listenerId = null) {
		if (null === $this->eventManagers) {
			...
			foreach ($this->connections as $n => $id) {
				$arguments = $this->container->getDefinition($id)
						->getArguments();
				$this->eventManagers[$n] = $this->container
						->getDefinition((string) $arguments[2]);
			}
		}
		...
		if (!isset($this->eventManagers[$name])) {
			throw new \InvalidArgumentException(
					sprintf(
							'Doctrine connection "%s" does not exist but is referenced in the "%s" event listener.',
							$name, $listenerId));
		}
