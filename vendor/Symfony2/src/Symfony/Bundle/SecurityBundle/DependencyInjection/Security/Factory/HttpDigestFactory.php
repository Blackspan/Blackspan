...
class HttpDigestFactory implements SecurityFactoryInterface {
	...
	public function addConfiguration(NodeDefinition $node) {
		$node->children()->scalarNode('provider')->end()->scalarNode('realm')
				->defaultValue('Secured Area')->end()->scalarNode('key')
				->isRequired()->cannotBeEmpty()->end()->end();
	}
	...
	protected function createEntryPoint($container, $id, $config,
		...
		$container
				...
				->addArgument($config['realm'])->addArgument($config['key']);
