...
class PhpEngine implements EngineInterface, \ArrayAccess {
	...
	public function __construct(TemplateNameParserInterface $parser,
			LoaderInterface $loader, array $helpers = array()) {
		$this->parser = $parser;
		$this->loader = $loader;
		$this->parents = array();
		$this->stack = array();
	...
	public function render($name, array $parameters = array()) {
		...
		if (false === $content = $this->evaluate($storage, $parameters)) {
			throw new \RuntimeException(
					sprintf('The template "%s" cannot be rendered.',
	...
	protected function evaluate(Storage $template, array $parameters = array()) {
		...
		if (isset($parameters['__template__'])) {
			throw new \InvalidArgumentException(
					'Invalid parameter (__template__)');
		}
	...
	public function offsetUnset($name) {
		throw new \LogicException(
				sprintf('You can\'t unset a helper (%s).', $name));
	}
	...
	public function getEscaper($context) {
		if (!isset($this->escapers[$context])) {
			throw new \InvalidArgumentException(
					sprintf('No registered escaper for context "%s".', $context));
		}
		...
		$this->escapers = array(
				...
				function ($value) use ($that) {
					...
					return is_string($value) ? htmlspecialchars($value,
									ENT_QUOTES, $that->getCharset(), false)
							: $value;
				},
				'js' => /**
				...
				function ($value) use ($that) {
					...
					$callback = function ($matches) use ($that) {
						...
						// \uHHHH
						$char = $that
								->convertEncoding($char, 'UTF-16BE', 'UTF-8');
					...
					if ('UTF-8' != $that->getCharset()) {
						$value = $that
								->convertEncoding($value, $that->getCharset(),
										'UTF-8');
					}
