...
class XmlFileLoader extends FileLoader {
	...
	public function loadClassMetadata(ClassMetadata $metadata) {
		if (null === $this->classes) {
			...
			foreach ($xml->namespace as $namespace) {
				$this->namespaces[(string) $namespace['prefix']] = trim(
						(string) $namespace);
			}
		...
		if (isset($this->classes[$metadata->getClassName()])) {
				...
				foreach ($this->parseConstraints($property->constraint) as $constraint) {
					$metadata
							->addPropertyConstraint(
									(string) $property['name'], $constraint);
				}
			}
			...
			foreach ($xml->getter as $getter) {
				foreach ($this->parseConstraints($getter->constraint) as $constraint) {
					$metadata
							->addGetterConstraint(
									(string) $getter['property'], $constraint);
				}
			}
	...
	protected function parseFile($file) {
		...
		if (!$dom
				->schemaValidate(
						__DIR__
								. '/schema/dic/constraint-mapping/constraint-mapping-1.0.xsd')) {
			throw new MappingException(implode("\n", $this->getXmlErrors()));
		}
