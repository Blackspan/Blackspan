...
class Application {
	...
	public function __construct($name = 'UNKNOWN', $version = 'UNKNOWN') {
		...
		$this->helperSet = new HelperSet(
				array(new FormatterHelper(), new DialogHelper(),));
		...
		$this->definition = new InputDefinition(
				array(
						...
						new InputOption('--ansi', '', InputOption::VALUE_NONE,
								'Force ANSI output.'),
						new InputOption('--no-ansi', '',
								InputOption::VALUE_NONE,
								'Disable ANSI output.'),
						new InputOption('--no-interaction', '-n',
								InputOption::VALUE_NONE,
	...
	public function run(InputInterface $input = null,
		...
		try {
			...
			$statusCode = is_numeric($statusCode) && $statusCode ? $statusCode
					: 1;
		}
	...
	public function doRun(InputInterface $input, OutputInterface $output) {
		...
		if (true
				=== $input->hasParameterOption(array('--no-interaction', '-n'))) {
	...
	public function getHelp() {
		$messages = array($this->getLongVersion(), '',
		...
		foreach ($this->getDefinition()->getOptions() as $option) {
			$messages[] = sprintf('  %-29s %s %s',
					...
					$option->getShortcut() ? '<info>-' . $option->getShortcut()
									. '</info>' : '  ',
	...
	public function getLongVersion() {
		if ('UNKNOWN' !== $this->getName() && 'UNKNOWN' !== $this->getVersion()) {
			return sprintf('<info>%s</info> version <comment>%s</comment>',
	...
	public function findNamespace($namespace) {
		...
		foreach (explode(':', $namespace) as $i => $part) {
			$abbrevs = static::getAbbreviations(
					array_unique(
							array_values(
									array_filter(
											array_map(
			...
			if (count($abbrevs[$part]) > 1) {
				throw new \InvalidArgumentException(
						sprintf('The namespace "%s" is ambiguous (%s).',
								$namespace,
								$this
	...
	public function find($name) {
		...
		if (isset($abbrevs[$searchName]) && count($abbrevs[$searchName]) > 1) {
			$suggestions = $this
					->getAbbreviationSuggestions($abbrevs[$searchName]);

			throw new \InvalidArgumentException(
					sprintf('Command "%s" is ambiguous (%s).', $name,
							$suggestions));
		}
	...
	public function asText($namespace = null) {
		$commands = $namespace ? $this->all($this->findNamespace($namespace))
				: $this->commands;
		...
		$width = 0;
		foreach ($commands as $command) {
			$width = strlen($command->getName()) > $width ? strlen(
							$command->getName()) : $width;
		}
		$width += 2;
		...
		foreach ($this->sortCommands($commands) as $space => $commands) {
			...
			foreach ($commands as $name => $command) {
				$messages[] = sprintf("  <info>%-${width}s</info> %s", $name,
	...
	public function asXml($namespace = null, $asDom = false) {
		...
		foreach ($this->sortCommands($commands) as $space => $commands) {
			...
			foreach ($commands as $name => $command) {
				...
				$node = $command->asXml(true)->getElementsByTagName('command')
						->item(0);
	...
	public function renderException($e, $output) {
		...
		do {
			...
			$len = $strlen($title);
			$width = $this->getTerminalWidth() ? $this->getTerminalWidth() - 1
					: PHP_INT_MAX;
			$lines = array();
			...
			$messages = array(str_repeat(' ', $len),
					$title . str_repeat(' ', max(0, $len - $strlen($title))));
			...
			if (OutputInterface::VERBOSITY_VERBOSE === $output->getVerbosity()) {
				...
				array_unshift($trace,
						array('function' => '',
								'file' => $e->getFile() != null ? $e->getFile()
										: 'n/a',
								'line' => $e->getLine() != null ? $e->getLine()
										: 'n/a', 'args' => array(),));
				...
				for ($i = 0, $count = count($trace); $i < $count; $i++) {
					...
					$output
							->writeln(
									sprintf(' %s%s%s() at <info>%s:%s</info>',
											$class, $type, $function, $file,
											$line));
				}
	...
	protected function getTerminalWidth() {
		...
		if (preg_match("{rows.(\d+);.columns.(\d+);}i",
				exec('stty -a | grep columns'), $match)) {
			return $match[2];
		}
	}
	...
	protected function getTerminalHeight() {
		if (defined('PHP_WINDOWS_VERSION_BUILD')
				&& $ansicon = getenv('ANSICON')) {
			return preg_replace('{^\d+x\d+ \(\d+x(\d+)\)$}', '$1',
					trim($ansicon));
		}
	...
	private function getAbbreviationSuggestions($abbrevs) {
		return sprintf('%s, %s%s', $abbrevs[0], $abbrevs[1],
				count($abbrevs) > 2 ? sprintf(' and %d more',
								count($abbrevs) - 2) : '');
	}
