...
function list_files($directory, $extension) {
	...
	foreach ($iterator as $file) {
		if (!$file->isDot()
				&& substr($file->getFilename(), -strlen($extension))
						=== $extension) {
...
function genrb($source, $target) {
	exec('genrb -d ' . $target . ' ' . $source . DIRECTORY_SEPARATOR . '*.txt',
			$output, $result);
...
function genrb_file($target, $source, $locale) {
	exec(
			'genrb -v -d ' . $target . ' ' . $source . DIRECTORY_SEPARATOR
					. $locale . '.txt', $output, $result);
...
function load_resource_bundle($locale, $directory) {
	...
	if (null === $bundle) {
		bailout(
				'The resource bundle for locale ' . $locale
						. ' could not be loaded from directory ' . $directory);
	}
...
function get_data($index, $dataDir, $locale = 'en', $constraint = null) {
	$data = array();
	$bundle = load_resource_bundle($locale,
			__DIR__ . DIRECTORY_SEPARATOR . $dataDir);
...
foreach ($supportedLocales as $key => $supportedLocale) {
	...
	$localeBundleOrig = file_get_contents(
			$source . DIRECTORY_SEPARATOR . 'locales' . DIRECTORY_SEPARATOR
					. $supportedLocale . '.txt');
...
// is equal to "de"
$translatedLocales = array_unique(
		array_merge(list_files($langDir, '.res'),
...
foreach ($translatedLocales as $translatedLocale) {
	...
	foreach ($supportedLocales as $supportedLocale) {
		...
		if ($script) {
			...
			if ($langBundle->get('Languages')->get($lang . '_' . $script)) {
				$langName = $langBundle->get('Languages')
						->get($lang . '_' . $script);
				...
				if (strpos($langName, '(') !== false) {
					list($langName, $scriptName) = preg_split('/[\s()]/',
							$langName, null, PREG_SPLIT_NO_EMPTY);
	...
	$file = fopen(
			$namesGeneratedDir . DIRECTORY_SEPARATOR . $translatedLocale
					. '.txt', 'w');
...
foreach ($bundle->get('CurrencyMeta') as $code => $data) {
	...
	if ('DEFAULT' == $code) {
		$defaultMeta = array('fractionDigits' => $data[0],
...
$countries = get_data('Countries', 'region', $defaultLocale,
		$countriesConstraint);
