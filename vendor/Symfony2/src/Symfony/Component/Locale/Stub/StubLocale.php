...
class StubLocale {
	 ...
	 * @throws MethodNotImplementedException
	 */
	static public function filterMatches($langtag, $locale,
			$canonicalize = false) {
		throw new MethodNotImplementedException(__METHOD__);
	}
	 ...
	 */
	static public function lookup(array $langtag, $locale,
			$canonicalize = false, $default = null) {
		throw new MethodNotImplementedException(__METHOD__);
	}
	...
	static private function getStubData($locale, $cacheVariable, $stubDataDir) {
		if ('en' !== $locale) {
			throw new \InvalidArgumentException(
					sprintf('Only the \'en\' locale is supported. %s',
							NotImplementedException::INTL_INSTALL_MESSAGE));
		}

		if (empty(self::${$cacheVariable})) {
			self::${$cacheVariable} = include __DIR__
					. '/../Resources/data/stub/' . $stubDataDir . '/en.php';
		}
