...
class FullTransformer {
	...
	public function __construct($pattern, $timezone) {
		...
		$implementedCharsMatch = $this
				->buildCharsMatch($this->implementedChars);
		$notImplementedCharsMatch = $this
				->buildCharsMatch($this->notImplementedChars);
		$this->regExp = "/($this->quoteMatch|$implementedCharsMatch|$notImplementedCharsMatch)/";

		$this->transformers = array('M' => new MonthTransformer(),
				'L' => new MonthTransformer(), 'y' => new YearTransformer(),
				'd' => new DayTransformer(), 'q' => new QuarterTransformer(),
				'Q' => new QuarterTransformer(),
				'h' => new Hour1201Transformer(),
				'D' => new DayOfYearTransformer(),
				'E' => new DayOfWeekTransformer(),
				'a' => new AmPmTransformer(), 'H' => new Hour2400Transformer(),
				'K' => new Hour1200Transformer(),
				'k' => new Hour2401Transformer(),
				'm' => new MinuteTransformer(), 's' => new SecondTransformer(),
	...
	public function format(\DateTime $dateTime) {
		...
		$formatted = preg_replace_callback($this->regExp,
	...
	public function formatReplace($dateChars, $dateTime) {
		...
		if (false !== strpos($this->notImplementedChars, $dateChars[0])) {
			throw new NotImplementedException(
					sprintf(
							"Unimplemented date character '%s' in format '%s'",
							$dateChars[0], $this->pattern));
		}
	}
	...
	public function parse(\DateTime $dateTime, $value) {
		$reverseMatchingRegExp = $this
				->getReverseMatchingRegExp($this->pattern);
	...
	public function getReverseMatchingRegExp($pattern) {
		...
		$reverseMatchingRegExp = preg_replace_callback($this->regExp,
	...
	protected function normalizeArray(array $data) {
		...
		foreach ($data as $key => $value) {
			...
			$ret[$key[0]] = array('value' => $value, 'pattern' => $key);
		}
	 ...
	 */
	protected function calculateUnixTimestamp(\DateTime $dateTime,
			array $options) {
		...
		if (in_array('yy', $matches[0])) {
			...
			$year = $year > $dateTime->format('y') + 20 ? 1900 + $year
					: 2000 + $year;
		}
	...
	private function getDefaultValueForOptions(array $options) {
		return array(
				...
				'hourInstance' => isset($options['hourInstance']) ? $options['hourInstance']
						: null,
