...
class TimeZoneTransformer extends Transformer {
	...
	public function format(\DateTime $dateTime, $length) {
		...
		if (!in_array($timeZone, array('Etc', 'UTC'))) {
			throw new NotImplementedException(
					'Time zone different than GMT or UTC is not supported as a formatting output.');
		}
	...
	public function extractDateOptions($matched, $length) {
		return array('timezone' => self::getEtcTimeZoneId($matched));
	}
	...
	static public function getEtcTimeZoneId($formattedTimeZone) {
		...
		throw new \InvalidArgumentException(
				'The GMT time zone \'%s\' does not match with the supported formats GMT[+-]HH:MM or GMT[+-]HHMM.');
	}
}
