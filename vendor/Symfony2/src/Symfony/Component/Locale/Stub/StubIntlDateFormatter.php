...
class StubIntlDateFormatter {
	 ...
	 * @var array
	 */
	private $defaultDateFormats = array(self::NONE => '',
			self::FULL => 'EEEE, LLLL d, y', self::LONG => 'LLLL d, y',
			self::MEDIUM => 'LLL d, y', self::SHORT => 'M/d/yy',);
	 ...
	 * @var array
	 */
	private $defaultTimeFormats = array(self::FULL => 'h:mm:ss a zzzz',
			self::LONG => 'h:mm:ss a z', self::MEDIUM => 'h:mm:ss a',
			self::SHORT => 'h:mm a',);
	 ...
	 */
	public function __construct($locale, $datetype, $timetype, $timezone = null,
			$calendar = self::GREGORIAN, $pattern = null) {
		if ('en' !== $locale) {
	 ...
	 * @throws MethodArgumentValueNotImplementedException  When $locale different than 'en' is passed
	 */
	static public function create($locale, $datetype, $timetype,
			$timezone = null, $calendar = self::GREGORIAN, $pattern = null) {
		return new self($locale, $datetype, $timetype, $timezone, $calendar,
				$pattern);
	}
	...
	public function format($timestamp) {
		...
		if (is_array($timestamp)) {
			$message = version_compare(\PHP_VERSION, '5.3.4', '>=') ? 'Only integer unix timestamps and DateTime objects are supported'
		...
		if (null !== $argumentError) {
			StubIntl::setError(StubIntl::U_ILLEGAL_ARGUMENT_ERROR,
					$argumentError);
		...
		$transformer = new FullTransformer($this->getPattern(),
				$this->getTimeZoneId());
	...
	public function setTimeZoneId($timeZoneId) {
		...
		if ('GMT' !== $timeZoneId && 0 === strpos($timeZoneId, 'GMT')) {
			try {
				$timeZoneId = DateFormat\TimeZoneTransformer::getEtcTimeZoneId(
						$timeZoneId);
