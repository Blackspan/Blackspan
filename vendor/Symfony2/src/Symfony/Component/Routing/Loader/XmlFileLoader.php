...
class XmlFileLoader extends FileLoader {
	 ...
	 * @param string          $file
	 */
	protected function parseNode(RouteCollection $collection, \DOMElement $node,
			$path, $file) {
		switch ($node->tagName) {
		case 'route':
		...
		default:
			throw new \InvalidArgumentException(
					sprintf('Unable to parse tag "%s"', $node->tagName));
		}
	}
	 ...
	 */
	protected function parseRoute(RouteCollection $collection,
			\DOMElement $definition, $file) {
		$defaults = array();
		$requirements = array();
		$options = array();
		...
		foreach ($definition->childNodes as $node) {
			...
			switch ($node->tagName) {
			case 'default':
				$defaults[(string) $node->getAttribute('key')] = trim(
						(string) $node->nodeValue);
				break;
			case 'option':
				$options[(string) $node->getAttribute('key')] = trim(
						(string) $node->nodeValue);
				break;
			case 'requirement':
				$requirements[(string) $node->getAttribute('key')] = trim(
						(string) $node->nodeValue);
				break;
			default:
				throw new \InvalidArgumentException(
		...
		$route = new Route((string) $definition->getAttribute('pattern'),
