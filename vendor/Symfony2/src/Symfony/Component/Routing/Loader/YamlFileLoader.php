...
class YamlFileLoader extends FileLoader {
	private static $availableKeys = array('type', 'resource', 'prefix',
			'pattern', 'options', 'defaults', 'requirements');
	...
	public function load($file, $type = null) {
		...
		if (!is_array($config)) {
			throw new \InvalidArgumentException(
					sprintf('The file "%s" must contain a YAML array.', $file));
		}
	...
	protected function parseRoute(RouteCollection $collection, $name, $config,
		...
		if (!isset($config['pattern'])) {
			throw new \InvalidArgumentException(
					sprintf('You must define a "pattern" for the "%s" route.',
							$name));
		}
