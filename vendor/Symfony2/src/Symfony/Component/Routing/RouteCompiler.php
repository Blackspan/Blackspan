...
class RouteCompiler implements RouteCompilerInterface {
	...
	public function compile(Route $route) {
		...
		$variables = array();
		$pos = 0;
		preg_match_all('#.\{([\w\d_]+)\}#', $pattern, $matches,
		...
		foreach ($matches as $match) {
			...
			if ($req = $route->getRequirement($var)) {
				...
				$regexp = sprintf('[^%s]+?',
						preg_quote(implode('', array_unique($seps)), '#'));
			}
		...
		return new CompiledRoute($route,
				...
				sprintf("#^%s$#s", $regexp), array_reverse($tokens),
				$variables);
	}
	...
	private function computeRegexp(array $tokens, $index, $firstOptional) {
		...
		if ('text' === $token[0]) {
			...
			if (0 === $index && 0 === $firstOptional && 1 == count($tokens)) {
				...
				return sprintf('%s(?P<%s>%s)?', preg_quote($token[1], '#'),
						$token[3], $token[2]);
			} else {
				$nbTokens = count($tokens);
				$regexp = sprintf('%s(?P<%s>%s)', preg_quote($token[1], '#'),
						$token[3], $token[2]);
