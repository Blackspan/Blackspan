...
class PhpMatcherDumper extends MatcherDumper {
	...
	public function dump(array $options = array()) {
		...
		return $this->startClass($options['class'], $options['base_class'])
				. $this->addConstructor()
				. $this->addMatcher($supportsRedirections) . $this->endClass();
	}
	...
	private function addMatcher($supportsRedirections) {
		...
		$code = implode("\n",
				$this
						->compileRoutes(clone $this->getRoutes(),
...
EOF;
	...
	private function compileRoutes(RouteCollection $routes,
			$supportsRedirections, $parentPrefix = null) {
		$code = array();
		...
		foreach ($routeIterator as $name => $route) {
			...
			if ($route instanceof RouteCollection) {
				...
				$optimizable = $prefix && count($route->all()) > 1
						&& false === strpos($route->getPrefix(), '{');
				$indent = '';
				if ($optimizable) {
					for ($j = $i; $j < $keysCount; $j++) {
						...
						$testPrefix = $isCollection ? $testRoute->getPrefix()
								: $testRoute->getPattern();
				...
				foreach ($this
						->compileRoutes($route, $supportsRedirections, $prefix) as $line) {
					foreach (explode("\n", $line) as $l) {
						if ($l) {
							$code[] = $indent . $l;
						} else {
				...
				foreach ($this
						->compileRoute($route, $name, $supportsRedirections,
								$parentPrefix) as $line) {
					$code[] = $line;
				}
			}
		}
	...
	private function compileRoute(Route $route, $name, $supportsRedirections,
		...
		$matches = false;
		if (!count($compiledRoute->getVariables())
				&& false
						!== preg_match('#^(.)\^(?P<url>.*?)\$\1#',
			...
			if ($supportsRedirections && substr($m['url'], -1) === '/') {
				$conditions[] = sprintf("rtrim(\$pathinfo, '/') === %s",
						var_export(
								rtrim(str_replace('\\', '', $m['url']), '/'),
								true));
				$hasTrailingSlash = true;
			} else {
				$conditions[] = sprintf("\$pathinfo === %s",
						var_export(str_replace('\\', '', $m['url']), true));
			}
		} else {
			if ($compiledRoute->getStaticPrefix()
					&& $compiledRoute->getStaticPrefix() != $parentPrefix) {
			...
			$conditions[] = sprintf("preg_match(%s, \$pathinfo, \$matches)",
		...
		if ($scheme = $route->getRequirement('_scheme')) {
			if (!$supportsRedirections) {
				throw new \LogicException(
						'The "_scheme" requirement is only supported for route dumper that implements RedirectableUrlMatcherInterface.');
			}
...
EOF
		...
		if (true === $matches && $compiledRoute->getDefaults()) {
			...
			$code[] = sprintf('            return %s;',
					str_replace("\n", '',
							var_export(
									array_merge($compiledRoute->getDefaults(),
											array('_route' => $name)), true)));
		} else {
