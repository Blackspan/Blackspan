...
class ApacheMatcherDumper extends MatcherDumper {
	...
	public function dump(array $options = array()) {
		$options = array_merge(
				array('script_name' => 'app.php', 'base_uri' => '',), $options);
		...
		foreach ($this->getRoutes()->all() as $name => $route) {
			...
			if (strlen($regex) < 2 || 0 === $regexPatternEnd) {
				throw new \LogicException(
						'The "%s" route regex "%s" is invalid', $name, $regex);
			}
			$regex = preg_replace('/\?P<.+?>/', '',
					substr($regex, 1, $regexPatternEnd - 1));
			$regex = '^'
					. self::escape(
							preg_quote($options['base_uri'])
									. substr($regex, 1), ' ', '\\');
			...
			if ($req = $route->getRequirement('_method')) {
				...
				$rule[] = sprintf(
						...
						implode('|', $methods));
				$rule[] = sprintf('RewriteRule .* - [S=%d,%s]',
						$hasTrailingSlash ? 2 : 1, implode(',', $allow));
			}
		...
		if (0 < count($methodVars)) {
			...
			foreach ($methodVars as $i => $methodVar) {
				$rule[] = sprintf('RewriteCond %%{_ROUTING__allow_%s} !-z%s',
						$methodVar, isset($methodVars[$i + 1]) ? ' [OR]' : '');
			}
			$rule[] = sprintf('RewriteRule .* %s [QSA,L]',
					$options['script_name']);
