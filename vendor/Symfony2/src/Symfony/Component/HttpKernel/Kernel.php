...
abstract class Kernel implements KernelInterface {
	...
	public function __construct($environment, $debug) {
		...
		$this->name = preg_replace('/[^a-zA-Z0-9_]+/', '',
	...
	public function getBundle($name, $first = true) {
		if (!isset($this->bundleMap[$name])) {
			throw new \InvalidArgumentException(
					sprintf(
							'Bundle "%s" does not exist or it is not enabled. Maybe you forgot to add it in the registerBundles() function of your %s.php file?',
							$name, get_class($this)));
		}
	...
	public function locateResource($name, $dir = null, $first = true) {
		if ('@' !== $name[0]) {
			throw new \InvalidArgumentException(
					sprintf('A resource name must start with @ ("%s" given).',
							$name));
		}
	...
	public function loadClassCache($name = 'classes', $extension = '.php') {
		if (!$this->booted && file_exists($this->getCacheDir() . '/classes.map')) {
			ClassCollectionLoader::load(
					...
					$this->getCacheDir(), $name, $this->debug, false,
					$extension);
		}
	}
	...
	protected function initializeBundles() {
		...
		foreach ($this->registerBundles() as $bundle) {
			...
			if ($parentName = $bundle->getParent()) {
				...
				if ($parentName == $name) {
					throw new \LogicException(
							sprintf('Bundle "%s" can not extend itself.', $name));
				}
		...
		if (count(
				$diff = array_values(
						array_diff(array_keys($directChildren),
								array_keys($this->bundles))))) {
			throw new \LogicException(
					sprintf(
	...
	protected function getContainerClass() {
		return $this->name . ucfirst($this->environment)
	...
	protected function initializeContainer() {
		...
		$cache = new ConfigCache($this->getCacheDir() . '/' . $class . '.php',
				$this->debug);
		$fresh = true;
		if (!$cache->isFresh()) {
			$container = $this->buildContainer();
			$this
					->dumpContainer($cache, $container, $class,
	...
	protected function getKernelParameters() {
		...
		return array_merge(
				array('kernel.root_dir' => $this->rootDir,
	...
	protected function buildContainer() {
		foreach (array('cache' => $this->getCacheDir(),
				'logs' => $this->getLogDir()) as $name => $dir) {
			if (!is_dir($dir)) {
				...
				throw new \RuntimeException(
						sprintf("Unable to write in the %s directory (%s)\n",
								$name, $dir));
			}
		}
	...
	protected function dumpContainer(ConfigCache $cache,
		...
		$content = $dumper
				->dump(array('class' => $class, 'base_class' => $baseClass));
		if (!$this->debug) {
