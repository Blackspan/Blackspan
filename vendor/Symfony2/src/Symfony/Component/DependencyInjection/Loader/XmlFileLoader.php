...
class XmlFileLoader extends FileLoader {
	...
	public function load($file, $type = null) {
		...
		$xml
				->registerXPathNamespace('container',
	...
	private function parseParameters(SimpleXMLElement $xml, $file) {
		...
		$this->container->getParameterBag()
				->add($xml->parameters->getArgumentsAsPhp('parameter'));
	}
	...
	private function parseImports(SimpleXMLElement $xml, $file) {
		...
		foreach ($imports as $import) {
			...
			$this
					->import((string) $import['resource'], null,
							(Boolean) $import
									->getAttributeAsPhp('ignore-errors'),
							$file);
		}
	}
	...
	private function parseDefinition($id, $service, $file) {
		if ((string) $service['alias']) {
			...
			$this->container
					->setAlias($id,
							new Alias((string) $service['alias'], $public));
		...
		foreach (array('class', 'scope', 'public', 'factory-class',
				'factory-method', 'factory-service', 'synthetic', 'abstract') as $key) {
			if (isset($service[$key])) {
				$method = 'set' . str_replace('-', '', $key);
				$definition
						->$method((string) $service->getAttributeAsPhp($key));
			}
		}
		...
		if (isset($service->configurator)) {
			if (isset($service->configurator['function'])) {
				$definition
						->setConfigurator(
								(string) $service->configurator['function']);
			} else {
				if (isset($service->configurator['service'])) {
					$class = new Reference(
							(string) $service->configurator['service'],
		...
		foreach ($service->call as $call) {
			$definition
					->addMethodCall((string) $call['method'],
	...
	private function processAnonymousServices(SimpleXMLElement $xml, $file) {
		...
		if (false
				=== $nodes = $xml
						->xpath(
								'//container:argument[@type="service"][not(@id)]')) {
			return $xml;
		}
		...
		if (false
				=== $nodes = $xml
						->xpath(
								'//container:services/container:service[not(@id)]')) {
			return $xml;
		}
		foreach ($nodes as $node) {
			// give it a unique name
	...
	private function validateSchema(\DOMDocument $dom, $file) {
		...
		if ($element = $dom->documentElement
			...
			for ($i = 0, $nb = count($items); $i < $nb; $i += 2) {
				...
				if (($extension = $this->container->getExtension($items[$i]))
						&& false !== $extension->getXsdValidationBasePath()) {
					...
					if (!file_exists($path)) {
						throw new \RuntimeException(
								sprintf(
										'Extension "%s" references a non-existent XSD file "%s"',
	...
	private function getXmlErrors() {
		...
		foreach (libxml_get_errors() as $error) {
			$errors[] = sprintf('[%s %s] %s (in %s - line %d, column %d)',
					...
					$error->file ? $error->file : 'n/a', $error->line,
					$error->column);
		}
