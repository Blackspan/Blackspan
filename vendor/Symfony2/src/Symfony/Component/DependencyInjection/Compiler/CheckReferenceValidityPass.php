...
class CheckReferenceValidityPass implements CompilerPassInterface {
		...
		foreach ($arguments as $argument) {
			if (is_array($argument)) {
				...
				if (null !== $targetDefinition
						&& $targetDefinition->isAbstract()) {
					throw new \RuntimeException(
							sprintf(
									'The definition "%s" has a reference to an abstract definition "%s". '
											. 'Abstract definitions cannot be the target of references.',
									$this->currentId, $argument));
				}
	 ...
	 */
	private function validateScope(Reference $reference,
		...
		if (in_array($scope, $this->currentScopeChildren, true)) {
			throw new ScopeWideningInjectionException($this->currentId,
					$this->currentScope, $id, $scope);
		}

		if (!in_array($scope, $this->currentScopeAncestors, true)) {
			throw new ScopeCrossingInjectionException($this->currentId,
					$this->currentScope, $id, $scope);
		}
	}
