...
class PhpDumper extends Dumper {
	...
	public function dump(array $options = array()) {
		$options = array_merge(
				array('class' => 'ProjectServiceContainer',
						'base_class' => 'Container',), $options);
		...
		$code .= $this->addServices() . $this->addDefaultParametersMethod()
				. $this->endClass();
		...
		foreach ($localDefinitions as $iDefinition) {
			$this
					->getServiceCallsFromArguments(
							$iDefinition->getArguments(), $calls, $behavior);
			$this
					->getServiceCallsFromArguments(
							$iDefinition->getMethodCalls(), $calls, $behavior);
			$this
					->getServiceCallsFromArguments(
							$iDefinition->getProperties(), $calls, $behavior);
		}
		...
		foreach ($calls as $id => $callCount) {
			...
			if ($callCount > 1) {
				...
				if (ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE
						=== $behavior[$id]) {
					$code .= sprintf($template, $name,
							$this->getServiceCall($id));
				} else {
					$code .= sprintf($template, $name,
							$this
									->getServiceCall($id,
	...
	private function addServiceInlinedDefinitions($id, $definition) {
		...
		foreach ($inlinedDefinitions as $sDefinition) {
			...
			if ($nbOccurrences->offsetGet($sDefinition) > 1
					|| count($sDefinition->getMethodCalls()) > 0
				...
				if (null !== $sDefinition->getFactoryMethod()) {
					if (null !== $sDefinition->getFactoryClass()) {
						$code .= sprintf(
								"        \$%s = call_user_func(array(%s, '%s')%s);\n",
								$name,
								$this
										->dumpValue(
												$sDefinition->getFactoryClass()),
								$sDefinition->getFactoryMethod(),
								count($arguments) > 0 ? ', '
												. implode(', ', $arguments) : '');
						...
						throw new \RuntimeException(
								'Factory service or factory class must be defined in service definition for '
										. $id);
					}
	...
	private function addServiceInstance($id, $definition) {
		...
		if (0 === strpos($class, "'")
			...
			throw new \InvalidArgumentException(
					sprintf(
							'"%s" is not a valid class name for the "%s" service.',
							$class, $id));
		}
		...
		if (ContainerInterface::SCOPE_CONTAINER === $definition->getScope()) {
			$instantiation = "\$this->services['$id'] = "
					. ($simple ? '' : '$instance');
		} elseif (ContainerInterface::SCOPE_PROTOTYPE
		...
		if (null !== $definition->getFactoryMethod()) {
			if (null !== $definition->getFactoryClass()) {
				$code = sprintf(
						"        $return{$instantiation}call_user_func(array(%s, '%s')%s);\n",
						$this->dumpValue($definition->getFactoryClass()),
						$definition->getFactoryMethod(),
						$arguments ? ', ' . implode(', ', $arguments) : '');
			} elseif (null !== $definition->getFactoryService()) {
				$code = sprintf(
						"        $return{$instantiation}%s->%s(%s);\n",
						$this->getServiceCall($definition->getFactoryService()),
						$definition->getFactoryMethod(),
						implode(', ', $arguments));
			} else {
				throw new \RuntimeException(
						'Factory method requires a factory service or factory class in service definition for '
								. $id);
			}
		} elseif (false !== strpos($class, '$')) {
			$code = sprintf(
	...
	private function isSimpleInstance($id, $definition) {
		foreach (array_merge(array($definition),
				$this->getInlinedDefinitions($definition)) as $sDefinition) {
			if ($definition !== $sDefinition
					&& !$this
							->hasReference($id, $sDefinition->getMethodCalls())) {
				continue;
			}

			if ($sDefinition->getMethodCalls() || $sDefinition->getProperties()
	 ...
	 * @return string
	 */
	private function addServiceMethodCalls($id, $definition,
		...
		foreach ($definition->getMethodCalls() as $call) {
			...
			$calls .= $this
					->wrapServiceConditionals($call[1],
							sprintf("        \$%s->%s(%s);\n", $variableName,
	...
	private function addServiceProperties($id, $definition,
			$variableName = 'instance') {
		$code = '';
	...
	private function addServiceInlinedDefinitionsSetup($id, $definition) {
		...
		foreach ($this->getInlinedDefinitions($definition) as $iDefinition) {
			...
			if ($iDefinition->getMethodCalls()) {
				$code .= $this
						->addServiceMethodCalls(null, $iDefinition,
								(string) $this->definitionVariables
										->offsetGet($iDefinition));
			}
			if ($iDefinition->getConfigurator()) {
				$code .= $this
						->addServiceConfigurator(null, $iDefinition,
								(string) $this->definitionVariables
										->offsetGet($iDefinition));
			}
		}
	 ...
	 * @return string
	 */
	private function addServiceConfigurator($id, $definition,
			$variableName = 'instance') {
		...
		if (is_array($callable)) {
			if (is_object($callable[0]) && $callable[0] instanceof Reference) {
				return sprintf("        %s->%s(\$%s);\n",
						$this->getServiceCall((string) $callable[0]),
						$callable[1], $variableName);
			}
	...
	private function addService($id, $definition) {
		...
		if ($definition->isSynthetic()) {
			...
			$return = sprintf("@return %s A %s instance.",
					0 === strpos($class, '%') ? 'Object' : $class, $class);
		} elseif ($definition->getFactoryClass()) {
			$return = sprintf(
					'@return Object An instance returned by %s::%s().',
...
EOF;
		...
		if (ContainerInterface::SCOPE_CONTAINER !== $scope
				&& ContainerInterface::SCOPE_PROTOTYPE !== $scope) {
			$code .= <<<EOF
...
EOF;
		...
		if ($definition->isSynthetic()) {
			$code .= sprintf(
					"        throw new \RuntimeException('You have requested a synthetic service (\"%s\"). The DIC does not know how to construct this service.');\n    }\n",
					$id);
		} else {
			$code .= $this->addServiceInclude($id, $definition)
					...
					. $this
							->addServiceInlinedDefinitionsSetup($id,
									$definition)
...
EOF;
	...
	private function startClass($class, $baseClass) {
		$bagClass = $this->container->isFrozen() ? 'use Symfony\Component\DependencyInjection\ParameterBag\FrozenParameterBag;'
...
EOF;
	...
	private function addConstructor() {
		$arguments = $this->container->getParameterBag()->all() ? 'new ParameterBag($this->getDefaultParameters())'
				: null;
...
EOF;
		if (count($scopes = $this->container->getScopes()) > 0) {
			...
			$code .= "        \$this->scopes = " . $this->dumpValue($scopes)
					. ";\n";
			$code .= "        \$this->scopeChildren = "
					. $this->dumpValue($this->container->getScopeChildren())
					. ";\n";
		}
...
EOF;
	...
	private function addDefaultParametersMethod() {
		...
		$parameters = $this
				->exportParameters($this->container->getParameterBag()->all());
...
EOF;
	...
	private function exportParameters($parameters, $path = '', $indent = 12) {
		...
		foreach ($parameters as $key => $value) {
			if (is_array($value)) {
				$value = $this
						->exportParameters($value, $path . '/' . $key, $indent
								+ 4);
			} elseif ($value instanceof Variable) {
				throw new \InvalidArgumentException(
						sprintf(
								'You cannot dump a container with parameters that contain variable references. Variable "%s" found in "%s".',
								$value, $path . '/' . $key));
			} elseif ($value instanceof Definition) {
				throw new \InvalidArgumentException(
						sprintf(
								'You cannot dump a container with parameters that contain service definitions. Definition for "%s" found in "%s".',
								$value->getClass(), $path . '/' . $key));
			} elseif ($value instanceof Reference) {
				throw new \InvalidArgumentException(
						sprintf(
								'You cannot dump a container with parameters that contain references to other services (reference to service "%s" found in "%s").',
								$value, $path . '/' . $key));
			} else {
				$value = var_export($value, true);
			}

			$php[] = sprintf('%s%s => %s,', str_repeat(' ', $indent),
...
EOF;
	 ...
	 * @return void
	 */
	private function getServiceCallsFromArguments(array $arguments,
			array &$calls, array &$behavior) {
		foreach ($arguments as $argument) {
			if (is_array($argument)) {
				$this
						->getServiceCallsFromArguments($argument, $calls,
								$behavior);
				...
				} elseif (ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE
						!== $behavior[$id]) {
					$behavior[$id] = $argument->getInvalidBehavior();
				}
		...
		foreach ($arguments as $argument) {
			if (is_array($argument)) {
				$definitions = array_merge($definitions,
						$this->getDefinitionsFromArguments($argument));
	...
	private function dumpValue($value, $interpolate = true) {
		if (is_array($value)) {
			...
			if (null !== $this->definitionVariables
					&& $this->definitionVariables->contains($value)) {
				return $this
						->dumpValue(
								$this->definitionVariables->offsetGet($value),
								$interpolate);
			}
			...
			if (null !== $value->getConfigurator()) {
				throw new \RuntimeException(
						'Cannot dump definitions which have a configurator.');
			}
			...
			if (false !== strpos($class, '$')) {
				throw new \RuntimeException(
						'Cannot dump definitions which have a variable class name.');
			}

			if (null !== $value->getFactoryMethod()) {
				if (null !== $value->getFactoryClass()) {
					return sprintf("call_user_func(array(%s, '%s')%s)",
							$this->dumpValue($value->getFactoryClass()),
							$value->getFactoryMethod(),
							count($arguments) > 0 ? ', '
											. implode(', ', $arguments) : '');
				} elseif (null !== $value->getFactoryService()) {
					return sprintf("%s->%s(%s)",
							$this->getServiceCall($value->getFactoryService()),
							$value->getFactoryMethod(),
							implode(', ', $arguments));
				} else {
					throw new \RuntimeException(
							'Cannot dump definitions which have factory method without factory service or factory class.');
				}
			}

			return sprintf("new \\%s(%s)",
					substr(str_replace('\\\\', '\\', $class), 1, -1),
					implode(', ', $arguments));
		} elseif (is_object($value) && $value instanceof Variable) {
			return '$' . $value;
		} elseif (is_object($value) && $value instanceof Reference) {
			if (null !== $this->referenceVariables
					&& isset($this->referenceVariables[$id = (string) $value])) {
				return $this
						->dumpValue($this->referenceVariables[$id],
								$interpolate);
			}
			...
			if (preg_match('/^%([^%]+)%$/', $value, $match)) {
				...
				$code = str_replace('%%', '%',
						preg_replace_callback('/(?<!%)(%)([^%]+)\1/',
								$replaceParameters, var_export($value, true)));
	...
	public function dumpParameter($name) {
		if ($this->container->isFrozen()
			...
			return $this
					->dumpValue($this->container->getParameter($name), false);
		}
	...
	private function getServiceCall($id, Reference $reference = null) {
		...
		if (null !== $reference
			...
			return sprintf(
					'$this->get(\'%s\', ContainerInterface::NULL_ON_INVALID_REFERENCE)',
					$id);
		} else {
