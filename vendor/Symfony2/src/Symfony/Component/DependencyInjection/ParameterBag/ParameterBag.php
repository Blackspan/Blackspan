	...
	public function resolveValue($value, array $resolving = array()) {
		if (is_array($value)) {
			...
			foreach ($value as $k => $v) {
				$args[$this->resolveValue($k, $resolving)] = $this
	...
	public function resolveString($value, array $resolving = array()) {
		...
		if (preg_match('/^%([^%\s]+)%$/', $value, $match)) {
			...
			if (isset($resolving[$key])) {
				throw new ParameterCircularReferenceException(
						array_keys($resolving));
			}

			$resolving[$key] = true;

			return $this->resolved ? $this->get($key)
					: $this->resolveValue($this->get($key), $resolving);
		}
		...
		return preg_replace_callback('/%%|%([^%\s]+)%/',
				function ($match) use ($self, $resolving, $value) {
					...
					return $self->isResolved() ? $resolved
							: $self->resolveString($resolved, $resolving);
				}, $value);
	}
