...
class Form implements \IteratorAggregate, FormInterface {
	...
	public function __construct($name, EventDispatcherInterface $dispatcher,
		...
		foreach ($clientTransformers as $transformer) {
			if (!$transformer instanceof DataTransformerInterface) {
				throw new UnexpectedTypeException($transformer,
		...
		foreach ($validators as $validator) {
			if (!$validator instanceof FormValidatorInterface) {
				throw new UnexpectedTypeException($validator,
	...
	public function setData($appData) {
		...
		if (!$this->clientTransformers && !$this->normTransformers
				&& is_scalar($appData)) {
	...
	public function bindRequest(Request $request) {
		...
		default:
			throw new FormException(
					sprintf('The request method "%s" is not supported',
							$request->getMethod()));
		}
	...
	public function isEmpty() {
		...
		return array() === $this->appData || null === $this->appData
				|| '' === $this->appData;
	}
	...
	public function get($name) {
		...
		throw new \InvalidArgumentException(
				sprintf('Field "%s" does not exist.', $name));
	}
