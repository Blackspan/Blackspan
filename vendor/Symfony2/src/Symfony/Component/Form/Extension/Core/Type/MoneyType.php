...
class MoneyType extends AbstractType {
		...
		$builder
				->appendClientTransformer(
						new MoneyToLocalizedStringTransformer(
								$options['precision'], $options['grouping'],
								null, $options['divisor']))
	...
	public function buildView(FormView $view, FormInterface $form) {
		$view
				->set('money_pattern',
						self::getPattern($form->getAttribute('currency')));
	}
	...
	public function getDefaultOptions(array $options) {
		return array('precision' => 2, 'grouping' => false, 'divisor' => 1,
				'currency' => 'EUR',);
	}
	...
	static private function getPattern($currency) {
		...
		if (!isset(self::$patterns[\Locale::getDefault()][$currency])) {
			$format = new \NumberFormatter(\Locale::getDefault(),
			...
			if (!empty($matches[1])) {
				self::$patterns[\Locale::getDefault()] = $matches[1]
						. ' {{ widget }}';
