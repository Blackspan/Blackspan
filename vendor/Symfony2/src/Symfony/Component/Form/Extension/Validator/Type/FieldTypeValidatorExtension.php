...
class FieldTypeValidatorExtension extends AbstractTypeExtension {
	...
	public function buildForm(FormBuilder $builder, array $options) {
		$options['validation_groups'] = empty($options['validation_groups']) ? null
				: (array) $options['validation_groups'];

		$builder
				->setAttribute('validation_groups',
						$options['validation_groups'])
				->setAttribute('validation_constraint',
						$options['validation_constraint'])
				->addValidator(new DelegatingValidator($this->validator));
	}

	public function getDefaultOptions(array $options) {
		return array('validation_groups' => null,
