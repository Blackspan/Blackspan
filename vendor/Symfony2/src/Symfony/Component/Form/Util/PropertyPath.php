	...
	public function __construct($propertyPath) {
		if (null === $propertyPath) {
			throw new InvalidPropertyPathException(
					'The property path must not be empty');
		}
		...
		if (!empty($remaining)) {
			throw new InvalidPropertyPathException(
					sprintf(
							...
							$propertyPath, $remaining{0}, $position));
		}
		...
		for ($i = 0; $i < $this->length; ++$i) {
			if (is_object($objectOrArray)) {
				...
				if (!array_key_exists($property, $objectOrArray)) {
					$objectOrArray[$property] = $i + 1 < $this->length ? array()
							: null;
				}
				$value = &$objectOrArray[$property];
			} else {
				throw new UnexpectedTypeException($objectOrArray,
						'object or array');
			}
		...
		if ($this->isIndex[$currentIndex]) {
			if (!$object instanceof \ArrayAccess) {
				throw new InvalidPropertyException(
						sprintf(
								'Index "%s" cannot be read from object of type "%s" because it doesn\'t implement \ArrayAccess',
				...
				if (!$reflClass->getProperty($property)->isPublic()) {
					throw new PropertyAccessDeniedException(
							sprintf(
									...
									$property, $reflClass->getName(), $getter,
									$isser));
				}
	...
	protected function writeProperty(&$objectOrArray, $currentIndex, $value) {
		...
		if (is_object($objectOrArray) && $this->isIndex[$currentIndex]) {
			if (!$objectOrArray instanceof \ArrayAccess) {
				throw new InvalidPropertyException(
						sprintf(
								'Index "%s" cannot be modified in object of type "%s" because it doesn\'t implement \ArrayAccess',
								$property, get_class($objectOrArray)));
			}
			...
			if ($reflClass->hasMethod($setter)) {
				...
				if (!$reflClass->getProperty($property)->isPublic()) {
					throw new PropertyAccessDeniedException(
							sprintf(
									'Property "%s" is not public in class "%s". Maybe you should create the method "%s()"?',
									$property, $reflClass->getName(), $setter));
				}
				...
				throw new InvalidPropertyException(
						sprintf(
								'Neither element "%s" nor method "%s()" exists in class "%s"',
								$property, $setter, $reflClass->getName()));
			}
		} else {
			$objectOrArray[$property] = $value;
		}
