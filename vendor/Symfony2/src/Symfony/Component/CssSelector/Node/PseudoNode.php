...
class PseudoNode implements NodeInterface {
	static protected $unsupported = array('indeterminate', 'first-line',
			'first-letter', 'selection', 'before', 'after', 'link', 'visited',
			'active', 'focus', 'hover',);
	...
	public function __construct($element, $type, $ident) {
		...
		if (!in_array($type, array(':', '::'))) {
			throw new ParseException(
					sprintf(
							'The PseudoNode type can only be : or :: (%s given).',
							$type));
		}
	...
	public function __toString() {
		return sprintf('%s[%s%s%s]', __CLASS__, $this->element, $this->type,
				$this->ident);
	}
	...
	public function toXpath() {
		...
		if (in_array($this->ident, self::$unsupported)) {
			throw new ParseException(
					sprintf('The pseudo-class %s is unsupported', $this->ident));
		}
	...
	protected function xpath_checked($xpath) {
		...
		$xpath
				->addCondition(
						"(@selected or @checked) and (name(.) = 'input' or name(.) = 'option')");
