...
abstract class AbstractAuthenticationListener implements ListenerInterface {
	...
	public function __construct(SecurityContextInterface $securityContext,
			...
			HttpUtils $httpUtils, $providerKey, array $options = array(),
	 ...
	 */
	public function setRememberMeServices(
	...
	public final function handle(GetResponseEvent $event) {
		...
		try {
			...
			if ($returnValue instanceof TokenInterface) {
				...
				throw new \RuntimeException(
						'attemptAuthentication() must either return a Response, an implementation of TokenInterface, or null.');
			}
	...
	protected function requiresAuthentication(Request $request) {
		return $this->httpUtils
				->checkRequestPath($request, $this->options['check_path']);
	}
	...
	private function onFailure(GetResponseEvent $event, Request $request,
			AuthenticationException $failed) {
		if (null !== $this->logger) {
			$this->logger
					->info(
							sprintf('Authentication request failed: %s',
									$failed->getMessage()));
		}
		...
		if (null !== $this->failureHandler) {
			return $this->failureHandler
					->onAuthenticationFailure($request, $failed);
		}
		...
		if ($this->options['failure_forward']) {
			if (null !== $this->logger) {
				$this->logger
						->debug(
								sprintf('Forwarding to %s',
										$this->options['failure_path']));
			}

			$subRequest = $this->httpUtils
					->createRequest($request, $this->options['failure_path']);
			$subRequest->attributes
					->set(SecurityContextInterface::AUTHENTICATION_ERROR,
							$failed);

			return $event->getKernel()
		...
		if (null !== $this->logger) {
			$this->logger
					->debug(
							sprintf('Redirecting to %s',
									$this->options['failure_path']));
		}

		$request->getSession()
				->set(SecurityContextInterface::AUTHENTICATION_ERROR, $failed);
	...
	private function onSuccess(GetResponseEvent $event, Request $request,
	...
	private function determineTargetUrl(Request $request) {
		...
		if ($targetUrl = $request
				->get($this->options['target_path_parameter'], null, true)) {
			return $targetUrl;
		}
