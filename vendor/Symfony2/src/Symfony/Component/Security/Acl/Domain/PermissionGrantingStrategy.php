...
class PermissionGrantingStrategy implements PermissionGrantingStrategyInterface {
	 ...
	 * {@inheritDoc}
	 */
	public function isGranted(AclInterface $acl, array $masks, array $sids,
		...
		try {
			try {
				...
				return $this
						->hasSufficientPermissions($acl, $aces, $masks, $sids,
								$administrativeMode);
			...
			if ($acl->isEntriesInheriting()
					&& null !== $parentAcl = $acl->getParentAcl()) {
				return $parentAcl
						->isGranted($masks, $sids, $administrativeMode);
			}
	 ...
	 * {@inheritDoc}
	 */
	public function isFieldGranted(AclInterface $acl, $field, array $masks,
		...
		try {
			...
			if ($acl->isEntriesInheriting()
					&& null !== $parentAcl = $acl->getParentAcl()) {
				return $parentAcl
						->isFieldGranted($field, $masks, $sids,
								$administrativeMode);
			}
	 ...
	 */
	private function hasSufficientPermissions(AclInterface $acl, array $aces,
			array $masks, array $sids, $administrativeMode) {
