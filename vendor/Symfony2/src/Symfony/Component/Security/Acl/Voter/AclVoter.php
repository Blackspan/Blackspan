...
class AclVoter implements VoterInterface {
	...
	public function vote(TokenInterface $token, $object, array $attributes) {
		foreach ($attributes as $attribute) {
			if (null
					=== $masks = $this->permissionMap
							->getMasks($attribute, $object)) {
				continue;
			}
					...
					$this->logger
							->debug(
									sprintf(
											...
											$this
													->allowIfObjectIdentityUnavailable ? 'grant access'
													: 'abstain'));
				}

				return $this->allowIfObjectIdentityUnavailable ? self::ACCESS_GRANTED
						: self::ACCESS_ABSTAIN;
			...
			if ($object instanceof ObjectIdentityInterface) {
				$oid = $object;
					...
					=== $oid = $this->objectIdentityRetrievalStrategy
			...
			try {
				...
				if (null !== $this->logger) {
					$this->logger
							->debug(
									'ACL found, insufficient permissions. Voting to deny access.');
				}
				...
				if (null !== $this->logger) {
					$this->logger
							->debug(
									'ACL found, no ACE applicable. Voting to deny access.');
				}
