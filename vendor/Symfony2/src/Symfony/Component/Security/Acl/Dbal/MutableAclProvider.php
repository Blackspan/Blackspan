 ...
 */
class MutableAclProvider extends AclProvider implements
	...
	public function __construct(Connection $connection,
			...
			array $options, AclCacheInterface $cache = null) {
		parent::__construct($connection, $permissionGrantingStrategy, $options,
				$cache);
	...
	public function createAcl(ObjectIdentityInterface $oid) {
		if (false !== $this->retrieveObjectIdentityPrimaryKey($oid)) {
			throw new AclAlreadyExistsException(
					sprintf('%s is already associated with an ACL.', $oid));
		}
		...
		try {
			...
			$this->connection
					->executeQuery(
							$this->getInsertObjectIdentityRelationSql($pk, $pk));
	...
	public function findAcls(array $oids, array $sids = array()) {
		...
		foreach ($result as $oid) {
			...
			if (false === $this->propertyChanges->contains($acl)
					&& $acl instanceof MutableAclInterface) {
			...
			while (null !== $parentAcl) {
				if (false === $this->propertyChanges->contains($parentAcl)
						&& $acl instanceof MutableAclInterface) {
	 ...
	 * @return void
	 */
	public function propertyChanged($sender, $propertyName, $oldValue,
			$newValue) {
		if (!$sender instanceof MutableAclInterface
				&& !$sender instanceof EntryInterface) {
		...
		if (false === $this->propertyChanges->contains($sender)) {
			throw new \InvalidArgumentException(
					'$sender is not being tracked by this provider.');
		}
		...
		if (null === $ace) {
			if (isset($propertyChanges[$propertyName])) {
				...
				if ($oldValue === $newValue) {
					...
					$propertyChanges[$propertyName] = array($oldValue,
							$newValue);
				}
			} else {
			...
			$acePropertyChanges = $propertyChanges['aces']->contains($ace) ? $propertyChanges['aces']
							->offsetGet($ace) : array();
			...
			if (isset($acePropertyChanges[$propertyName])) {
				...
				if ($oldValue === $newValue) {
					...
					$acePropertyChanges[$propertyName] = array($oldValue,
							$newValue);
				}
			} else {
	...
	public function updateAcl(MutableAclInterface $acl) {
		if (!$this->propertyChanges->contains($acl)) {
			throw new \InvalidArgumentException(
					'$acl is not tracked by this provider.');
		}
		...
		try {
			...
			if (isset($propertyChanges['parentAcl'])) {
				if (null === $propertyChanges['parentAcl'][1]) {
					...
					$sets[] = 'parent_object_identity_id = '
							. intval($propertyChanges['parentAcl'][1]->getId());
				}
			...
			if (isset($propertyChanges['objectAces'])) {
				$this
						->updateAceProperty('objectAces',
								$propertyChanges['objectAces']);
			}
			if (isset($propertyChanges['objectFieldAces'])) {
				$this
						->updateFieldAceProperty('objectFieldAces',
			...
			if (count($sharedPropertyChanges) > 0) {
				$classAcesProperty = new \ReflectionProperty(
				...
				foreach ($this
						->loadedAcls[$acl->getObjectIdentity()->getType()] as $sameTypeAcl) {
					if (isset($sharedPropertyChanges['classAces'])) {
						if ($acl !== $sameTypeAcl
								&& $classAcesProperty->getValue($sameTypeAcl)
						...
						$classAcesProperty
								->setValue($sameTypeAcl,
										$sharedPropertyChanges['classAces'][1]);
					}

					if (isset($sharedPropertyChanges['classFieldAces'])) {
						if ($acl !== $sameTypeAcl
								&& $classFieldAcesProperty
										->getValue($sameTypeAcl)
						...
						$classFieldAcesProperty
								->setValue($sameTypeAcl,
										$sharedPropertyChanges['classFieldAces'][1]);
					}
				}
			}
		...
		if (null !== $this->cache) {
			if (count($sharedPropertyChanges) > 0) {
				...
				$this->cache
						->evictFromCacheByIdentity($acl->getObjectIdentity());
	...
	protected function getDeleteAccessControlEntriesSql($oidPK) {
		return sprintf('DELETE FROM %s WHERE object_identity_id = %d',
				$this->options['entry_table_name'], $oidPK);
	}
	...
	protected function getDeleteAccessControlEntrySql($acePK) {
		return sprintf('DELETE FROM %s WHERE id = %d',
				$this->options['entry_table_name'], $acePK);
	}
	...
	protected function getDeleteObjectIdentitySql($pk) {
		return sprintf('DELETE FROM %s WHERE id = %d',
				$this->options['oid_table_name'], $pk);
	}
	...
	protected function getDeleteObjectIdentityRelationsSql($pk) {
		return sprintf('DELETE FROM %s WHERE object_identity_id = %d',
				$this->options['oid_ancestors_table_name'], $pk);
	}
	 ...
	 * @return string
	 */
	protected function getInsertAccessControlEntrySql($classId,
			$objectIdentityId, $field, $aceOrder, $securityIdentityId,
			$strategy, $mask, $granting, $auditSuccess, $auditFailure) {
		$query = <<<QUERY
            INSERT INTO %s (
                class_id,
                object_identity_id,
...
QUERY;
		return sprintf($query, $this->options['entry_table_name'], $classId,
				null === $objectIdentityId ? 'NULL' : intval($objectIdentityId),
				null === $field ? 'NULL' : $this->connection->quote($field),
				$aceOrder, $securityIdentityId, $mask,
				$this->connection->getDatabasePlatform()
						->convertBooleans($granting),
				$this->connection->quote($strategy),
				$this->connection->getDatabasePlatform()
						->convertBooleans($auditSuccess),
				$this->connection->getDatabasePlatform()
	...
	protected function getInsertClassSql($classType) {
		return sprintf('INSERT INTO %s (class_type) VALUES (%s)',
	 ...
	 * @return string
	 */
	protected function getInsertObjectIdentityRelationSql($objectIdentityId,
			$ancestorId) {
		return sprintf(
				'INSERT INTO %s (object_identity_id, ancestor_id) VALUES (%d, %d)',
				$this->options['oid_ancestors_table_name'], $objectIdentityId,
				$ancestorId);
	}
	 ...
	 * @return string
	 */
	protected function getInsertObjectIdentitySql($identifier, $classId,
			$entriesInheriting) {
		$query = <<<QUERY
              INSERT INTO %s (class_id, object_identifier, entries_inheriting)
              VALUES (%d, %s, %s)
QUERY;
		return sprintf($query, $this->options['oid_table_name'], $classId,
				$this->connection->quote($identifier),
				$this->connection->getDatabasePlatform()
						->convertBooleans($entriesInheriting));
	}
	 ...
	 * @return string
	 */
	protected function getInsertSecurityIdentitySql(
	 ...
	 * @return string
	 */
	protected function getSelectAccessControlEntryIdSql($classId, $oid, $field,
			$order) {
		return sprintf(
				'SELECT id FROM %s WHERE class_id = %d AND %s AND %s AND ace_order = %d',
				$this->options['entry_table_name'], $classId,
				null === $oid ? $this->connection->getDatabasePlatform()
								->getIsNullExpression('object_identity_id')
						: 'object_identity_id = ' . intval($oid),
				null === $field ? $this->connection->getDatabasePlatform()
	...
	protected function getSelectClassIdSql($classType) {
		return sprintf('SELECT id FROM %s WHERE class_type = %s',
				$this->options['class_table_name'],
				$this->connection->quote($classType));
	}
	 ...
	 * @return string
	 */
	protected function getSelectSecurityIdentityIdSql(
			SecurityIdentityInterface $sid) {
		if ($sid instanceof UserSecurityIdentity) {
			$identifier = $sid->getClass() . '-' . $sid->getUsername();
			$username = true;
	...
	protected function getUpdateObjectIdentitySql($pk, array $changes) {
		...
		return sprintf('UPDATE %s SET %s WHERE id = %d',
				$this->options['oid_table_name'], implode(', ', $changes), $pk);
	}
	...
	protected function getUpdateAccessControlEntrySql($pk, array $sets) {
		...
		return sprintf('UPDATE %s SET %s WHERE id = %d',
				$this->options['entry_table_name'], implode(', ', $sets), $pk);
	}
				...
				->executeQuery(
						$this
								->getInsertObjectIdentitySql(
										$oid->getIdentifier(), $classId, true));
	}
	...
	private function createOrRetrieveClassId($classType) {
		if (false
				!== $id = $this->connection
						->executeQuery($this->getSelectClassIdSql($classType))
						->fetchColumn()) {
			return $id;
		}
	 ...
	 * @return integer
	 */
	private function createOrRetrieveSecurityIdentityId(
			SecurityIdentityInterface $sid) {
		if (false
				!== $id = $this->connection
						->executeQuery(
								$this->getSelectSecurityIdentityIdSql($sid))
						->fetchColumn()) {
			return $id;
		}

		$this->connection
				->executeQuery($this->getInsertSecurityIdentitySql($sid));

		return $this->connection
				->executeQuery($this->getSelectSecurityIdentityIdSql($sid))
				->fetchColumn();
	}
	...
	private function deleteAccessControlEntries($oidPK) {
		$this->connection
				->executeQuery($this->getDeleteAccessControlEntriesSql($oidPK));
	}
	...
	private function deleteObjectIdentityRelations($pk) {
		$this->connection
				->executeQuery($this->getDeleteObjectIdentityRelationsSql($pk));
	}
	...
	private function regenerateAncestorRelations(AclInterface $acl) {
		...
		while (null !== $parentAcl) {
			$this->connection
					->executeQuery(
							$this
									->getInsertObjectIdentityRelationSql($pk,
											$parentAcl->getId()));
			...
			for ($i = 0, $c = count($new); $i < $c; $i++) {
				...
				if (null === $ace->getId()) {
					...
					if ($classIds->contains($oid)) {
						...
						$classId = $this
								->createOrRetrieveClassId($oid->getType());
					}

					$objectIdentityId = $name === 'classFieldAces' ? null
							: $ace->getAcl()->getId();
					...
					$aceIdProperty = new \ReflectionProperty(
			...
			for ($i = 0, $c = count($old); $i < $c; $i++) {
				...
				if (!isset($currentIds[$ace->getId()])) {
					$this->connection
							->executeQuery(
									$this
											->getDeleteAccessControlEntrySql(
													$ace->getId()));
	...
	private function updateAceProperty($name, array $changes) {
		...
		for ($i = 0, $c = count($new); $i < $c; $i++) {
			...
			if (null === $ace->getId()) {
				...
				$objectIdentityId = $name === 'classAces' ? null
						: $ace->getAcl()->getId();
	...
	private function updateAces(\SplObjectStorage $aces) {
		foreach ($aces as $ace) {
			...
			if (isset($propertyChanges['strategy'])) {
				$sets[] = sprintf('granting_strategy = %s',
						$this->connection->quote($propertyChanges['strategy']));
			}
			if (isset($propertyChanges['aceOrder'])) {
				$sets[] = sprintf('ace_order = %d',
						$propertyChanges['aceOrder'][1]);
			}
			if (isset($propertyChanges['auditSuccess'])) {
				$sets[] = sprintf('audit_success = %s',
						$this->connection->getDatabasePlatform()
								->convertBooleans(
										$propertyChanges['auditSuccess'][1]));
			}
			if (isset($propertyChanges['auditFailure'])) {
				$sets[] = sprintf('audit_failure = %s',
						$this->connection->getDatabasePlatform()
								->convertBooleans(
										$propertyChanges['auditFailure'][1]));
			}
