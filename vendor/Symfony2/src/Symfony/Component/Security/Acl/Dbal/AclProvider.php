...
class AclProvider implements AclProviderInterface {
	...
	public function findChildren(ObjectIdentityInterface $parentOid,
		...
		foreach ($this->connection->executeQuery($sql)->fetchAll() as $data) {
			$children[] = new ObjectIdentity($data['object_identifier'],
					$data['class_type']);
		}
	...
	public function findAcls(array $oids, array $sids = array()) {
		...
		for ($i = 0, $c = count($oids); $i < $c; $i++) {
			...
			if (!$aclFound
					&& isset(
							$this
									->loadedAcls[$oid->getType()][$oid
											->getIdentifier()])) {
				$acl = $this
						->loadedAcls[$oid->getType()][$oid->getIdentifier()];
				...
				if (!$acl->isSidLoaded($sids)) {
					...
					throw new \RuntimeException(
							'This is not supported by the default implementation.');
				} else {
				...
				if (null !== $acl) {
					if ($acl->isSidLoaded($sids)) {
						...
						while (null !== $parentAcl) {
							...
							if (isset(
									$this
											->loadedAcls[$parentOid->getType()][$parentOid
													->getIdentifier()])) {
								$acl
										->setParentAcl(
												$this
														->loadedAcls[$parentOid
																->getType()][$parentOid
																->getIdentifier()]);
								break;
							} else {
			...
			if ((self::MAX_BATCH_SIZE === count($currentBatch)
					|| ($i + 1) === $c) && count($currentBatch) > 0) {
				$loadedBatch = $this
						->lookupObjectIdentities($currentBatch, $sids,
								$oidLookup);
				...
				foreach ($loadedBatch as $loadedOid) {
					...
					if (isset(
							$oidLookup[$loadedOid->getIdentifier()
									. $loadedOid->getType()])) {
			...
			if (!$result->contains($oid)) {
				if (1 === count($oids)) {
					throw new AclNotFoundException(
							sprintf('No ACL found for %s.', $oid));
				}
	...
	protected function getLookupSql(array $ancestorIds) {
		...
		$sql = <<<SELECTCLAUSE
            ...
            LEFT JOIN {$this->options['entry_table_name']} e ON (
                e.class_id = o.class_id AND (e.object_identity_id = o.id OR {$this
...
FINDCHILDREN;
	 ...
	 * @return string
	 */
	protected function getSelectObjectIdentityIdSql(
		...
		$query = <<<QUERY
            ...
            WHERE o.object_identifier = %s AND c.class_type = %s
QUERY;
		return sprintf($query, $this->options['oid_table_name'],
	 ...
	 * @return integer
	 */
	protected final function retrieveObjectIdentityPrimaryKey(
			ObjectIdentityInterface $oid) {
		return $this->connection
				->executeQuery($this->getSelectObjectIdentityIdSql($oid))
				->fetchColumn();
	}
	...
	private function updateAceIdentityMap(AclInterface $acl) {
		foreach (array('classAces', 'classFieldAces', 'objectAces',
				'objectFieldAces') as $property) {
	 ...
	 */
	private function lookupObjectIdentities(array $batch, array $sids,
			array $oidLookup) {
	 ...
	 */
	private function hydrateObjectIdentities(Statement $stmt, array $oidLookup,
			array $sids) {
		...
		$aclReflection = new \ReflectionClass(
		...
		$aclClassFieldAcesProperty = $aclReflection
				->getProperty('classFieldAces');
		...
		$aclObjectFieldAcesProperty = $aclReflection
				->getProperty('objectFieldAces');
		...
		foreach ($stmt->fetchAll(\PDO::FETCH_NUM) as $data) {
			...
			if (isset($acls[$aclId])) {
				...
				if (!isset($oidCache[$objectIdentifier . $classType])) {
					$oidCache[$objectIdentifier . $classType] = $acl
							->getObjectIdentity();
				}
				...
				if (!isset($oidCache[$oidLookupKey])) {
					$oidCache[$oidLookupKey] = new ObjectIdentity(
							$objectIdentifier, $classType);
				}

				$acl = new Acl((integer) $aclId, $oidCache[$oidLookupKey],
				...
				if (null !== $parentObjectIdentityId) {
					if (isset($acls[$parentObjectIdentityId])) {
						$aclParentAclProperty
								->setValue($acl, $acls[$parentObjectIdentityId]);
					} else {
			...
			if (null !== $aceId) {
				...
				if (!isset($aces[$aclId])) {
					$aces[$aclId] = array($emptyArray, $emptyArray,
				...
				if (!isset($loadedAces[$aceId])) {
					if (!isset(
							$sids[$key = ($username ? '1' : '0')
									. $securityIdentifier])) {
						if ($username) {
							$sids[$key] = new UserSecurityIdentity(
									substr($securityIdentifier,
											1
													+ $pos = strpos(
															$securityIdentifier,
															'-')),
							...
							$sids[$key] = new RoleSecurityIdentity(
									$securityIdentifier);
						}
					}

					if (null === $fieldName) {
						$loadedAces[$aceId] = new Entry((integer) $aceId, $acl,
								$sids[$key], $grantingStrategy,
								(integer) $mask, !!$granting, !!$auditFailure,
								!!$auditSuccess);
					} else {
						$loadedAces[$aceId] = new FieldEntry((integer) $aceId,
								$acl, $fieldName, $sids[$key],
								$grantingStrategy, (integer) $mask,
