 ...
 */
class XmlEncoder extends SerializerAwareEncoder implements EncoderInterface,
		DecoderInterface, NormalizationAwareInterface {
	private $dom;
	private $format;
	...
	private function buildXml($parentNode, $data) {
		...
		throw new UnexpectedValueException(
				'An unexpected value could not be serialized: '
	...
	private function selectNodeType($node, $val) {
		if (is_array($val)) {
			...
			return $this
					->buildXml($node,
							$this->serializer->normalize($val, $this->format));
