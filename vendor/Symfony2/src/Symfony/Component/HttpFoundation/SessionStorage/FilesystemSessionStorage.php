...
class FilesystemSessionStorage extends NativeSessionStorage {
	...
	public function start() {
		...
		session_set_cookie_params($this->options['lifetime'],
				$this->options['path'], $this->options['domain'],
				$this->options['secure'], $this->options['httponly']);

		if (!ini_get('session.use_cookies') && isset($this->options['id'])
				&& $this->options['id'] && $this->options['id'] != session_id()) {
		...
		$this->data = file_exists($file) ? unserialize(file_get_contents($file))
				: array();
		$this->started = true;
	}
	...
	public function write($key, $data) {
		...
		file_put_contents($this->path . '/' . session_id() . '.session',
				serialize($this->data));
	}
