...
class Request {
	...
	public function __construct(array $query = array(),
		...
		$this
				->initialize($query, $request, $attributes, $cookies, $files,
						$server, $content);
	}
	 ...
	 * @api
	 */
	public function initialize(array $query = array(), array $request = array(),
	...
	static public function createFromGlobals() {
		$request = new static($_GET, $_POST, array(), $_COOKIE, $_FILES,
				$_SERVER);

		if (0
				=== strpos($request->server->get('CONTENT_TYPE'),
	 ...
	 * @api
	 */
	static public function create($uri, $method = 'GET', $parameters = array(),
			$cookies = array(), $files = array(), $server = array(),
			$content = null) {
		$defaults = array('SERVER_NAME' => 'localhost',
				...
				'SCRIPT_FILENAME' => '', 'SERVER_PROTOCOL' => 'HTTP/1.1',
				'REQUEST_TIME' => time(),);
		...
		if (isset($components['port'])) {
			...
			$defaults['HTTP_HOST'] = $defaults['HTTP_HOST'] . ':'
					. $components['port'];
		}
		...
		$queryString = isset($components['query']) ? html_entity_decode(
						$components['query']) : '';
		...
		$server = array_replace($defaults, $server,
				array('REQUEST_METHOD' => strtoupper($method),
						'PATH_INFO' => '', 'REQUEST_URI' => $uri,
	...
	public function duplicate(array $query = null, array $request = null,
			array $attributes = null, array $cookies = null,
	...
	public function getClientIp($proxy = false) {
		if ($proxy) {
			...
			} elseif (self::$trustProxy
					&& $this->server->has('HTTP_X_FORWARDED_FOR')) {
				$clientIp = explode(',',
						$this->server->get('HTTP_X_FORWARDED_FOR'), 2);
	...
	public function getScriptName() {
		return $this->server
				->get('SCRIPT_NAME', $this->server->get('ORIG_SCRIPT_NAME', ''));
	}
	...
	public function isSecure() {
		return ((strtolower($this->server->get('HTTPS')) == 'on'
				...
				|| (self::$trustProxy
						&& strtolower($this->headers->get('X_FORWARDED_PROTO'))
								== 'https'));
	}
	...
	public function getHost() {
		if (self::$trustProxy
				&& $host = $this->headers->get('X_FORWARDED_HOST')) {
	...
	public function setLocale($locale) {
		if (!$this->hasSession()) {
			throw new \LogicException(
					'Forward compatibility for Request::setLocale() requires the session to be set.');
		}
	...
	public function getLocale() {
		if (!$this->hasSession()) {
			throw new \LogicException(
					'Forward compatibility for Request::getLocale() requires the session to be set.');
		}
	...
	public function getPreferredLanguage(array $locales = null) {
		...
		$preferredLanguages = array_values(
				array_intersect($preferredLanguages, $locales));
	...
	public function getLanguages() {
		...
		$languages = $this
				->splitHttpAcceptHeader($this->headers->get('Accept-Language'));
	...
	public function getAcceptableContentTypes() {
		...
		return $this->acceptableContentTypes = array_keys(
				$this->splitHttpAcceptHeader($this->headers->get('Accept')));
	}
	...
	protected function prepareBaseUrl() {
		...
		} elseif (basename($this->server->get('ORIG_SCRIPT_NAME'))
				=== $filename) {
	...
	protected function preparePathInfo() {
		...
		if ((null !== $baseUrl)
				&& (false
						=== ($pathInfo = substr(urldecode($requestUri),
