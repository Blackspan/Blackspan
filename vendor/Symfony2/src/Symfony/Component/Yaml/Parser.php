...
class Parser {
	...
	public function parse($value) {
		...
		if (function_exists('mb_detect_encoding')
				&& false === mb_detect_encoding($value, 'UTF-8', true)) {
			...
			if (preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+?))?\s*$#u',
				...
				if (!isset($values['value'])
					...
					if (isset($values['leadspaces'])
							&& ' ' == $values['leadspaces']
							&& preg_match(
									'#^(?P<key>' . Inline::REGEX_QUOTED_STRING
											. '|[^ \'"\{\[].*?) *\:(\s+(?P<value>.+?))?\s*$#u',
									$values['value'], $matches)) {
				...
				if ('<<' === $key) {
					if (isset($values['value'])
						...
						if (!array_key_exists($isInPlace, $this->refs)) {
							throw new ParseException(
									sprintf('Reference "%s" does not exist.',
											$isInPlace),
	...
	private function parseValue($value) {
		...
		if (preg_match(
			...
			return $this
					->parseFoldedScalar($matches['separator'],
	 ...
	 */
	private function parseFoldedScalar($separator, $indicator = '',
			$indentation = 0) {
		$separator = '|' == $separator ? "\n" : ' ';
		$text = '';
		...
		while ($this->currentLineNb + 1 < count($this->lines)) {
			...
			if (preg_match(
				...
				$text .= str_repeat(' ',
						$diff = strlen($matches['indent'])
								- strlen($textIndent)) . $matches['text']
						. ($diff ? "\n" : $separator);
			} elseif (preg_match('#^(?P<text> *)$#', $this->currentLine,
					$matches)) {
				$text .= preg_replace('#^ {1,' . strlen($textIndent) . '}#',
						'', $matches['text']) . "\n";
			} else {
				$this->moveToPreviousLine();
