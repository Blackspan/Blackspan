...
class EntityChoiceList extends ArrayChoiceList {
	...
	public function __construct(EntityManager $em, $class, $property = null,
			$queryBuilder = null, $choices = null) {
		// If a query builder was passed, it must be a closure or QueryBuilder
		// instance
		if (!(null === $queryBuilder || $queryBuilder instanceof QueryBuilder
				|| $queryBuilder instanceof \Closure)) {
			throw new UnexpectedTypeException($queryBuilder,
					'Doctrine\ORM\QueryBuilder or \Closure');
		}
		...
		$this->identifier = $em->getClassMetadata($class)
			...
			if ($this->propertyPath) {
				...
				if (!method_exists($entity, '__toString')) {
					throw new FormException(
							sprintf(
									'Entity "%s" passed to the choice field must have a "__toString()" method defined (or you can also override the "property" option).',
									$this->class));
				}
	...
	public function getEntity($key) {
		...
		try {
			if (count($this->identifier) > 1) {
				...
				$where = $qb->expr()
						->eq($alias . '.' . current($this->identifier), $key);
	...
	private function getReflProperty($property) {
		if (!isset($this->reflProperties[$property])) {
			$this->reflProperties[$property] = new \ReflectionProperty(
					$this->class, $property);
