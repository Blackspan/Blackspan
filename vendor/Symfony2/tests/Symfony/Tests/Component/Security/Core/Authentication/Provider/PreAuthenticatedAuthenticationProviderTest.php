...
class PreAuthenticatedAuthenticationProviderTest extends
	...
	public function testSupports() {
		...
		$token->expects($this->once())->method('getProviderKey')
	...
	public function testAuthenticate() {
		...
		$token = $provider
				->authenticate($this->getSupportedToken('fabien', 'pass'));
		$this
				->assertInstanceOf(
						'Symfony\Component\Security\Core\Authentication\Token\PreAuthenticatedToken',
						$token);
		...
		$this
				->assertEquals(array('foo' => 'bar'), $token->getAttributes(),
	...
	protected function getSupportedToken($user = false, $credentials = false) {
		...
		if (false !== $credentials) {
			$token->expects($this->once())->method('getCredentials')
					->will($this->returnValue($credentials));
		}

		$token->expects($this->any())->method('getProviderKey')
	...
	protected function getProvider($user = false, $userChecker = false) {
		...
		return new PreAuthenticatedAuthenticationProvider($userProvider,
				$userChecker, 'key');
	}
}
