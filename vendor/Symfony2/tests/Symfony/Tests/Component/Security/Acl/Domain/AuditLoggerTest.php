...
class AuditLoggerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testLogIfNeeded($granting, $audit) {
		...
		if (true === $granting) {
			$ace->expects($this->once())->method('isAuditSuccess')
					->will($this->returnValue($audit));

			$ace->expects($this->never())->method('isAuditFailure');
		} else {
			$ace->expects($this->never())->method('isAuditSuccess');

			$ace->expects($this->once())->method('isAuditFailure')
					->will($this->returnValue($audit));
		}

		if (true === $audit) {
			$logger->expects($this->once())->method('doLog')
					->with($this->equalTo($granting), $this->equalTo($ace));
		} else {
			$logger->expects($this->never())->method('doLog');
		}
	...
	public function getTestLogData() {
		return array(array(true, false), array(true, true),
