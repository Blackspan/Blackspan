...
class MutableAclProviderTest extends \PHPUnit_Framework_TestCase {
	...
	public static function assertAceEquals(EntryInterface $a, EntryInterface $b) {
		...
		self::assertTrue(
				$a->getSecurityIdentity()->equals($b->getSecurityIdentity()));
	...
	public function testCreateAcl() {
		...
		$this
				->assertInstanceOf(
						'Symfony\Component\Security\Acl\Domain\Acl', $acl);
	...
	public function testFindAclsAddsPropertyListenerToParentAcls() {
		...
		$this
				->importAcls($provider,
						array(
								'main' => array('object_identifier' => '1',
										'class_type' => 'foo',
										'parent_acl' => 'parent',),
								'parent' => array('object_identifier' => '1',
	...
	public function testPropertyChangedDoesNotTrackUnmanagedAcls() {
		...
		$acl = new Acl(1, new ObjectIdentity(1, 'foo'),
				new PermissionGrantingStrategy(), array(), false);
	...
	public function testPropertyChangedTracksChangesToAceProperties() {
		$provider = $this->getProvider();
		$acl = $provider->createAcl(new ObjectIdentity(1, 'Foo'));
		$ace = new Entry(1, $acl, new UserSecurityIdentity('foo', 'FooClass'),
				'all', 1, true, true, true);
		$ace2 = new Entry(2, $acl, new UserSecurityIdentity('foo', 'FooClass'),
				'all', 1, true, true, true);
		$propertyChanges = $this->getField($provider, 'propertyChanges');
	...
	public function testUpdateDoesNothingWhenThereAreNoChanges() {
		$con = $this
				->getMock('Doctrine\DBAL\Connection', array(), array(), '',
						false);
		$con->expects($this->never())->method('beginTransaction');
		$con->expects($this->never())->method('executeQuery');

		$provider = new MutableAclProvider($con,
				new PermissionGrantingStrategy(), array());
		$acl = new Acl(1, new ObjectIdentity(1, 'Foo'),
				new PermissionGrantingStrategy(), array(), true);
		$propertyChanges = $this->getField($provider, 'propertyChanges');
	...
	public function testUpdateAcl() {
		...
		$this
				->assertSame($acl->isEntriesInheriting(),
						$reloadedAcl->isEntriesInheriting());
	...
	public function testUpdateAclWorksForChangingTheParentAcl() {
		...
		$this->assertNotSame($acl, $reloadedAcl);
		$this
				->assertSame($parentAcl->getId(),
						$reloadedAcl->getParentAcl()->getId());
	}
	...
	public function testUpdateAclUpdatesChildAclsCorrectly() {
		...
		$newParentParentAcl = $provider
				->createAcl(new ObjectIdentity(2, 'Baz'));
		...
		$reloadedAcl = $reloadProvider->findAcl(new ObjectIdentity(1, 'Foo'));
		$this
				->assertEquals($newParentParentAcl->getId(),
						$reloadedAcl->getParentAcl()->getParentAcl()->getId());
	}
	...
	protected function importAcls(AclProvider $provider, array $data) {
		...
		try {
			foreach ($data as $name => $aclData) {
				if (!isset($aclData['object_identifier'],
						$aclData['class_type'])) {
					throw new \InvalidArgumentException(
							'"object_identifier", and "class_type" must be present.');
				}

				$this
						->callMethod($provider, 'createObjectIdentity',
								array(
										new ObjectIdentity(
												$aclData['object_identifier'],
												$aclData['class_type'])));
				...
				$sql = $this
						->callMethod($provider,
								'getInsertObjectIdentityRelationSql',
								array($aclId, $aclId));
				...
				if (isset($aclData['parent_acl'])) {
					if (isset($aclIds[$aclData['parent_acl']])) {
						$con
								->executeQuery(
										"UPDATE acl_object_identities SET parent_object_identity_id = "
												. $aclIds[$aclData['parent_acl']]
												. " WHERE id = " . $aclId);
						$con
								->executeQuery(
										$this
	...
	protected function setUp() {
		...
		if (!class_exists('PDO')
			...
			self::markTestSkipped(
					'This test requires SQLite support in your environment');
		}
	...
	protected function getOptions() {
		return array('oid_table_name' => 'acl_object_identities',
	...
	protected function getProvider($cache = null) {
		return new MutableAclProvider($this->con, $this->getStrategy(),
				$this->getOptions(), $cache);
	}
}
