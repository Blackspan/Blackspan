...
class AclVoterTest extends \PHPUnit_Framework_TestCase {
	...
	public function testSupportsAttribute($attribute, $supported) {
		...
		$permissionMap->expects($this->once())->method('contains')
	...
	public function getSupportsAttributeTests() {
		return array(array('foo', true), array('foo', false),);
	}
	...
	public function getSupportsClassTests() {
		return array(array('foo'), array('bar'), array('moo'),);
	}

	public function testVote() {
		list($voter, , $permissionMap, , ) = $this->getVoter();
		$permissionMap->expects($this->atLeastOnce())->method('getMasks')
				->will($this->returnValue(null));

		$this
				->assertSame(VoterInterface::ACCESS_ABSTAIN,
						$voter
								->vote($this->getToken(), null,
	 ...
	 */
	public function testVoteWhenNoObjectIsPassed(
			$allowIfObjectIdentityUnavailable) {
		list($voter, , $permissionMap, , ) = $this
				->getVoter($allowIfObjectIdentityUnavailable);
		$permissionMap->expects($this->once())->method('getMasks')
		...
		$this
				->assertSame($vote,
						$voter->vote($this->getToken(), null, array('VIEW')));
	}
	...
	public function testVoteWhenOidStrategyReturnsNull($allowIfUnavailable) {
		list($voter, , $permissionMap, $oidStrategy, ) = $this
				->getVoter($allowIfUnavailable);
		$permissionMap->expects($this->once())->method('getMasks')
				->will($this->returnValue(array()));

		$oidStrategy->expects($this->once())->method('getObjectIdentity')
				->will($this->returnValue(null));
		...
		$this
				->assertSame($vote,
						$voter
								->vote($this->getToken(), new \stdClass(),
										array('VIEW')));
	}
	...
	public function testVoteNoAclFound() {
		list($voter, $provider, $permissionMap, $oidStrategy, $sidStrategy) = $this
				->getVoter();
		...
		$sidStrategy->expects($this->once())->method('getSecurityIdentities')
				->will(
						$this
								->returnValue(
										$sids = array(
		...
		$provider->expects($this->once())->method('findAcl')
				->with($this->equalTo($oid), $this->equalTo($sids))
				->will(
						$this
								->throwException(
										new AclNotFoundException('Not found.')));
	...
	public function testVoteGrantsAccess($grant) {
		...
		$acl->expects($this->once())->method('isGranted')
				->with($this->identicalTo($masks), $this->equalTo($sids),
						$this->isFalse())->will($this->returnValue($grant));
	...
	public function testVoteGrantsFieldAccess($grant) {
		...
		$acl->expects($this->once())->method('isFieldGranted')
				->with($this->identicalTo('foo'), $this->identicalTo($masks),
						$this->equalTo($sids), $this->isFalse())
		...
		$this
				->assertSame($vote,
						$voter
								->vote($this->getToken(),
										new FieldVote(new \stdClass(), 'foo'),
										array('VIEW')));
	}
	...
	public function testWhenReceivingAnObjectIdentityInterfaceWeDontRetrieveANewObjectIdentity() {
		...
		$oidStrategy->expects($this->never())->method('getObjectIdentity');
	...
	protected function getVoter($allowIfObjectIdentityUnavailable = true) {
		...
		return array(
				new AclVoter($provider, $oidStrategy, $sidStrategy,
						$permissionMap, null,
						$allowIfObjectIdentityUnavailable), $provider,
				$permissionMap, $oidStrategy, $sidStrategy,);
	}
}
