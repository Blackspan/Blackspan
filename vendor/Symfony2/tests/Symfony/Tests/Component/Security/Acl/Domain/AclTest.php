...
class AclTest extends \PHPUnit_Framework_TestCase {
	public function testConstructor() {
		$acl = new Acl(1, $oid = new ObjectIdentity('foo', 'foo'),
				$permissionStrategy = new PermissionGrantingStrategy(),
				array(), true);
	...
	public function testDeleteAce($type) {
		...
		$listener = $this
				->getListener(
						array($type . 'Aces', 'aceOrder', 'aceOrder',
								$type . 'Aces',));
	...
	public function getDeleteAceTests() {
		return array(array('class'), array('object'),);
	}
	...
	public function testDeleteFieldAce($type) {
		...
		$acl
				->{'insert' . $type . 'Ace'}('foo',
						new RoleSecurityIdentity('foo'), 1, 0);
		$acl
				->{'insert' . $type . 'Ace'}('foo',
						new RoleSecurityIdentity('foo'), 2, 1);
		$acl
				->{'insert' . $type . 'Ace'}('foo',
						new RoleSecurityIdentity('foo'), 3, 2);
		...
		$this
				->assertEquals(2,
						count($aces = $acl->{'get' . $type . 'Aces'}('foo')));
		$this->assertEquals(2, $aces[0]->getMask());
		$this->assertEquals(3, $aces[1]->getMask());
	...
	public function getDeleteFieldAceTests() {
		return array(array('classField'), array('objectField'),);
	}
	...
	public function testInsertAce($property, $method) {
		...
		$listener = $this
				->getListener(
						array($property, 'aceOrder', $property, 'aceOrder',
								$property));
		$acl->addPropertyChangedListener($listener);
	 ...
	 * @dataProvider getInsertAceTests
	 */
	public function testInsertClassAceThrowsExceptionOnInvalidIndex($property,
			$method) {
		$acl = $this->getAcl();
	...
	public function getInsertAceTests() {
		return array(array('classAces', 'insertClassAce'),
	...
	public function testInsertClassFieldAce($property, $method) {
		...
		$listener = $this
				->getListener(
						array($property, $property, 'aceOrder', $property,
	 ...
	 * @dataProvider getInsertFieldAceTests
	 */
	public function testInsertClassFieldAceThrowsExceptionOnInvalidIndex(
			$property, $method) {
		$acl = $this->getAcl();
	...
	public function getInsertFieldAceTests() {
		return array(array('classFieldAces', 'insertClassFieldAce'),
				array('objectFieldAces', 'insertObjectFieldAce'),);
	}

	public function testIsFieldGranted() {
		$sids = array(new RoleSecurityIdentity('ROLE_FOO'),
				new RoleSecurityIdentity('ROLE_IDDQD'));
		$masks = array(1, 2, 4);
		$strategy = $this
				->getMock(
						'Symfony\Component\Security\Acl\Model\PermissionGrantingStrategyInterface');
		$acl = new Acl(1, new ObjectIdentity(1, 'foo'), $strategy, array(),
				true);

		$strategy->expects($this->once())->method('isFieldGranted')
				->with($this->equalTo($acl), $this->equalTo('foo'),
						$this->equalTo($masks), $this->equalTo($sids),
						$this->isTrue())->will($this->returnValue(true));
	...
	public function testIsGranted() {
		...
		$strategy->expects($this->once())->method('isGranted')
				->with($this->equalTo($acl), $this->equalTo($masks),
						$this->equalTo($sids), $this->isTrue())
	...
	public function testIsSidLoaded() {
		$acl = new Acl(1, new ObjectIdentity('1', 'foo'),
				new PermissionGrantingStrategy(),
				array(new UserSecurityIdentity('foo', 'Foo'),
						new UserSecurityIdentity('johannes', 'Bar')), true);
		...
		$this
				->assertFalse(
						$acl->isSidLoaded(new RoleSecurityIdentity('ROLE_FOO')));
		$this
				->assertFalse(
						$acl
								->isSidLoaded(
										new UserSecurityIdentity(
												'schmittjoh@gmail.com', 'Moo')));
		$this
				->assertFalse(
						$acl
								->isSidLoaded(
	 ...
	 * @expectedException \OutOfBoundsException
	 */
	public function testUpdateAceThrowsOutOfBoundsExceptionOnInvalidIndex(
			$type) {
		$acl = $this->getAcl();
		$acl->{'update' . $type}(0, 1);
	}
	...
	public function getUpdateAceTests() {
		return array(array('classAce'), array('objectAce'),);
	}
	...
	public function testUpdateFieldAce($type) {
		...
		$acl
				->{'insert' . $type}('foo',
						new UserSecurityIdentity('foo', 'Foo'), 1);
	...
	public function getUpdateFieldAceTests() {
		return array(array('classFieldAce'), array('objectFieldAce'),);
	}
	...
	public function testUpdateAuditing($type) {
		...
		$listener = $this
				->getListener(
						array('auditFailure', 'auditSuccess', 'auditFailure',));
		$acl->addPropertyChangedListener($listener);
	...
	public function testUpdateFieldAuditingThrowsExceptionOnInvalidIndex($type) {
		$acl = $this->getAcl();
		$acl->{'insert' . $type . 'Ace'}('foo',
						new RoleSecurityIdentity('foo'), 1);
	...
	public function testUpdateFieldAuditing($type) {
		...
		$listener = $this
				->getListener(
						array('auditSuccess', 'auditSuccess', 'auditFailure',));
		$acl->addPropertyChangedListener($listener);
	...
	protected function getListener($expectedChanges) {
		$aceProperties = array('aceOrder', 'mask', 'strategy', 'auditSuccess',
				'auditFailure');
		...
		foreach ($expectedChanges as $index => $property) {
			...
			$listener->expects($this->at($index))->method('propertyChanged')
	...
	protected function getAcl() {
		return new Acl(1, new ObjectIdentity(1, 'foo'),
				new PermissionGrantingStrategy(), array(), true);
	}
