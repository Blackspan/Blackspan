...
class ObjectIdentityTest extends \PHPUnit_Framework_TestCase {
	...
	public function testFromDomainObjectPrefersInterfaceOverGetId() {
		...
		$domainObject->expects($this->once())->method('getObjectIdentifier')
				->will($this->returnValue('getObjectIdentifier()'));
		$domainObject->expects($this->never())->method('getId')
	...
	public function getCompareData() {
		return array(
				array(new ObjectIdentity('123', 'foo'),
						new ObjectIdentity('123', 'foo'), true),
				array(new ObjectIdentity('123', 'foo'),
						new ObjectIdentity(123, 'foo'), true),
				array(new ObjectIdentity('1', 'foo'),
						new ObjectIdentity('2', 'foo'), false),
				array(new ObjectIdentity('1', 'bla'),
						new ObjectIdentity('1', 'blub'), false),);
	}
