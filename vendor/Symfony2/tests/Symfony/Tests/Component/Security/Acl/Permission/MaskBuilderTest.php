...
class MaskBuilderTest extends \PHPUnit_Framework_TestCase {
	...
	public function getInvalidConstructorData() {
		return array(array(234.463), array('asdgasdf'), array(array()),
				array(new \stdClass()),);
	}
	...
	public function testAddAndRemove() {
		...
		$builder->add('view')->add('eDiT')->add('ownEr');
		$mask = $builder->get();

		$this
				->assertEquals(MaskBuilder::MASK_VIEW,
						$mask & MaskBuilder::MASK_VIEW);
		$this
				->assertEquals(MaskBuilder::MASK_EDIT,
						$mask & MaskBuilder::MASK_EDIT);
		$this
				->assertEquals(MaskBuilder::MASK_OWNER,
	...
	public function testGetPattern() {
		...
		$builder->add('owner');
		$this
				->assertEquals(str_repeat('.', 24) . 'N......V',
						$builder->getPattern());

		$builder->add(1 << 10);
		$this
				->assertEquals(
						str_repeat('.', 21) . MaskBuilder::ON . '..N......V',
						$builder->getPattern());
	}
