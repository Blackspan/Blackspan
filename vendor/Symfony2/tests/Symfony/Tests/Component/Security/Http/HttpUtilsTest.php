...
class HttpUtilsTest extends \PHPUnit_Framework_TestCase {
	public function testCreateRedirectResponse() {
		...
		// absolute path
		$response = $utils
				->createRedirectResponse($this->getRequest(), '/foobar');
		$this->assertTrue($response->isRedirect('http://localhost/foobar'));
		$this->assertEquals(302, $response->getStatusCode());

		// absolute URL
		$response = $utils
				->createRedirectResponse($this->getRequest(),
						'http://symfony.com/');
		...
		$router->expects($this->any())->method('getContext')
				->will(
						$this
								->returnValue(
										$this
	...
	public function testCreateRequest() {
		...
		// absolute URL
		$subRequest = $utils
				->createRequest($this->getRequest(), 'http://symfony.com/');
	...
	public function testCheckRequestPath() {
		...
		$this
				->assertFalse(
						$utils->checkRequestPath($this->getRequest(), '/foo'));

		$router = $this->getMock('Symfony\Component\Routing\RouterInterface');
		$router->expects($this->any())->method('match')
