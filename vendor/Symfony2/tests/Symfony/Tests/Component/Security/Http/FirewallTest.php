...
class FirewallTest extends \PHPUnit_Framework_TestCase {
	public function testOnKernelRequestRegistersExceptionListener() {
		...
		$listener = $this
				->getMock(
						...
						array(), array(), '', false);
		$listener->expects($this->once())->method('register')
		...
		$map->expects($this->once())->method('getListeners')
	...
	public function testOnKernelRequestStopsWhenThereIsAResponse() {
		...
		$first->expects($this->once())->method('handle');

		$second = $this
				->getMock(
						'Symfony\Component\Security\Http\Firewall\ListenerInterface');
		$second->expects($this->never())->method('handle');
		...
		$event->expects($this->once())->method('hasResponse')
