...
class TokenBasedRememberMeServicesTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAutoLoginThrowsExceptionOnNonExistentUser() {
		...
		$request->cookies
				->set('foo',
						$this
								->getCookie('fooclass', 'foouser', time()
												+ 3600, 'foopass'));
	...
	public function testAutoLoginDoesNotAcceptCookieWithInvalidHash() {
		...
		$request->cookies
				->set('foo',
						base64_encode(
								'class:' . base64_encode('foouser')
										. ':123456789:fooHash'));

		$user = $this
				->getMock('Symfony\Component\Security\Core\User\UserInterface');
		$user->expects($this->once())->method('getPassword')
	...
	public function testAutoLoginDoesNotAcceptAnExpiredCookie() {
		...
		$request->cookies
				->set('foo',
						$this
								->getCookie('fooclass', 'foouser', time() - 1,
										'foopass'));
	...
	public function testLoginSuccess() {
		...
		$token->expects($this->atLeastOnce())->method('getUser')
				->will($this->returnValue($user));
	...
	protected function getService($userProvider = null, $options = array(),
		...
		$service = new TokenBasedRememberMeServices(array($userProvider),
				'fookey', 'fookey', $options, $logger);
