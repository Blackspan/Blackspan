...
class ContainerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testConstructor() {
		$sc = new Container();
		$this
				->assertSame($sc, $sc->get('service_container'),
	...
	public function testGetParameterBag() {
		$sc = new Container();
		$this
				->assertEquals(array(), $sc->getParameterBag()->all(),
	...
	public function testGetSetParameter() {
		...
		$this
				->assertEquals('foo', $sc->getParameter('bar'),
						'->setParameter() sets the value of a new parameter');

		$sc->setParameter('foo', 'baz');
		$this
				->assertEquals('baz', $sc->getParameter('foo'),
						'->setParameter() overrides previously set parameter');

		$sc->setParameter('Foo', 'baz1');
		$this
				->assertEquals('baz1', $sc->getParameter('foo'),
	...
	public function testGetServiceIds() {
		...
		$this
				->assertEquals(array('service_container', 'foo', 'bar'),
						$sc->getServiceIds(),
		...
		$this
				->assertEquals(
						array('scoped', 'scoped_foo', 'bar', 'foo_bar',
								'foo.baz', 'circular', 'throw_exception',
								'service_container'), $sc->getServiceIds(),
						'->getServiceIds() returns defined service ids by getXXXService() methods');
	}
	...
	public function testGet() {
		...
		$sc->set('foo', $foo = new \stdClass());
		$this
				->assertEquals($foo, $sc->get('foo'),
						'->get() returns the service for the given id');
		$this
				->assertEquals($sc->__bar, $sc->get('bar'),
						'->get() returns the service for the given id');
		$this
				->assertEquals($sc->__foo_bar, $sc->get('foo_bar'),
						'->get() returns the service if a get*Method() is defined');
		$this
				->assertEquals($sc->__foo_baz, $sc->get('foo.baz'),
						'->get() returns the service if a get*Method() is defined');

		$sc->set('bar', $bar = new \stdClass());
		$this
				->assertEquals($bar, $sc->get('bar'),
		...
		try {
			...
			$this
					->fail(
							'->get() throws a \InvalidArgumentException exception if the service is empty');
		} catch (\Exception $e) {
			$this
					->assertInstanceOf(
	...
	public function testGetCircularReference() {
		...
		try {
			...
			$this
					->fail(
							'->get() throws a ServiceCircularReferenceException if it contains circular reference');
		} catch (\Exception $e) {
			$this
					->assertInstanceOf(
			...
			$this
					->assertStringStartsWith(
							'Circular reference detected for service "circular"',
							$e->getMessage(),
	...
	public function testLeaveScopeNotActive() {
		...
		try {
			...
			$this
					->fail(
							'->leaveScope() throws a \LogicException if the scope is not active yet');
		} catch (\Exception $e) {
			$this
					->assertInstanceOf('\LogicException', $e,
							'->leaveScope() throws a \LogicException if the scope is not active yet');
			$this
					->assertEquals('The scope "foo" is not active.',
							$e->getMessage(),
							'->leaveScope() throws a \LogicException if the scope is not active yet');
		}
		...
		try {
			...
			$this
					->fail(
							'->leaveScope() throws a \LogicException if the scope does not exist');
		} catch (\Exception $e) {
			$this
					->assertInstanceOf('\LogicException', $e,
							'->leaveScope() throws a \LogicException if the scope does not exist');
			$this
					->assertEquals('The scope "bar" is not active.',
							$e->getMessage(),
							'->leaveScope() throws a \LogicException if the scope does not exist');
		}
	}
	...
	public function testAddScope() {
		...
		$this
				->assertSame(array('foo' => 'container', 'bar' => 'foo'),
						$this->getField($c, 'scopes'));
		$this
				->assertSame(array('foo' => array('bar'), 'bar' => array()),
						$this->getField($c, 'scopeChildren'));
	}
	...
	public function testGetThrowsException() {
		...
		try {
			...
			$this
					->assertEquals('Something went terribly wrong!',
							$e->getMessage());
		}
	...
	public function getInvalidParentScopes() {
		return array(array(ContainerInterface::SCOPE_PROTOTYPE), array('bar'),);
	}

	public function getBuiltInScopes() {
		return array(array(ContainerInterface::SCOPE_CONTAINER),
...
class ProjectServiceContainer extends Container {
	...
	protected function getScopedService() {
		...
		return $this->services['scoped'] = $this
				->scopedServices['foo']['scoped'] = new \stdClass();
	}
	...
	protected function getScopedFooService() {
		...
		return $this->services['scoped_foo'] = $this
				->scopedServices['foo']['scoped_foo'] = new \stdClass();
	}
