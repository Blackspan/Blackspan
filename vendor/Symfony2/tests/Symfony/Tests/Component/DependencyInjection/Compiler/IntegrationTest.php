...
class IntegrationTest extends \PHPUnit_Framework_TestCase {
	...
	public function testProcessRemovesAndInlinesRecursively() {
		...
		$a = $container->register('a', '\stdClass')
				->addArgument(new Reference('c'));

		$b = $container->register('b', '\stdClass')
				->addArgument(new Reference('c'))->setPublic(false);
	...
	public function testProcessInlinesWhenThereAreMultipleReferencesButFromTheSameDefinition() {
		...
		$container->register('b', '\stdClass')->addArgument(new Reference('c'))
				->setPublic(false);
		...
		$this->assertFalse($container->hasDefinition('b'));
		$this
				->assertFalse($container->hasDefinition('c'),
						'Service C was not inlined.');
	}
}
