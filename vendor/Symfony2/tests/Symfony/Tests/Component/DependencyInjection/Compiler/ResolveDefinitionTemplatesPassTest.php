...
class ResolveDefinitionTemplatesPassTest extends \PHPUnit_Framework_TestCase {
	public function testProcess() {
		...
		$container->setDefinition('child', new DefinitionDecorator('parent'))
				->replaceArgument(0, 'a')->setProperty('foo', 'bar')
				->setClass('bar');
	...
	public function testProcessAppendsMethodCallsAlways() {
		...
		$container->register('parent')->addMethodCall('foo', array('bar'));
	...
	public function testProcessDoesNotCopyScope() {
		...
		$container->register('parent')->setScope('foo');
		...
		$def = $container->getDefinition('child');
		$this
				->assertEquals(ContainerInterface::SCOPE_CONTAINER,
						$def->getScope());
	}
	...
	public function testProcessHandlesMultipleInheritance() {
		...
		$container->register('parent', 'foo')
				->setArguments(array('foo', 'bar', 'c'));

		$container->setDefinition('child2', new DefinitionDecorator('child1'))
				->replaceArgument(1, 'b');
