...
class HeaderBagTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAll() {
		$bag = new HeaderBag(array('foo' => 'bar'));
		$this
				->assertEquals(array('foo' => array('bar')), $bag->all(),
	...
	public function testReplace() {
		...
		$this
				->assertEquals(array('nope' => array('BAR')), $bag->all(),
	...
	public function testGet() {
		...
		$this
				->assertEquals(array('bar'), $bag->get('foo', 'nope', false),
						'->get return the value as array');

		// defaults
		$this
				->assertNull($bag->get('none'),
						'->get unknown values returns null');
		$this
				->assertEquals('default', $bag->get('none', 'default'),
						'->get unknown values returns default');
		$this
				->assertEquals(array('default'),
	...
	public function testContains() {
		$bag = new HeaderBag(array('foo' => 'bar', 'fuzz' => 'bizz'));
		$this
				->assertTrue($bag->contains('foo', 'bar'),
						'->contains first value');
		$this
				->assertTrue($bag->contains('fuzz', 'bizz'),
						'->contains second value');
		$this
				->assertFalse($bag->contains('nope', 'nope'),
						'->contains unknown value');
		$this
				->assertFalse($bag->contains('foo', 'nope'),
						'->contains unknown value');
		...
		$this
				->assertTrue($bag->contains('foo', 'bar'),
						'->contains first value');
		$this
				->assertTrue($bag->contains('foo', 'bor'),
						'->contains second value');
		$this
				->assertFalse($bag->contains('foo', 'nope'),
						'->contains unknown value');
	...
	public function testCacheControlDirectiveParsing() {
		...
		$this
				->assertEquals('max-age=10, public, s-maxage=100',
