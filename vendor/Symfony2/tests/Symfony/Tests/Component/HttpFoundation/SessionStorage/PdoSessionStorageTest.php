...
class PdoSessionStorageTest extends \PHPUnit_Framework_TestCase {
	...
	public function testMultipleInstances() {
		$storage1 = new PdoSessionStorage($this->pdo, array(),
				array('db_table' => 'sessions'));
		$storage1->sessionWrite('foo', 'bar');

		$storage2 = new PdoSessionStorage($this->pdo, array(),
				array('db_table' => 'sessions'));
		$this
				->assertEquals('bar', $storage2->sessionRead('foo'),
						'values persist between instances');
	}

	public function testSessionDestroy() {
		$storage = new PdoSessionStorage($this->pdo, array(),
				array('db_table' => 'sessions'));
		$storage->sessionWrite('foo', 'bar');
		$this
				->assertEquals(1,
						count(
								$this->pdo->query('SELECT * FROM sessions')
										->fetchAll()));
