...
class ResponseHeaderBagTest extends \PHPUnit_Framework_TestCase {
	public function testCacheControlHeader() {
		...
		$this
				->assertEquals('private, must-revalidate',
		...
		$bag = new ResponseHeaderBag(
				array('Expires' => 'Wed, 16 Feb 2011 14:17:43 GMT'));
		$this
				->assertEquals('private, must-revalidate',
						$bag->get('Cache-Control'));
		...
		$this
				->assertEquals('max-age=3600, private',
						$bag->get('Cache-Control'));
		...
		$bag = new ResponseHeaderBag(
				array('Etag' => 'abcde', 'Last-Modified' => 'abcde'));
		$this
				->assertEquals('private, must-revalidate',
						$bag->get('Cache-Control'));
		...
		$bag = new ResponseHeaderBag(
				array('cache-control' => 'public, max-age=100'));
	...
	public function testToStringIncludesCookieHeaders() {
		...
		$this
				->assertContains("Set-Cookie: foo=bar; httponly",
						explode("\r\n", $bag->__toString()));
		...
		$this
				->assertContains(
						"Set-Cookie: foo=deleted; expires="
	...
	public function testCookiesWithSameNames() {
		...
		$this
				->assertContains(
						"Set-Cookie: foo=bar; path=/path/foo; domain=foo.bar; httponly",
						$headers);
		$this
				->assertContains(
						"Set-Cookie: foo=bar; path=/path/foo; domain=foo.bar; httponly",
		...
		$this
				->assertContains(
						"Set-Cookie: foo=bar; path=/path/bar; domain=bar.foo; httponly",
						$headers);
