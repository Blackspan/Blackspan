...
class ResponseTest extends \PHPUnit_Framework_TestCase {
	public function testIsValidateable() {
		$response = new Response('', 200,
				array(
						'Last-Modified' => $this->createDateTimeOneHourAgo()
								->format(DATE_RFC2822)));
		$this
				->assertTrue($response->isValidateable(),
		...
		$this
				->assertFalse($response->isValidateable(),
	...
	public function testGetDate() {
		$response = new Response('', 200,
				array(
						'Date' => $this->createDateTimeOneHourAgo()
								->format(DATE_RFC2822)));
		$this
				->assertEquals(0,
						$this->createDateTimeOneHourAgo()
								->diff($response->getDate())->format('%s'),
		...
		$this
				->assertLessThan(1,
						$date->diff(new \DateTime(), true)->format('%s'),
		...
		$this
				->assertEquals(0,
						$now->diff($response->getDate())->format('%s'),
	...
	public function testGetMaxAge() {
		...
		$response->headers
				->set('Expires',
						$this->createDateTimeOneHourLater()
								->format(DATE_RFC2822));
		$this
				->assertEquals(3600, $response->getMaxAge(),
	...
	public function testExpire() {
		...
		$response->expire();
		$this
				->assertEquals(100, $response->headers->get('Age'),
		...
		$response->expire();
		$this
				->assertEquals(500, $response->headers->get('Age'),
		...
		$response->expire();
		$this
				->assertEquals(1000, $response->headers->get('Age'),
						'->expire() does nothing when the response is already stale/expired');

		$response = new Response();
		$response->expire();
		$this
				->assertFalse($response->headers->has('Age'),
	...
	public function testGetTtl() {
		...
		$response->headers
				->set('Expires',
						$this->createDateTimeOneHourLater()
								->format(DATE_RFC2822));
		$this
				->assertLessThan(1, 3600 - $response->getTtl(),
		...
		$this
				->assertLessThan(0, $response->getTtl(),
						'->getTtl() returns negative values when Expires is in part');

		$response = new Response();
		$response->headers->set('Cache-Control', 'max-age=60');
		$this
				->assertLessThan(1, 60 - $response->getTtl(),
	...
	public function testGetVary() {
		$response = new Response();
		$this
				->assertEquals(array(), $response->getVary(),
						'->getVary() returns an empty array if no Vary header is present');

		$response = new Response();
		$response->headers->set('Vary', 'Accept-Language');
		$this
				->assertEquals(array('Accept-Language'), $response->getVary(),
		...
		$this
				->assertEquals(
						array('Accept-Language', 'User-Agent', 'X-Foo'),
						$response->getVary(),
	...
	public function testSetVary() {
		...
		$this
				->assertEquals(array('Accept-Language', 'User-Agent'),
						$response->getVary(),
		...
		$this
				->assertEquals(array('Accept-Language', 'User-Agent'),
						$response->getVary(),
						'->setVary() doesn\'t change the Vary header if replace is set to false');
	}
	...
	public function testDefaultContentType() {
		$headerMock = $this
				->getMock(
						...
						array('set'));
		$headerMock->expects($this->at(0))->method('set')
				->with('Content-Type', 'text/html; charset=UTF-8');
		$headerMock->expects($this->at(1))->method('set')
	...
	public function testSetCache() {
		...
		$response->setCache($options);
		$this
				->assertEquals($response->getLastModified()->getTimestamp(),
						$now->getTimestamp());
	...
	public function testSetExpires() {
		...
		$this
				->assertEquals($response->getExpires()->getTimestamp(),
						$now->getTimestamp());
	}
	...
	public function validContentProvider() {
		return array('obj' => array(new StringableObject),
				'string' => array('Foo'), 'int' => array(2),);
	}

	public function invalidContentProvider() {
		return array('obj' => array(new \stdClass), 'array' => array(array()),
				'bool' => array(true, '1'),);
	}
