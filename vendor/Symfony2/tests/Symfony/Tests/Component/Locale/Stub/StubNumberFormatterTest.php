...
class StubNumberFormatterTest extends LocaleTestCase {
	...
	public function testConstructorWithUnsupportedStyle() {
		$formatter = new StubNumberFormatter('en',
				StubNumberFormatter::PATTERN_DECIMAL);
	}
	...
	public function testSetAttributeInvalidRoundingMode() {
		$formatter = $this->getStubFormatterWithDecimalStyle();
		$ret = $formatter
				->setAttribute(StubNumberFormatter::ROUNDING_MODE, null);
	}
	...
	public function testCreateIntl() {
		...
		$this
				->assertInstanceOf('\NumberFormatter',
						\NumberFormatter::create('en',
	 ...
	 */
	public function testFormatCurrencyWithDecimalStyleStub($value, $currency,
			$expected) {
		$formatter = $this->getStubFormatterWithDecimalStyle();
		$this
	 ...
	 * @dataProvider formatCurrencyWithDecimalStyleProvider
	 */
	public function testFormatCurrencyWithDecimalStyleIntl($value, $currency,
			$expected) {
		$this->skipIfIntlExtensionIsNotLoaded();
	...
	public function formatCurrencyWithDecimalStyleProvider() {
		return array(array(100, 'ALL', '100'), array(100, 'BRL', '100.00'),
				array(100, 'CRC', '100'), array(100, 'JPY', '100'),
				array(100, 'CHF', '100'), array(-100, 'ALL', '-100'),
				array(-100, 'BRL', '-100'), array(-100, 'CRC', '-100'),
				array(-100, 'JPY', '-100'), array(-100, 'CHF', '-100'),
	 ...
	 */
	public function testFormatCurrencyWithCurrencyStyleStub($value, $currency,
			$expected) {
	 ...
	 * @dataProvider formatCurrencyWithCurrencyStyleProvider
	 */
	public function testFormatCurrencyWithCurrencyStyleIntl($value, $currency,
			$expected) {
		$this->skipIfIntlExtensionIsNotLoaded();
	...
	public function formatCurrencyWithCurrencyStyleProvider() {
		return array(array(100, 'ALL', 'ALL100'),
				...
				array(-100, 'BRL', '(R$100.00)'),
				array(1000.12, 'BRL', 'R$1,000.12'), array(100, 'CRC', '₡100'),
				array(-100, 'CRC', '(₡100)'), array(1000.12, 'CRC', '₡1,000'),
				array(100, 'JPY', '¥100'), array(-100, 'JPY', '(¥100)'),
				array(1000.12, 'JPY', '¥1,000'), array(100, 'EUR', '€100.00'),
	 ...
	 */
	public function testFormatCurrencyWithCurrencyStyleSwissRoundingStub(
			$value, $currency, $symbol, $expected) {
		$formatter = $this->getStubFormatterWithCurrencyStyle();
		$this
				->assertEquals(sprintf($expected, 'CHF'),
						$formatter->formatCurrency($value, $currency));
	}
	 ...
	 * @dataProvider formatCurrencyWithCurrencyStyleSwissRoundingProvider
	 */
	public function testFormatCurrencyWithCurrencyStyleSwissRoundingIntl(
			$value, $currency, $symbol, $expected) {
		$this->skipIfIntlExtensionIsNotLoaded();
		$this->skipIfICUVersionIsTooOld();
		$formatter = $this->getIntlFormatterWithCurrencyStyle();
		$this
				->assertEquals(sprintf($expected, $symbol),
						$formatter->formatCurrency($value, $currency));
	}
	...
	public function formatCurrencyWithCurrencyStyleSwissRoundingProvider() {
		...
		$chf = $this->isIntlExtensionLoaded()
				&& $this->isLowerThanIcuVersion('4.4') ? 'Fr.' : 'CHF';

		return array(array(100, 'CHF', $chf, '%s100.00'),
	 ...
	 */
	public function testFormatTypeInt32Intl($formatter, $value, $expected,
			$message = '') {
		$this->skipIfIntlExtensionIsNotLoaded();
		$formattedValue = $formatter
				->format($value, \NumberFormatter::TYPE_INT32);
	...
	public function testFormatTypeInt64Intl($formatter, $value, $expected) {
		$this->skipIfIntlExtensionIsNotLoaded();
		$formattedValue = $formatter
				->format($value, \NumberFormatter::TYPE_INT64);
	...
	public function testFormatTypeDoubleIntl($formatter, $value, $expected) {
		$this->skipIfIntlExtensionIsNotLoaded();
		$formattedValue = $formatter
				->format($value, \NumberFormatter::TYPE_DOUBLE);
		$this->assertEquals($expected, $formattedValue);
	}
	...
	public function formatTypeDoubleProvider() {
		...
		return array(array($df, 1, '1'), array($df, 1.1, '1.1'),
				array($cf, 1, 'SFD1.00'), array($cf, 1.1, 'SFD1.10'),);
	}
	...
	public function testFormatTypeCurrencyReturnStub() {
		...
		$this
				->assertFalse(
						@$formatter
								->format(1, StubNumberFormatter::TYPE_CURRENCY));
	}
	...
	public function testFormatTypeCurrencyIntl($formatter, $value) {
		$this->skipIfIntlExtensionIsNotLoaded();
		$formattedValue = $formatter
				->format($value, \NumberFormatter::TYPE_CURRENCY);
	}
	...
	public function formatTypeCurrencyProvider() {
		...
		return array(array($df, 1), array($df, 1),);
	}

	/**
	 * @dataProvider formatFractionDigitsProvider
	 */
	public function testFormatFractionDigitsStub($value, $expected,
			$fractionDigits = null, $expectedFractionDigits = 1) {
		$formatter = $this->getStubFormatterWithDecimalStyle();

		if (null !== $fractionDigits) {
			$attributeRet = $formatter
					->setAttribute(StubNumberFormatter::FRACTION_DIGITS,
							$fractionDigits);
		}
		...
		$this
				->assertSame($expectedFractionDigits,
						$formatter
								->getAttribute(
										StubNumberFormatter::FRACTION_DIGITS));
	 ...
	 * @dataProvider formatFractionDigitsProvider
	 */
	public function testFormatFractionDigitsIntl($value, $expected,
			$fractionDigits = null, $expectedFractionDigits = 1) {
		$this->skipIfIntlExtensionIsNotLoaded();
		$formatter = $this->getIntlFormatterWithDecimalStyle();
	...
	public function formatFractionDigitsProvider() {
		return array(array(1.123, '1.123', null, 0), array(1.123, '1', 0, 0),
				array(1.123, '1.1', 1, 1), array(1.123, '1.12', 2, 2),
				array(1.123, '1', -1, 0), array(1.123, '1', 'abc', 0));
	}

	/**
	 * @dataProvider formatGroupingUsedProvider
	 */
	public function testFormatGroupingUsedStub($value, $expected,
			$groupingUsed = null, $expectedGroupingUsed = 1) {
		$formatter = $this->getStubFormatterWithDecimalStyle();

		if (null !== $groupingUsed) {
			$attributeRet = $formatter
					->setAttribute(StubNumberFormatter::GROUPING_USED,
							$groupingUsed);
		}
		...
		$this->assertSame($expected, $formattedValue);
		$this
				->assertSame($expectedGroupingUsed,
						$formatter
								->getAttribute(
										StubNumberFormatter::GROUPING_USED));
	 ...
	 * @dataProvider formatGroupingUsedProvider
	 */
	public function testFormatGroupingUsedIntl($value, $expected,
			$groupingUsed = null, $expectedGroupingUsed = 1) {
		$this->skipIfIntlExtensionIsNotLoaded();
		$formatter = $this->getIntlFormatterWithDecimalStyle();
		...
		if (null !== $groupingUsed) {
			$attributeRet = $formatter
					->setAttribute(\NumberFormatter::GROUPING_USED,
							$groupingUsed);
		}
	...
	public function formatGroupingUsedProvider() {
		return array(array(1000, '1,000', null, 1), array(1000, '1000', 0, 0),
				array(1000, '1,000', 1, 1), array(1000, '1,000', 2, 1),
				array(1000, '1000', 'abc', 0), array(1000, '1,000', -1, 1),);
	}
	...
	public function testFormatRoundingModeStubRoundHalfUp($value, $expected) {
		...
		$formatter
				->setAttribute(StubNumberFormatter::ROUNDING_MODE,
						StubNumberFormatter::ROUND_HALFUP);
		$this
				->assertSame($expected, $formatter->format($value),
						'->format() with ROUND_HALFUP rounding mode.');
	}
	...
	public function formatRoundingModeRoundHalfUpProvider() {
		...
		return array(array(1.121, '1.12'), array(1.123, '1.12'),
				// array(1.125, '1.13'),
				array(1.127, '1.13'), array(1.129, '1.13'),);
	}
	...
	public function testFormatRoundingModeStubRoundHalfDown($value, $expected) {
		...
		$formatter
				->setAttribute(StubNumberFormatter::ROUNDING_MODE,
						StubNumberFormatter::ROUND_HALFDOWN);
		$this
				->assertSame($expected, $formatter->format($value),
						'->format() with ROUND_HALFDOWN rounding mode.');
	}
	...
	public function formatRoundingModeRoundHalfDownProvider() {
		return array(array(1.121, '1.12'), array(1.123, '1.12'),
				array(1.125, '1.12'), array(1.127, '1.13'),
				array(1.129, '1.13'),);
	}
	...
	public function testFormatRoundingModeStubRoundHalfEven($value, $expected) {
		...
		$formatter
				->setAttribute(StubNumberFormatter::ROUNDING_MODE,
						StubNumberFormatter::ROUND_HALFEVEN);
		$this
				->assertSame($expected, $formatter->format($value),
						'->format() with ROUND_HALFEVEN rounding mode.');
	}
	...
	public function testParseStub($value, $expected, $message = '') {
		...
		$parsedValue = $formatter
				->parse($value, StubNumberFormatter::TYPE_DOUBLE);
		...
		$this->assertSame($errorCode, StubIntl::getErrorCode());
		$this
				->assertSame($errorCode !== 0,
						StubIntl::isFailure(StubIntl::getErrorCode()));
		$this->assertSame($errorMessage, $formatter->getErrorMessage());
		$this->assertSame($errorCode, $formatter->getErrorCode());
		$this
				->assertSame($errorCode !== 0,
						StubIntl::isFailure($formatter->getErrorCode()));
	}
	...
	public function parseProvider() {
		return array(
				array('prefix1', false,
						'->parse() does not parse a number with a string prefix.'),
				array('1suffix', (float) 1,
						'->parse() parses a number with a string suffix.'),);
	}
	...
	public function testParseTypeInt32Stub($value, $expected, $message = '') {
		$formatter = $this->getStubFormatterWithDecimalStyle();
		$parsedValue = $formatter
				->parse($value, StubNumberFormatter::TYPE_INT32);
	...
	public function parseTypeInt32Provider() {
		return array(array('1', 1), array('1.1', 1),
				array('2,147,483,647', 2147483647),
				array('-2,147,483,648', -2147483647 - 1),
				array('2,147,483,648', false,
						'->parse() TYPE_INT32 returns false when the number is greater than the integer positive range.'),
				array('-2,147,483,649', false,
						'->parse() TYPE_INT32 returns false when the number is greater than the integer negative range.'));
	}
	...
	public function testParseTypeInt64StubWith32BitIntegerInPhp32Bit() {
		...
		$parsedValue = $formatter
				->parse('2,147,483,647', \NumberFormatter::TYPE_INT64);
		...
		$parsedValue = $formatter
				->parse('-2,147,483,648', \NumberFormatter::TYPE_INT64);
	...
	public function testParseTypeInt64StubWith64BitIntegerInPhp32Bit() {
		...
		$this->assertInternalType('float', $parsedValue);
		$this
				->assertEquals(2147483648, $parsedValue,
						'->parse() TYPE_INT64 does not use true 64 bit integers, using only the 32 bit range.');

		$parsedValue = $formatter
				->parse('-2,147,483,649', \NumberFormatter::TYPE_INT64);
		$this->assertInternalType('float', $parsedValue);
		$this
				->assertEquals(-2147483649, $parsedValue,
						'->parse() TYPE_INT64 does not use true 64 bit integers, using only the 32 bit range.');
	}
	...
	public function testParseTypeInt64StubWith64BitIntegerInPhp64Bit() {
		...
		$this->assertInternalType('integer', $parsedValue);
		$this
				->assertEquals(-2147483647 - 1, $parsedValue,
						'->parse() TYPE_INT64 does not use true 64 bit integers, using only the 32 bit range.');
		...
		$this->assertInternalType('integer', $parsedValue);
		$this
				->assertEquals(2147483647, $parsedValue,
						'->parse() TYPE_INT64 does not use true 64 bit integers, using only the 32 bit range.');
	}
	...
	public function parseTypeDoubleProvider() {
		return array(array('1', (float) 1), array('1.1', 1.1),
	...
	public function testParseWithNullPositionValueIntl() {
		...
		$formatter = $this->getIntlFormatterWithDecimalStyle();
		$parsedValue = $formatter
				->parse('123', \NumberFormatter::TYPE_DOUBLE, $position);
