...
class CheckboxTypeTest extends TypeTestCase {
	...
	public function testTransformedData($data, $expected) {
		...
		$form = $this->builder->create('expedited_shipping', 'checkbox')
				->prependClientTransformer($transformer)->getForm();
		$form->setData($data);
		$view = $form->createView();
	...
	public function proviceTransformedData() {
		return array(array('expedited', true), array('standard', false),);
	}
}
