...
class EnsureCsrfFieldListenerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->form = $this
				->getMock('Symfony\\Tests\\Component\\Form\\FormInterface');
		$this->field = $this
				->getMock('Symfony\\Tests\\Component\\Form\\FormInterface');
	...
	public function testAddField() {
		$this->formFactory->expects($this->once())->method('createNamed')
				->with('csrf', '_token', null, array())
				->will($this->returnValue($this->field));
		$this->form->expects($this->once())->method('add')
				->with(
						$this
								->isInstanceOf(
	...
	public function testIntention() {
		$this->formFactory->expects($this->once())->method('createNamed')
				->with('csrf', '_token', null,
						array('intention' => 'something'))
				->will($this->returnValue($this->field));
		$this->form->expects($this->once())->method('add')
				->with(
						$this
