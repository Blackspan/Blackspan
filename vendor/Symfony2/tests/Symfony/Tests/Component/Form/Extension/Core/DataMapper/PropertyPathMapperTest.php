...
class PropertyPathMapperTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->propertyPath = $this
				->getMockBuilder('Symfony\Component\Form\Util\PropertyPath')
	...
	private function getForm(PropertyPath $propertyPath = null,
			$synchronized = true, $readOnly = false) {
		$form = $this->getMock('Symfony\Tests\Component\Form\FormInterface');

		$form->expects($this->any())->method('getAttribute')
				->with('property_path')
				->will($this->returnValue($propertyPath));

		$form->expects($this->any())->method('isSynchronized')
				->will($this->returnValue($synchronized));

		$form->expects($this->any())->method('isReadOnly')
	...
	public function testMapDataToForm() {
		...
		$this->propertyPath->expects($this->once())->method('getValue')
				->with($data)->will($this->returnValue('foobar'));

		$form = $this->getForm($this->propertyPath);

		$form->expects($this->once())->method('setData')->with('foobar');
	...
	public function testMapFormToDataIgnoresReadOnlyForm() {
		...
		$this->propertyPath->expects($this->never())->method('setValue');
