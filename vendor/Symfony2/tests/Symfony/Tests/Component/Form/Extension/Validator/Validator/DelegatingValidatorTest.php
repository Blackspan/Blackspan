...
class DelegatingValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function getConstraintViolation($propertyPath) {
		return new ConstraintViolation($this->message, $this->params, null,
				$propertyPath, null);
	}
	...
	protected function getBuilder($name = 'name', $propertyPath = null) {
		...
		$builder
				->setAttribute('property_path',
						new PropertyPath($propertyPath ? : $name));
	...
	public function testUseValidateValueWhenValidationConstraintExist() {
		...
		$form = $this->getBuilder('name')
				->setAttribute('validation_constraint', $constraint)->getForm();
	...
	public function testFormErrorsOnForm() {
		...
		$this->delegate->expects($this->once())->method('validate')
				->will(
						$this
								->returnValue(
										array(
						...
						$this
								->returnValue(
										array(
												$this
														->getConstraintViolation(
																'children[address].data.street.constrainedProp'))));
	...
	public function testFormErrorsOnGrandChild() {
		...
		$this
				->assertEquals(array($this->getFormError()),
						$grandChild->getErrors());
	}
						...
						$this
								->returnValue(
										array(
												$this
														->getConstraintViolation(
																'children[address].constrainedProp'))));
						...
						$this
								->returnValue(
										array(
												$this
														->getConstraintViolation(
																'children[lastName].constrainedProp'))));
						...
						$this
								->returnValue(
										array(
												$this
														->getConstraintViolation(
																'children[0].data.firstName'),
												$this
														->getConstraintViolation(
																'children[1].data.firstName'),)));
						...
						$this
								->returnValue(
										array(
												$this
														->getConstraintViolation(
																'data.firstName.constrainedProp'))));
						...
						$this
								->returnValue(
										array(
												$this
														->getConstraintViolation(
																'data.address.street.constrainedProp'))));
	...
	public function testDataErrorsOnCollectionForm() {
		...
		$this
				->assertFalse($parent->hasErrors(),
						'->hasErrors() returns false for parent form');
		$this
				->assertFalse($child->hasErrors(),
						'->hasErrors() returns false for child form');
	...
	public function testMappedError() {
		$parent = $this->getBuilder()
				->setAttribute('error_mapping',
						array('passwordPlain' => 'password',))->getForm();
						...
						$this
								->returnValue(
										array(
												$this
														->getConstraintViolation(
																'data.passwordPlain.constrainedProp'))));
	...
	public function testMappedNestedError() {
		$parent = $this->getBuilder()
				->setAttribute('error_mapping',
						array('address.streetName' => 'address.street',))
				->getForm();
		$child = $this->getForm('address');
		$grandChild = $this->getForm('street');
						...
						$this
								->returnValue(
										array(
												$this
														->getConstraintViolation(
																'data.address.streetName.constrainedProp'))));
	...
	public function testNestedMappingUsingForm() {
		...
		$child = $this->getBuilder('address')
				->setAttribute('error_mapping',
						array('streetName' => 'street',))->getForm();
		$grandChild = $this->getForm('street');
						...
						$this
								->returnValue(
										array(
												$this
														->getConstraintViolation(
																'children[address].data.streetName.constrainedProp'))));
	...
	public function testNestedMappingVirtualForm() {
		$parent = $this->getBuilder()
				->setAttribute('error_mapping',
						array('streetName' => 'street',))->getForm();
		$child = $this->getBuilder('address')->setAttribute('virtual', true)
				->getForm();
		$grandChild = $this->getForm('street');
						...
						$this
								->returnValue(
										array(
												$this
														->getConstraintViolation(
																'data.streetName.constrainedProp'))));
	...
	public function testValidateFormData() {
		...
		$graphWalker->expects($this->at(0))->method('walkReference')
				->with($object, 'group1', 'data', true);
		$graphWalker->expects($this->at(1))->method('walkReference')
	...
	public function testValidateFormDataUsesInheritedValidationGroup() {
		...
		$parent = $this->getBuilder()
				->setAttribute('validation_groups', 'group')->getForm();
		$child = $this->getBuilder()->setAttribute('validation_groups', null)
				->getForm();
		$parent->add($child);

		$child->setData($object);

		$graphWalker->expects($this->once())->method('walkReference')
	...
	public function testValidateFormDataDoesNotWalkScalars() {
		...
		$form = $this->getBuilder()
				->appendClientTransformer($clientTransformer)->getForm();
	...
	public function testValidateIgnoresNonRoot() {
		...
		$form->expects($this->once())->method('isRoot')
				->will($this->returnValue(false));

		$this->delegate->expects($this->never())->method('validate');
