...
class ResizeFormListenerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testPreSetDataResizesForm() {
		...
		$this->factory->expects($this->at(0))->method('createNamed')
				->with('text', 1, null,
						array('property_path' => '[1]', 'max_length' => 10))
				->will($this->returnValue($this->getForm('1')));
		$this->factory->expects($this->at(1))->method('createNamed')
				->with('text', 2, null,
						array('property_path' => '[2]', 'max_length' => 10))
				->will($this->returnValue($this->getForm('2')));

		$data = array(1 => 'string', 2 => 'string');
		$event = new DataEvent($this->form, $data);
		$listener = new ResizeFormListener($this->factory, 'text',
				array('max_length' => '10'), false, false);
	...
	public function testPreSetDataRequiresArrayOrTraversable() {
		...
		$event = new DataEvent($this->form, $data);
		$listener = new ResizeFormListener($this->factory, 'text', array(),
				false, false);
		$listener->preSetData($event);
	}
	...
	public function testPreBindResizesUpIfAllowAdd() {
		...
		$this->factory->expects($this->once())->method('createNamed')
				->with('text', 1, null,
						array('property_path' => '[1]', 'max_length' => 10))
				->will($this->returnValue($this->getForm('1')));
		...
		$event = new DataEvent($this->form, $data);
		$listener = new ResizeFormListener($this->factory, 'text',
				array('max_length' => 10), true, false);
