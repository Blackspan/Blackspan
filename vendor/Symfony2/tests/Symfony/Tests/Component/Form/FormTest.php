...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->factory = $this
				->getMock('Symfony\Component\Form\FormFactoryInterface');
	...
	public function testConstructExpectsValidValidators() {
		...
		new Form('name', $this->dispatcher, array(), array(), array(), null,
				$validators);
	}

	public function testDataIsInitializedEmpty() {
		$norm = new FixedDataTransformer(
				array('' => 'foo',));
		$client = new FixedDataTransformer(
				array('foo' => 'bar',));

		$form = new Form('name', $this->dispatcher, array(), array($client),
				array($norm));
	...
	public function testBind() {
		...
		$child->expects($this->once())->method('bind')
				->with($this->equalTo('Bernhard'));

		$this->form->bind(array('firstName' => 'Bernhard'));

		$this
				->assertEquals(array('firstName' => 'Bernhard'),
						$this->form->getData());
	}
	...
	public function testBindIsIgnoredIfReadOnly() {
		$form = $this->getBuilder()->setReadOnly(true)->setData('initial')
				->getForm();
	...
	public function testGetRootReturnsRootOfParent() {
		...
		$parent->expects($this->once())->method('getRoot')
	...
	public function testNotEmptyIfChildNotEmpty() {
		...
		$child->expects($this->once())->method('isEmpty')
	...
	public function testValidIfBoundAndReadOnlyWithChildren() {
		$this->factory->expects($this->once())->method('createNamedBuilder')
				->with('text', 'name', null, array())
				->will($this->returnValue($this->getBuilder('name')));

		$form = $this->getBuilder('person')->setReadOnly(true)
	...
	public function testNotValidIfChildNotValid() {
		$child = $this->getMockForm();
		$child->expects($this->once())->method('isValid')
				->will($this->returnValue(false));
	...
	public function testIterator() {
		...
		$this
				->assertSame($this->form->getChildren(),
						iterator_to_array($this->form));
	}
	...
	public function testSetDataExecutesTransformationChain() {
		...
		$form = $this->getBuilder('name', new EventDispatcher())
				->addEventSubscriber(
						new FixedFilterListener(
								array(
										'onSetData' => array(
												'app' => 'filtered',),)))
				->appendNormTransformer(
						new FixedDataTransformer(
								array('' => '', 'filtered' => 'norm',)))
	...
	public function testNotSynchronizedIfTransformationFailed() {
		...
		$transformer->expects($this->once())->method('reverseTransform')
				->will(
						$this
								->throwException(
										new TransformationFailedException()));

		$form = $this->getBuilder()->appendClientTransformer($transformer)
				->getForm();
	...
	public function testEmptyDataCreatedBeforeTransforming() {
		$form = $this->getBuilder()->setEmptyData('foo')
				->appendClientTransformer(
						new FixedDataTransformer(
								array('' => '', 
								// direction is reversed!
	...
	public function testAddMapsClientDataToForm() {
		...
		$form = $this->getBuilder()->setDataMapper($mapper)
				->appendClientTransformer(
						new FixedDataTransformer(
								array('' => '', 'foo' => 'bar',)))
				->setData('foo')->getForm();
		...
		$mapper->expects($this->once())->method('mapDataToForm')
				->with('bar', $child);
	...
	public function testSetDataMapsClientDataToChildren() {
		...
		$form = $this->getBuilder()->setDataMapper($mapper)
				->appendClientTransformer(
						new FixedDataTransformer(
								array('' => '', 'foo' => 'bar',)))->getForm();

		$form->add($child1 = $this->getBuilder('firstName')->getForm());
		$form->add($child2 = $this->getBuilder('lastName')->getForm());

		$mapper->expects($this->once())->method('mapDataToForms')
				->with('bar',
						array('firstName' => $child1, 'lastName' => $child2));
	...
	public function testBindMapsBoundChildrenOntoExistingClientData() {
		...
		$mapper->expects($this->once())->method('mapFormsToData')
				->with(array('firstName' => $child1, 'lastName' => $child2),
						'bar')
				->will(
						$this
								->returnCallback(
										function ($children, $bar) use ($test) {
											$test
													->assertEquals('Bernhard',
															$children['firstName']
																	->getData());
											$test
													->assertEquals('Schussek',
															$children['lastName']
																	->getData());
										}));
	...
	public function testBindMapsBoundChildrenOntoEmptyData() {
		...
		$object = new \stdClass();
		$form = $this->getBuilder()->setDataMapper($mapper)
				->setEmptyData($object)->setData(null)->getForm();
		...
		$form->bind(array('name' => 'Bernhard',));
	}

	public function testBindValidatesAfterTransformation() {
		$test = $this;
		$validator = $this->getFormValidator();
		$form = $this->getBuilder()->addValidator($validator)->getForm();

		$validator->expects($this->once())->method('validate')->with($form)
				->will(
						$this
								->returnCallback(
										function ($form) use ($test) {
	...
	public function requestMethodProvider() {
		return array(array('POST'), array('PUT'),);
	}
	...
	public function testBindPostOrPutRequest($method) {
		...
		$values = array(
				'author' => array('name' => 'Bernhard',
						'image' => array('filename' => 'foobar.png'),),);

		$files = array(
				'author' => array('error' => array('image' => UPLOAD_ERR_OK),
		...
		$file = new UploadedFile($path, 'upload.png', 'image/png', 123,
				UPLOAD_ERR_OK);
	...
	public function testBindGetRequest() {
		$values = array(
				'author' => array('firstName' => 'Bernhard',
						'lastName' => 'Schussek',),);

		$request = new Request($values, array(), array(), array(), array(),
	...
	public function testCreateView() {
		...
		$type1Extension = $this
				->getMock('Symfony\Component\Form\FormTypeExtensionInterface');
		$type1->expects($this->any())->method('getExtensions')
				->will($this->returnValue(array($type1Extension)));
		$type2 = $this->getMock('Symfony\Component\Form\FormTypeInterface');
		$type2Extension = $this
				->getMock('Symfony\Component\Form\FormTypeExtensionInterface');
		$type2->expects($this->any())->method('getExtensions')
				->will($this->returnValue(array($type2Extension)));
		$calls = array();

		$type1->expects($this->once())->method('buildView')
				->will(
						$this
								->returnCallback(
										function (FormView $view, Form $form) use (
												$test, &$calls) {
		...
		$type1Extension->expects($this->once())->method('buildView')
				->will(
						$this
								->returnCallback(
										function (FormView $view, Form $form) use (
		...
		$type2->expects($this->once())->method('buildView')
				->will(
						$this
								->returnCallback(
										function (FormView $view, Form $form) use (
		...
		$type2Extension->expects($this->once())->method('buildView')
				->will(
						$this
								->returnCallback(
										function (FormView $view, Form $form) use (
		...
		$type1->expects($this->once())->method('buildViewBottomUp')
				->will(
						$this
								->returnCallback(
										function (FormView $view, Form $form) use (
		...
		$type1Extension->expects($this->once())->method('buildViewBottomUp')
				->will(
						$this
								->returnCallback(
										function (FormView $view, Form $form) use (
		...
		$type2->expects($this->once())->method('buildViewBottomUp')
				->will(
						$this
								->returnCallback(
										function (FormView $view, Form $form) use (
		...
		$type2Extension->expects($this->once())->method('buildViewBottomUp')
				->will(
						$this
								->returnCallback(
										function (FormView $view, Form $form) use (
		...
		$this
				->assertEquals(
						array(0 => 'type1::buildView',
								...
								7 => 'type2ext::buildViewBottomUp',), $calls);
	}
	...
	protected function getBuilder($name = 'name',
		...
		return new FormBuilder($name, $this->factory,
	...
	protected function getMockForm($name = 'name') {
		...
		$form->expects($this->any())->method('getName')
	...
	protected function getDataTransformer() {
		return $this
				->getMock('Symfony\Component\Form\DataTransformerInterface');
	}
