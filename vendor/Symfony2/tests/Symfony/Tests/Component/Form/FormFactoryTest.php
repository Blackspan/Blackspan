...
class FormFactoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCreateNamedBuilderDoesNotOverrideExistingDataOption() {
		...
		$builder = $this->factory
				->createNamedBuilder('foo', 'bar', 'xyz',
						array('data' => 'abc',));
	...
	public function testCreateNamedBuilderExpectsDataOptionToBeSupported() {
		...
		$type->expects($this->any())->method('getName')
				->will($this->returnValue('foo'));
		$type->expects($this->any())->method('getExtensions')
				->will($this->returnValue(array()));
		$type->expects($this->any())->method('getAllowedOptionValues')
				->will($this->returnValue(array()));
		$type->expects($this->any())->method('getDefaultOptions')
				->will(
						$this
								->returnValue(
						...
						$this
								->returnValue(
										array('data' => null,
												'required' => false,
												'max_length' => null,)));
		$type->expects($this->any())->method('createBuilder')
	...
	public function testCreateNamedBuilderExpectsOptionsToExist() {
		...
		$this->factory
				->createNamedBuilder('foo', 'bar', null,
						array('invalid' => 'xyz',));
	}
	...
	public function testCreateBuilderForPropertyCreatesFieldWithHighestConfidence() {
		$this->guesser1->expects($this->once())->method('guessType')
		...
		$this->guesser2->expects($this->once())->method('guessType')
				->with('Application\Author', 'firstName')
				->will(
						$this
								->returnValue(
										new TypeGuess('password',
												array('max_length' => 7),
												Guess::HIGH_CONFIDENCE)));

		$factory = $this->createMockFactory(array('createNamedBuilder'));

		$factory->expects($this->once())->method('createNamedBuilder')
	...
	public function testOptionsCanBeOverridden() {
		...
		$builder = $factory
				->createBuilderForProperty('Application\Author', 'firstName',
						null, array('max_length' => 11));

		$this->assertEquals('builderInstance', $builder);
	}

	public function testCreateBuilderUsesMaxLengthIfFound() {
		$this->guesser1->expects($this->once())->method('guessMaxLength')
				->with('Application\Author', 'firstName')
				->will(
						$this
								->returnValue(
		...
		$this->guesser2->expects($this->once())->method('guessMaxLength')
				->with('Application\Author', 'firstName')
				->will(
						$this
								->returnValue(
	...
	public function testCreateBuilderUsesRequiredSettingWithHighestConfidence() {
		$this->guesser1->expects($this->once())->method('guessRequired')
				->with('Application\Author', 'firstName')
				->will(
						$this
								->returnValue(
		...
		$this->guesser2->expects($this->once())->method('guessRequired')
				->with('Application\Author', 'firstName')
				->will(
						$this
								->returnValue(
										new ValueGuess(false,
												Guess::HIGH_CONFIDENCE)));
	...
	private function createMockFactory(array $methods = array()) {
		return $this->getMockBuilder('Symfony\Component\Form\FormFactory')
				->setMethods($methods)
				->setConstructorArgs(
						array(array($this->extension1, $this->extension2)))
				->getMock();
	}
}
