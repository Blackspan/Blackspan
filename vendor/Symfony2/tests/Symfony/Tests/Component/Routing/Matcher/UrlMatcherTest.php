...
class UrlMatcherTest extends \PHPUnit_Framework_TestCase {
	...
	public function testMethodNotAllowed() {
		...
		$coll
				->add('foo',
						new Route('/foo', array(), array('_method' => 'post')));
	...
	public function testHeadAllowedWhenRequirementContainsGet() {
		...
		$coll
				->add('foo',
						new Route('/foo', array(), array('_method' => 'get')));
	...
	public function testMethodNotAllowedAggregatesAllowedMethods() {
		...
		try {
			...
			$this
					->assertEquals(array('POST', 'PUT', 'DELETE'),
							$e->getAllowedMethods());
		}
	}
	...
	public function testMatch() {
		...
		$this
				->assertEquals(array('_route' => 'foo', 'bar' => 'baz'),
						$matcher->match('/foo/baz'));

		// test that defaults are merged
		$collection = new RouteCollection();
		$collection
				->add('foo', new Route('/foo/{bar}', array('def' => 'test')));
		$matcher = new UrlMatcher($collection, new RequestContext(), array());
		$this
				->assertEquals(
						array('_route' => 'foo', 'bar' => 'baz',
								'def' => 'test'), $matcher->match('/foo/baz'));
		...
		$collection
				->add('bar',
						new Route('/{bar}/foo', array('bar' => 'bar'),
								array('bar' => 'foo|bar')));
		$matcher = new UrlMatcher($collection, new RequestContext(), array());
		$this
				->assertEquals(array('_route' => 'bar', 'bar' => 'bar'),
						$matcher->match('/bar/foo'));
		$this
				->assertEquals(array('_route' => 'bar', 'bar' => 'foo'),
						$matcher->match('/foo/foo'));

		$collection = new RouteCollection();
		$collection
				->add('bar',
						new Route('/{bar}', array('bar' => 'bar'),
								array('bar' => 'foo|bar')));
		$matcher = new UrlMatcher($collection, new RequestContext(), array());
		$this
				->assertEquals(array('_route' => 'bar', 'bar' => 'foo'),
	...
	public function testMatchWithPrefixes() {
		...
		$matcher = new UrlMatcher($collection, new RequestContext(), array());
		$this
				->assertEquals(array('_route' => 'foo', 'foo' => 'foo'),
	...
	public function testMatchWithDynamicPrefix() {
		...
		$this
				->assertEquals(
						array('_locale' => 'fr', '_route' => 'foo',
								'foo' => 'foo'), $matcher->match('/fr/b/foo'));
	}
		...
		$collection
				->add('foo',
						new Route('/{foo}/bar', array(),
								array('foo' => '[' . preg_quote($chars) . ']+')));

		$matcher = new UrlMatcher($collection, new RequestContext(), array());
		$this
				->assertEquals(array('_route' => 'foo', 'foo' => $chars),
						$matcher->match('/' . urlencode($chars) . '/bar'));
		$this
				->assertEquals(array('_route' => 'foo', 'foo' => $chars),
						$matcher
								->match(
	...
	public function testMatchWithDotMetacharacterInRequirements() {
		...
		$matcher = new UrlMatcher($collection, new RequestContext(), array());
		$this
				->assertEquals(array('_route' => 'foo', 'foo' => "\n"),
	...
	public function testMatchRegression() {
		...
		$matcher = new UrlMatcher($coll, new RequestContext());
		$this
				->assertEquals(array('foo' => 'bar', '_route' => 'bar'),
