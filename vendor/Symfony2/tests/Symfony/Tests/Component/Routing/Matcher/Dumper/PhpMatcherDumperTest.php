...
class PhpMatcherDumperTest extends \PHPUnit_Framework_TestCase {
	public function testDump() {
		...
		$this
				->assertStringEqualsFile(
						__DIR__ . '/../../Fixtures/dumper/url_matcher1.php',
						$dumper->dump(),
						'->dump() dumps basic routes to the correct PHP file.');
		...
		$collection
				->add('secure',
						new Route('/secure', array(),
								array('_scheme' => 'https')));

		// force HTTP redirection
		$collection
				->add('nonsecure',
						new Route('/nonsecure', array(),
								array('_scheme' => 'http')));
		...
		$this
				->assertStringEqualsFile(
						__DIR__ . '/../../Fixtures/dumper/url_matcher2.php',
						$dumper
								->dump(
										array(
												'base_class' => 'Symfony\Tests\Component\Routing\Fixtures\RedirectableUrlMatcher')),
						'->dump() dumps basic routes to the correct PHP file.');
	}
	...
	protected function getRouteCollection() {
		...
		$collection
				->add('foo',
						new Route('/foo/{bar}', array('def' => 'test'),
								array('bar' => 'baz|symfony')));
		// method requirement
		$collection
				->add('bar',
						new Route('/bar/{foo}', array(),
								array('_method' => 'GET|head')));
		// GET method requirement automatically adds HEAD as valid
		$collection
				->add('barhead',
						new Route('/barhead/{foo}', array(),
								array('_method' => 'GET')));
		// simple
		...
		$collection
				->add('foofoo',
						new Route('/foofoo', array('def' => 'test')));
		// pattern with quotes
		$collection
				->add('quoter',
						new Route('/{quoter}', array(),
								array('quoter' => '[\']+')));
		// space in pattern
