...
class RouteCollectionTest extends \PHPUnit_Framework_TestCase {
	public function testRoute() {
		...
		$this
				->assertEquals(array('foo' => $route), $collection->all(),
						'->add() adds a route');
		$this
				->assertEquals($route, $collection->get('foo'),
						'->get() returns a route by name');
		$this
				->assertNull($collection->get('bar'),
	...
	public function testIteratorWithOverridenRoutes() {
		...
		$this
				->assertEquals('/foo1',
						$this->getFirstNamedRoute($collection, 'foo')
								->getPattern());
	}

	protected function getFirstNamedRoute(RouteCollection $routeCollection,
			$name) {
	...
	public function testAddCollection() {
		...
		$collection->addCollection($collection1);
		$this
				->assertEquals(array('foo' => $foo1, 'bar' => $bar1),
						$collection->all(),
						'->addCollection() adds routes from another collection');
		...
		$this
				->assertEquals('/foo/foo1',
						$collection->get('foo')->getPattern(),
						'->addCollection() can add a prefix to all merged routes');
		...
		$collection->addCollection($collection1);
		$this
				->assertEquals(array($foo, $foo1), $collection->getResources(),
	...
	public function testResource() {
		...
		$collection
				->addResource(
						$foo = new FileResource(__DIR__ . '/Fixtures/foo.xml'));
		$this
				->assertEquals(array($foo), $collection->getResources(),
