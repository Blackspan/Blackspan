...
class FileValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testTooLargeBytes() {
		...
		$constraint = new File(
				array('maxSize' => 10, 'maxSizeMessage' => 'myMessage',));
	...
	public function testTooLargeKiloBytes() {
		...
		$constraint = new File(
				array('maxSize' => '1k', 'maxSizeMessage' => 'myMessage',));
	...
	public function testTooLargeMegaBytes() {
		...
		$constraint = new File(
				array('maxSize' => '1M', 'maxSizeMessage' => 'myMessage',));
	...
	public function testFileNotFound() {
		$constraint = new File(
				array('notFoundMessage' => 'myMessage',));
	...
	public function testValidMimeType() {
		...
		$file->expects($this->once())->method('getMimeType')
				->will($this->returnValue('image/jpg'));

		$constraint = new File(
				array('mimeTypes' => array('image/png', 'image/jpg'),));
	...
	public function testInvalidMimeType() {
		...
		$file->expects($this->once())->method('getPathname')
				->will($this->returnValue($this->path));
		$file->expects($this->exactly(2))->method('getMimeType')
	...
	public function testUploadedFileError($error, $message) {
		$file = new UploadedFile('/path/to/file', 'originalName', 'mime', 0,
				$error);
	...
	public function uploadedFileErrorProvider() {
		return array(array(UPLOAD_ERR_INI_SIZE, 'uploadIniSizeErrorMessage'),
	...
	protected function assertFileValid($filename, File $constraint,
			$valid = true) {
		$this
				->assertEquals(
						$this->validator->isValid($filename, $constraint),
						$valid);
		if (file_exists($filename)) {
			$this
					->assertEquals(
							$this->validator
									->isValid(new FileObject($filename),
											$constraint), $valid);
		}
	}
}
