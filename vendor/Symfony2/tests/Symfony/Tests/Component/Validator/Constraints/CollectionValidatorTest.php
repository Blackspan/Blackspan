 ...
 */
class TestArrayObject implements \ArrayAccess, \IteratorAggregate, \Countable,
		\Serializable {
	private $array;
...
class CollectionValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->walker = $this
				->getMock('Symfony\Component\Validator\GraphWalker', array(),
						array(), '', false);
		$metadataFactory = $this
				->getMock(
						'Symfony\Component\Validator\Mapping\ClassMetadataFactoryInterface');

		$this->context = new ExecutionContext('Root', $this->walker,
				$metadataFactory);
	...
	public function testWalkSingleConstraint($array) {
		...
		foreach ($array as $key => $value) {
			$this->walker->expects($this->once())->method('walkConstraint')
					->with($this->equalTo($constraint), $this->equalTo($value),
							$this->equalTo('MyGroup'),
							$this->equalTo('foo[' . $key . ']'));
		}
	...
	public function testWalkMultipleConstraints($array) {
		...
		foreach ($array as $key => $value) {
			$this->walker->expects($this->exactly(2))->method('walkConstraint')
					->with($this->equalTo($constraint), $this->equalTo($value),
							$this->equalTo('MyGroup'),
							$this->equalTo('foo[' . $key . ']'));
		}
	...
	public function testNullNotConsideredExtraField() {
		...
		$this
				->assertTrue(
						$this->validator
								->isValid(new \ArrayObject($array), $collection));
		$this
				->assertTrue(
						$this->validator
								->isValid(new TestArrayObject($array),
										$collection));
	}

	public function testExtraFieldsAllowed() {
		$array = array('foo' => 5, 'bar' => 6,);
		$collection = new Collection(
				array(
						'fields' => array('foo' => new Min(4),),
	...
	public function testObjectShouldBeLeftUnchanged() {
		$value = new \ArrayObject(array('foo' => 3));
		$this->validator
				->isValid($value,
						new Collection(
								array(
										'fields' => array('foo' => new Min(2),))));

		$this
				->assertEquals(array('foo' => 3), (array) $value);
	}
