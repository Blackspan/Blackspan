...
class CallbackValidatorTest_Object {
	public function validateOne(ExecutionContext $context) {
		...
		$context
				->addViolation('My message', array('parameter'), 'invalidValue');
	}

	public function validateTwo(ExecutionContext $context) {
		$context
				->addViolation('Other message', array('other parameter'),
						'otherInvalidValue');
	}
}
...
class CallbackValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testNullIsValid() {
		$this
				->assertTrue(
						$this->validator
								->isValid(null, new Callback(array('foo'))));
	}
	...
	public function testCallbackSingleMethod() {
		...
		$this
				->assertTrue(
						$this->validator
								->isValid($object,
										new Callback(array('validateOne'))));

		$violations = new ConstraintViolationList();
		$violations
				->add(
						new ConstraintViolation('My message',
								array('parameter'), 'Root', 'foo.bar',
								'invalidValue'));

		$this->assertEquals($violations, $this->context->getViolations());
		$this->assertEquals('InitialClass', $this->context->getCurrentClass());
		$this
				->assertEquals('initialProperty',
						$this->context->getCurrentProperty());
		$this->assertEquals('InitialGroup', $this->context->getGroup());
		$this
				->assertEquals('initial.property.path',
	...
	public function testCallbackSingleStaticMethod() {
		...
		$violations
				->add(
						new ConstraintViolation('Static message',
								array('parameter'), 'Root', 'foo.bar',
								'invalidValue'));
	...
	public function testCallbackMultipleMethods() {
		...
		$violations
				->add(
						new ConstraintViolation('Other message',
								array('other parameter'), 'Root',
								'initial.property.path', 'otherInvalidValue'));
	...
	public function testExpectValidCallbacks() {
		...
		$this->validator
				->isValid($object, new Callback(array(array('foo', 'bar'))));
	}
