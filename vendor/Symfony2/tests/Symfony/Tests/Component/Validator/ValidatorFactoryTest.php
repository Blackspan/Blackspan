...
class ValidatorFactoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testOverrideClassMetadataFactory() {
		...
		$this
				->assertSame($factory1,
						$this->defaultContext->getClassMetadataFactory());
	...
	public function testOverrideConstraintValidatorFactory() {
		...
		$this
				->assertSame($factory1,
						$this->defaultContext->getConstraintValidatorFactory());
	...
	public function testGetValidator() {
		...
		$this->defaultContext->setClassMetadataFactory($metadataFactory)
		...
		$this
				->assertEquals(
						new Validator($metadataFactory, $validatorFactory),
						$validator);
	}
	...
	public function testBuildDefaultFromYaml() {
		...
		$context
				->setClassMetadataFactory(
						new ClassMetadataFactory(
								new YamlFilesLoader(array($path))))
				->setConstraintValidatorFactory(
						new ConstraintValidatorFactory());
	...
	public function testBuildDefaultFromStaticMethod() {
		$path = __DIR__ . '/Mapping/Loader/constraint-mapping.yml';
		$factory = ValidatorFactory::buildDefault(array(), false,
				'loadMetadata');
	...
	public function testBuildDefaultFromMultipleLoaders() {
		...
		$factory = ValidatorFactory::buildDefault(array($xmlPath, $yamlPath),
				true, 'loadMetadata');

		$chain = new LoaderChain(
				array(new XmlFilesLoader(array($xmlPath)),
						new YamlFilesLoader(array($yamlPath)),
						new AnnotationLoader(new AnnotationReader()),
						new StaticMethodLoader('loadMetadata'),));

		$context = new ValidatorContext();
		$context->setClassMetadataFactory(new ClassMetadataFactory($chain))
				->setConstraintValidatorFactory(
						new ConstraintValidatorFactory());
	...
	public function testBuildDefaultThrowsExceptionIfUnknownExtension() {
		ValidatorFactory::buildDefault(
				array(__DIR__ . '/Mapping/Loader/StaticMethodLoaderTest.php'));
	}
}
