...
class GraphWalkerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->walker = new GraphWalker('Root', $this->factory,
	...
	public function testWalkObjectUpdatesContext() {
		...
		$this
				->assertEquals(
						...
						$this->getContext()->getCurrentClass());
	}
	...
	public function testWalkObjectTwiceInDifferentGroupsValidatesTwice() {
		$this->metadata->addConstraint(new ConstraintA());
		$this->metadata
				->addConstraint(new ConstraintA(array('groups' => 'Custom')));
	...
	public function testWalkObjectInDefaultGroupTraversesGroupSequence() {
		...
		$this->metadata
				->setGroupSequence(
						array('First', $this->metadata->getDefaultGroup()));
		...
		$violations
				->add(
						new ConstraintViolation('', array(), 'Root',
								'firstName', ''));
	...
	public function testWalkObjectInGroupSequencePropagatesDefaultGroup() {
		...
		$this->metadata
				->setGroupSequence(array($this->metadata->getDefaultGroup()));
	...
	public function testWalkObjectInOtherGroupTraversesNoGroupSequence() {
		...
		$this->walker
				->walkObject($this->metadata, $entity,
	...
	public function testWalkPropertyUpdatesContext() {
		...
		$this->walker
				->walkPropertyValue($this->metadata, 'firstName', 'value',
						'Default', '');
		...
		$this
				->assertEquals('firstName',
						$this->getContext()->getCurrentProperty());
	}
	...
	public function testWalkCascadedPropertyValidatesReferences() {
		...
		$this->walker
				->walkPropertyValue($this->metadata, 'reference', $entity,
						// object!
						'Default', 'path');
		...
		$violations
				->add(
						new ConstraintViolation('', array(), 'Root', 'path',
								$entity));
	...
	public function testWalkCascadedPropertyValidatesArraysByDefault() {
		...
		$this->walker
				->walkPropertyValue($this->metadata, 'reference',
	...
	public function testWalkCascadedPropertyValidatesTraversableByDefault() {
		...
		$this->walker
				->walkPropertyValue($this->metadata, 'reference',
						new \ArrayIterator(array('key' => $entity)), 'Default',
						'path');
	...
	public function testWalkCascadedPropertyDoesNotValidateTraversableIfDisabled() {
		...
		// validate array when validating the property "reference"
		$this->metadata
				->addPropertyConstraint('reference',
						new Valid(
	...
	public function testWalkCascadedPropertyDoesNotValidateNestedScalarValues() {
		...
		$this->walker
				->walkPropertyValue($this->metadata, 'reference',
						array('scalar', 'values'), 'Default', 'path');
	...
	public function testWalkCascadedPropertyDoesNotValidateNullValues() {
		...
		$this->walker
				->walkPropertyValue($this->metadata, 'reference', null,
						'Default', '');
	...
	public function testWalkCascadedPropertyRequiresObjectOrArray() {
		...
		$this->walker
				->walkPropertyValue($this->metadata, 'reference', 'no object',
						'Default', '');
	}
	...
	public function testWalkConstraintBuildsAViolationIfFailed() {
		...
		$this->walker
				->walkConstraint($constraint, 'foobar', 'Default',
						'firstName.path');
		...
		$violations
				->add(
						new ConstraintViolation('message',
								array('param' => 'value'), 'Root',
								'firstName.path', 'foobar'));
	...
	public function testWalkConstraintBuildsNoViolationIfSuccessful() {
		...
		$this->walker
				->walkConstraint($constraint, 'VALID', 'Default',
						'firstName.path');
	...
	public function testWalkObjectUsesCorrectPropertyPathInViolationsWhenUsingCollections() {
		$constraint = new Collection(
				array('foo' => new ConstraintA(), 'bar' => new ConstraintA(),));

		$this->walker
				->walkConstraint($constraint, array('foo' => 'VALID'),
						'Default', 'collection');
	...
	public function testWalkObjectUsesCorrectPropertyPathInViolationsWhenUsingNestedCollections() {
		$constraint = new Collection(
				array(
						'foo' => new Collection(
								array('foo' => new ConstraintA(),
										'bar' => new ConstraintA(),)),));

		$this->walker
				->walkConstraint($constraint,
						array('foo' => array('foo' => 'VALID')), 'Default',
						'collection');
		$violations = $this->walker->getViolations();
		$this
				->assertEquals('collection[foo]',
