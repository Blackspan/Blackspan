...
class XmlEncoderTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAttributes() {
		$obj = new ScalarDummy;
		$obj->xmlFoo = array(
				'foo-bar' => array('@id' => 1, '@name' => 'Bar'),
				'Foo' => array('Bar' => "Test", '@Type' => 'test'),
				'föo_bär' => 'a', "Bar" => array(1, 2, 3), 'a' => 'b',);
		$expected = '<?xml version="1.0"?>' . "\n" . '<response>'
				. '<foo-bar id="1" name="Bar"/>'
				. '<Foo Type="test"><Bar><![CDATA[Test]]></Bar></Foo>'
				. '<föo_bär><![CDATA[a]]></föo_bär>' . '<Bar>1</Bar>'
				. '<Bar>2</Bar>' . '<Bar>3</Bar>' . '<a><![CDATA[b]]></a>'
				. '</response>' . "\n";
		$this->assertEquals($expected, $this->encoder->encode($obj, 'xml'));
	}
	...
	public function testElementNameValid() {
		$obj = new ScalarDummy;
		$obj->xmlFoo = array('foo-bar' => 'a', 'foo_bar' => 'a',
				'föo_bär' => 'a',);

		$expected = '<?xml version="1.0"?>' . "\n" . '<response>'
				. '<foo-bar><![CDATA[a]]></foo-bar>'
				. '<foo_bar><![CDATA[a]]></foo_bar>'
				. '<föo_bär><![CDATA[a]]></föo_bär>' . '</response>' . "\n";
	...
	public function testEncodeRootAttributes() {
		$array = array('firstname' => 'Paul', '@gender' => 'm');
	...
	public function testEncodeScalarWithAttribute() {
		$array = array('person' => array('@gender' => 'M', '#' => 'Peter'),);
	...
	public function testDecode() {
		...
		$this
				->assertEquals(get_object_vars($obj),
	...
	public function testDecodeScalarWithAttribute() {
		...
		$expected = array('person' => array('@gender' => 'M', '#' => 'Peter'),);
	...
	public function testDecodeRootAttributes() {
		...
		$expected = array('firstname' => 'Peter', 'lastname' => 'Mac Calloway',
				'@gender' => 'M');
	...
	public function testDecodeArray() {
		...
		$expected = array(
				'people' => array(
						'person' => array(
								array('firstname' => 'Benjamin',
										'lastname' => 'Alexandre'),
								array('firstname' => 'Damien',
										'lastname' => 'Clay'))));
	...
	public function testPreventsComplexExternalEntities() {
		...
		try {
			$decoded = $this->encoder
					->decode(
							'<?xml version="1.0"?><!DOCTYPE scan[<!ENTITY test SYSTEM "php://filter/read=convert.base64-encode/resource=XmlEncoderTest.php">]><scan>&test;</scan>',
							'xml');
			chdir($oldCwd);
	...
	protected function getObject() {
		...
		$obj->baz = array('key' => 'val', 'key2' => 'val', 'A B' => 'bar',
				"Barry" => array('FooBar' => array("Baz" => "Ed", "@id" => 1)));
		$obj->qux = "1";
