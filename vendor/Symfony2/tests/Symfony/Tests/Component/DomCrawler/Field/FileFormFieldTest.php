...
class FileFormFieldTest extends FormFieldTestCase {
	public function testInitialize() {
		...
		$this
				->assertEquals(
						array('name' => '', 'type' => '', 'tmp_name' => '',
								'error' => UPLOAD_ERR_NO_FILE, 'size' => 0),
						$field->getValue(),
						'->initialize() sets the value of the field to no file uploaded');
		...
		try {
			...
			$this
					->fail(
							'->initialize() throws a \LogicException if the node is not an input field');
		...
		try {
			...
			$this
					->fail(
							'->initialize() throws a \LogicException if the node is not a file input field');
		} catch (\LogicException $e) {
			$this
					->assertTrue(true,
							'->initialize() throws a \LogicException if the node is not a file input field');
	...
	public function testSetErrorCode() {
		...
		$value = $field->getValue();
		$this
				->assertEquals(UPLOAD_ERR_FORM_SIZE, $value['error'],
