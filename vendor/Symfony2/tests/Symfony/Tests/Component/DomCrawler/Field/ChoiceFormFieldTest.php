...
class ChoiceFormFieldTest extends FormFieldTestCase {
	public function testInitialize() {
		...
		try {
			...
			$this
					->fail(
							'->initialize() throws a \LogicException if the node is not an input or a select');
	...
	public function testGetType() {
		$node = $this
				->createNode('input', '',
						array('type' => 'radio', 'name' => 'name',
								'value' => 'foo'));
		$field = new ChoiceFormField($node);

		$this
				->assertEquals('radio', $field->getType(),
						'->getType() returns radio for radio buttons');

		$node = $this
				->createNode('input', '',
						array('type' => 'checkbox', 'name' => 'name',
								'value' => 'foo'));
		$field = new ChoiceFormField($node);

		$this
				->assertEquals('checkbox', $field->getType(),
						'->getType() returns radio for a checkbox');
		...
		$this
				->assertEquals('select', $field->getType(),
						'->getType() returns radio for a select');
	}
	...
	public function testIsMultiple() {
		...
		$this
				->assertEquals(false, $field->isMultiple(),
						'->isMultiple() returns false for radio buttons');
		...
		$this
				->assertEquals(false, $field->isMultiple(),
						'->isMultiple() returns false for checkboxes');
		...
		$node = $this
				->createNode('select', '', array('multiple' => 'multiple'));
		$field = new ChoiceFormField($node);

		$this
				->assertEquals(true, $field->isMultiple(),
	...
	public function testMultipleSelects() {
		$node = $this
				->createSelectNode(array('foo' => false, 'bar' => false),
		...
		$field->setValue('foo');
		$this
				->assertEquals(array('foo'), $field->getValue(),
						'->setValue() returns an array of options if multiple is true');

		$field->setValue('bar');
		$this
				->assertEquals(array('bar'), $field->getValue(),
						'->setValue() returns an array of options if multiple is true');

		$field->setValue(array('foo', 'bar'));
		$this
				->assertEquals(array('foo', 'bar'), $field->getValue(),
						'->setValue() returns an array of options if multiple is true');

		$node = $this
				->createSelectNode(array('foo' => true, 'bar' => true),
						array('multiple' => 'multiple'));
		$field = new ChoiceFormField($node);
	...
	public function testRadioButtons() {
		...
		$this
				->assertFalse($field->hasValue(),
						'->hasValue() returns false when no radio button is selected');
		$this
				->assertEquals(null, $field->getValue(),
						'->getValue() returns null if no radio button is selected');
		$this
				->assertFalse($field->isMultiple(),
						'->isMultiple() returns false for radio buttons');
		...
		$this
				->assertTrue($field->hasValue(),
						'->hasValue() returns true when a radio button is selected');
		$this
				->assertEquals('bar', $field->getValue(),
	...
	public function testCheckboxes() {
		...
		$this
				->assertFalse($field->isMultiple(),
						'->hasValue() returns false for checkboxes');
		try {
			$field->addChoice(new \DOMNode());
			$this
					->fail(
							'->addChoice() throws a \LogicException for checkboxes');
		} catch (\LogicException $e) {
			$this
					->assertTrue(true,
							'->initialize() throws a \LogicException for checkboxes');
		}
	...
	public function testTick() {
		...
		try {
			...
			$this
					->assertTrue(true,
							'->tick() throws a \LogicException for select boxes');
		}
	...
	public function testUntick() {
		...
		try {
			...
			$this
					->assertTrue(true,
							'->untick() throws a \LogicException for select boxes');
		}
	...
	public function testSelect() {
		...
		$this
				->assertEquals(1, $field->getValue(),
						'->select() changes the value of the field');
		$field->select(false);
		$this
				->assertEquals(null, $field->getValue(),
						'->select() changes the value of the field');
	...
	public function testOptionWithNoValue() {
		$node = $this
				->createSelectNodeWithEmptyOption(
						array('foo' => false, 'bar' => false));
		$field = new ChoiceFormField($node);
		$field->select('foo');
		$this
				->assertEquals('foo', $field->getValue(),
	...
	protected function createSelectNodeWithEmptyOption($options,
			$attributes = array()) {
		$document = new \DOMDocument();
		$node = $document->createElement('select');
