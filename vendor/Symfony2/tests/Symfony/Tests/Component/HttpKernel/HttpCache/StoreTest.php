...
class StoreTest extends \PHPUnit_Framework_TestCase {
	...
	public function testSetsTheXContentDigestResponseHeaderBeforeStoring() {
		...
		$this
				->assertEquals('ena94a8fe5ccb19ba61c4c0873d391e987982fbbd3',
	...
	public function testRestoresResponseHeadersProperlyWithLookup() {
		...
		$this
				->assertEquals($response->headers->all(),
						array_merge(
								array('content-length' => 4,
										'x-body-file' => array(
												$this
														->getStorePath(
																$response
	...
	public function testRestoresResponseContentFromEntityStoreWithLookup() {
		...
		$response = $this->store->lookup($this->request);
		$this
				->assertEquals($this->getStorePath('en' . sha1('test')),
						$response->getContent());
	}
	...
	public function testStoresMultipleResponsesForEachVaryCombination() {
		...
		$this
				->assertEquals($this->getStorePath('en' . sha1('test 3')),
						$this->store->lookup($req3)->getContent());
		$this
				->assertEquals($this->getStorePath('en' . sha1('test 2')),
						$this->store->lookup($req2)->getContent());
		$this
				->assertEquals($this->getStorePath('en' . sha1('test 1')),
	...
	protected function storeSimpleEntry($path = null, $headers = array()) {
		...
		$this->request = Request::create($path, 'get', array(), array(),
				array(), $headers);
		$this->response = new Response('test', 200,
				array('Cache-Control' => 'max-age=420'));
