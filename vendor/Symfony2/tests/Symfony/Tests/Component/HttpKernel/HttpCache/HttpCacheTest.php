...
class HttpCacheTest extends HttpCacheTestCase {
	...
	public function testDoesNotCacheWithAuthorizationRequestHeaderAndNonPublicResponse() {
		...
		$this
				->request('GET', '/',
						array('HTTP_AUTHORIZATION' => 'basic foobarbaz'));
	...
	public function testDoesCacheWithAuthorizationRequestHeaderAndPublicResponse() {
		$this
				->setNextResponse(200,
						array('Cache-Control' => 'public', 'ETag' => '"Foo"'));
		$this
				->request('GET', '/',
						array('HTTP_AUTHORIZATION' => 'basic foobarbaz'));
	...
	public function testRespondsWith304WhenIfNoneMatchMatchesETag() {
		$this
				->setNextResponse(200,
						array('Cache-Control' => 'public', 'ETag' => '12345',
								'Content-Type' => 'text/plain'), 'Hello World');
	...
	public function testRespondsWith304OnlyIfIfNoneMatchAndIfModifiedSinceBothMatch() {
		...
		$this
				->request('GET', '/',
						array('HTTP_IF_NONE_MATCH' => '12345',
	...
	public function testReloadsResponsesWhenCacheHitsButNoCacheRequestDirectivePresentWhenAllowReloadIsSetTrue() {
		...
		$this
				->setNextResponse(200,
						array('Cache-Control' => 'public, max-age=10000'), '',
	...
	public function testDoesNotCacheSomeStatusCodeResponses() {
		foreach (array_merge(range(201, 202), range(204, 206), range(303, 305),
				range(400, 403), range(405, 409), range(411, 417),
				range(500, 505)) as $code) {
			$time = \DateTime::createFromFormat('U', time() + 5);
			$this
					->setNextResponse($code,
							array('Expires' => $time->format(DATE_RFC2822)));
	...
	public function testCachesResponsesWithAMaxAgeDirective() {
		$this
				->setNextResponse(200,
						array('Cache-Control' => 'public, max-age=5'));
	...
	public function testCachesResponsesWithAnETagValidatorButNoFreshnessInformation() {
		$this
				->setNextResponse(200,
						array('Cache-Control' => 'public', 'ETag' => '"123456"'));
		...
		$this
				->assertTrue(
						strtotime($this->responses[0]->headers->get('Date'))
								- strtotime(
										$this->response->headers->get('Date'))
								< 2);
	...
	public function testHitsCachedResponseWithMaxAgeDirective() {
		...
		$this
				->setNextResponse(200,
						array('Date' => $time->format(DATE_RFC2822),
	...
	public function testDoesNotAssignDefaultTtlWhenResponseHasMustRevalidateDirective() {
		$this
				->setNextResponse(200,
						array('Cache-Control' => 'must-revalidate'));
	...
	public function testFetchesFullResponseWhenCacheStaleAndNoValidatorsPresent() {
		...
		$m
				->invoke($this->store, 'md' . sha1('http://localhost/'),
						serialize($tmp));
	...
	public function testValidatesCachedResponsesWithLastModifiedAndNoFreshnessInformation() {
		$time = \DateTime::createFromFormat('U', time());
		$this
				->setNextResponse(200, array(), 'Hello World',
						function ($request, $response) use ($time) {
							$response->headers->set('Cache-Control', 'public');
							$response->headers
									->set('Last-Modified',
											$time->format(DATE_RFC2822));
							if ($time->format(DATE_RFC2822)
									== $request->headers
	...
	public function testReplacesCachedResponsesWhenValidationResultsInNon304Response() {
		...
		$this
				->setNextResponse(200, array(), 'Hello World',
						function ($request, $response) use ($time, &$count) {
							$response->headers
									->set('Last-Modified',
											$time->format(DATE_RFC2822));
							$response->headers->set('Cache-Control', 'public');
	...
	public function testPassesHeadRequestsThroughDirectlyOnPass() {
		...
		$this
				->setNextResponse(200, array(), 'Hello World',
						function ($request, $response) use ($that) {
							$response->setContent('');
							$response->setStatusCode(200);
	...
	public function testUsesCacheToRespondToHeadRequestsWhenFresh() {
		...
		$this->assertEquals('', $this->response->getContent());
		$this
				->assertEquals(strlen('Hello World'),
	...
	public function testSendsNoContentWhenFresh() {
		...
		$this
				->setNextResponse(200, array(), 'Hello World',
						function ($request, $response) use ($that, $time) {
							$response->headers
									->set('Cache-Control', 'public, max-age=10');
							$response->headers
									->set('Last-Modified',
	...
	public function testServesFromCacheWhenHeadersMatch() {
		...
		$this
				->setNextResponse(200,
						array('Cache-Control' => 'max-age=10000'), '',
						function ($request, $response) use (&$count) {
							$response->headers
	...
	public function testEsiCacheSendsTheLowestTtl() {
		$responses = array(
				...
				array('status' => 200, 'body' => 'Hello World!',
						'headers' => array('Cache-Control' => 's-maxage=300'),),
				array('status' => 200, 'body' => 'My name is Bobby.',
	...
	public function testEsiRecalculateContentLengthHeader() {
		$responses = array(
				array('status' => 200,
						...
						'headers' => array('Content-Length' => 26,
								'Cache-Control' => 's-maxage=300',
								'Surrogate-Control' => 'content="ESI/1.0"',),),
				array('status' => 200, 'body' => 'Hello World!',
						'headers' => array(),),);
