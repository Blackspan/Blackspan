...
class EsiTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAddSurrogateEsiCapability() {
		...
		$this
				->assertEquals('symfony2="ESI/1.0"',
						$request->headers->get('Surrogate-Capability'));

		$esi->addSurrogateEsiCapability($request);
		$this
				->assertEquals('symfony2="ESI/1.0", symfony2="ESI/1.0"',
						$request->headers->get('Surrogate-Capability'));
	}
	...
	public function testProcess() {
		...
		$this
				->assertEquals(
						'foo <?php echo $this->esi->handle($this, \'...\', \'alt\', true) ?>'
								. "\n", $response->getContent());
		...
		$this
				->assertEquals(
						'foo <?php echo $this->esi->handle($this, \'...\', \'\', false) ?>'
								. "\n", $response->getContent());
	}
	...
	public function testProcessEscapesPhpTags() {
		...
		$this
				->assertEquals(
						'foo <?php echo "<?"; ?>php die("foo"); ?><?php echo "<%"; ?>= "lala" %>',
						$response->getContent());
	}
	...
	public function testHandleWhenResponseIsNot200AndAltIsPresent() {
		...
		$cache = $this
				->getCache(Request::create('/'), array($response1, $response2));
		$this->assertEquals('bar', $esi->handle($cache, '/', '/alt', false));
	}

	protected function getCache($request, $response) {
		$cache = $this
				->getMock('Symfony\Component\HttpKernel\HttpCache\HttpCache',
						array('getRequest', 'handle'), array(), '', false);
		$cache->expects($this->any())->method('getRequest')
				->will($this->returnValue($request));
		if (is_array($response)) {
			$cache->expects($this->any())->method('handle')
					->will(
