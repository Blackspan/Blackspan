...
class HttpKernelTest extends \PHPUnit_Framework_TestCase {
	...
	public function testHandleWhenControllerThrowsAnExceptionAndRawIsTrue() {
		...
		$kernel
				->handle(new Request(), HttpKernelInterface::MASTER_REQUEST,
						true);
	}
	...
	public function testHandleWhenTheControllerIsAnObjectWithInvoke() {
		$dispatcher = new EventDispatcher();
		$kernel = new HttpKernel($dispatcher,
				$this->getResolver(new Controller()));

		$this
				->assertResponseEquals(new Response('foo'),
						$kernel->handle(new Request()));
	}
	...
	public function testHandleWhenTheControllerIsAFunction() {
		...
		$kernel = new HttpKernel($dispatcher,
				$this
						->getResolver(
								'Symfony\Tests\Component\HttpKernel\controller_func'));
	...
	public function testHandleWhenTheControllerIsAnArray() {
		$dispatcher = new EventDispatcher();
		$kernel = new HttpKernel($dispatcher,
				$this->getResolver(array(new Controller(), 'controller')));
	...
	public function testHandleWhenTheControllerIsAStaticArray() {
		...
		$kernel = new HttpKernel($dispatcher,
				$this
						->getResolver(
								array(
										'Symfony\Tests\Component\HttpKernel\Controller',
										'staticcontroller')));
	...
	protected function getResolver($controller = null) {
		...
		$resolver->expects($this->any())->method('getController')
				->will($this->returnValue($controller));
		$resolver->expects($this->any())->method('getArguments')
				->will($this->returnValue(array()));

		return $resolver;
	}

	protected function assertResponseEquals(Response $expected,
			Response $actual) {
