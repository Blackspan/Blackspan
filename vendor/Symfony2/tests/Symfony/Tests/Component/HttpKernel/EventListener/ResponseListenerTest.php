...
class ResponseListenerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->dispatcher
				->addListener(KernelEvents::RESPONSE,
						array($listener, 'onKernelResponse'));
	...
	public function testFilterDoesNothingForSubRequests() {
		...
		$event = new FilterResponseEvent($this->kernel, new Request(),
				HttpKernelInterface::SUB_REQUEST, $response);
	...
	public function testFilterDoesNothingIfRequestFormatIsNotDefined() {
		...
		$event = new FilterResponseEvent($this->kernel, Request::create('/'),
				HttpKernelInterface::MASTER_REQUEST, $response);
		$this->dispatcher->dispatch(KernelEvents::RESPONSE, $event);
	...
	public function testFilterSetContentType() {
		...
		$event = new FilterResponseEvent($this->kernel, $request,
				HttpKernelInterface::MASTER_REQUEST, $response);
		$this->dispatcher->dispatch(KernelEvents::RESPONSE, $event);
	...
	public function testFilterSetsNonDefaultCharsetIfNotOverridden() {
		...
		$this->dispatcher
				->addListener(KernelEvents::RESPONSE,
						array($listener, 'onKernelResponse'), 1);
