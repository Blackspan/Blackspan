...
class KernelTest extends \PHPUnit_Framework_TestCase {
	...
	public function testBootInitializesBundlesAndContainer() {
		$kernel = $this
				->getMockBuilder(
						'Symfony\Tests\Component\HttpKernel\KernelForTest')
				->disableOriginalConstructor()
				->setMethods(
						array('initializeBundles', 'initializeContainer',
								'getBundles'))->getMock();
		$kernel->expects($this->once())->method('initializeBundles');
		$kernel->expects($this->once())->method('initializeContainer');
		$kernel->expects($this->once())->method('getBundles')
	...
	public function testShutdownCallsShutdownOnAllBundles() {
		...
		$kernel = $this
				->getMockBuilder(
						'Symfony\Tests\Component\HttpKernel\KernelForTest')
				->disableOriginalConstructor()->setMethods(array('getBundles'))
				->getMock();
		$kernel->expects($this->once())->method('getBundles')
				->will($this->returnValue(array($bundle)));
	...
	public function testShutdownGivesNullContainerToAllBundles() {
		$bundle = $this
				->getMockBuilder('Symfony\Component\HttpKernel\Bundle\Bundle')
				->disableOriginalConstructor()->getMock();
		$bundle->expects($this->once())->method('setContainer')->with(null);
	...
	public function testHandleCallsHandleOnHttpKernel() {
		...
		$httpKernelMock = $this
				...
				->disableOriginalConstructor()->getMock();
		$httpKernelMock->expects($this->once())->method('handle')
		...
		$kernel = $this
				->getMockBuilder(
						'Symfony\Tests\Component\HttpKernel\KernelForTest')
				->disableOriginalConstructor()
				->setMethods(array('getHttpKernel'))->getMock();

		$kernel->expects($this->once())->method('getHttpKernel')
	...
	public function testHandleBootsTheKernel() {
		...
		$kernel = $this
				->getMockBuilder(
						'Symfony\Tests\Component\HttpKernel\KernelForTest')
				->disableOriginalConstructor()
				->setMethods(array('getHttpKernel', 'boot'))->getMock();
	...
	public function testStripComments() {
		if (!function_exists('token_get_all')) {
			$this
					->markTestSkipped(
							'The function token_get_all() is not available.');
...
EOF;
	...
	public function testIsClassInActiveBundleFalse() {
		...
		$this
				->assertFalse(
						$kernel->isClassInActiveBundle('Not\In\Active\Bundle'));
	}
	...
	public function testIsClassInActiveBundleFalseNoNamespace() {
		...
		$this
				->assertFalse(
						$kernel->isClassInActiveBundle('NotNamespacedClass'));
	}
	...
	public function testLocateResourceThrowsExceptionWhenNameIsUnsafe() {
		$this->getKernelForInvalidLocateResource()
	...
	public function testLocateResourceThrowsExceptionWhenResourceDoesNotExist() {
		...
		$kernel->expects($this->once())->method('getBundle')
				->will(
						$this
								->returnValue(
										array(
	...
	public function testLocateResourceReturnsTheFirstThatMatches() {
		...
		$this
				->assertEquals(__DIR__ . '/Fixtures/Bundle1Bundle/foo.txt',
	...
	public function testLocateResourceReturnsTheFirstThatMatchesWithParent() {
		...
		$kernel = $this->getKernel();
		$kernel->expects($this->exactly(2))->method('getBundle')
				->will($this->returnValue(array($child, $parent)));

		$this
				->assertEquals(__DIR__ . '/Fixtures/Bundle2Bundle/foo.txt',
						$kernel->locateResource('@ParentAABundle/foo.txt'));
		$this
				->assertEquals(__DIR__ . '/Fixtures/Bundle1Bundle/bar.txt',
	...
	public function testLocateResourceReturnsAllMatches() {
		...
		$this
				->assertEquals(
						array(__DIR__ . '/Fixtures/Bundle2Bundle/foo.txt',
	...
	public function testLocateResourceReturnsTheDirOneForResourcesAndBundleOnes() {
		...
		$this
				->assertEquals(
						array(
								__DIR__
										. '/Fixtures/Resources/Bundle1Bundle/foo.txt',
								__DIR__
										. '/Fixtures/Bundle1Bundle/Resources/foo.txt'),
						$kernel
								->locateResource(
										'@Bundle1Bundle/Resources/foo.txt',
										__DIR__ . '/Fixtures/Resources', false));
	}

	public function testLocateResourceOverrideBundleAndResourcesFolders() {
		$parent = $this
				->getBundle(__DIR__ . '/Fixtures/BaseBundle', null,
						'BaseBundle', 'BaseBundle');
		$child = $this
				->getBundle(__DIR__ . '/Fixtures/ChildBundle', 'ParentBundle',
						'ChildBundle', 'ChildBundle');

		$kernel = $this->getKernel();
		$kernel->expects($this->exactly(4))->method('getBundle')
				->will($this->returnValue(array($child, $parent)));
		...
		$this
				->assertEquals(
						array(
								__DIR__
										. '/Fixtures/Resources/ChildBundle/foo.txt',
								__DIR__
										. '/Fixtures/ChildBundle/Resources/foo.txt',
								__DIR__
										. '/Fixtures/BaseBundle/Resources/foo.txt',),
						$kernel
								->locateResource(
										'@BaseBundle/Resources/foo.txt',
										__DIR__ . '/Fixtures/Resources', false));
		...
		try {
			$kernel
					->locateResource('@BaseBundle/Resources/hide.txt',
							__DIR__ . '/Fixtures/Resources', false);
			$this
					->fail(
							'Hidden resources should raise an exception when returning an array of matching paths');
		...
		try {
			$kernel
					->locateResource('@BaseBundle/Resources/hide.txt',
							__DIR__ . '/Fixtures/Resources', true);
			$this
					->fail(
							'Hidden resources should raise an exception when returning the first matching path');
		} catch (\RuntimeException $e) {
		}
	}
	...
	public function testLocateResourceOnDirectories() {
		...
		$this
				->assertEquals(__DIR__ . '/Fixtures/Resources/FooBundle/',
						$kernel
								->locateResource('@FooBundle/Resources/',
										__DIR__ . '/Fixtures/Resources'));
		$this
				->assertEquals(__DIR__ . '/Fixtures/Resources/FooBundle',
						$kernel
		...
		$this
				->assertEquals(__DIR__ . '/Fixtures/Bundle1Bundle/Resources/',
	...
	public function testInitializeBundles() {
		...
		$kernel = $this->getKernel();
		$kernel->expects($this->once())->method('registerBundles')
	...
	public function testInitializeBundlesSupportInheritanceCascade() {
		...
		$map = $kernel->getBundleMap();
		$this
				->assertEquals(array($child, $parent, $grandparent),
	...
	protected function getBundle($dir = null, $parent = null, $className = null,
		...
		$bundle = $this
				->getMockBuilder(
						'Symfony\Tests\Component\HttpKernel\BundleForTest')
		...
		$bundle->expects($this->any())->method('getName')
				->will(
						$this
								->returnValue(
										null === $bundleName ? get_class(
														$bundle) : $bundleName));

		$bundle->expects($this->any())->method('getPath')
				->will($this->returnValue($dir));

		$bundle->expects($this->any())->method('getParent')
