...
class ArgvInputTest extends \PHPUnit_Framework_TestCase {
	public function testConstructor() {
		...
		$this
				->assertEquals(array('foo'), $p->getValue($input),
	...
	public function testParser() {
		...
		$this
				->assertEquals(array('name' => 'foo'), $input->getArguments(),
		...
		$this
				->assertEquals(array('foo' => true), $input->getOptions(),
		...
		$this
				->assertEquals(array('foo' => 'bar'), $input->getOptions(),
		...
		try {
			...
			$this
					->fail(
							'->parse() throws a \RuntimeException if no value is passed to an option when it is required');
		} catch (\Exception $e) {
			$this
					->assertInstanceOf('\RuntimeException', $e,
							'->parse() throws a \RuntimeException if no value is passed to an option when it is required');
			$this
					->assertEquals('The "--foo" option requires a value.',
							$e->getMessage(),
							'->parse() throws a \RuntimeException if no value is passed to an option when it is required');
		}
		...
		$input
				->bind(
						new InputDefinition(
								array(new InputArgument('name'),
										new InputOption('foo', 'f',
												InputOption::VALUE_OPTIONAL),
										new InputOption('bar', 'b'))));
		$this
				->assertEquals(array('foo' => null, 'bar' => true),
						$input->getOptions(),
		...
		try {
			...
			$this
					->fail(
							'->parse() throws a \RuntimeException if a value is passed to an option which does not take one');
		} catch (\Exception $e) {
			$this
					->assertInstanceOf('\RuntimeException', $e,
							'->parse() throws a \RuntimeException if a value is passed to an option which does not take one');
			$this
					->assertEquals('The "-o" option does not exist.',
							$e->getMessage(),
							'->parse() throws a \RuntimeException if a value is passed to an option which does not take one');
		}
		...
		try {
			...
			$this
					->fail(
							'->parse() throws a \RuntimeException if too many arguments are passed');
		} catch (\Exception $e) {
			$this
					->assertInstanceOf('\RuntimeException', $e,
							'->parse() throws a \RuntimeException if too many arguments are passed');
		...
		try {
			...
			$this
					->fail(
							'->parse() throws a \RuntimeException if an unknown long option is passed');
		} catch (\Exception $e) {
			$this
					->assertInstanceOf('\RuntimeException', $e,
							'->parse() throws a \RuntimeException if an unknown long option is passed');
			$this
					->assertEquals('The "--foo" option does not exist.',
							$e->getMessage(),
							'->parse() throws a \RuntimeException if an unknown long option is passed');
		}
		...
		try {
			...
			$this
					->fail(
							'->parse() throws a \RuntimeException if an unknown short option is passed');
		} catch (\Exception $e) {
			$this
					->assertInstanceOf('\RuntimeException', $e,
							'->parse() throws a \RuntimeException if an unknown short option is passed');
				...
				->bind(
						new InputDefinition(
								array(new InputOption('foo', 'f'),
										new InputOption('bar', 'b'))));
		$this
				->assertEquals(array('foo' => true, 'bar' => true),
						$input->getOptions(),
						...
						new InputDefinition(
								array(new InputOption('foo', 'f'),
										new InputOption('bar', 'b',
												InputOption::VALUE_REQUIRED))));
		$this
				->assertEquals(array('foo' => true, 'bar' => 'bar'),
						$input->getOptions(),
						...
						new InputDefinition(
								array(
										...
										new InputOption('bar', 'b',
												InputOption::VALUE_OPTIONAL))));
		$this
				->assertEquals(array('foo' => 'bbar', 'bar' => null),
						$input->getOptions(),
		...
		try {
			...
			$this
					->assertNotEquals('Too many arguments.', $e->getMessage(),
							'->parse() parses array arguments');
		}

		$input = new ArgvInput(
				array('cli.php', '--name=foo', '--name=bar', '--name=baz'));
		$input
				->bind(
						new InputDefinition(
								array(
		...
		$this
				->assertEquals(array('name' => array('foo', 'bar', 'baz')),
						$input->getOptions());
	}
	...
	public function testHasParameterOption() {
		...
		$this
				->assertTrue($input->hasParameterOption('-f'),
						'->hasParameterOption() returns true if the given short option is in the raw input');

		$input = new ArgvInput(array('cli.php', '--foo', 'foo'));
		$this
				->assertTrue($input->hasParameterOption('--foo'),
						'->hasParameterOption() returns true if the given short option is in the raw input');

		$input = new ArgvInput(array('cli.php', 'foo'));
		$this
				->assertFalse($input->hasParameterOption('--foo'),
	...
	public function testGetParameterOptionEqualSign($argv, $key, $expected) {
		...
		$this
				->assertEquals($expected, $input->getParameterOption($key),
	...
	public function provideGetParameterOptionValues() {
		return array(
				...
				array(array('app/console', 'foo:bar', '--env=dev'), '--env',
						'dev'),
				array(array('app/console', 'foo:bar', '-e', 'dev'),
						array('-e', '--env'), 'dev'),
