...
class OutputTest extends \PHPUnit_Framework_TestCase {
	...
	public function testWrite() {
		...
		$this
				->assertEquals("\033[33;41;5mfoo\033[0m\n", $output->output,
		...
		try {
			...
			$this
					->fail(
							'->writeln() throws an \InvalidArgumentException when the type does not exist');
		} catch (\Exception $e) {
			$this
					->assertInstanceOf('\InvalidArgumentException', $e,
							'->writeln() throws an \InvalidArgumentException when the type does not exist');
			$this
					->assertEquals('Unknown output type given (24)',
							$e->getMessage());
		}
		...
		$this
				->assertEquals('<bar>foo</bar>', $output->output,
