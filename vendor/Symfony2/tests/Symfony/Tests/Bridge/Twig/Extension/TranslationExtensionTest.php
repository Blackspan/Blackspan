...
class TranslationExtensionTest extends TestCase {
	...
	public function testEscaping() {
		$output = $this
				->getTemplate(
						'{% trans %}Percent: %value%%% (%msg%){% endtrans %}')
	...
	public function testTrans($template, $expected, array $variables = array()) {
		if ($expected != $this->getTemplate($template)->render($variables)) {
			...
			$twig = new \Twig_Environment($loader,
					array('debug' => true, 'cache' => false));
			$twig
					->addExtension(
							new TranslationExtension(
									new Translator('en', new MessageSelector())));
			...
			$this
					->assertEquals($expected,
							$this->getTemplate($template)->render($variables));
		}
	...
	public function getTransTests() {
		return array(
				...
				array('{% trans %}%name%{% endtrans %}', 'Symfony2',
						array('name' => 'Symfony2')),
				array('{% trans from elsewhere %}Hello{% endtrans %}', 'Hello'),
				array('{% trans %}Hello %name%{% endtrans %}',
						'Hello Symfony2', array('name' => 'Symfony2')),
				array(
						'{% trans with { \'%name%\': \'Symfony2\' } %}Hello %name%{% endtrans %}',
						'Hello Symfony2'),
				array(
						'{% set vars = { \'%name%\': \'Symfony2\' } %}{% trans with vars %}Hello %name%{% endtrans %}',
						'Hello Symfony2'),
				...
				array(
						...
						'There is 5 apples (Symfony2)',
						array('count' => 5, 'name' => 'Symfony2')),
				array(
				...
				array('{{ name|trans }}', 'Symfony2',
						array('name' => 'Symfony2')),
				array('{{ hello|trans({ \'%name%\': \'Symfony2\' }) }}',
						'Hello Symfony2', array('hello' => 'Hello %name%')),
				array(
						'{% set vars = { \'%name%\': \'Symfony2\' } %}{{ hello|trans(vars) }}',
						'Hello Symfony2', array('hello' => 'Hello %name%')),
				...
				array(
						...
						'There is 5 apples', array('count' => 5)),
				array('{{ text|transchoice(5, {\'%name%\': \'Symfony2\'}) }}',
						'There is 5 apples (Symfony2)',
						array(
								'text' => '{0} There is no apples|{1} There is one apple|]1,Inf] There is %count% apples (%name%)')),
				array(
						'{{ "{0} There is no apples|{1} There is one apple|]1,Inf] There is %count% apples"|transchoice(count, {}, "messages", "fr") }}',
						'There is 5 apples', array('count' => 5)),);
	}
