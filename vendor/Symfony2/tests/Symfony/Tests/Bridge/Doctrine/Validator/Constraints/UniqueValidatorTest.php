...
class UniqueValidatorTest extends DoctrineOrmTestCase {
	protected function createRegistryMock($entityManagerName, $em) {
		...
		$registry->expects($this->any())->method('getEntityManager')
	...
	protected function createMetadataFactoryMock($metadata) {
		...
		$metadataFactory->expects($this->any())->method('getClassMetadata')
	...
	protected function createValidatorFactory($uniqueValidator) {
		...
		$validatorFactory->expects($this->any())->method('getInstance')
				->with(
						$this
								->isInstanceOf(
	...
	public function createValidator($entityManagerName, $em,
			$validateClass = null, $uniqueFields = null) {
		if (!$validateClass) {
		...
		$schemaTool
				->createSchema(
						array(
								$em
										->getClassMetadata(
												'Symfony\Tests\Bridge\Doctrine\Fixtures\SingleIdentEntity'),
								$em
										->getClassMetadata(
												'Symfony\Tests\Bridge\Doctrine\Fixtures\CompositeIdentEntity'),
								$em
										->getClassMetadata(
												'Symfony\Tests\Bridge\Doctrine\Fixtures\AssociationEntity'),));
	}
	...
	public function testValidateUniqueness() {
		...
		$this
				->assertEquals(0, $violationsList->count(),
						"No violations found on entity before it is saved to the database.");
		...
		$this
				->assertEquals(0, $violationsList->count(),
						"No violations found on entity after it was saved to the database.");

		$entity2 = new SingleIdentEntity(2, 'Foo');

		$violationsList = $validator->validate($entity2);
		$this
				->assertEquals(1, $violationsList->count(),
						"No violations found on entity after it was saved to the database.");
	...
	public function testValidateUniquenessWithNull() {
		...
		$this
				->assertEquals(0, $violationsList->count(),
						"No violations found on entity having a null value.");
	}
	...
	public function testValidateUniquenessAfterConsideringMultipleQueryResults() {
		...
		$this
				->assertEquals(1, $violationsList->count(),
						'Violation found on entity with conflicting entity existing in the database.');
	...
	public function testAssociatedEntity() {
		...
		$this->createSchema($em);
		$validator = $this
				->createValidator($entityManagerName, $em,
						'Symfony\Tests\Bridge\Doctrine\Fixtures\AssociationEntity',
						array('single'));
