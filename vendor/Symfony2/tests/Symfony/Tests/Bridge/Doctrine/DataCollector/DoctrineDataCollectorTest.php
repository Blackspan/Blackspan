...
class DoctrineDataCollectorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCollectQueryCount() {
		...
		$queries = array(
				array('sql' => "SELECT * FROM table1", 'params' => array(),
						'types' => array(), 'executionMS' => 0));
	...
	public function testCollectTime() {
		...
		$queries = array(
				array('sql' => "SELECT * FROM table1", 'params' => array(),
						'types' => array(), 'executionMS' => 1),
				array('sql' => "SELECT * FROM table2", 'params' => array(),
						'types' => array(), 'executionMS' => 2));
		$c = $this->createCollector($queries);
		$c->collect(new Request(), new Response());
	...
	public function testCollectQueries($param, $expected) {
		$queries = array(
				array('sql' => "SELECT * FROM table1 WHERE field1 = ?1",
						'params' => array($param), 'types' => array(),
						'executionMS' => 1));
		$c = $this->createCollector($queries);
		$c->collect(new Request(), new Response());
	...
	public function paramProvider() {
		return array(array('some value', 'some value'), array(1, '1'),
				array(true, 'true'), array(null, 'null'),
	...
	private function createCollector($queries) {
		...
		$registry->expects($this->any())->method('getConnectionNames')
				->will(
						$this
								->returnValue(
										array(
												'default' => 'doctrine.dbal.default_connection')));
		$registry->expects($this->any())->method('getEntityManagerNames')
				->will(
						$this
								->returnValue(
										array(
