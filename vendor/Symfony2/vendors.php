...
if (isset($argv[1])
		&& in_array($argv[1],
				array('--transport=http', '--transport=https',
						'--transport=git'))) {
...
$deps = array(
		...
		array('swiftmailer', 'http://github.com/swiftmailer/swiftmailer.git',
				'v4.1.7'),
		array('twig', 'http://github.com/fabpot/Twig.git', 'v1.7.0'),);

foreach ($deps as $dep) {
	list($name, $url, $rev) = $dep;

	if ($transport) {
		$url = preg_replace('/^(http:|https:|git:)(.*)/', $transport . ':$2',
				$url);
	}
	...
	if (!is_dir($installDir)) {
		system(
				sprintf('git clone %s %s', escapeshellarg($url),
						escapeshellarg($installDir)));
	}

	system(
			sprintf('cd %s && git fetch origin && git reset --hard %s',
