...
class ControllerListener {
	...
	public function onCoreController(FilterControllerEvent $event) {
		...
		$method = new MethodInvocation($controller[0], $controller[1],
				$controller[0]);
		...
		if (null
				=== $metadata = $this->converter
						->convertMethodAnnotations($method, $annotations)) {
			return;
		}
		...
		$jmsSecurityExtra__interceptor = $this->container
				->get('security.access.method_interceptor');
		...
		$closureCode .= '$jmsSecurityExtra__method->setArguments(array('
				. implode(', ', $paramNames) . '));';
	...
	public function convertMethodAnnotations(\ReflectionMethod $method,
		...
		foreach ($annotations as $annotation) {
			if ($annotation instanceof Secure) {
				...
				$methodMetadata
						->addParamPermissions($parameters[$annotation->name],
...
class MethodInvocation extends \ReflectionMethod {
	...
	public function __construct($class, $name, $object,
